...
#define VTSS_MRP_CNT                                                                               \
    (VTSS_PORTS / 2) // The maximum number of MRP instances. One instance
                     // requires at least two ports
...
typedef struct {
    ...
    BOOL                 mra;    // MRP is MRA. The actual role is given by 'ring_role' and can be
                                 // changed with vtss_mrp_ring_role_set()
...
// MRP ring state.
typedef enum { VTSS_MRP_RING_STATE_CLOSED, VTSS_MRP_RING_STATE_OPEN } vtss_mrp_ring_state_t;
...
typedef struct {
    ...
    u32 tst_mon_count; // The TST monitoring count of intervals without TST
                       // generating TST LOC

    u32 itst_interval;  // The expected ITST frame reception interval in
                        // microseconds
    u32 itst_mon_count; // The ITST monitoring count of intervals without ITST
                        // generating ITST LOC
...
typedef struct {
    BOOL tst_to_cpu;  // Copy MRP_Test PDUs from remote MRM to CPU if we are
                      // currently an MRM.
    BOOL itst_to_cpu; // Copy MRP_InTest PDUs from remote MIM to CPU if we are a
                      // MIM-RC.
...
// MRP counters.
typedef struct {
    u64 tst_rx_count;  // Number of received valid MRP_Test frames subject to
                       // MRP_Test processing.
    u64 itst_rx_count; // Number of received valid MRP_InTest frames subject to
...
// get. inst      [IN]  Target instance reference. mrp_idx   [IN]  Index of
// polled MRPE instance. events    [OUT] Masks of detected pending events during
// get.
typedef struct {
    u32 p_mask;
    u32 s_mask;
