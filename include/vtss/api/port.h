...
typedef struct {
    ...
    vtss_port_counter_t
        rx_etherStatsCRCAlignErrors; /**< Rx CRC/alignment errors */
    ...
    vtss_port_counter_t
        rx_etherStatsPkts65to127Octets; /**< Rx 65-127 byte packets */
    vtss_port_counter_t
    ...
    vtss_port_counter_t
        tx_etherStatsPkts65to127Octets; /**< Tx 65-127 byte packets */
    vtss_port_counter_t
...
typedef struct {
...
#if defined(VTSS_FEATURE_PORT_CNT_ETHER_LIKE)
    vtss_port_counter_t
        dot3StatsSingleCollisionFrames; /**< Tx single collisions */
    vtss_port_counter_t
        dot3StatsMultipleCollisionFrames; /**< Tx multiple collisions */
    ...
    vtss_port_counter_t
        dot3StatsExcessiveCollisions; /**< Tx excessive collisions */
    vtss_port_counter_t
        dot3StatsCarrierSenseErrors; /**< Tx carrier sense errors */
#endif                               /* VTSS_FEATURE_PORT_CNT_ETHER_LIKE */
...
typedef struct {
    ...
    vtss_port_counter_t
        rx_green_discard[VTSS_PRIOS]; /**< Rx green discarded frames */
    vtss_port_counter_t
        rx_yellow_discard[VTSS_PRIOS];        /**< Rx yellow discarded frames */
...
typedef enum {
    VTSS_SPEED_FIBER_NOT_SUPPORTED_OR_DISABLED =
    ...
    VTSS_SPEED_FIBER_DISABLED =
        5, /**< Obsolete - use VTSS_SPEED_FIBER_NOT_SUPPORTED_OR_DISABLED
              instead*/
...
typedef struct {
    ...
    BOOL unidirectional_ability; /**<TRUE: PHY able to transmit from media
                                    independent interface regardless of whether
                                    the PHY has determined that a valid link has
                                    been established.FALSE: PHY able to transmit
                                    from media independent interface only when
                                    the PHY has determined that a valid link has
                                    been established. Note This bit is only
                                    applicable to 100BASE-FX and 1000BASE-X
                                    fiber media modes.*/
    vtss_aneg_t aneg;            /**< Auto negotiation result */
    BOOL mdi_cross; /**< Indication of if Auto-MDIX crossover is performed */
    BOOL fiber;  /**< Indication of if the link is a fiber link, TRUE if link is
                    a fiber link. FALSE if link is cu link or No Media */
    BOOL copper; /**< Indication of if the link is a copper link, TRUE if link
