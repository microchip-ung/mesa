...
/** \page qos Quality of Service (QoS)
    ...
    The Quality of Service (QoS) functions are used to control the various QoS
   features in the switch.
*/

#if defined(VTSS_FEATURE_QOS_WRED_V2) || defined(VTSS_FEATURE_QOS_WRED_V3)
/**
 * \brief Random Early Detection version 2. Select if max means max drop
 *probability or max fill level
 **/
typedef enum {
...
/**
 * \brief Random Early Detection configuration struct version 2 (per queue, per
 *dpl - switch global)
 **/
typedef struct {
...
/** \page qos
    ...
    The global configuration functions are used to control the global switch
...
typedef struct {
    ...
    vtss_wred_conf_t red_v3[VTSS_QUEUE_ARRAY_SIZE] /**< Random Early Detection -
                                                      per queue (0..7), per dpl
                                                      (1..3), per group (0..2) */
                           [VTSS_WRED_DPL_CNT][VTSS_WRED_GROUP_CNT];
#endif /* defined(VTSS_FEATURE_QOS_WRED_V3) */
} vtss_qos_conf_t;
 ...
 **/
typedef struct {
    BOOL frame_rate; /**< Measure rates in frames per seconds instead of bits
                        per second */
...
/** \page qos
    ...
    The MEP policer configuration functions are used to control the MEP
   policers.\n The following functions are available:
    - vtss_mep_policer_conf_get() is used to get MEP policer configuration for a
   specific port and priority.
    - vtss_mep_policer_conf_set() is used to set MEP policer configuration for a
   specific port and priority.
*/
...
typedef enum {
    ...
    VTSS_DSCP_MODE_SEL,  /**< DSCP values selected above (dscp_remark) are
                            remarked */
...
typedef enum {
    ...
    VTSS_DSCP_EMODE_REMAP,   /**< DSCP remarked with DSCP value from analyzer
                                remapped through global remap table  */
#if defined(VTSS_FEATURE_QOS_DSCP_REMARK_DP_AWARE)
    VTSS_DSCP_EMODE_REMAP_DPA /**< DSCP remarked with DSCP value from analyzer
                                 remapped through global remap dp aware tables */
...
/** \page qos
    ...
    The port configuration functions are used to control the per port
   parameters.\n The following functions are available:
    - vtss_qos_port_conf_get() is used to get port configuration for a specific
   port.
    - vtss_qos_port_conf_set() is used to set port configuration for a specific
   port.
*/
...
typedef struct {
    ...
    vtss_dp_level_t dp_level_map[VTSS_PCP_ARRAY_SIZE]
                                [VTSS_DEI_ARRAY_SIZE]; /**< Ingress mapping for
                                                          tagged frames from PCP
                                                          and DEI to DP level */
    BOOL dscp_class_enable; /**< Ingress classification of QoS class and DP
                               level based on DSCP */
    ...
    vtss_qos_ingress_map_id_t ingress_map; /**< Ingress map to use for
...
/** \page qos
    ...
    Advanced QoS classification rules can be done using the QoS Control List
   (QCL). Each QoS Control Entry (QCE) is identified by a QCE ID
   (::vtss_qce_id_t).\n The following functions are available:
    - vtss_qce_init() is used to initialize a QCE (::vtss_qce_t) to default
   values.
    - vtss_qce_add() is used to add or modify a QCE.
    - vtss_qce_del() is used to delete a QCE.

    The QCEs are ordered in a list of rules based on the QCE IDs. When adding a
   rule, the QCE ID of the rule and the QCE ID of the next rule in the list must
   be specified. A special value #VTSS_QCE_ID_LAST is used to specify that the
   rule must be added at the end of the list.\n Each QCE includes a key
   structure (::vtss_qce_key_t) with fields used for matching received frames
   and an action structure (::vtss_qce_action_t) with the classification
...
typedef struct {
    ...
    vtss_vcap_u48_t smac; /**< SMAC - Only the 24 most significant bits (OUI)
                             are supported on Jaguar1, rest are wildcards */
} vtss_qce_mac_t;
...
typedef struct {
    vtss_vcap_vr_t   dscp;  /**< DSCP field (6 bit) */
    vtss_vcap_u8_t   proto; /**< Protocol */
    vtss_vcap_u128_t sip;   /**< Source IP address (32 LSB on L26 and J1, 64 LSB
                               on Serval when key_type = mac_ip_addr) */
#if defined(VTSS_FEATURE_QCL_KEY_DIP)
    vtss_vcap_u128_t dip; /**< Destination IP address - 64 LSB on Serval when
 ...
 * \param qce_id [IN]  QCE ID. The QCE will be added before the entry with this
 *ID. VTSS_QCE_ID_LAST is reserved for inserting last.
...
/** \page qos
    ...
    Each ingress map is identified by an ingress map id
   (::vtss_qos_ingress_map_id_t).\n The following functions are available:
    - vtss_qos_ingress_map_init() is used to initialize an ingress map
   (::vtss_qos_ingress_map_t) to default values.
...
/** \page qos
    ...
    Each egress map is identified by an egress map id
   (::vtss_qos_egress_map_id_t).\n The following functions are available:
    - vtss_qos_egress_map_init() is used to initialize an egress map
   (::vtss_qos_egress_map_t) to default values.
