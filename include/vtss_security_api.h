...
/** \page security Security
    ...
    The Security functions are used to control Port Authentication and Access
   Control List.

    \section port_auth Port Authentication (802.1X)

    The the 802.1X forwarding state for a port can be setup.
    - vtss_auth_port_state_get() is used to get the authentication state for a
   port.
    - vtss_auth_port_state_set() is used to set the authentication state for a
   port.
...
/** \page security
    ...
    Advanced frame processing can be setup using the Access Control List (ACL)
   of the switch. Each rule in the list is called an Access Control Entry (ACE).
   For each frame received on an ingress port, the ACL is searched until an ACE
   matching the ingress port and frame properties is found. The action of the
   first matching ACE determines the forwarding of the frame. If no matching ACE
   is found, the default action of the ingress port is used.

    By default, the ACL is empty and the default port actions allow forwarding
   of frames.

    \subsection ace Access Control Entry

    Each ACE is idenfied by an ACE ID used for identification and ordering of
   ACEs. The following ACE functions are available:
    ...
    The ::vtss_ace_t structure used when adding an ACE can be divided into three
   parts:
    - ACE ID (::vtss_ace_id_t) used for identifying the rule.
    - ACE action field (::vtss_acl_action_t) used to describe the forwarding of
   matching frames, including:
    ...
    The port configuration includes the default ACL action and the ACL policy
   number (::vtss_acl_policy_no_t) which can be used to form groups of ports
   matching the same ACEs. ACEs matching an ACL policy number can be added to
   match frames from ports with the same policy number. The value
   #VTSS_ACL_POLICY_NO_NONE is used to disable ACL processing on a port.

    \subsection acl_policer Policer Configuration

    Each policer is identified by an ACL policer ID (::vtss_acl_policer_no_t)
   which can be used when mapping an ACL action to a policer. The following
   functions are available per ACL policer:
...
/** \page security
    ...
    The switch supports Hierarchical Access Control Lists, meaning that multiple
   ACLs are available. The following ACLs can be used, listed in the order from
   ingress to egress processing:
    - I-PACL: Ingress Port ACL (ACL rules for ingress ports matching any frame
   type)
    - I-VACL: Ingress VLAN ACL (ACL rules for ingress ports matching
   ETYPE/LLC/SNAP frames)
    - I-RACL: Ingress Router ACL (ACL rules for ingress router legs matching
   IPv4/IPv6 frames)
    - E-RACL: Egress Router ACL (ACL rules for egress router legs matching
   IPv4/IPv6 frames)
    - E-VACL: Egress VLAN ACL (ACL rules for egress ports matching
   ETYPE/LLC/SNAP frames)
    - E-PACL: Egress Port ACL (ACL rules for egress ports matching any frame
   type)
...
typedef enum {
    ...
    VTSS_ACL_PTP_ACTION_ONE_STEP_SUB_DELAY_1, /**< PTP one-step time-stamping,
                                                 Serval: subtract delay 1, Jr2:
                                                 Add IDLY1 */
    VTSS_ACL_PTP_ACTION_ONE_STEP_SUB_DELAY_2, /**< PTP one-step time-stamping,
                                                 Serval: subtract delay 2, Jr2:
                                                 Add IDLY2 */
#endif
#if defined(VTSS_ARCH_LUTON26)
    VTSS_ACL_PTP_ACTION_ONE_AND_TWO_STEP, /**< PTP one-step and two-step
                                             time-stamping */
...
/** \brief ACL Source IP index. For IPv6 entries, it must be divisible by 4 and
 * consumes 4 rules **/
