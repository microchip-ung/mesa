...
typedef struct {
    ...
    u32 auto_copy_period[2]; // Timer period values in microseconds, used for
                             // auto copy to CPU (auto "hit me once"). A value
                             // of 0 disables the timer.
    u8 auto_copy_ccm_valid;  // #auto_copy_period timer (0 or 1) to use for auto
                             // copy next valid CCM to CPU.
    u8 auto_copy_ccm_tlv;    // #auto_copy_period timer (0 or 1) to use for auto
...
typedef struct {
    ...
    // LBM Transaction ID to use on Tx. VTSS_VOE_LBM_TRANSACTION_ID_NONE means
    // unchanged. Auto increment of Tx LBM transaction ID is always enabled
    u32 trans_id;
} vtss_voe_lb_conf_t;
