...
def tod_internal_mode_ingress_node_test
    ...
    # Update the TX FIFO in AIL. This will cause callback to Json with the TX timestamp
    ...
    #Calculate the TX TOD nanoseconds based on TX time stamp
    ...
    if ((diff < 0) || (diff > 4000))
        t_e("SYNC PDU reserved field not as expected.   frame_ts #{frame_ts}   tod_nano_tx #{tod_nano_tx}   diff #{diff}")
    end
...
def tod_internal_mode_egress_node_test
    ...
    # calculate the smallest expected correction value. The tod_nano_tx is not egress time in this node but close
