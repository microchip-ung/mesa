...
#    $ts.dut.run("mesa-cmd mac dump")
    ...
    console("Calculated frames per sec at line speed: #{sec_count_in}")

    pre_tx = with_pre_tx ? 1 : 0    # Calculate the possible pre tx time in seconds
    console("Start Easy Frame transmitting #{sec*sec_count_in} frames of size #{size} with #{pre_tx} sec of pre TX and 2 sec of post TX. Speed is 1 Gbps.")
