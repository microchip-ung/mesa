...
def voe_test(tag_vid = 0)
    ...
    console("Assure that LOC is detected")
    sleep(4)
    ...
    console("Transmit valid CCM frame in front of VOE to clear LOC")
    frametx = frame.dup
    ...
    console("Transmit valid CCM frame in front of VOE with lower level. Check that frame is level filtered.")
    frametx = frame.dup
    ...
    console("Transmit valid CCM frame behind VOE with lower level. Check that frame is level filtered.")
    frame_tx(frametx, $port1, "", "", " ", " ")

    console("Transmit valid CCM frame behind VOE with higher level. Check that frame is NOT level filtered.")
    frametx = frame.dup
    frametx += ccm_pdu_create($level+1, "MESA_VOE_CCM_PERIOD_1_SEC", 0, 1, $megid)
    frame_tx(frametx, $port1, frametx, "", " ", " ")

    console("Transmit valid CCM frame in front of VOE with higher level. Check that frame is NOT level filtered.")
    ...
    console("Transmit valid LBM frame in front of VOE")
    ...
    console("Inject and expect valid CCM frame from the VOE")
    ...
    console("Increment sequence number")
