...
check_capabilities do
    $cap_vop_v1 = ($ts.dut.call("mesa_capability", "MESA_CAP_VOP_V1") != 0) ? true : false
    $cap_vop_v2 = ($ts.dut.call("mesa_capability", "MESA_CAP_VOP_V2") != 0) ? true : false
    $cap_vop_cfm = ($ts.dut.call("mesa_capability", "MESA_CAP_VOP_CFM") != 0) ? true : false
    assert(($cap_vop_v1 == true) || ($cap_vop_v2 == true), "VOP must be V1 or V2,  (cap_vop_v1=#{$cap_vop_v1} $cap_vop_v2=#{$cap_vop_v1})")
...
def voe_test(tag_vid = 0)
    ...
    if ($cap_vop_v2)    # V1 does not support CCM level check status
    ...
    frame = frame_create(MC_STRING, SC_STRING, tag_vid)
    exp_seq = $cap_oam_v2 ? 0 : lan966x ? 0 : 1     #LAN966X start with 0 and Serval start with 1 sequence number
    ccm_pdu = ccm_pdu_create($level, $period, exp_seq, 2, $megid)
