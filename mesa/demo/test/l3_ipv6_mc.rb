...
# 6) MC frame is insterted on the local port and routing is validated on the other.
  ...
  [
    {
      ...
      rt_ttl_limit: 0,
      rt_rpf: false,
      frm_dmac: "33:33:01:02:03:01",
      frm_vid: 2,
      frm_sip: "01::1:1",
      frm_mcast: "ff00::0102:0304",
      frm_ttl: 64,
      expect_rx: true
    },
    {
      ...
      rt_ttl_limit: 0,
      rt_rpf: false,
      frm_dmac: "33:33:01:02:03:02",
      frm_vid: 3,
      frm_sip: "02::2:2",
      frm_mcast: "ff00::0102:0305",
      frm_ttl: 64,
      expect_rx: true
    }
  ],
...
$conf_table.each do |pair|
    test "l3-mc-conf" do
        ...
        test "Add multicast routes" do
            pair.each_with_index do |e, idx|
                if e.key?(:rt_sip)
                    sip = ipv6_str2arr(e[:rt_sip])
                    dip = ipv6_str2arr(e[:rt_mcast])
                else
                    sip = ipv6_str2arr(e[:frm_sip])
                    dip = ipv6_str2arr(e[:frm_mcast])
                end
                ipv6_mc = {
    ...
    test "mc-frame-routing-io" do
        ...
        pair.each_with_index do |entry, idx_tx|
            ...
            cmd += "et 0x86dd ipv6 sip #{tx_ip_src} dip #{tx_ip_dst} hlim #{ttl} "
            ...
            pair.each_with_index do |rxentry, idx_rx|
                ...
                cmd += "name f#{idx_rx} eth dmac #{tx_dmac} smac #{$router_mac} et 0x86dd "
                cmd += "ipv6 sip #{tx_ip_src} dip #{tx_ip_dst} hlim #{ttl} "
                if entry[:expect_rx]
        ...
        if (rleg1[:ipv6mc_transmitted_frames] != 0) &&
              (rleg1[:ipv6mc_transmitted_frames] != rleg2[:ipv6mc_received_frames]) &&
              (rleg2[:ipv6mc_transmitted_frames] != rleg1[:ipv6mc_received_frames])
