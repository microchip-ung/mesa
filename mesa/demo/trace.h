...
#define T(_grp, _lvl, _fmt, ...)                                                                   \
    {                                                                                              \
        if (trace_groups[_grp].level >= _lvl)                                                      \
            mscc_appl_trace_printf(trace_module.name, trace_groups[_grp].name, _lvl, __FILE__,     \
                                   __LINE__, __FUNCTION__, _fmt, ##__VA_ARGS__);                   \
    }
#define T_HEX(_grp, _lvl, _byte, _cnt)                                                             \
    {                                                                                              \
        if (trace_groups[_grp].level >= _lvl)                                                      \
            mscc_appl_trace_hex(trace_module.name, trace_groups[_grp].name, _lvl, __FILE__,        \
                                __LINE__, __FUNCTION__, _byte, _cnt);                              \
    }
