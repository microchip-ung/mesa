...
#define T(_grp, _lvl, _fmt, ...)                                               \
    {                                                                          \
        if (trace_groups[_grp].level >= _lvl)                                  \
            mscc_appl_trace_printf(trace_module.name, trace_groups[_grp].name, \
                                   _lvl, __FILE__, __LINE__, __FUNCTION__,     \
                                   _fmt, ##__VA_ARGS__);                       \
    }
#define T_HEX(_grp, _lvl, _byte, _cnt)                                         \
    {                                                                          \
        if (trace_groups[_grp].level >= _lvl)                                  \
            mscc_appl_trace_hex(trace_module.name, trace_groups[_grp].name,    \
                                _lvl, __FILE__, __LINE__, __FUNCTION__, _byte, \
                                _cnt);                                         \
    }
#define T_EG(_grp, _fmt, ...)                                                  \
    T(_grp, MESA_TRACE_LEVEL_ERROR, _fmt, ##__VA_ARGS__)
#define T_E(_fmt, ...) T_EG(0, _fmt, ##__VA_ARGS__)
#define T_EG_HEX(_grp, _byte, _cnt)                                            \
    T_HEX(_grp, MESA_TRACE_LEVEL_ERROR, _byte, _cnt)
#define T_E_HEX(_byte, _cnt) T_EG_HEX(0, _byte, _cnt)
#define T_IG(_grp, _fmt, ...)                                                  \
    T(_grp, MESA_TRACE_LEVEL_INFO, _fmt, ##__VA_ARGS__)
#define T_I(_fmt, ...) T_IG(0, _fmt, ##__VA_ARGS__)
#define T_IG_HEX(_grp, _byte, _cnt)                                            \
    T_HEX(_grp, MESA_TRACE_LEVEL_INFO, _byte, _cnt)
#define T_I_HEX(_byte, _cnt) T_IG_HEX(0, _byte, _cnt)
#define T_DG(_grp, _fmt, ...)                                                  \
    T(_grp, MESA_TRACE_LEVEL_DEBUG, _fmt, ##__VA_ARGS__)
#define T_D(_fmt, ...) T_DG(0, _fmt, ##__VA_ARGS__)
#define T_DG_HEX(_grp, _byte, _cnt)                                            \
    T_HEX(_grp, MESA_TRACE_LEVEL_DEBUG, _byte, _cnt)
#define T_D_HEX(_byte, _cnt) T_DG_HEX(0, _byte, _cnt)
#define T_NG(_grp, _fmt, ...)                                                  \
    T(_grp, MESA_TRACE_LEVEL_NOISE, _fmt, ##__VA_ARGS__)
#define T_N(_fmt, ...) T_NG(0, _fmt, ##__VA_ARGS__)
#define T_NG_HEX(_grp, _byte, _cnt)                                            \
    T_HEX(_grp, MESA_TRACE_LEVEL_NOISE, _byte, _cnt)
