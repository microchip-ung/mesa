...
//   gcc -Wall -shared -fPIC -fasynchronous-unwind-tables btsig.c -lubacktrace
//   -o btsig.so
// If using glibc, compile with
//   gcc -Wall -shared -fPIC -fasynchronous-unwind-tables btsig.c -lbacktrace -o
//   btsig.so
//
// Test with
...
static const btsig_entry_t btsig_captured_signals[] = {
    CAPTURE(SIGINT, TRUE),   CAPTURE(SIGQUIT, TRUE),  CAPTURE(SIGILL, TRUE),
    CAPTURE(SIGFPE, TRUE),   CAPTURE(SIGSEGV, TRUE),  CAPTURE(SIGBUS, TRUE),
    CAPTURE(SIGUSR1, FALSE), CAPTURE(SIGUSR2, FALSE), CAPTURE(SIGPWR, FALSE),
};
...
static void btsig_signal_handler(int sig)
{
    ...
    // If printing to stdout, we must call fflush(), because stdout by default
    // is buffered.
    fflush(fd);
