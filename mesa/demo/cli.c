...
static mscc_appl_trace_module_t trace_module = {.name = "cli"};
...
void mscc_appl_cli_cmd_reg(cli_cmd_t *cmd)
{
    ...
    for (cur = cli_cmd_list; cur != NULL; prev = cur, cur = cur->next) {
        ...
        if (strstr(cur->syntax, "Help") == cur->syntax ||
            strstr(cur->syntax, "Exit") == cur->syntax)
            continue;
            ...
            for (i_cmd = i_parm, i_stx = i_parm; i_parm && i_stx < stx_count; i_stx++) {
                ...
                case CLI_STATE_DONE:
                    if (end && !separator)
                        ...
                        skip_parm = 1;
                    break;
                default: cli_printf("Illegal state: %d\n", state); return -1;
                }
                end = (strstr(stx, ")") != NULL);
...
static void cli_socket_init(void)
{
    ...
    if ((fd = socket(AF_UNIX, SOCK_STREAM, 0)) < 0) {
        T_E("socket failed");
    } else if (bind(fd, (struct sockaddr *)&local,
                    sizeof(local.sun_family) + strlen(local.sun_path)) < 0) {
        T_E("bind failed");
        close(fd);
