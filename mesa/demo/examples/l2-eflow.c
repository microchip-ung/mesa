...
static int eflow_run(int argc, const char *argv[])
{
    ...
    mesa_vid_t     avid =
        ARGV_RUN_OPT_INT("avid", "Access port VID, default untagged", EFLOW_VID_UNTAG);
    ...
    mesa_bitrate_t rate = ARGV_RUN_OPT_INT("rate", "VLAN policer rate [kbps], default disabled",
