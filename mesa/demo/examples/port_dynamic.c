...
static int port_dynamic_init(int argc, const char *argv[])
{
    ...
    // Get the ports capability, but for test reason do not break the test if
    // the capabilities are not found
    ...
    // Now remove/attach phys on all 4 ports and verify that the API is
...
static int port_dynamic_clean() { return 0; }
