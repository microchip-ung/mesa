...
typedef enum {
    ...
    MESA_ACL_PTP_ACTION_ONE_STEP_SUB_DELAY_2
        CAP(ACL_PTP_DELAY), // PTP one-step time-stamping, Serval: subtract
                            // delay 2, Jr2: Add IDLY2
    MESA_ACL_PTP_ACTION_ONE_AND_TWO_STEP
        CAP(ACL_PTP_ONE_AND_TWO_STEP), // PTP one-step and two-step time-stamping
    MESA_ACL_PTP_ACTION_TWO_STEP       // PTP two-step time-stamping
} mesa_acl_ptp_action_t;

// ACL Source IP index. For IPv6 entries, it must be divisible by 4 and consumes
// 4 rules
...
typedef enum {
    ...
    MESA_ACL_PTP_RSP_DLY_REQ_RSP_TS_UPD, // Auto response to Delay_Req, includes
                                         // receiveTimestamp update
    MESA_ACL_PTP_RSP_DLY_REQ_RSP_NO_TS   // Auto response to Delay_Req, exludes
                                         // receiveTimestamp update
...
typedef struct {
    ...
    mesa_bool_t ifh_flag
        CAP(ACL_IFH_FLAG); // Control one target specific bit in IFH (match_id bit 1), if supported
    mesa_bool_t acl_hit_disable CAP(PACKET_RX_MATCH_ID); // Disable ACL hit flag via match_id bit 0
...
// Router leg list
typedef struct {
    uint8_t _private[MESA_RLEG_LIST_ARRAY_SIZE]; // Router leg list, access
                                                 // using utilities below
} mesa_rleg_list_t;
...
typedef struct {
    ...
    mesa_bool_t ifh_flag; // Control one target specific bit in IFH (I-PACL/I-VACL only), match_id
                          // bit 1, if supported
    mesa_bool_t acl_hit_disable CAP(PACKET_RX_MATCH_ID); // Disable ACL hit flag via match_id bit 0
...
mesa_rc mesa_hace_add(const mesa_inst_t        inst,
                      ...
                      const mesa_ace_id_t      ace_id_next,
                      const mesa_hace_t *const hace) CAP(ACL_HACL);
...
mesa_rc mesa_hace_counter_get(const mesa_inst_t         inst,
                              const mesa_hacl_type_t    type,
                              const mesa_ace_id_t       ace_id,
                              mesa_ace_counter_t *const counter) CAP(ACL_HACL);
