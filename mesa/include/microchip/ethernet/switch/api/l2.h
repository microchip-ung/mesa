...
//              By default, the VLAN port configuration is used for all ports in
//              all VLANs.
...
//      maximum information rate that is based on actual burst size. Higher
//      burst size give higher max rate.
...
//               By default, the destination group for each port is equal to the
//               port number.
...
mesa_rc mesa_ipv4_mc_add(const mesa_inst_t       inst,
                         ...
                         const mesa_ip_t         sip,
                         const mesa_ip_t         dip,
                         const mesa_port_list_t *port_list) CAP(L2_IPV4_MC_SIP);
...
mesa_rc mesa_ipv4_mc_del(const mesa_inst_t inst,
                         const mesa_vid_t  vid,
                         const mesa_ip_t   sip,
                         const mesa_ip_t   dip) CAP(L2_IPV4_MC_SIP);
...
// are flooded using the multicast port list or the IPv6 multicast port list.
...
mesa_rc mesa_ipv6_mc_add(const mesa_inst_t       inst,
                         ...
                         const mesa_port_list_t *port_list) CAP(L2_IPV6_MC_SIP);
...
mesa_rc mesa_ipv6_mc_del(const mesa_inst_t inst,
                         const mesa_vid_t  vid,
                         const mesa_ipv6_t sip,
                         const mesa_ipv6_t dip) CAP(L2_IPV6_MC_SIP);
...
// Alternatively, MESA_PORT_NO_NONE can be used to indicate an internal
// connection to the neighbour RedBox. Assume that switch port X and Y are
// candidates for use as RedBox port A and B. The table below shows the legal
// combinations and how the switch port roles are affected. The switch port
// roles in the table mean:
