...
typedef struct {
    ...
    mesa_pct_t
        max; // Maximum drop probability or fill level - selected by max_unit
    mesa_wred_max_t max_unit; // Selects the unit for max
} mesa_wred_conf_t CAP(QOS_WRED);

// QoS configuration per (DPL, GROUP)
typedef struct {
    mesa_wred_conf_t wred[MESA_QUEUE_ARRAY_SIZE] CAP(
        QOS_WRED); // WRED configuration per priority
...
typedef struct {
    ...
    mesa_bool_t    frame_rate; // Measure rates in frames per seconds instead of
                               // bits per second
    mesa_dp_level_t dp_bypass_level
        CAP(QOS_PORT_POLICER_EXT_DPBL); // Drop Predence bypass level

    mesa_bool_t known_unicast
        CAP(QOS_PORT_POLICER_EXT_TTM); // Known unicast frames are policed
    mesa_bool_t known_multicast
        CAP(QOS_PORT_POLICER_EXT_TTM); // Known multicast frames are policed
    mesa_bool_t known_broadcast
        CAP(QOS_PORT_POLICER_EXT_TTM); // Known broadcast frames are policed
    mesa_bool_t unknown_unicast
        CAP(QOS_PORT_POLICER_EXT_TTM); // Unknown unicast frames are policed
    ...
    mesa_bool_t learning
        CAP(QOS_PORT_POLICER_EXT_TTM); // Learning frames are policed
...
typedef enum {
    ...
    MESA_DSCP_EMODE_REMAP_DPA
        CAP(QOS_DSCP_REMARK_DP_AWARE) // DSCP remarked with classified DSCP
...
typedef struct {
    mesa_prio_t prio; // Ingress mapping for tagged frames from PCP and DEI to
                      // priority (QOS class)
    mesa_dp_level_t
        dpl; // Ingress mapping for tagged frames from PCP and DEI to DP level
} mesa_qos_port_pcp_dei_conf_t;

// Egress Tag Remark Mode
typedef enum {
    MESA_TAG_REMARK_MODE_CLASSIFIED =
        0, // Use classified PCP/DEI values as tag PCP/DEI
    MESA_TAG_REMARK_MODE_DEFAULT =
        2, // Use configured default egress PCP/DEI values as tag PCP/DEI
    MESA_TAG_REMARK_MODE_MAPPED =
        3 // Use mapped versions of classified QOS class and DP level (see
...
typedef enum {
    ...
    MESA_SHAPER_MODE_FRAME
        CAP(QOS_EGRESS_SHAPER_FRAME) // Use frame-rate for the shaper
...
typedef struct {
    ...
    mesa_burst_level_t ebs
        CAP(QOS_EGRESS_SHAPERS_DLB); // EBS (Excess Burst Size).  Unit: bytes
    mesa_bitrate_t eir
        CAP(QOS_EGRESS_SHAPERS_DLB); // EIR (Excess Information Rate). Unit:
                                     // kbps. Use MESA_BITRATE_DISABLED to
                                     // disable DLB
    mesa_shaper_mode_t mode CAP(QOS_EGRESS_SHAPERS_RT); // RT (Rate type)
    mesa_bool_t credit_enable
        CAP(QOS_EGRESS_QUEUE_SHAPERS_CRB); // Queue shaper only: Credit-based
                                           // shaping
} mesa_shaper_t;
...
typedef struct {
    ...
    mesa_pct_t
        pct; // The DWRR Queue percentage if DWRR is enabled ('dwrr_enable')
    mesa_bool_t excess_enable
        CAP(QOS_EGRESS_QUEUE_SHAPERS_EB); // Allow this queue to use excess
                                          // bandwidth
    mesa_bool_t cut_through_enable
        CAP(QOS_EGRESS_QUEUE_CUT_THROUGH); // Allow this queue to use cut
                                           // through feature
...
typedef struct {
    ...
    mesa_bool_t dwrr_enable; // Enable Deficit Weighted Round Robin (DWRR)
                             // fairness scheduling.
    uint8_t dwrr_cnt
        CAP(QOS_SCHEDULER_CNT_DWRR); // Number of queues running in DWRR mode
                                     // starting from queue 0. Value 3 means that
                                     // queue 0 to 2 is running DWRR scheduling.
    mesa_bool_t dmac_dip
        CAP(QOS_QCL_DMAC_DIP); // QCL lookup matching on DMAC/DIP (default is
                               // SMAC/SIP)
    mesa_vcap_key_type_t key_type
        CAP(QOS_QCL_KEY_TYPE); // QCL lookup key type for received frames
    ...
    mesa_qos_port_queue_conf_t
        queue[MESA_QUEUE_ARRAY_SIZE]; // Per priority (QOS class/Queue)
                                      // configuration

    mesa_bool_t ot_dwrr_enable
        CAP(QOS_OT); // Enable DWRR fairness scheduling between OT queues.
    uint8_t ot_dwrr_cnt
        CAP(QOS_OT); // Number of OT queues running in DWRR mode.
    mesa_qos_port_ot_queue_conf_t ot_queue
        [MESA_QUEUE_ARRAY_SIZE] CAP(QOS_OT); // Per OT queue configuration

    mesa_bool_t ot_it_dwrr_enable
        CAP(QOS_OT); // Enable DWRR fairness scheduling
                     // between OT and IT traffic.
    mesa_pct_t ot_pct
        CAP(QOS_OT); // The DWRR OT traffic percentage. Rest is IT traffic.
...
typedef struct {
    ...
    mesa_vcap_u48_t dmac
        CAP(QOS_QCL_KEY_DMAC); // DMAC - Serval: key_type = mac_ip_addr
...
typedef struct {
    ...
    mesa_vcap_ip_t dip
        CAP(QOS_QCL_KEY_DIP); // Destination IP address - Serval: key_type =
                              // ip_addr and mac_ip_addr
...
typedef struct {
    ...
    mesa_vcap_u128_t dip
        CAP(QOS_QCL_KEY_DIP); // Destination IP address - 64 LSB on Serval when
...
typedef struct {
    ...
    mesa_bool_t map_id_enable
        CAP(QOS_INGRESS_MAP_CNT); // Enable classification via ingress map
...
typedef struct {
    ...
    mesa_cosid_t
        cosid; // The classified COS ID is set to cosid if action.cosid is TRUE
    mesa_cosid_t
        path_cosid; // Path COS ID value used by OAM MEP if action.path is TRUE
...
typedef union {
    ...
    mesa_qos_ingress_map_values_t
        pcp_dei[8][2];                      // MESA_QOS_INGRESS_MAP_KEY_PCP_DEI
...
typedef union {
    ...
    mesa_qos_egress_map_values_t
        cosid_dpl[8][4];                   // MESA_QOS_EGRESS_MAP_KEY_COSID_DPL
    mesa_qos_egress_map_values_t dscp[64]; // MESA_QOS_EGRESS_MAP_KEY_DSCP
    mesa_qos_egress_map_values_t
        dscp_dpl[64][4];                     // MESA_QOS_EGRESS_MAP_KEY_DSCP_DPL
    mesa_qos_egress_map_values_t mpls_tc[8]; // MESA_QOS_EGRESS_MAP_KEY_MPLS_TC
    mesa_qos_egress_map_values_t
        mpls_tc_dpl[8][4]; // MESA_QOS_EGRESS_MAP_KEY_MPLS_TC_DPL
