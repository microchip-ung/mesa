...
// All the MRP frames are forwarded according to IEC 62439-2 depending on ring
// role. MRP PDUs required for protocol processing are copied to CPU. Test and
// ITest Loss Of Continuity is calculated according to IEC 62439-2.
//
...
// inst     [IN]  Target instance reference.
// mrp_idx  [IN]  Index of the configured MRP instance.
// voe_idx  [OUT] Allocated VOE indices per port for MRP. VTSS_VOE_IDX_NONE
// indicates that no VOE is allocated.
...
// inst     [IN] Target instance reference.
// mrp_idx  [IN] Index of the configured MRP instance.
// port_no  [IN] The new primary port. Must be one of the already configured
// ports.
...
// Set an MRP instance ring state. This is the ring state inserted in the
// transmitted TST frames inst     [IN] Target instance reference. mrp_idx  [IN]
// Index of the configured MRP instance. state    [IN] The ring state.
...
// Set an MRP instance Interconnected ring state. This is the ring state
// inserted in the transmitted INTST frames inst     [IN] Target instance
// reference. mrp_idx [IN] Index of the configured MRP instance. state    [IN]
// The ring state.
...
typedef struct {
    uint32_t tst_interval;  // The expected TST frame reception interval in
                            // microseconds
    uint32_t tst_mon_count; // The TST monitoring count of intervals without TST
                            // generating TST LOC

    uint32_t itst_interval;  // The expected ITST frame reception interval in
                             // microseconds
    uint32_t itst_mon_count; // The ITST monitoring count of intervals without
                             // ITST generating ITST LOC
...
typedef struct {
    mesa_bool_t tst_to_cpu;  // Copy MRP_Test PDUs from remote MRM to CPU if we
                             // are currently MRM.
    mesa_bool_t itst_to_cpu; // Copy MRP_InTest PDUs from remote MIM to CPU if
                             // we are MIM-RC.
...
// MRP counters.
typedef struct {
    uint64_t tst_rx_count;  // Number of received valid MRP_Test frames subject
                            // to MRP_Test processing.
    uint64_t itst_rx_count; // Number of received valid MRP_InTest frames
                            // subject to MRP_InTest processing.
