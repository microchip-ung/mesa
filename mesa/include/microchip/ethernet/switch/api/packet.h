...
typedef enum {
    ...
    MESA_PACKET_OAM_TYPE_CCM_LM,    // Continuity Check Message with Loss
    ...
    MESA_PACKET_OAM_TYPE_DLR_BCN,   // DLR Beacon frame (CIP Networks Library
                                    // Volume 2)
    MESA_PACKET_OAM_TYPE_DLR_ADV,   // MRP Advertise frame (CIP Networks Library
                                    // Volume 2)
...
typedef enum {
    ...
    MESA_PACKET_ENCAP_TYPE_ETHER, // PDU as indicated in ptp_action or oam_type
                                  // ETHERNET encapsulated (no IP)
    MESA_PACKET_ENCAP_TYPE_IP4,   // PDU as indicated in ptp_action IPV4
                                  // encapsulated
    MESA_PACKET_ENCAP_TYPE_IP6,   // PDU as indicated in ptp_action IPV6
                                  // encapsulated
...
typedef struct {
    // The Ethernet type of the received frame, needed to decode
...
//
// In some cases, a frame received by the CPU should be dropped. This cannot
// always be detected by just looking at the decoded extraction header.
//
// This enum provides the caller of mesa_packet_rx_hdr_decode() with a means to
// determine whether to drop or forward the frame further to the application.
// Whether to actually drop it is fully up to the application.
//
...
typedef enum {
    ...
    // If a frame is received on a C-port with a "foreign" tag (i.e. an S-tag or
    // S-custom-tag), the frame should
    // possibly be dropped. This is indicated with this enum being member of the
    // hints flags.
    // The same goes for frames received on S-ports or S-custom-ports with
    // "foreign" tags.
    MESA_PACKET_RX_HINTS_VLAN_TAG_MISMATCH = 0x01,

    // If a tagged frame is received on a port that only should accept untagged
    // frames or if an untagged frame
    // is received on a port that should only accept tagged frames, then this
    // will be set.
    // In general, the application should not drop such frames, because e.g.
    // BPDUs are normally untagged.
...
// One indicating the value of the parameter (e.g. #tstamp_id), and another
// indicating if the parameter is actually decoded or not (e.g.
// #tstamp_id_decoded).
//
// The reason for having an XXX_decoded boolean for every parameter is that the
// information held in the extraction header is very different on the various
// devices, and even on the same chip type, the information may be overloaded,
// depending on the incoming frame type.
//
// Most parameters don't have a decoded field, and in that case, they are always
// decoded.
typedef struct {
    ...
    // This may be MESA_PORT_NO_NONE, if the physical source port is not a part
    // of the port map.
    ...
    // This is not set by the API, but may be used by the application to strip a
    ...
    // timestamps. On Serval, this field identifies Tx timestamps. Rx timestamps
    ...
    //
    // Serval: Two-step PTP frames have the 32-bit Rx timestamp saved in this
    // field.
    uint64_t hw_tstamp;
    ...
    // or if it's not due to sFlow in the first place (MESA_SFLOW_TYPE_NONE).
    mesa_sflow_type_t sflow_type;

    // sFlow port, if #sflow_type != MESA_SFLOW_TYPE_NONE.
    // Indicates the logical sFlow Rx or Tx port number that caused this frame
...
typedef struct {
    // Switch frame option.
    // If TRUE, the device will do switching lookups to determine the
    // destination port set. The frame must then include a VLAN tag for correct
    // classification. If FALSE, the destination port set is specified with
    // #dst_port_mask.
    ...
    //
    // #switch_frm == TRUE:
    //   The tag fields may be used for classification depending on the
    //   architecture.
    //
    // #switch_frm == FALSE:
    //   By default, the frame is sent without any modification on each egress
    //   port. If tag.tpid is zero and tag.vid is non-zero, frame modifications
    //   on egress will be enabled.
    mesa_vlan_tag_t tag;

    // The QoS class that this frame will be transmitted on (egress queue
    // number). If #switch_frm is FALSE, MESA_PRIO_SUPER may be used for
    // super-priority transmission.
    mesa_prio_t cos;
    ...
    // The PTP identifier, used when #ptp_action ==
    // MESA_PACKET_PTP_ACTION_TWO_STEP. The CPU can only use from IDs 0 through 3.
    uint8_t ptp_id;

    // PTP timestamp indicating when the injection started, used when
    // #ptp_action is != MESA_PACKET_PTP_ACTION_NONE. The rewriter can then
    // calculate a residence time based on this and the frame's transmission
    // timestamp. This is in 16 bit fraction of nano seconds. On Jaguar2 and
    // Fireant: Allocated timestamp id (TX FIFO) for a two step transmission
    // must be in the nano second part (id << 16) Sequence counter index for a
    // PTP  transmission must be in the nano second part (index << 16)
    uint64_t ptp_timestamp;

    // OAM type, used if #ptp_action is MESA_PACKET_PTP_ACTION_NONE (not used
    // for Luton26).
    ...
    // Ingress flow ID used for egress lookup, if not MESA_IFLOW_ID_NONE (not
    // used for Luton26).
    mesa_iflow_id_t iflow_id;

    // Masquerade port, used if #switch_frm is TRUE (not used for Luton26).
    // When masquerading, the frame will be handled as if it was received on the
    // masquerade port. Set to MESA_PORT_NO_NONE to disable masquerading.
    ...
    // There are a number of sequence number update counters. The 'sequence_idx'
    // is a pointer to a counter.
...
// MESA_arch_PACKET_HDR_SIZE_BYTES.
//
// info            [IN]   Tx header properties.
// bin_hdr_max_len [IN]   If bin_hdr is non-NULL, the maximum length of the
// binary injection header in bytes bin_hdr         [OUT]  NULL to get
// bin_hdr_len filled with required length in bytes.
//                        Non-NULL to get it filled in with the binary injection
//                        header.
// bin_hdr_len     [OUT]  If bin_hdr is NULL, the required length of binary
// injection header in bytes.
...
//
// frm                          [IN]  Pointer to the PHY inserted RX time in the
// packet rx_info                      [IN]  RX info structure message_type [IN]
// Type of the PTP message ts_props                     [IN]  Time stamping
