 ...
 * \param inst     [IN] handle to an API instance.
 * \param ts       [IN] pointer to a TimeStamp structure.
 * \param negative [IN] True if ts is subtracted from current time, else ts is
 * added. Architecture: All
 *
 * \return Return code.
 */
 ...
 * On Serval1:
 * Must be called after every one-second timer synchronization pulse interrupt
 * (MEBA_EVENT_SYNC)
 *
 ...
 * srvl_ts_timeofday_offset_set() + srvl_ts_timeofday_set() +
 * srvl_ts_timeofday_set_delta() will return error code
 *
 * \param inst [IN]     handle to an API instance.
 ...
 *  JR2,
 *  S5     : it must only be called when the PPS output pin is low, therefore it
 * shall be called at least 200 microseconds after the 1PPS interrupt
 *
 * \return Return code.
 */
 ...
 *  Jaguar2: tc = free running nanoseconds counter.  In 16 bit fraction of nano
 * seconds. SparX-5: tc = (seconds + nanoseconds + fractional nanoseconds). In
 * 16 bit fraction of nano seconds.
 *
 * \return Return code.
 */
mesa_rc mesa_ts_timeofday_get(const mesa_inst_t       inst,
                              mesa_timestamp_t *const ts,
                              uint64_t *const         tc) CAP(TS);

/**
 * \brief Get the current raw time in a Timestamp format, and the corresponding
 *        time counter. (like mesa_ts_timeofday_get but without correction for
 * outstanding adjustments.) Used to be able to poll the time and call the onsec
 * function in architectures that do not support interrupt.
 * \param inst [IN]     handle to an API instance
 * \param ts [OUT]      pointer to a TimeStamp structure
 * \param tc [OUT]      pointer to a time counter (internal hw format)
 * Architecture:
...
/**
 * \brief Get the current time in timestamp format from multiple domains
 * simultanesouly at same instance of time. This will be useful in comparing two
 * different clocks.
 * \param inst       [IN]   handle to an API instance.
 * \param domain_cnt [IN]   Number of supported clock domains from which
 * timestamps must be read.
 * \param ts         [OUT]  Array containing timestamps from each clock domain
 * at the time of execution. Timestamps are read starting from clock domain 0.
 * Architecture:
 ...
 * \param inst [IN]     handle to an API instance.
 * \param adj [IN]      Clock ratio frequency offset in units of 0,1 ppb (parts
 * pr billion). ratio > 0 => clock runs faster
 *
 * \return Return code.
 */
mesa_rc mesa_ts_adjtimer_set(const mesa_inst_t inst, const int32_t adj) CAP(TS);
 ...
 * \param inst   [IN]     handle to an API instance.
 * \param domain [IN]     Clock domain.
 * \param adj    [IN]     Clock ratio frequency offset in units of 0,1 ppb
 ...
 * \param inst   [IN]     handle to an API instance.
 * \param domain [IN]     Clock domain.
 * \param adj    [OUT]    Clock ratio frequency offset in units of 0,1 ppb
 * (parts pr billion). ratio > 0 => clock runs faster
 *
 * \return Return code.
 */
 ...
 * \param inst [IN]     handle to an API instance.
 * \param adj [OUT]     Clock ratio frequency offset in ppb (parts pr billion).
 *                      ratio > 0 => internal clock runs faster than external
 * clock
 *
 * \return Return code.
 */
 ...
 *
 * \param inst [IN]             handle to an API instance
 * \param saved [OUT]           latest saved value. In 16 bit fraction of nano
 * seconds.
 *
 * \return Return code.
 */
...
typedef struct mesa_ts_ext_clock_mode_t {
    ...
    uint32_t freq;      /**< clock output frequency (hz [1..25.000.000]) when
                           'enable' == True. */
 ...
 * \param inst    [IN]             handle to an API instance
 * \param io      [IN]             Input/Output pin number [0..3]
 * \param offset  [OUT]            The last output clock edge was generated with
 * this offset. Value 256 is one nano second.
 *
 * \return Return code.
 */
mesa_rc mesa_ts_output_clock_edge_offset_get(const mesa_inst_t inst,
                                             const uint32_t    io,
                                             uint32_t *const   offset) CAP(TS);
...
mesa_rc mesa_ts_p2p_delay_get(const mesa_inst_t          inst,
                              const mesa_port_no_t       port_no,
                              mesa_timeinterval_t *const p2p_delay) CAP(TS);
 ...
 * \param inst [IN]             handle to an API instance
 * \param port_no [IN]          port number
 * \param delay_asymmetry [IN]  delay asymmetry. The (signed) delay is
 * subtracted on egress and added on ingress.
 *
 * \return Return code.
 */
...
typedef struct mesa_ts_timestamp_t {
    ...
    mesa_bool_t ts_valid; /**< Timestamp is valid (can be not valid if timestamp
                             is not received */
 ...
 * \return Return code.
 */
mesa_rc mesa_tx_timestamp_update(const mesa_inst_t inst) CAP(TS);
...
mesa_rc mesa_rx_timestamp_get(const mesa_inst_t          inst,
                              ...
                              mesa_ts_timestamp_t *const ts) CAP(TS);
...
typedef void (*mesa_ts_timestamp_alloc_cb_t)(void                *context,
                                             uint32_t             port_no,
 ...
 * \return Return code.
 */
mesa_rc mesa_timestamp_age(const mesa_inst_t inst) CAP(TS);

/**
 * \brief Signal port status change (used to detect and compensate for the
 * internal ingress and egress latencies)
 * \param inst    [IN]          handle to an API instance
 * \param port_no [IN]          port number
 *
 * \return Return code.
 ...
 *     DelayResponce portIdentity.portNumber = (ptp_port_msb & ~bitMask) | ((API
 * Port number) & bitMask) ptp_port_individual FALSE: DelayResponce
...
mesa_rc mesa_ts_smac_get(const mesa_inst_t    inst,
                         const mesa_port_no_t port_no,
                         mesa_mac_t *const    smac) CAP(TS_DELAY_REQ_AUTO_RESP);

/**
 * \brief Set auto response behavior pr. domain.
 * \param inst    [IN]          handle to an API instance
 * \param sec_cntr[IN]          sequence counter number [0..255]. See
 * mesa_packet_tx_info_t.sequence_idx and mesa_packet_tx_info_t.ptp_timestamp.
