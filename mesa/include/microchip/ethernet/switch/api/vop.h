 ...
 *   Injection of OAM frames can be done using AFI or CPU single frame
 * injection. Hitting a Service VOE requires a TCE entry that maps Tx OAM frames
 * to VOE. Hitting a Port VOE requires that the frame did not hit a Service VOE.
 ...
 *   On interrupt from VOE (or any time) any active event source can be
 * determined. 7 Status information. Basic status information can be retrieved .
 ...
 * 3 Inject OAM frames.
 *   Injection of OAM frames can be done using AFI or CPU single frame
 * injection. Hitting a VOI is not required. 4 Extract OAM frames hitting VOI on
 * ingress. Extraction of frames can be controlled through VOI configuration.
...
// VOP configuration. Once the VOP is configured, VOEs and MRPs (see mrp.h) can
// be configured.
typedef struct {
    ...
    // auto_copy_xxx members are auto copy to CPU timer period selector for PDU
    // types. Value 0 or 1 select one of the two periods in 'auto_copy_period'
...
typedef enum {
    MESA_VOE_DMAC_CHECK_UNICAST,   // Check unicast DMAC against VOE configured
...
// }
//
// The above will give a clear timer that is longer that specified in
// G.8021 6.1.3. The clear timer should have been started when the last CCM PDU
// with unexpected Period was received.
//
// inst     [IN]  Target instance reference.
// voe_idx  [IN]  Index of the VOE instance.
