...
typedef struct {
    uint32_t fps; // Frame rate (in frames per second) at which a frame should be transmitted.
...
// For future compatibility, memset() this structure to 0 before filling it in.
typedef struct {
    // On some platforms, it may not be possible to achieve the requested frame rate in one single go.
    // A work-around for this is to inject the same frame with different frame rates.
    // Typically, a particular desired frame rate can be obtained by transmitting the same frame a number of times.
    // mesa_afi_alloc() fills in #actual_fps with the number of frames that the AFI will actually send.
    // It is guaranteed that #actual_fps <= #fps, so that mesa_afi_alloc() can be called repeatedly with the
    // missing frame count.
    // The API supports two different allocation methods.
    // Which one is currently active depends on the value of the MESA_OPT_AFI_OPTIMIZE_FOR_TIMERS compile-time flag.
    // If MESA_OPT_AFI_OPTIMIZE_FOR_TIMERS is 0 (default/legacy), the AFI code attempts to use as few frame-slots
    // as possible to achieve the desired rate. This is at the expense of timers.
    // Setting MESA_OPT_AFI_CAL_TYPE to 1 causes the AFI code to optimize for timers rather than frame-slots.
    // This might be a better solution, because there are much more frame-slots than timers.
    // When optimizing for timers, each timer is pre-configured to provide a fixed number of frames per second.
    // In this mode the application may have to call into the mesa_afi_alloc() function more times than had it been
    // in the "optimize-for-frame-slots" mode.
    uint32_t fps;
...
// The function returns an ID that can be used in subsequent calls to mesa_packet_tx_hdr_encode() and mesa_afi_free().
// The ID is only valid for one single frame.
// dscr   [IN]   Pointer to structure describing how the frame should be injected.
// actual [OUT]  Pointer to structure describing how the actual frame rate.
// id     [OUT]  Pointer receiving a unique ID that identifies the allocated resources for this frame.
...
// This function cancels any ongoing transmissions of the frame associated with the ID passed to this function,
// and frees the allocated resources.
// id [IN]  ID of AFI frame to cancel and free resources for.
mesa_rc mesa_afi_free(const mesa_inst_t inst,
                      mesa_afi_id_t     id)
    CAP(AFI_V1);

// Check that the frame just transmitted to the AFI indeed gets to get known by the AFI.
// After the frame is injected, make sure it gets known by H/W by calling this function.
// Especially if the already-applied AFI rate on the port this frame is supposed to egress is very high
// (higher than the port's link rate), will there be a chance that subsequent frames sent from S/W to the AFI
// won't make it to the AFI, because all B/W from the queue system towards the port is in use by the AFI itself,
// which has higher Tx priority than other frames from the queue system.
// You will not need to call this function if you know that the rate towards a given port is lower than
// the link speed of that port.
// id [IN]  ID of AFI frame to cancel and free resources for.
...
typedef struct {
    ...
    // The reason that the destination port number must be conveyed to the AFI in addition to conveying it
    // in the IFH is that once the AFI starts injection, it doesn't have access to the IFH.
    mesa_port_no_t port_no;

    // Priority on which the frame sequence shall be transmitted.
    // Valid values are in the range [0; MESA_PRIO_CNT[ and [MESA_PRIO_SUPER; MESA_PRIO_SUPER + 1].
    mesa_prio_t prio;

    // Number of frames in sequence, i.e. number of frames to be hijacked using mesa_afi_fast_inj_frm_hijack().
    // Maximum is MESA_AFI_FAST_INJ_FRM_CNT_MAX.
    uint32_t frm_cnt;

    // For up-flows, that is, when setting #port_no to MESA_PORT_NO_NONE, convey the port number on which
    // the flow ingresses, that is, the masquerade port. It is not used unless #port_no is set to MESA_PORT_NO_NONE.
...
// Free AFI fast injection resource. Before resources are freed, fast injection must be stopped.
...
typedef struct {
    // Frame size in bytes.
    // This is only used for calculating bandwidth and need not match the actual frame size.
    // If IFG and preamble shall be included in the bandwidth calculation, then this must be added to frm_size.
    uint32_t frm_size;
} mesa_afi_fast_inj_frm_cfg_t CAP(AFI_V2);

// Setup frame for fast injection.
// When a sequence of frames is configured, then the frame order is the order in which frames are setup.
// fastid [IN]  Fast injection ID.
// cfg    [IN]  Frame setup configuration.
...
typedef struct {
    ...
    // Number of sequences to inject. Value zero means inject until stopped. Maximum value is 2^31-1.
    uint32_t seq_cnt;
} mesa_afi_fast_inj_start_cfg_t CAP(AFI_V2);

// Structure defining properties of a fast injection.
typedef struct {
    // Actual injection rate (Bits per second), always smaller than or equal to the requested rate.
...
// If seq_cnt is set to 0 in mesa_afi_fast_inj_start() then the value read will be the number of frames injected.
// Otherwise the value read will be the remaining number of sequences to inject.
// fastid  [IN]   Fast injection ID.
...
// Free AFI slow injection resource. Before resources are freed, slow injection must be stopped.
...
typedef struct {
    ...
    // If FALSE, first frame is transmitted a randomized number of timer ticks after call to
    // mesa_afi_slow_inj_start() to minimize risk of bursting in case many flows are started "simultaneously".
