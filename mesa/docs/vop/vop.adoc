...
==== [#bookmark-ace]#ACE#.
If MESA capability `MESA_CAP_VOP_V1` is not zero the following applies::
On Port 2-3 one ACE entry is configured (`mesa_ace_t`) with key matching on VOI-PAG value and opcode RAPS (and RAPS MC-MAC). Action is to control copy to CPU and forwarding. +
 +
On Port 2-3 one ACE entry is configured (`mesa_ace_t`) with key matching on VOI-PAG value and opcode LTM (and Class-2 MC-MAC). Action is to control copy to CPU and forwarding. +
 +
On Port 2-3 one ACE entry is configured (`mesa_ace_t`) with key matching on VOI-PAG value and opcode LBM (and UC-MAC). Action is to control copy to CPU and forwarding. +
+
IMPORTANT: Note that the VOI-PAG value can be used by multiple/all VOIs. The RAPS and LTM ACE rules can be used by multiple/all VOIs. The LBM ACE must be per VOI due to different UC-MAC.
