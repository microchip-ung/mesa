...
/**
	\brief This API controls how packets that do not match any of the SA
		   matching rules are handled for a received packet when explicit
		   classification is enabled at the ingress packet interface. The result
		   of the control packet detector is overruled by the externally
		   provided indication.

	\param phy Pointer to GPHY data (\ref gpy211_device).
	\param nm_expl Pointer to SA No-Match flow ctrl action for explicit classified
			   frame (\ref sa_nonmatch_fca).
...
/**
	\brief This API controls how packets that do not match any of the SA
		   matching rules are handled for a transmitted packet when explicit
		   classification is enabled at the egress packet interface. The result
		   of the control packet detector is overruled by the externally
		   provided indication.
