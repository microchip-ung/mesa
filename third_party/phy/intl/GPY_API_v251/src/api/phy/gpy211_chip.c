		      ...
		      struct gpy211_led_brlvl_cfg *brtness_cfg)
{
	...
	/* Validate Sensing Period range */
	if (!(brtness_cfg->sensing_period >= LED_SENSING_MIN &&
	      brtness_cfg->sensing_period <= LED_SENSING_MAX)) {
		LOG_WARN("WARN: Invalid input (sensingPeriod) - Sensing Period\n");
		return -EINVAL;
	}

	/* Validate Discharge Slots range */
	if (brtness_cfg->discharge_slots < LED_DIS_SLOTS_MIN ||
	    brtness_cfg->discharge_slots > LED_DIS_SLOTS_MAX) {
		LOG_WARN("WARN: Invalid input (dischargeSlots) - Discharge Slots\n");
		return -EINVAL;
	}
	...
	/* Set LED Brightness 2 Level Switch Enable */
	if ((brtness_cfg->mode == ED_BRLVL_2LVL) ||
	    (brtness_cfg->mode == LED_BRLVL_MIX)) {
		FIELD_REPLACE(_brtness_cfg, 1, LED_BCTRL_PDI_REGISTERS_LED_BRT_CTRL_TWOSWEN);
	} else {
	...
	/* Read LED Sensing Period configuration */
	ret = SMDIO_READ(phy, LED_BCTRL_PDI_REGISTERS_LED_BCTRL_PDI_REGISTERS_LED_LSENS_CTRL);

	if (ret < 0) {
		LOG_CRIT("ERROR: LED_LSENS_CTRL read fails\n");
		goto END;
	}

	_sensing_cfg = (u16)ret;

	/* Modify LED Sensing configuration */
	/* Set LED Sensing Period */
	FIELD_REPLACE(_sensing_cfg, brtness_cfg->sensing_period,
		      LED_BCTRL_PDI_REGISTERS_LED_LSENS_CTRL_PERIOD);

	/* Set LED Brightness 16 Level Light Sensing Enable */
	if (brtness_cfg->mode >= LED_BRLVL_16LVL) {
		FIELD_REPLACE(_sensing_cfg, 1, LED_BCTRL_PDI_REGISTERS_LED_LSENS_CTRL_SENS_EN);
	} else {
		FIELD_REPLACE(_sensing_cfg, 0, LED_BCTRL_PDI_REGISTERS_LED_LSENS_CTRL_SENS_EN);
	}

	/* Set LED's Number of Slots for Discharge */
	FIELD_REPLACE(_sensing_cfg, brtness_cfg->discharge_slots,
		      LED_BCTRL_PDI_REGISTERS_LED_LSENS_CTRL_TD);
	...
	/* Write LED Sensing Period configuration */
	ret = SMDIO_WRITE(phy, LED_BCTRL_PDI_REGISTERS_LED_BCTRL_PDI_REGISTERS_LED_LSENS_CTRL,
			  _sensing_cfg);

	if (ret < 0) {
		LOG_CRIT("ERROR: LED_LSENS_CTRL write fails\n");
		goto END;
	}
...
{
	u16 _brtness_cfg, _sensing_cfg;
	u16 _brt_ctrl_en, _brt_ctrl_twosen, _led_lsens_en;
	int ret = -EINVAL;
	LOG_INFO("Entering func name :- %s\n", __func__);
	...
	/* If LED Light Sensing Enable */
	if ((_led_lsens_en = FIELD_GET(_sensing_cfg, LED_BCTRL_PDI_REGISTERS_LED_LSENS_CTRL_SENS_EN))) {
		/* Get LED Sensing Period */
		brtness_cfg->sensing_period = FIELD_GET(_sensing_cfg, LED_BCTRL_PDI_REGISTERS_LED_LSENS_CTRL_PERIOD);
	}

	/* Get LED's Number of Slots for Discharge */
	brtness_cfg->discharge_slots = FIELD_GET(_sensing_cfg, LED_BCTRL_PDI_REGISTERS_LED_LSENS_CTRL_TD);

	if ((_brt_ctrl_twosen == 1) && (_led_lsens_en == 1))
		brtness_cfg->mode = LED_BRLVL_MIX;
	else if ((_brt_ctrl_twosen == 1) && (_brt_ctrl_en == 1) && (_led_lsens_en == 0))
		brtness_cfg->mode = ED_BRLVL_2LVL;
	else if ((_brt_ctrl_twosen == 0) && (_brt_ctrl_en == 1) && (_led_lsens_en == 1))
		brtness_cfg->mode = LED_BRLVL_16LVL;
	else if ((_brt_ctrl_twosen == 0) && (_brt_ctrl_en == 1) && (_led_lsens_en == 0))
				   ...
				   time64_t ost_ctime)
{
	...
	ret = SMDIO_WRITE(phy, PM_PDI_PM_PDI_TX_OST_CFG_0, _ost_time);
	...
	/* Write Part of the 64-bit Tx OST Correction Timestamp */
	ret = SMDIO_WRITE(phy, PM_PDI_PM_PDI_TX_OST_CFG_1, _ost_time);
	...
	/* Write Part of the 64-bit Tx OST Correction Timestamp */
	ret = SMDIO_WRITE(phy, PM_PDI_PM_PDI_TX_OST_CFG_2, _ost_time);
	...
	/* Write Part of the 64-bit Tx OST Correction Timestamp */
	ret = SMDIO_WRITE(phy, PM_PDI_PM_PDI_TX_OST_CFG_3, _ost_time);
				   ...
				   time64_t *ost_ctime)
{
	...
	*ost_ctime = 0;
	...
	ret = SMDIO_READ(phy, PM_PDI_PM_PDI_TX_OST_CFG_0);
	...
	*ost_ctime = ret;
	...
	/* Get Part of the 64-bit Tx OST Correction Timestamp */
	ret = SMDIO_READ(phy, PM_PDI_PM_PDI_TX_OST_CFG_1);
	...
	*ost_ctime |= ((long long)ret << 16);
	...
	/* Get Part of the 64-bit Tx OST Correction Timestamp */
	ret = SMDIO_READ(phy, PM_PDI_PM_PDI_TX_OST_CFG_2);
	...
	*ost_ctime |= ((long long)ret << 32);
	...
	/* Get Part of the 64-bit Tx OST Correction Timestamp */
	ret = SMDIO_READ(phy, PM_PDI_PM_PDI_TX_OST_CFG_3);
				   ...
				   u8 sub_sec, u8 sub_nsec)
{
	...
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_TIMESTAMP_CONTROL_L);
	...
	if (0 != phy->ptp_clock) {
		sub_sec = SEC_TO_NANOSEC(1) / MHZ_TO_HZ(phy->ptp_clock);
		_rem_nsec = SEC_TO_NANOSEC(1) % MHZ_TO_HZ(phy->ptp_clock);
		_rem_nsec = _rem_nsec * 0x64;
		sub_nsec = _rem_nsec / MHZ_TO_HZ(phy->ptp_clock);
	}
	...
	if (1 == _tsc_low.dob_ro_ctrl) {
		...
		tmp = (u32)1000 * sub_sec / 465;

		if (tmp & ~0xFF) {
			LOG_CRIT("ERROR: sub_sec (%u) * 1000 / 465 exceeds 8-bit\n", (u32)sub_sec);
			return -EINVAL;
		}

		_sub_sec = (u8)tmp;

		tmp = (u32)1000 * sub_nsec / 465;

		if (tmp & ~0xFF) {
			LOG_CRIT("ERROR: sub_nsec (%u) * 1000 / 465 exceeds 8-bit\n", (u32)sub_nsec);
			return -EINVAL;
		}
	...
	ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_SUB_SECOND_INCREMENT_H, _sub_sec);
	ret2 = SMDIO_WRITE(phy, GMACL_EQOS_MAC_GMACL_MAC_SUB_SECOND_INCREMENT_H, _sub_sec);
	...
	/* Write Sub-second Increment Value */
	ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_SUB_SECOND_INCREMENT_L, (_sub_nsec << 8));
	ret2 = SMDIO_WRITE(phy, GMACL_EQOS_MAC_GMACL_MAC_SUB_SECOND_INCREMENT_L, (_sub_nsec << 8));
...
static int _gpy2xx_ptp_get_ssecinc(struct gpy211_device *phy,
				   u8 *sub_sec, u8 *sub_nsec)
{
	int ret = -EINVAL;
	u8 _sub_sec, _sub_nsec;
	...
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_SUB_SECOND_INCREMENT_H);

	if (ret < 0) {
		LOG_CRIT("ERROR: SUB_SECOND_INCREMENT_H read fails\n");
		return ret;
	}
	...
	/* Write Sub-second Increment Value */
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_SUB_SECOND_INCREMENT_L);
	...
	if (1 == _tsc_low.dob_ro_ctrl) {
		...
		/* If the System Time Nanoseconds register has a resolution of ~0.465 ns */
		_sub_sec = (465 * _sub_sec) / 1000;
		_sub_nsec = (465 * _sub_nsec) / 1000;
	}
				    ...
				    u32 nsec)
{
	...
	if (sec > MAX_SEC_IN_DAY || nsec > MAX_NSEC_IN_SEC) {
		LOG_WARN("WARN: Invalid sec or nano-sec input\n");
		return -EINVAL;
	}

	/* Make sure 'TSINIT' is in reset state before init'ing SysTime */
	ret = __is_rw_done(phy, GMACF_EQOS_MAC_GMACF_MAC_TIMESTAMP_CONTROL_L,
	...
	ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_SYSTEM_TIME_SECONDS_UPDATE_L, _sec_low);
	ret2 = SMDIO_WRITE(phy, GMACL_EQOS_MAC_GMACL_MAC_SYSTEM_TIME_SECONDS_UPDATE_L, _sec_low);
	...
	_sec_high = sec >> 16;
	ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_SYSTEM_TIME_SECONDS_UPDATE_H, _sec_high);
	ret2 = SMDIO_WRITE(phy, GMACL_EQOS_MAC_GMACL_MAC_SYSTEM_TIME_SECONDS_UPDATE_H, _sec_high);
	...
	if (1 == _tsc_low.dob_ro_ctrl) {
		...
		if (nsec > BIN_ROLLOVER_NSEC_MAX)
			nsec = BIN_ROLLOVER_NSEC_MAX;
	}

	/* Write low-part of 'System Time Nanoseconds' */
	_nsec_low = nsec & GMACF_MAC_SYSTEM_TIME_NANOSECONDS_UPDATE_L_TSSS_MASK;
	ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_SYSTEM_TIME_NANOSECONDS_UPDATE_L, _nsec_low);
	ret2 = SMDIO_WRITE(phy, GMACL_EQOS_MAC_GMACL_MAC_SYSTEM_TIME_NANOSECONDS_UPDATE_L, _nsec_low);
	...
	_nsec_high = nsec >> 16;
	ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_SYSTEM_TIME_NANOSECONDS_UPDATE_H, _nsec_high);
	ret2 = SMDIO_WRITE(phy, GMACL_EQOS_MAC_GMACL_MAC_SYSTEM_TIME_NANOSECONDS_UPDATE_H, _nsec_high);
	...
	ret = SMDIO_SET_BIT(phy, GMACF_EQOS_MAC_GMACF_MAC_TIMESTAMP_CONTROL_L,
			    GMACF_MAC_TIMESTAMP_CONTROL_L_TSINIT_POS);
	ret2 = SMDIO_SET_BIT(phy, GMACL_EQOS_MAC_GMACL_MAC_TIMESTAMP_CONTROL_L,
			     GMACL_MAC_TIMESTAMP_CONTROL_L_TSINIT_POS);
...
{
	int ret = -EINVAL, ret2 = -EINVAL;
	u32 _sec_updt, _nsec_updt, _dob_ro_ctrl;
	u16 _sec_low, _sec_high, _nsec_low, _nsec_high;
	struct gpy211_tsc_low _tsc_low;
	...
	_tsc_low.raw_ts_ctrl_low = ret;
	_dob_ro_ctrl = _tsc_low.dob_ro_ctrl;
	...
	if (add_sub) {
		...
		if (1 == _dob_ro_ctrl) {
			...
			/* If the System Time Nanoseconds register has a resolution of ~0.465 ns,
			 * then the programmed TSSS value should be 2 pow 31 - <sub-second_value>.
			 */
			_nsec_updt = 0x80000000 - nsec;
		}
	} else {
	...
	ret = SMDIO_SET_BIT(phy, GMACF_EQOS_MAC_GMACF_MAC_TIMESTAMP_CONTROL_L,
			    GMACF_MAC_TIMESTAMP_CONTROL_L_TSUPDT_POS);
	ret2 = SMDIO_SET_BIT(phy, GMACL_EQOS_MAC_GMACL_MAC_TIMESTAMP_CONTROL_L,
			     GMACL_MAC_TIMESTAMP_CONTROL_L_TSUPDT_POS);
...
static int _gpy2xx_ptp_set_addend(struct gpy211_device *phy, u32 addend)
{
	...
	ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_TIMESTAMP_ADDEND_L, _addend_low);
	ret2 = SMDIO_WRITE(phy, GMACL_EQOS_MAC_GMACL_MAC_TIMESTAMP_ADDEND_L, _addend_low);
	...
	ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_TIMESTAMP_ADDEND_H, _addend_high);
	ret2 = SMDIO_WRITE(phy, GMACL_EQOS_MAC_GMACL_MAC_TIMESTAMP_ADDEND_H, _addend_high);
	...
	ret = SMDIO_SET_BIT(phy, GMACF_EQOS_MAC_GMACF_MAC_TIMESTAMP_CONTROL_L,
			    GMACF_MAC_TIMESTAMP_CONTROL_L_TSADDREG_POS);
	ret2 = SMDIO_SET_BIT(phy, GMACL_EQOS_MAC_GMACL_MAC_TIMESTAMP_CONTROL_L,
			     GMACL_MAC_TIMESTAMP_CONTROL_L_TSADDREG_POS);
...
static int _gpy2xx_ptp_get_addend(struct gpy211_device *phy, u32 *addend)
{
	int ret = -EINVAL;
	u16 _addend_low, _addend_high;
	struct gpy211_tsc_low _tsc_low;
	...
	if (1 != _tsc_low.foc_updt) {
		LOG_CRIT("ERROR: Fine update (TSCFUPDT) method is not set in TIMESTAMP_CONTROL_L\n");
		return ret;
	}

	/* Make sure 'TSADDREG' is in reset state before reading Addend reg */
	ret = __is_rw_done(phy, GMACF_EQOS_MAC_GMACF_MAC_TIMESTAMP_CONTROL_L,
			   GMACF_MAC_TIMESTAMP_CONTROL_L_TSADDREG_POS);
	...
	/* Get low-part of 'Addend' */
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_TIMESTAMP_ADDEND_L);
	...
	/* Get high-part of 'Addend' */
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_TIMESTAMP_ADDEND_H);
	...
	*addend = (_addend_high << 16) | _addend_low;
		      ...
		      struct gpy211_ptp_ctrl *ptp_cfg)
{
	...
	struct gpy211_tsc_low _tsc_low;
	struct gpy211_tsc_high _tsc_hi;
	LOG_INFO("Entering func name :- %s\n", __func__);
	...
	ret = SMDIO_SET_BIT(phy, PM_PDI_PM_PDI_GMAC_CFG, PM_PDI_GMAC_CFG_GMAC_PTP_DET_POS);
	ret2 = SMDIO_SET_BIT(phy, PM_PDI_PM_PDI_GMACL_CFG, PM_PDI_GMACL_CFG_GMACL_PTP_DET_POS);
	...
	ret = SMDIO_SET_BIT(phy, GMACF_EQOS_MAC_GMACF_MAC_INTERRUPT_ENABLE_L, GMACF_MAC_INTERRUPT_ENABLE_L_TSIE_POS);
	ret2 = SMDIO_SET_BIT(phy, GMACL_EQOS_MAC_GMACL_MAC_INTERRUPT_ENABLE_L, GMACL_MAC_INTERRUPT_ENABLE_L_TSIE_POS);
	...
	_ptp_cfg.ts_ostcor_en = ptp_cfg->ts_ostcor_en;
	_ptp_cfg.tx_ts_phy = ptp_cfg->tx_ts_phy;
	_ptp_cfg.rx_ts_phy = ptp_cfg->rx_ts_phy;

	/* Set Timestamp config */
	ret = SMDIO_WRITE(phy, PM_PDI_PM_PDI_TS_CFG, _ptp_cfg.raw_ts_cfg);
	...
	if (ptp_cfg->ts_ostcor_en == 1) {
		...
		/* Set PTP config */
		ret = SMDIO_WRITE(phy, PM_PDI_PM_PDI_GMAC_PTP_CFG, _ptp_cfg.raw_ptp_cfg);
	...
	ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_TIMESTAMP_CONTROL_L, _tsc_low.raw_ts_ctrl_low);
	ret2 = SMDIO_WRITE(phy, GMACL_EQOS_MAC_GMACL_MAC_TIMESTAMP_CONTROL_L, _tsc_low.raw_ts_ctrl_low);
	...
	_tsc_hi.raw_ts_ctrl_high = 0;
	_tsc_hi.snap_type = 1;

	/* Set Timestamp Control high word */
	ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_TIMESTAMP_CONTROL_H, _tsc_hi.raw_ts_ctrl_high);
	ret2 = SMDIO_WRITE(phy, GMACL_EQOS_MAC_GMACL_MAC_TIMESTAMP_CONTROL_H, _tsc_hi.raw_ts_ctrl_high);
	...
	ret = _gpy2xx_ptp_set_ssecinc(phy, 1, 25);
	...
	ret = _gpy2xx_ptp_set_addend(phy, phy->def_addend);
...
int gpy2xx_ptp_disable(struct gpy211_device *phy)
{
	...
	ret = SMDIO_CLR_BIT(phy, PM_PDI_PM_PDI_GMAC_CFG, PM_PDI_GMAC_CFG_GMAC_PTP_DET_POS);
	ret2 = SMDIO_CLR_BIT(phy, PM_PDI_PM_PDI_GMACL_CFG, PM_PDI_GMACL_CFG_GMACL_PTP_DET_POS);
	...
	ret = SMDIO_CLR_BIT(phy, GMACF_EQOS_MAC_GMACF_MAC_INTERRUPT_ENABLE_L, GMACF_MAC_INTERRUPT_ENABLE_L_TSIE_POS);
	ret2 = SMDIO_CLR_BIT(phy, GMACL_EQOS_MAC_GMACL_MAC_INTERRUPT_ENABLE_L, GMACL_MAC_INTERRUPT_ENABLE_L_TSIE_POS);
	...
	_ptp_cfg = 0;
	/* Clear PTP config */
	ret = SMDIO_WRITE(phy, PM_PDI_PM_PDI_GMAC_PTP_CFG, _ptp_cfg);
	...
	ret = SMDIO_WRITE(phy, PM_PDI_PM_PDI_TS_CFG, _ts_cfg);
	...
	/* Set Timestamp Control high word */
	ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_TIMESTAMP_CONTROL_H, 0x0);
	ret2 = SMDIO_WRITE(phy, GMACL_EQOS_MAC_GMACL_MAC_TIMESTAMP_CONTROL_H, 0x0);
			  ...
			  struct gpy2xx_ts_ctrl *ts_cfg)
{
	...
	_ts_ctrl_low.en_all_pkts = ts_cfg->en_all_pkts;
	_ts_ctrl_low.ts_evtmsg_en = ts_cfg->ts_evtmsg_en;
	_ts_ctrl_low.ts_master_en = ts_cfg->ts_master_en;

	/* Set Timestamp Control low word */
	ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_TIMESTAMP_CONTROL_L, _ts_ctrl_low.raw_ts_ctrl_low);
	ret2 = SMDIO_WRITE(phy, GMACL_EQOS_MAC_GMACL_MAC_TIMESTAMP_CONTROL_L, _ts_ctrl_low.raw_ts_ctrl_low);
	...
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_TIMESTAMP_CONTROL_H);
	...
	_ts_ctrl_high.ts_stat_mode = ts_cfg->ts_stat_mode;

	/* Set Timestamp Control high word */
	ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_TIMESTAMP_CONTROL_H, _ts_ctrl_high.raw_ts_ctrl_high);
	ret2 = SMDIO_WRITE(phy, GMACL_EQOS_MAC_GMACL_MAC_TIMESTAMP_CONTROL_H, _ts_ctrl_high.raw_ts_ctrl_high);
	...
	if (ts_cfg->ssec_inc || ts_cfg->snsec_inc) {
		ret = _gpy2xx_ptp_set_ssecinc(phy, ts_cfg->ssec_inc, ts_cfg->snsec_inc);
				  ...
				  struct gpy2xx_ts_ctrl *ts_cfg)
{
	...
	_ts_ctrl_low.raw_ts_ctrl_low = ret;
	ts_cfg->en_all_pkts = _ts_ctrl_low.en_all_pkts;
	...
	ts_cfg->ts_stat_mode = _ts_ctrl_high.ts_stat_mode;

	ret = _gpy2xx_ptp_get_ssecinc(phy, &ts_cfg->ssec_inc, &ts_cfg->snsec_inc);

	if (ret < 0) {
		LOG_CRIT("ERROR: api _gpy2xx_ptp_get_ssecinc fails\n");
		goto END;
	}

	if (1 == _ts_ctrl_low.foc_updt) {
		ret = _gpy2xx_ptp_get_addend(phy, &ts_cfg->addend);

		if (ret < 0) {
			LOG_CRIT("ERROR: api _gpy2xx_ptp_get_addend fails\n");
			goto END;
		}
	}
			   ...
			   struct gpy2xx_pps_ctrl *pps_ctrl)
{
	...
	u16 _dob_ro_ctrl, _pps_ctrl_low;
	...
	/* Validate input params */
	if ((pps_ctrl->sec_tgttime > MAX_SEC_IN_DAY) ||
	    (pps_ctrl->nsec_tgttime > MAX_NSEC_IN_SEC) ||
	...
	ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_PPS_CONTROL_L, _pps_ctrl_low);
	...
	ret = __is_rw_done(phy, GMACF_EQOS_MAC_GMACF_MAC_PPS0_TARGET_TIME_NANOSECONDS_H,
	...
	ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_PPS0_TARGET_TIME_SECONDS_L, _sec_tt_low);
	...
	ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_PPS0_TARGET_TIME_SECONDS_H, _sec_tt_hi);
	...
	if (1 == _dob_ro_ctrl) {
		...
		tmp = 1000ULL * pps_ctrl->nsec_tgttime;
		_ttime_nsec = div_u64_rem(tmp, 465, NULL);

		if (_ttime_nsec > BIN_ROLLOVER_NSEC_MAX)
			_ttime_nsec = BIN_ROLLOVER_NSEC_MAX;
	}

	_nsec_tt_low = _ttime_nsec & GMACF_MAC_PPS0_TARGET_TIME_NANOSECONDS_L_TTSL0_MASK;
	/* Set PPS Target Time Nano-Seconds - low word */
	ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_PPS0_TARGET_TIME_NANOSECONDS_L, _nsec_tt_low);
	...
	ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_PPS0_TARGET_TIME_NANOSECONDS_H, _nsec_tt_hi);
	...
	ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_PPS0_INTERVAL_L, _units_int_low);
	...
	ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_PPS0_INTERVAL_H, _units_int_hi);
	...
	ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_PPS0_WIDTH_L, _units_wid_low);
	...
	/* Set PPS Interval - high word */
	ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_PPS0_WIDTH_H, _units_wid_hi);
				   ...
				   struct gpy2xx_pps_ctrl *pps_ctrl)
{
	...
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_PPS_CONTROL_L);
	...
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_PPS0_TARGET_TIME_SECONDS_L);
	...
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_PPS0_TARGET_TIME_SECONDS_H);
	...
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_PPS0_TARGET_TIME_NANOSECONDS_L);
	...
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_PPS0_TARGET_TIME_NANOSECONDS_H);
	...
	if (1 == _dob_ro_ctrl) {
		...
		/* If the System Time Nanoseconds register has a resolution of ~0.465 ns */
		_ttime_nsec = (465 * _ttime_nsec) / 1000;
	}

	pps_ctrl->nsec_tgttime = _ttime_nsec;

	/* Get PPS Interval - low word */
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_PPS0_INTERVAL_L);
	...
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_PPS0_INTERVAL_H);
	...
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_PPS0_WIDTH_L);
	...
	/* Get PPS Interval - high word */
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_PPS0_WIDTH_H);
			   ...
			   struct gpy2xx_pto_ctrl *pto_ctrl)
{
	...
	ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_PTO_CONTROL_L, pto_ctrl->ptoctrl_low.raw_pto_ctrl_low);
	...
	for (i = 0; i < PTP_SPORT_ID_LEN_HWORD; i++) {
		...
		ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_SOURCE_PORT_IDENTITY0_L + (i * 4), _sport_id[i]);
	...
	ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_LOG_MESSAGE_INTERVAL_L, pto_ctrl->logmsg_lvl.raw_logm_lvl_low);
	...
	ret = SMDIO_WRITE(phy, GMACF_EQOS_MAC_GMACF_MAC_LOG_MESSAGE_INTERVAL_H, pto_ctrl->logmsg_lvl.raw_logm_lvl_hi);
				   ...
				   struct gpy2xx_pto_ctrl *pto_ctrl)
{
	...
	/* Set PTO Control low word */
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_PTO_CONTROL_L);
	...
	for (i = 0; i < PTP_SPORT_ID_LEN_HWORD; i++) {
		/* Set Source Port Identity 0 */
		ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_SOURCE_PORT_IDENTITY0_L + (i * 4));
	...
	/* Set Log Message Interval - low word */
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_LOG_MESSAGE_INTERVAL_L);
	...
	/* Set Log Message Interval - high word */
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_LOG_MESSAGE_INTERVAL_H);
...
int gpy2xx_ptp_adjfreq(struct gpy211_device *phy, s32 ppb)
{
	...
	def_addend = phy->def_addend;

	/* Actual addend in integer arithmetic becomes
	 * addend = (def_addend * 1000000000) / (1000000000 - ppb)
	 */
	adj = (def_addend * SEC_TO_NANOSEC(1));
	diff = (SEC_TO_NANOSEC(1) - ppb);

	addend = div_u64_rem(adj, diff, NULL);

	ret = _gpy2xx_ptp_set_addend(phy, addend);
...
int gpy2xx_ptp_gettime(struct gpy211_device *phy, struct timespec64 *ts)
{
	...
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_SYSTEM_TIME_SECONDS_L);
	...
	/* Get Seconds Part of the System Time */
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_SYSTEM_TIME_SECONDS_H);
	...
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_SYSTEM_TIME_NANOSECONDS_L);
	...
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_SYSTEM_TIME_NANOSECONDS_H);
	...
	if (1 == _tsc_low.dob_ro_ctrl) {
		...
		/* If the System Time Nanoseconds register has a resolution of ~0.465 ns */
		ts->tv_nsec = ((465 * (ts->tv_nsec)) / 1000);
	}
			...
			struct gpy211_ts_fifo_stat *fifo_status)
{
	...
	/* Read FIFO statu */
	ret = SMDIO_READ(phy, PM_PDI_PM_PDI_TS_FIFO_STAT);
	...
	/* Write 1 to clear */
	ret = SMDIO_WRITE(phy, PM_PDI_PM_PDI_TS_FIFO_STAT, _fifo_status);
...
int gpy2xx_ptp_resetfifo(struct gpy211_device *phy)
{
	...
	ret = SMDIO_READ(phy, PM_PDI_PM_PDI_TS_CFG);
			...
			struct gpy211_ts_stat *ts_status)
{
	...
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_TIMESTAMP_STATUS_L);
	...
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_TIMESTAMP_STATUS_H);
	...
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_TX_TIMESTAMP_STATUS_NANOSECONDS_L);
	...
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_TX_TIMESTAMP_STATUS_NANOSECONDS_H);
	...
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_TX_TIMESTAMP_STATUS_SECONDS_L);
	...
	ret = SMDIO_READ(phy, GMACF_EQOS_MAC_GMACF_MAC_TX_TIMESTAMP_STATUS_SECONDS_H);
		       ...
		       struct gpy211_rx_ts *rx_ts)
{
	...
	_rx_ts = SMDIO_READ(phy, PM_PDI_PM_PDI_GMAC_RX_TS_STAT_0);
	...
	/* Get Part of the 64-bit Rx Timestamp */
	_rx_ts = SMDIO_READ(phy, PM_PDI_PM_PDI_GMAC_RX_TS_STAT_1);
	...
	/* Store Part of the 64-bit Rx Timestamp */
	_ts_nsec |= ((_rx_ts & 0x7FFFF) << 16);

	//printf("\nNano-sec %x", _ts_nsec);
	/* Get Part of the 64-bit Rx Timestamp */
	_rx_ts = SMDIO_READ(phy, PM_PDI_PM_PDI_GMAC_RX_TS_STAT_2);
	...
	/* Get Part of the 64-bit Rx Timestamp */
	_rx_ts = SMDIO_READ(phy, PM_PDI_PM_PDI_GMAC_RX_TS_STAT_3);
	...
	_rx_crc = SMDIO_READ(phy, PM_PDI_PM_PDI_GMAC_RX_CRC_STAT_0);
	...
	/* Get Part of the 32-bit Tx Frame CRC */
	_rx_crc = SMDIO_READ(phy, PM_PDI_PM_PDI_GMAC_RX_CRC_STAT_1);
		       ...
		       struct gpy211_tx_ts *tx_ts)
{
	...
	_tx_ts = SMDIO_READ(phy, PM_PDI_PM_PDI_GMAC_TX_TS_STAT_0);
	...
	/* Get Part of the 64-bit Tx Timestamp */
	_tx_ts = SMDIO_READ(phy, PM_PDI_PM_PDI_GMAC_TX_TS_STAT_1);
	...
	/* Store Part of the 64-bit Tx Timestamp */
	_ts_nsec |= ((_tx_ts & 0x7FFF) << 16);
	//printf("\nNano-sec %x", _ts_nsec);

	/* Get Part of the 64-bit Tx Timestamp */
	_tx_ts = SMDIO_READ(phy, PM_PDI_PM_PDI_GMAC_TX_TS_STAT_2);
	...
	/* Get Part of the 64-bit Tx Timestamp */
	_tx_ts = SMDIO_READ(phy, PM_PDI_PM_PDI_GMAC_TX_TS_STAT_3);
	...
	tx_ts->tx_crc_stat = 0;
	/* Get Part of the 32-bit Tx Frame CRC */
	_tx_crc = SMDIO_READ(phy, PM_PDI_PM_PDI_GMAC_TX_CRC_STAT_0);
	...
	/* Get Part of the 32-bit Tx Frame CRC */
	_tx_crc = SMDIO_READ(phy, PM_PDI_PM_PDI_GMAC_TX_CRC_STAT_1);
		      ...
		      struct gpy211_ptp_cfg *ptp_cfg)
{
	...
	ret = SMDIO_READ(phy, PM_PDI_PM_PDI_GMAC_PTP_CFG);
