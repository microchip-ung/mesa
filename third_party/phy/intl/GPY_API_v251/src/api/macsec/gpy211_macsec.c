...
static int _gpy2xx_msec_detect_ing_eip160(struct gpy211_device *phy)
{
	...
	LOG_CRIT("\nEIP num raw %x, stored %x", _data, eip_ver.raw_eip160_ver);
	LOG_CRIT("\nEIP num %x compliment %x, patch-level %x, minor-ver %x, major-ver %x, reserved %x",
...
static int _gpy2xx_msec_config_ing_sa_nm_xcp(struct gpy211_device *phy)
{
	...
	struct sa_nonmatch_fca nm_expl = {
		.pkt_type = SA_NM_SECY_UNTAGGED,
		.flow_type = SA_NM_FCA_FLOW_DROP,
		.dest_port = SA_NM_FCA_DPT_CONTROL,
		.drop_non_reserved = 0,
		.drop_action = SA_NM_FCA_DROP_AS_CRC,
	...
	/* Set 'SA_NM_FLOW_EXPL flow control action */
	ret = gpy2xx_msec_config_ing_sa_nm_expl(phy, &nm_expl);

	if (ret < 0) {
		LOG_CRIT("ERROR: api gpy2xx_msec_config_ing_sa_nm_expl fails\n");
		goto END;
	}
...
int gpy2xx_msec_init_ing_dev(struct gpy211_device *phy)
{
	...
	pr_debug("The GPHY device addr = %px", phy);
	return 1;
#endif
	...
	LOG_CRIT("\nEIP cfg raw %x, stored %x", _data, eip_cfg.raw_eip160_cfg);
	LOG_CRIT("\nSA nums %x CCs %x, match-sci %x, reserved %x",
...
static int _gpy2xx_msec_config_egr_sa_nm_xcp(struct gpy211_device *phy)
{
	...
	/* Set 'SA_NM_FLOW_EXPL flow control action */
	ret = gpy2xx_msec_config_egr_sa_nm_expl(phy, &nm_expl);

	if (ret < 0) {
		LOG_CRIT("ERROR: api gpy2xx_msec_config_egr_sa_nm_expl fails\n");
		goto END;
	}
...
int gpy2xx_msec_init_egr_dev(struct gpy211_device *phy)
{
	...
	phy->nr_of_sas =
		FIELD_GET(_data, EIPE_EIP160_CONFIG_NR_OF_SAS);
#ifdef VERIFY_ENDIANNESS
	struct eip160_config eip_cfg;
	eip_cfg.raw_eip160_cfg = _data;
	LOG_CRIT("\nEIP cfg raw %x, stored %x", _data, eip_cfg.raw_eip160_cfg);
...
/* Populate AES Key in Little Endian order */
...
/* Populate AES HKey in Little Endian order */
...
/* Populate SCI in Little Endian order */
...
int gpy2xx_msec_config_ing_sa_nm_expl(struct gpy211_device *phy,
				      const struct sa_nonmatch_fca *nm_expl)
{
	int ret;
	u32 _base_addr, _offset, _target_addr, _nm_expl = 0;
	LOG_INFO("Entering func name :- %s\n", __func__);

	if (phy == NULL || nm_expl == NULL) {
		LOG_WARN("WARN: NULL input pointer(s)\n");
		return -EINVAL;
	}
	...
	/* Validate input params */
	/* Validate SA non-match EXPL params */
	if ((nm_expl->flow_type > SA_NM_FCA_FLOW_DROP) ||
	    (nm_expl->dest_port > SA_NM_FCA_DPT_UNCONTROL) ||
	    (nm_expl->drop_non_reserved > 1) ||
	    (nm_expl->drop_action > SA_NM_FCA_NO_DROP)) {
		LOG_WARN("WARN: Invalid input SA non-match EXPL params\n");
		return -EINVAL;
	}
	...
	_base_addr = EIPI_EIP_160S_I_32_CLASSIFIER_VARIOUS_CONTROL_REGS_MODULE_BASE;
	_offset = EIPI_SAM_NM_FLOW_EXPL;
	_target_addr = _base_addr + _offset;

#ifdef USE_READ_RMW //Read-Modify_write

	/* Get SAM_NM_FLOW_EXPL */
	ret = PHY_MBOX_READ(phy, _target_addr, &_nm_expl);

	if (ret < 0) {
		LOG_CRIT("ERROR: SAM_NM_FLOW_EXPL get fails\n");
		goto END;
	}

#endif

	FIELD_REPLACE(_nm_expl, nm_expl->flow_type, EIPI_SAM_NM_FLOW_EXPL_FLOW_TYPE0);
	FIELD_REPLACE(_nm_expl, nm_expl->dest_port, EIPI_SAM_NM_FLOW_EXPL_DEST_PORT0);
	FIELD_REPLACE(_nm_expl, nm_expl->drop_non_reserved, EIPI_SAM_NM_FLOW_EXPL_DROP_NON_RESERVED0);
	FIELD_REPLACE(_nm_expl, nm_expl->drop_action, EIPI_SAM_NM_FLOW_EXPL_DROP_ACTION0);

	/* Set SAM_NM_FLOW_EXPL data */
	ret = PHY_MBOX_WRITE(phy, _target_addr, _nm_expl);

	if (ret < 0) {
		LOG_CRIT("ERROR: SAM_NM_FLOW_EXPL set fails\n");
		goto END;
	}
...
int gpy2xx_msec_config_egr_sa_nm_expl(struct gpy211_device *phy,
				      const struct sa_nonmatch_fca *nm_expl)
{
	int ret;
	u32 _base_addr, _offset, _target_addr, _nm_expl = 0;
	...
	FIELD_REPLACE(_nm_expl, nm_expl->flow_type, EIPE_SAM_NM_FLOW_EXPL_FLOW_TYPE0);
	FIELD_REPLACE(_nm_expl, nm_expl->dest_port, EIPE_SAM_NM_FLOW_EXPL_DEST_PORT0);
	FIELD_REPLACE(_nm_expl, nm_expl->drop_action, EIPE_SAM_NM_FLOW_EXPL_DROP_ACTION0);
