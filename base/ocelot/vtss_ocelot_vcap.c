...
static vtss_rc srvl_vcap_data_get(const tcam_props_t *tcam,
                                  vtss_vcap_idx_t    *idx,
                                  srvl_tcam_data_t   *data)
{
    u32 i, col, offset, count, cnt, base, width = tcam->tg_width;

    VTSS_N("%s, row: %u, col: %u, key_size: %s", tcam->name, idx->row, idx->col,
    ...
    case VTSS_VCAP_KEY_SIZE_QUARTER:
        ...
        break;
    default: VTSS_E("illegal key size"); return VTSS_RC_ERROR;
    }
    col = (count - idx->col - 1);
                                   ...
                                   vtss_vcap_idx_t *idx)
{
    ...
    VTSS_I("%s, row: %u, col: %u, key_size: %s", tcam->name, idx->row, idx->col,
           vtss_vcap_key_size2txt(idx->key_size));
...
static void srvl_vcap_data_copy(srvl_tcam_data_t *src,
                                srvl_tcam_data_t *dst,
                                u32               key_len,
                                u32               action_len,
                                u32               counter_len)
{
                                  ...
                                  BOOL          clear)
{
    ...
    if (clear) {
        ...
        VTSS_RC(srvl_vcap_port_cmd(vtss_state, tcam, port, VTSS_TCAM_CMD_WRITE,
                                   VTSS_TCAM_SEL_COUNTER));
    }
    return VTSS_RC_OK;
}
...
#undef MASK
    ...
    case VTSS_IS0_TYPE_MLBS: {
        ...
        srvl_vcap_action_set(data, IS0_AO_MLBS_TC_MAP, IS0_AL_MLBS_TC_MAP,
...
static void srvl_is1_l4port_update(vtss_vcap_vr_t      *vr,
                                   u32                  range,
                                   vtss_vcap_u16_t     *val,
                                   srvl_is1_key_info_t *info,
                                   BOOL                *l4_used)
                                  ...
                                  u32               counter)
{
    ...
    case VTSS_IS1_TYPE_ETYPE:
        ...
        if (vtss_srvl_is1_oam_adv(vtss_state) && info.etype.value[0] == 0x89 &&
            info.etype.value[1] == 0x02 && info.etype.mask[0] == 0xff &&
            info.etype.mask[1] == 0xff) {
    ...
    if (idx->key_size == VTSS_VCAP_KEY_SIZE_QUARTER) {
        ...
        /* Full key */
        srvl_vcap_key_set(data, IS1_FKO_TYPE, IS1_FKL_TYPE, IS1_TYPE_7TUPLE,
                          VTSS_BITMASK(IS1_FKL_TYPE));
...
static vtss_rc srvl_debug_is1(srvl_debug_info_t *info)
{
    ...
    if (data->tg_sw == VCAP_TG_QUARTER) {
        ...
        pr("(%s) ", type == IS1_TYPE_NORMAL       ? "normal"
    ...
    pr("(%s) ", type == IS1_TYPE_NORMAL_IP6   ? "normal_ip6"
                                   ...
                                   u32                counter)
{
    ...
    if (discard && action->cpu_once == 0 && action->cpu == 0) {
        ...
        act = VTSS_ACL_PORT_ACTION_NONE; // Use mode 0 to make Rx red counters
                                         // increase
        ...
        if (vtss_vcap_udp_tcp_rule(proto)) {
            ...
            if (ptp->enable) {
                ...
                srvl_vcap_key_set(data, IS2_FKO_IP6_TCP_UDP_L4_1588_VER,
                                  IS2_FKL_IP6_TCP_UDP_L4_1588_VER, 0, 0);
            }
        ...
        if (vtss_vcap_udp_tcp_rule(proto)) {
            ...
            if (ptp->enable) {
                ...
                srvl_vcap_key_set(data, IS2_HKO_IP4_TCP_UDP_L4_1588_VER,
                                  IS2_HKL_IP4_TCP_UDP_L4_1588_VER, 0, 0);
            }
        } else if (proto->mask == 0) {
            /* Any IP protocol match */
            type = IS2_TYPE_IP_UDP_TCP;
...
static void srvl_debug_is2_l4(srvl_debug_info_t *info, u32 offset)
{
    ...
    srvl_debug_bits(info, "1588_ver", IS2_HKO_IP4_TCP_UDP_L4_1588_VER,
...
static vtss_rc srvl_debug_is2(srvl_debug_info_t *info)
{
    ...
    if (data->tg_sw == VCAP_TG_QUARTER) {
        ...
        pr("(%s) ", type == IS2_TYPE_IP6_TCP_UDP ? "ip6_tcp_udp"
...
static vtss_rc srvl_vcap_init(vtss_state_t *vtss_state)
{
    ...
    for (i = 0; i < VTSS_VCAP_RANGE_CHK_CNT; i++) {
        ...
        case 1:  entry->type = VTSS_VCAP_RANGE_TYPE_DPORT; break;
        case 2:  entry->type = VTSS_VCAP_RANGE_TYPE_SPORT; break;
        case 3:  entry->type = VTSS_VCAP_RANGE_TYPE_SDPORT; break;
        case 4:  entry->type = VTSS_VCAP_RANGE_TYPE_VID; break;
        case 5:  entry->type = VTSS_VCAP_RANGE_TYPE_DSCP; break;
        default: entry->type = VTSS_VCAP_RANGE_TYPE_IDLE; break;
        }
