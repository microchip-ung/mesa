                              ...
                              const char   *name)
{
    ...
    vtss_srvl_debug_reg(vtss_state, ss, addr, buf.s);
}
...
static vtss_rc srvl_fdma_flush(vtss_state_t *vtss_state)
{
    ...
    /* Be sure to flush to avoid PCIe bus lock-up. And 2 times due to buffer
     * architecture */
    for (i = 0; i < max_qframes; i++) {
        ...
        if (val & VTSS_F_ICPU_CFG_MANUAL_XTRINJ_MANUAL_INTR_INTR_XTR_ANY_RDY) {
            ...
            SRVL_RD(VTSS_ICPU_CFG_MANUAL_XTRINJ_MANUAL_XTR(4096 - 1),
                    &dummy); /* bring manual DMA manual logic into known state */
        } else {
            break;
        }
    }
...
vtss_rc vtss_cil_init_conf_set(vtss_state_t *vtss_state)
{
    ...
    if (!vtss_state->warm_start_cur && !vtss_state->sys_config.using_pcie &&
        ...
        if (vtss_state->sys_config.using_pcie) {
            ...
            if (value & VTSS_F_ICPU_CFG_MANUAL_XTRINJ_MANUAL_CFG_XTR_ENA) {
                ...
                SRVL_WRM_CLR(VTSS_ICPU_CFG_MANUAL_XTRINJ_MANUAL_CFG,
                             VTSS_F_ICPU_CFG_MANUAL_XTRINJ_MANUAL_CFG_XTR_ENA);
            }
        } else {
