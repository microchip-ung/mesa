                                      ...
                                      const vtss_eee_port_conf_t *const conf)
{
    ...
    if (port_no >= VTSS_PORT_ARRAY_SIZE) {
        VTSS_E("Out of bounds: chip_port:%d, VTSS_PORT_ARRAY_SIZE:%d", port_no,
    ...
    // EEE holdoff timer
    eee_cfg_reg |= VTSS_F_DEV_PORT_MODE_EEE_CFG_EEE_TIMER_HOLDOFF(0x5) |
                   VTSS_F_DEV_PORT_MODE_EEE_CFG_EEE_TIMER_AGE(0x23);
 ...
 * FAN speed control
 * =================================================================*/
static vtss_rc srvl_fan_controller_init(vtss_state_t *vtss_state, const vtss_fan_conf_t *const spec)
{
    ...
    // Set GPIO alternate functions. TACHO is bit 20 - See Ocelot Datasheet
    // Table 163.
    VTSS_RC(vtss_srvl_gpio_mode(vtss_state, 0, 20, VTSS_GPIO_ALT_1));
    // Set GPIO alternate functions. PWM is bit 21 - See Ocelot Datasheet Table
    // 163.
    ...
    if (spec->fan_pwm_freq < (sizeof(pwm_freq) / sizeof(pwm_freq[0]))) {
        ...
        VTSS_E("spec.fan_pwm_freq = %u is out of range 0..%u", spec->fan_pwm_freq,
               (u32)(sizeof(pwm_freq) / sizeof(pwm_freq[0]) - 1));
    }
    ...
    if (spec->type == VTSS_FAN_3_WIRE_TYPE) {
        ...
        SRVL_WRM(VTSS_DEVCPU_GCB_FAN_CTRL_FAN_CFG, VTSS_F_DEVCPU_GCB_FAN_CTRL_FAN_CFG_GATE_ENA,
                 VTSS_F_DEVCPU_GCB_FAN_CTRL_FAN_CFG_GATE_ENA);
    } else {
        // For 4-wire fan types we need to disable gating (2-wire types doesn't
        // matter)
        SRVL_WRM(VTSS_DEVCPU_GCB_FAN_CTRL_FAN_CFG, 0, VTSS_F_DEVCPU_GCB_FAN_CTRL_FAN_CFG_GATE_ENA);
    }
...
static vtss_rc srvl_fan_cool_lvl_set(vtss_state_t *vtss_state, u8 lvl)
{
    ...
    SRVL_WRM(VTSS_DEVCPU_GCB_FAN_CTRL_FAN_CFG, VTSS_F_DEVCPU_GCB_FAN_CTRL_FAN_CFG_DUTY_CYCLE(lvl),
...
/*
 * Help function for keeping track of the fan rotation speed. This function mist
 * be called every one second with update = TRUE.
 *
 * In :  update -  When the function is called with update = TRUE, the fan
 * rotation count is read and the count for the last second is stored.
 *
 *       value  - Only needed when update is set to FALSE. The cnt for the last
 * second is store in this pointer.
 *
 * Return : VTSS_OK if count was updated correctly, or value contains valid
...
/*
 * Help function for keeping track of the fan rotation speed. Must be called
 * every one second.
 *
                              ...
                              u32           voe_idx)
{
    ...
    SRVL_WR(VTSS_ANA_IPT_OAM_MEP_CFG(isdx),
            VTSS_F_ANA_IPT_OAM_MEP_CFG_MEP_IDX(voe_idx) |
                ...
                ((voe_idx == VTSS_VOE_IDX_NONE) || (voe_idx >= VTSS_PORT_VOE_BASE_IDX)
                     ? 0
                     : VTSS_F_ANA_IPT_OAM_MEP_CFG_MEP_IDX_ENA)); /* Do not point
                                                                    to a Port
                                                                    VOE */
...
static vtss_rc srvl_ptp_event_poll(vtss_state_t *vtss_state, vtss_ptp_event_type_t *ev_mask)
{
    ...
    *ev_mask |= (sticky & VTSS_X_DEVCPU_PTP_PTP_CFG_PTP_PIN_INTR_INTR_PTP(1 << 0))
                    ? VTSS_PTP_PIN_0_SYNC_EV
                    : 0;
    *ev_mask |= (sticky & VTSS_X_DEVCPU_PTP_PTP_CFG_PTP_PIN_INTR_INTR_PTP(1 << 1))
                    ? VTSS_PTP_PIN_1_SYNC_EV
                    : 0;
    *ev_mask |= (sticky & VTSS_X_DEVCPU_PTP_PTP_CFG_PTP_PIN_INTR_INTR_PTP(1 << 2))
                    ? VTSS_PTP_PIN_2_SYNC_EV
                    : 0;
    *ev_mask |= (sticky & VTSS_X_DEVCPU_PTP_PTP_CFG_PTP_PIN_INTR_INTR_PTP(1 << 3))
...
static vtss_rc srvl_intr_pol_negation(vtss_state_t *vtss_state) { return VTSS_RC_OK; }
...
#define IRQ_DEST(conf)                                                                             \
    ((u32)((conf->external ? SRVL_IRQ_DEST_EXT0 : SRVL_IRQ_DEST_CPU0) + conf->destination))

static vtss_rc srvl_misc_irq_destination(vtss_state_t *vtss_state, u32 mask, u32 destination)
{
                                 ...
                                 const vtss_irq_conf_t *const conf)
{
    ...
    if (conf->destination > 1) {
        ...
        case VTSS_IRQ_SOFTWARE:
            rc = srvl_misc_irq_remap(vtss_state, VTSS_BIT(SRVL1_IRQ_SW0) | VTSS_BIT(SRVL1_IRQ_SW1),
                                     conf);
            break;
...
static vtss_rc srvl_misc_irq_status(vtss_state_t *vtss_state, vtss_irq_status_t *status)
{
    ...
    if (vtss_state->sys_config.using_pcie) {
        ...
        dest = (val & VTSS_F_ICPU_CFG_PCIE_PCIE_INTR_COMMON_CFG_LEGACY_MODE_INTR_SEL) == 0
                   ? SRVL_IRQ_DEST_EXT0
                   : SRVL_IRQ_DEST_EXT1;
    } else {
        // When running on the internal CPU, SRVL_IRQ_DEST_CPU1 is - by
        // convention and in agreement with the kernel - used in user-space.
                                    ...
                                    const BOOL       enable)
{
    u32 mask = 0;
    switch (irq) {
    case VTSS_IRQ_XTR: mask = VTSS_BIT(SRVL1_IRQ_XTR_RDY); break;
    case VTSS_IRQ_FDMA_XTR:
        ...
        break;
    case VTSS_IRQ_SOFTWARE: mask = VTSS_BIT(SRVL1_IRQ_SW0) | VTSS_BIT(SRVL1_IRQ_SW1); break;
    case VTSS_IRQ_PTP_RDY:  mask = VTSS_BIT(SRVL1_IRQ_PTP_RDY); break;
    case VTSS_IRQ_PTP_SYNC: mask = VTSS_BIT(SRVL1_IRQ_PTP_SYNC); break;
    case VTSS_IRQ_EXT0:     mask = VTSS_BIT(SRVL1_IRQ_EXT0); break;
    case VTSS_IRQ_EXT1:
#if defined(VTSS_OPSYS_ECOS)
...
#endif /* VTSS_OPSYS_ECOS */
        break;
    case VTSS_IRQ_OAM:     mask = VTSS_BIT(SRVL1_IRQ_OAM_MEP); break;
    case VTSS_IRQ_SGPIO:   mask = VTSS_BIT(SRVL1_IRQ_SGPIO); break;
    case VTSS_IRQ_DEV_ALL: mask = VTSS_BIT(SRVL1_IRQ_DEV_ALL); break;
                                       ...
                                       const BOOL               enable)
{
    ...
    if (enable) {
        ...
        SRVL_WRM(VTSS_DEVCPU_GCB_SIO_CTRL_SIO_INTR(bit), mask,
        ...
        /* Enable rising edge triggered interrupt - From datasheet 11: Interrupt
        ...
        if (i == 32) {
            SRVL_WRM(VTSS_DEVCPU_GCB_SIO_CTRL_SIO_CFG, 0,
                                   ...
                                   const vtss_sgpio_conf_t *const conf)
{
    ...
    for (port = 0; port < 32; port++) {
        for (bit_idx = 0; bit_idx < 4; bit_idx++) {
            ...
            VTSS_D("group:%d, port:%d, bit_idx:%d, int_pol_high:%d", group, port, bit_idx,
                   pol_high);
        }
    }
...
#define SRVL_DEBUG_GPIO(ss, addr, name)                                                            \
    vtss_srvl_debug_reg(vtss_state, ss, VTSS_DEVCPU_GCB_GPIO_GPIO_##addr, "GPIO_" name)
#define SRVL_DEBUG_SIO(ss, addr, name)                                                             \
    vtss_srvl_debug_reg(vtss_state, ss, VTSS_DEVCPU_GCB_SIO_CTRL_SIO_##addr, "SIO_" name)
#define SRVL_DEBUG_SIO_INST(ss, addr, i, name)                                                     \
    vtss_srvl_debug_reg_inst(vtss_state, ss, VTSS_DEVCPU_GCB_SIO_CTRL_SIO_##addr, i, "SIO_" name)
                               ...
                               const vtss_debug_info_t *const info)
{
    ...
    vtss_srvl_debug_reg(vtss_state, ss, VTSS_ICPU_CFG_CPU_SYSTEM_CTRL_GENERAL_STAT, "GENERAL_STAT");
    vtss_srvl_debug_reg(vtss_state, ss, VTSS_ICPU_CFG_INTR_INTR_IDENT, "INTR_IDENT");
    vtss_srvl_debug_reg(vtss_state, ss, VTSS_ICPU_CFG_INTR_INTR_RAW, "INTR_RAW");
    vtss_srvl_debug_reg(vtss_state, ss, VTSS_ICPU_CFG_INTR_INTR_ENA, "INTR_ENA");
    vtss_srvl_debug_reg(vtss_state, ss, VTSS_ICPU_CFG_INTR_INTR_BYPASS, "INTR_BYPASS");
    vtss_srvl_debug_reg(vtss_state, ss, VTSS_ICPU_CFG_FDMA_FDMA_INTR_IDENT, "FDMA_INTR_IDENT");
    vtss_srvl_debug_reg(vtss_state, ss, VTSS_ICPU_CFG_FDMA_FDMA_INTR_ENA, "FDMA_INTR_ENA");
    for (i = 0; i < 4; i++) {
        vtss_srvl_debug_reg_inst(vtss_state, ss, VTSS_ICPU_CFG_INTR_DST_INTR_MAP(i), i,
                                 "DST_INTR_MAP");
        vtss_srvl_debug_reg_inst(vtss_state, ss, VTSS_ICPU_CFG_INTR_DST_INTR_IDENT(i), i,
                                 "DST_INTR_IDENT");
    }
    // DEV_ALL IRQ
    vtss_srvl_debug_reg(vtss_state, ss, VTSS_ICPU_CFG_INTR_DEV_INTR_RAW, "DEV_INTR_RAW");
    vtss_srvl_debug_reg(vtss_state, ss, VTSS_ICPU_CFG_INTR_DEV_INTR_STICKY, "DEV_INTR_STICKY");
    vtss_srvl_debug_reg(vtss_state, ss, VTSS_ICPU_CFG_INTR_DEV_INTR_ENA, "DEV_INTR_ENA");
    vtss_srvl_debug_reg(vtss_state, ss, VTSS_ICPU_CFG_INTR_DEV_INTR_IDENT, "DEV_INTR_IDENT");
    if (vtss_state->sys_config.using_pcie) {
        vtss_srvl_debug_reg(vtss_state, ss, VTSS_ICPU_CFG_PCIE_PCIE_CFG, "PCIE_CFG");
        vtss_srvl_debug_reg(vtss_state, ss, VTSS_ICPU_CFG_PCIE_PCIE_STAT, "PCIE_STAT");
        vtss_srvl_debug_reg(vtss_state, ss, VTSS_ICPU_CFG_PCIE_PCIE_AUX_CFG, "PCIE_AUX_CFG");
        vtss_srvl_debug_reg(vtss_state, ss, VTSS_ICPU_CFG_PCIE_PCIE_INTR, "PCIE_INTR");
        vtss_srvl_debug_reg(vtss_state, ss, VTSS_ICPU_CFG_PCIE_PCIE_INTR_ENA, "PCIE_INTR_ENA");
        vtss_srvl_debug_reg(vtss_state, ss, VTSS_ICPU_CFG_PCIE_PCIE_INTR_IDENT, "PCIE_INTR_IDENT");
        vtss_srvl_debug_reg(vtss_state, ss, VTSS_ICPU_CFG_PCIE_PCIE_INTR_COMMON_CFG,
                            "PCIE_INTR_COMMON_CFG");
        for (i = 0; i < 2; i++)
            vtss_srvl_debug_reg_inst(vtss_state, ss, VTSS_ICPU_CFG_PCIE_PCIE_INTR_CFG(i), i,
                                     "PCIE_INTR_CFG");
    }
...
vtss_rc vtss_srvl_misc_debug_print(vtss_state_t                  *vtss_state,
                                   ...
                                   const vtss_debug_info_t *const info)
{
    return vtss_debug_print_group(VTSS_DEBUG_GROUP_MISC, srvl_debug_misc, vtss_state, ss, info);
}
        ...
        if (!(enable & 1 << port))
            for (bit = 0; bit < 4; ++bit) /* port is not enabled - check if it
                ...
                if (vtss_state->misc.sgpio_event_enabled[0][0].enable[port][bit]) {
                    rc = srvl_sgpio_event_enable(vtss_state, 0, 0, port, bit,
