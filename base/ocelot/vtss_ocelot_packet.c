...
#define VTSS_AFI_SLOT_DIVIDER                                                  \
    16 /* Where to place the slot divider (TIMED_ENTRY_FAST_CNT) */
#define VTSS_AFI_SLOTS_PER_CC 8 /* Number of slots visited in one clock cycle */

// Unfortunately, the AFI timers aren't defined as a replication (due to
// distinct defaults), so we have to define our own address.
#define VTSS_QSYS_TIMED_FRAME_CFG_TFRM_TIMER_CFG(x)                            \
    VTSS_IOREG(VTSS_TO_QSYS, 0x44c6 + (x))
                                      ...
                                      u32                       maxuse)
{
    ...
    pr("%4u %-8s %7s %9d %4u %5u %6u\n", idx,
       srvl_afi_qsys_resource_to_str(resource),
       srvl_afi_chip_port_to_str(vtss_state, chip_port, buf), chip_port, prio,
       inuse, maxuse);
}
...
static vtss_rc srvl_afi_cancel(vtss_state_t *vtss_state, vtss_afi_id_t afi_id)
{
    ...
    if ((val & VTSS_F_QSYS_TIMED_FRAME_DB_TIMED_FRAME_ENTRY_TFRM_VLD) == 0) {
        VTSS_IG(VTSS_TRACE_GROUP_AFI,
                "Cancelling a frame (ID = %u) that is not in H/W", afi_id);
        return VTSS_RC_OK;
    }

    // Do cancel frame
    SRVL_WRM(VTSS_QSYS_TIMED_FRAME_CFG_TFRM_MISC,
             VTSS_F_QSYS_TIMED_FRAME_CFG_TFRM_MISC_TIMED_CANCEL_SLOT(afi_id) |
                 VTSS_F_QSYS_TIMED_FRAME_CFG_TFRM_MISC_TIMED_CANCEL_1SHOT,
             VTSS_M_QSYS_TIMED_FRAME_CFG_TFRM_MISC_TIMED_CANCEL_SLOT |
                 VTSS_F_QSYS_TIMED_FRAME_CFG_TFRM_MISC_TIMED_CANCEL_1SHOT);
    ...
    while (1) {
        ...
        if ((val & VTSS_F_QSYS_TIMED_FRAME_CFG_TFRM_MISC_TIMED_CANCEL_1SHOT) ==
            0) {
            goto do_exit;
        }

        if (VTSS_MTIMER_TIMEOUT(&timer)) {
            VTSS_EG(VTSS_TRACE_GROUP_AFI,
                    "Timeout waiting for cancelling of frame (afi_id = %u)",
                    afi_id);
            rc = VTSS_RC_ERROR;
            goto do_exit;
        }
                                          ...
                                          BOOL          *link_up)
{
    ...
    VTSS_IG(VTSS_TRACE_GROUP_AFI, "Link %s event on iport %u",
            *link_up ? "up" : "down", port_no);
    ...
    for (afi_id = 0; afi_id < VTSS_ARRSZ(vtss_state->afi.slots); afi_id++) {
        ...
        if (!resume && slot_conf->state == VTSS_AFI_SLOT_STATE_ENABLED &&
            slot_conf->port_no == port_no) {
            // Pausing an enabled frame. This will remove the frame from the
            // switch core, but keep the software state.
            slot_conf->state = VTSS_AFI_SLOT_STATE_PAUSED;

            VTSS_IG(VTSS_TRACE_GROUP_AFI,
                    "iport = %u: Cancelling (pausing) afi_id = %u", port_no,
                    afi_id);
                              ...
                              vtss_afi_id_t *const       afi_id)
{
    ...
    if (timer == VTSS_ARRSZ(vtss_state->afi.timers)) {
        ...
        // for the request.
        VTSS_EG(VTSS_TRACE_GROUP_AFI,
                "Requested %u fps, but couldn't go slower than %u fps",
                dscr->fps,
                vtss_state->afi.timers[VTSS_ARRSZ(vtss_state->afi.timers) - 1]
                    .fps);
    ...
    for (attempt = attempt_min; attempt < 2; attempt++) {
        ...
        for (sub_slot = 0; sub_slot < 8; sub_slot++) {
            for (slot = min_slot + sub_slot; slot <= max_slot; slot += 8) {
                if (vtss_state->afi.slots[slot].state ==
                    VTSS_AFI_SLOT_STATE_FREE) {
                    // Also check that H/W doesn't have a frame now that it
                    // isn't supposed to have (can happen if the egress port is
                    // oversubscribed and the user hasn't called the hijack
                    // function).
                    SRVL_RD(VTSS_QSYS_TIMED_FRAME_DB_TIMED_FRAME_ENTRY(slot),
                            &val);
                    if ((val &
                         VTSS_F_QSYS_TIMED_FRAME_DB_TIMED_FRAME_ENTRY_TFRM_VLD) !=
                        0) {
                        VTSS_IG(VTSS_TRACE_GROUP_AFI,
                                "Cancelling an unknown frame in slot %u", slot);
        ...
        if (!found && attempt == 1) {
            // Out of slots.
            VTSS_EG(VTSS_TRACE_GROUP_AFI,
                    "Unable to find a usable AFI slot (fps = %u)",
                    dscr->actual_fps);
            return VTSS_RC_ERROR;
        }
    }
    ...
    slot_conf->state =
        VTSS_AFI_SLOT_STATE_RESERVED; // Only reserved. We don't know the
                                      // destination port yet.
    ...
    *afi_id = slot;
    ...
    // Figure out the closest, smaller fps that we can obtain with the AFI at
    // hand.
    ticks =
        VTSS_DIV64(VTSS_PS_PER_SEC, ((u64)dscr->fps * VTSS_QSYS_TIMER_UNIT_PS));
    if (ticks == 0) {
        VTSS_EG(VTSS_TRACE_GROUP_AFI, "Got ticks == 0 with fps == %u",
                dscr->fps);
        return VTSS_RC_ERROR;
    }

    dscr->actual_fps =
        VTSS_DIV64(VTSS_PS_PER_SEC, (ticks * VTSS_QSYS_TIMER_UNIT_PS));

    // Since we are truncating in the ticks computation, ticks will always be <=
    // than had we used floating point. This means that dscr->actual_fps will
    // always be >= dscr->fps.
    ...
    if (ticks <= 0 || ticks > 0xFFFFFFFFULL) {
        VTSS_EG(VTSS_TRACE_GROUP_AFI,
                "Unable to fulfil user request for fps = %u", dscr->fps);
        return VTSS_RC_ERROR;
    }
    ...
    if (timer == VTSS_ARRSZ(vtss_state->afi.timers)) {
        // Didn't find a suitable already-configured timer. See if there are any
        // free timers.
        for (timer = 0; timer < VTSS_ARRSZ(vtss_state->afi.timers); timer++) {
    ...
    if (timer == VTSS_ARRSZ(vtss_state->afi.timers)) {
        // No free timers.
        VTSS_EG(VTSS_TRACE_GROUP_AFI,
                "Unable to find a usable AFI timer (fps = %u)",
                dscr->actual_fps);
        return VTSS_RC_ERROR;
    }

    // Find a free slot. The first 16 slots are reserved for frames faster than
    // 60 us.
    ...
    for (slot = min_slot; slot <= max_slot; slot++) {
        if (vtss_state->afi.slots[slot].state == VTSS_AFI_SLOT_STATE_FREE) {
            ...
            if ((val & VTSS_F_QSYS_TIMED_FRAME_DB_TIMED_FRAME_ENTRY_TFRM_VLD) !=
                0) {
                VTSS_IG(VTSS_TRACE_GROUP_AFI,
                        "Cancelling an unknown frame in slot %u", slot);
                VTSS_RC(srvl_afi_cancel(vtss_state, slot));
...
static vtss_rc srvl_afi_free(vtss_state_t *vtss_state, vtss_afi_id_t afi_id)
{
    ...
    if (slot_conf->state != VTSS_AFI_SLOT_STATE_FREE) {
        ...
        VTSS_IG(VTSS_TRACE_GROUP_AFI, "Freeing afi_id = %u on port_no = %u",
                afi_id, slot_conf->port_no);
        VTSS_RC(srvl_afi_cancel(vtss_state, afi_id));
...
#if VTSS_OPT_AFI_OPTIMIZE_FOR_TIMERS
        {
            ...
            u32 section =
                afi_id >= VTSS_AFI_SLOT_DIVIDER * VTSS_AFI_SLOTS_PER_CC ? 1 : 0;
            if (vtss_state->afi.fps_per_section[section] < timer_fps) {
                VTSS_EG(
                    VTSS_TRACE_GROUP_AFI,
                    "AFI ID = %u: Error updating section info. afi.fps_per_section[0, 1] = %u, %u. Timer = %u. FPS = %u",
                    afi_id, vtss_state->afi.fps_per_section[0],
                    vtss_state->afi.fps_per_section[1], slot_conf->timer_idx,
                    timer_fps);
                return VTSS_RC_ERROR;
            }
...
static vtss_rc srvl_afi_hijack(vtss_state_t *vtss_state, vtss_afi_id_t afi_id)
{
    ...
    if (slot_conf->state != VTSS_AFI_SLOT_STATE_ENABLED) {
        ...
        VTSS_IG(VTSS_TRACE_GROUP_AFI,
                "Frame with AFI ID %u has not been sent towards the AFI",
                afi_id);
        return VTSS_RC_OK;
    }
    ...
    while (1) {
        ...
        if (VTSS_MTIMER_TIMEOUT(&timer)) {
            VTSS_EG(
                VTSS_TRACE_GROUP_AFI,
                "Timeout waiting for frame to hit the AFI (afi_id = %u). Reads so far = %u",
                afi_id, cnt);
            rc = VTSS_RC_ERROR;
            goto do_exit;
        }
...
static vtss_rc srvl_afi_init(vtss_state_t *vtss_state)
{
    ...
    vtss_state->afi.timers[4].fps =
        300; // Want to support 3.33 ms CCM with a single frame flow to space
             // frames evenly
    vtss_state->afi.timers[5].fps = 100; // Ditto with 10 ms CCM
    vtss_state->afi.timers[6].fps = 10;  // Ditto with 100 ms CCM
    vtss_state->afi.timers[7].fps =
        1; // If writing anything but 1 here, 1 FPS would not be achievable...

    // Configure the timers.
    for (timer = 0; timer < VTSS_ARRSZ(vtss_state->afi.timers); timer++) {
        ticks = VTSS_DIV64(VTSS_PS_PER_SEC,
                           ((u64)vtss_state->afi.timers[timer].fps *
                            VTSS_QSYS_TIMER_UNIT_PS)) +
                1;
        SRVL_WR(VTSS_QSYS_TIMED_FRAME_CFG_TFRM_TIMER_CFG(timer), ticks);

        // Update our state with the actually achieved rate
        vtss_state->afi.timers[timer].fps =
            VTSS_DIV_ROUND_UP(VTSS_PS_PER_SEC, ticks * VTSS_QSYS_TIMER_UNIT_PS);
    }
...
static vtss_rc srvl_npi_mask_set(vtss_state_t *vtss_state)
{
    ...
    if (port_no != VTSS_PORT_NO_NONE) {
        ...
        val =
            VTSS_F_QSYS_SYSTEM_EXT_CPU_CFG_EXT_CPU_PORT(VTSS_CHIP_PORT(port_no)) |
...
static vtss_rc srvl_npi_update(vtss_state_t *vtss_state)
{
    ...
    if (conf->port_no != VTSS_PORT_NO_NONE) {
        ...
        value = (conf->enable ? VTSS_F_SYS_SYSTEM_PORT_MODE_INCL_INJ_HDR(3) |
                                    VTSS_F_SYS_SYSTEM_PORT_MODE_INCL_XTR_HDR(3)
                              : 0);
...
static vtss_rc srvl_npi_conf_set(vtss_state_t *vtss_state,
...
static vtss_rc srvl_packet_phy_cnt_to_ts_cnt(vtss_state_t *vtss_state,
                                             u32           phy_cnt,
                                             u64          *ts_cnt)
{
                                        ...
                                        u64          *ts_cnt)
{
    ...
    *ts_cnt =
        (u64)tc << 16; /* Must return in 16 bit fractions of nanoseconds */
                                      ...
                                      BOOL *timestamp_ok)
{
    ...
    *timestamp_ok = rx_info->hw_tstamp_decoded;
    ...
    VTSS_D("Raw timestamp %" PRIu64 ", hw_tstamp_decoded %u", *rxTime,
           *timestamp_ok);
...
static vtss_rc srvl_rx_conf_set(vtss_state_t *vtss_state)
{
    ...
    for (queue = 0; queue < vtss_state->packet.rx_queue_count; queue++) {
        SRVL_WR(VTSS_QSYS_RES_CTRL_RES_CFG(512 /* egress */ +
                                           VTSS_CHIP_PORT_CPU * VTSS_PRIOS +
                                           queue),
                conf->queue[queue].size / SRVL_BUFFER_CELL_SZ);
    }
    SRVL_WR(VTSS_QSYS_RES_CTRL_RES_CFG(512 /* egress */ +
                                       224 /* per-port reservation */ +
                                       VTSS_CHIP_PORT_CPU),
    ...
    for (port_no = VTSS_PORT_NO_START; port_no < vtss_state->port_count;
        ...
        for (i = 0, bpdu = 0, garp = 0; i < 16; i++) {
            bpdu |= srvl_cpu_fwd_mask_get(port_conf->bpdu_reg[i],
                                          reg->bpdu_cpu_only, i);
            garp |= srvl_cpu_fwd_mask_get(port_conf->garp_reg[i],
    ...
    value =
        VTSS_F_ANA_COMMON_CPUQ_CFG_CPUQ_SFLOW(map->sflow_queue !=
                                                      VTSS_PACKET_RX_QUEUE_NONE
                                                  ? map->sflow_queue -
                                                        VTSS_PACKET_RX_QUEUE_START
                                                  : VTSS_PACKET_RX_QUEUE_START) |
        VTSS_F_ANA_COMMON_CPUQ_CFG_CPUQ_MIRROR(map->mac_vid_queue -
                                               VTSS_PACKET_RX_QUEUE_START) |
        VTSS_F_ANA_COMMON_CPUQ_CFG_CPUQ_LRN(map->learn_queue -
                                            VTSS_PACKET_RX_QUEUE_START) |
        VTSS_F_ANA_COMMON_CPUQ_CFG_CPUQ_MAC_COPY(map->mac_vid_queue -
                                                 VTSS_PACKET_RX_QUEUE_START) |
        VTSS_F_ANA_COMMON_CPUQ_CFG_CPUQ_SRC_COPY(map->mac_vid_queue -
                                                 VTSS_PACKET_RX_QUEUE_START) |
        VTSS_F_ANA_COMMON_CPUQ_CFG_CPUQ_LOCKED_PORTMOVE(
            map->mac_vid_queue - VTSS_PACKET_RX_QUEUE_START) |
        VTSS_F_ANA_COMMON_CPUQ_CFG_CPUQ_ALLBRIDGE(map->bpdu_queue -
                                                  VTSS_PACKET_RX_QUEUE_START) |
        VTSS_F_ANA_COMMON_CPUQ_CFG_CPUQ_IPMC_CTRL(map->ipmc_ctrl_queue -
                                                  VTSS_PACKET_RX_QUEUE_START) |
        VTSS_F_ANA_COMMON_CPUQ_CFG_CPUQ_IGMP(map->igmp_queue -
                                             VTSS_PACKET_RX_QUEUE_START) |
        VTSS_F_ANA_COMMON_CPUQ_CFG_CPUQ_MLD(map->igmp_queue -
                                            VTSS_PACKET_RX_QUEUE_START);
    SRVL_WR(VTSS_ANA_COMMON_CPUQ_CFG, value);

    /* Setup each of the BPDU, GARP and CCM 'address' extraction queues */
    for (i = 0; i < 16; i++) {
        value = VTSS_F_ANA_COMMON_CPUQ_8021_CFG_CPUQ_BPDU_VAL(
                    map->bpdu_queue - VTSS_PACKET_RX_QUEUE_START) |
                VTSS_F_ANA_COMMON_CPUQ_8021_CFG_CPUQ_GARP_VAL(
                    map->garp_queue - VTSS_PACKET_RX_QUEUE_START) |
                VTSS_F_ANA_COMMON_CPUQ_8021_CFG_CPUQ_CCM_VAL(
                    VTSS_PACKET_RX_QUEUE_START); /* Fixme */
    ...
    if (!vtss_state->packet.manual_mode) {
        ...
        SRVL_WR(VTSS_DEVCPU_QS_XTR_XTR_GRP_CFG(0),
                VTSS_F_DEVCPU_QS_XTR_XTR_GRP_CFG_MODE(1) |
                    (byte_swap ? VTSS_F_DEVCPU_QS_XTR_XTR_GRP_CFG_BYTE_SWAP
                               : 0));
        SRVL_WR(VTSS_DEVCPU_QS_INJ_INJ_GRP_CFG(0),
                VTSS_F_DEVCPU_QS_INJ_INJ_GRP_CFG_MODE(1) |
                    (byte_swap ? VTSS_F_DEVCPU_QS_INJ_INJ_GRP_CFG_BYTE_SWAP
                               : 0));
...
static vtss_rc srvl_rx_frame_discard_grp(vtss_state_t              *vtss_state,
                                         const vtss_packet_rx_grp_t xtr_grp)
{
    ...
    while (!done) {
        ...
        case XTR_EOF_0:
            ...
            break;
        case XTR_ESCAPE:
            SRVL_RD(VTSS_DEVCPU_QS_XTR_XTR_RD(xtr_grp),
                    &val); /* Escaped data */
            break;
        case XTR_NOT_READY:
        default:            break;
 ...
 */
static int srvl_rx_frame_word(vtss_state_t        *vtss_state,
                                          ...
                                          u32 *frm_length) /* Including FCS */
{
    ...
    *frm_length = bytes_got = 0;
    ...
    for (i = 0; i < 4; i++) {
        if (srvl_rx_frame_word(vtss_state, grp, TRUE, &val, &bytes_valid) !=
            0) {
                                     ...
                                     const vtss_vid_t                  vid)
{
    ...
    SRVL_WR(VTSS_DEVCPU_QS_INJ_INJ_CTRL(grp),
            VTSS_F_DEVCPU_QS_INJ_INJ_CTRL_GAP_SIZE(1) |
                VTSS_F_DEVCPU_QS_INJ_INJ_CTRL_SOF);
    ...
    for (w = 0; w < count; w++, buf += 4) {
        if (w == 3 && vid != VTSS_VID_NULL) {
            /* Insert C-tag */
            SRVL_WR(VTSS_DEVCPU_QS_INJ_INJ_WR(grp),
    ...
    SRVL_WR(VTSS_DEVCPU_QS_INJ_INJ_CTRL(grp),
            VTSS_F_DEVCPU_QS_INJ_INJ_CTRL_GAP_SIZE(1) |
                VTSS_F_DEVCPU_QS_INJ_INJ_CTRL_VLD_BYTES(length < 60 ? 0
                                                                    : last) |
                                  ...
                                  vtss_packet_rx_info_t *const info)
{
    ...
    for (i = 0; i < 2; i++) {
        int offset = 8 * i;
        ifh[i] = ((u64)xtr_hdr[offset + 0] << 56) |
                 ((u64)xtr_hdr[offset + 1] << 48) |
                 ((u64)xtr_hdr[offset + 2] << 40) |
                 ((u64)xtr_hdr[offset + 3] << 32) |
                 ((u64)xtr_hdr[offset + 4] << 24) |
                 ((u64)xtr_hdr[offset + 5] << 16) |
                 ((u64)xtr_hdr[offset + 6] << 8) |
                 ((u64)xtr_hdr[offset + 7] << 0);
    }
    ...
    rew_op =
        VTSS_EXTRACT_BITFIELD64(ifh[0], IFH_OFF_REW_OP - 64, IFH_LEN_REW_OP);
    ...
    // sflow_id:
    // 0-11 : Frame has been sFlow sampled by a Tx sampler on port given by
    // @sflow_id. 12   : Frame has been sFlow sampled by an Rx sampler on port
    // given by @src_port. 13-14: Reserved. 15   : Frame has not been sFlow
    // sampled.
                             ...
                             vtss_packet_rx_info_t *rx_info)
{
    ...
    if (val) {
        ...
        VTSS_RC(srvl_rx_frame_get_internal(vtss_state, grp, ifh, data, buflen,
                                           &length));
...
/*****************************************************************************/
static vtss_rc srvl_oam_type_to_ifh(vtss_packet_oam_type_t oam_type,
                                    u32                   *result)
{
    vtss_rc rc = VTSS_RC_OK;

    switch (oam_type) {
    case VTSS_PACKET_OAM_TYPE_NONE:    *result = 0; break;
    case VTSS_PACKET_OAM_TYPE_CCM:     *result = 2; break;
    case VTSS_PACKET_OAM_TYPE_CCM_LM:  *result = 3; break;
    case VTSS_PACKET_OAM_TYPE_LBM:     *result = 4; break;
    case VTSS_PACKET_OAM_TYPE_LBR:     *result = 5; break;
    case VTSS_PACKET_OAM_TYPE_LMM:     *result = 6; break;
    case VTSS_PACKET_OAM_TYPE_LMR:     *result = 7; break;
    case VTSS_PACKET_OAM_TYPE_DMM:     *result = 8; break;
    case VTSS_PACKET_OAM_TYPE_DMR:     *result = 9; break;
    case VTSS_PACKET_OAM_TYPE_1DM:     *result = 10; break;
    case VTSS_PACKET_OAM_TYPE_LTM:     *result = 11; break;
    case VTSS_PACKET_OAM_TYPE_LTR:     *result = 12; break;
    case VTSS_PACKET_OAM_TYPE_GENERIC: *result = 13; break;
    default:
...
/*****************************************************************************/
static vtss_rc srvl_ptp_action_to_ifh(vtss_packet_ptp_action_t ptp_action,
                                      u8                       ptp_id,
                                      u32                     *result)
{
    vtss_rc rc = VTSS_RC_OK;
    ...
    switch (ptp_action) {
    case VTSS_PACKET_PTP_ACTION_NONE: *result = 0; break;
    case VTSS_PACKET_PTP_ACTION_ONE_STEP:
        *result = 2; // Residence compensation must be done in S/W, so bit 5 is
                     // not set.
        break;
    case VTSS_PACKET_PTP_ACTION_TWO_STEP:
        *result =
            (3 | (ptp_id
                  << 3)); // Select a PTP timestamp identifier for two-step PTP.
        break;
    case VTSS_PACKET_PTP_ACTION_ORIGIN_TIMESTAMP: *result = 5; break;
    default:
                                  ...
                                  u32 *const bin_hdr_len)
{
    ...
    *bin_hdr_len = 16;
    ...
    inj_hdr[0] =
        VTSS_ENCODE_BITFIELD64(!info->switch_frm, 127 - 64, 1); // BYPASS
    inj_hdr[1] = 0;
    ...
    if (info->switch_frm) {
        if (info->masquerade_port != VTSS_PORT_NO_NONE) {
            ...
            inj_hdr[0] |=
                VTSS_ENCODE_BITFIELD64(1, 126 - 64, 1); // Enable masquerading.
            inj_hdr[0] |= VTSS_ENCODE_BITFIELD64(chip_port, 122 - 64,
                                                 4); // Masquerade port.
        }

        if (info->tag.tpid) {
            inj_hdr[1] |=
                VTSS_ENCODE_BITFIELD64(1, 28 - 0,
                                       2); // POP_CNT = 1, i.e. pop one tag,
                                           // expected inserted by application.
        }
    } else {
        u32 mi_port, port_cnt, rew_op = 0, rew_val = 0,
                               pop_cnt = 3 /* Disable rewriter */;
        u64            chip_port_mask;
        ...
        // Add mirror port if CPU ingress mirroring is enabled. Mirroring of
        // directed frames must occur through the port mask.
        ...
        if (miroring &&
            state->l2.port_state[mi_port]) { // Mirroring is requested and the
                                             // link is up
            chip_port_mask |=
                VTSS_BIT64(VTSS_CHIP_PORT_FROM_STATE(state, mi_port));
        }

        if (info->ptp_action != VTSS_PACKET_PTP_ACTION_NONE) {
            VTSS_RC(srvl_ptp_action_to_ifh(info->ptp_action, info->ptp_id,
                                           &rew_op));
            ...
            // Get the frame classified to info->tag.vid, and rewritten
            // accordingly.
            pop_cnt = 0;
        }
        ...
        if (info->ptp_action == VTSS_PACKET_PTP_ACTION_NONE &&
            ...
            // The rew_val is re-used for AFI, so we can't periodically transmit
            // a frame if ptp_action != VTSS_PACKET_PTP_ACTION_NONE
            if (port_cnt != 1) {
                VTSS_E(
                    "When using AFI, exactly one port must be specified (dst_port_mask = 0x%" PRIu64
                    ").",
                    info->dst_port_mask);
                return VTSS_RC_ERROR;
            }
            ...
            rew_val = info->afi_id;
            inj_hdr[1] |=
                VTSS_ENCODE_BITFIELD64(slot_conf->timer_idx + 1, 37 - 0, 4);
        }
#endif

        inj_hdr[0] |= VTSS_ENCODE_BITFIELD64(rew_op, IFH_OFF_REW_OP - 64,
                                             IFH_LEN_REW_OP); // REW_OP
        inj_hdr[0] |= VTSS_ENCODE_BITFIELD64(rew_val, IFH_OFF_REW_VAL - 64,
                                             32); // REW_VAL
        inj_hdr[0] |=
            VTSS_ENCODE_BITFIELD64(chip_port_mask >> 8, 64 - 64,
                                   3); // Don't send to the CPU port (hence
                                       // length == 3 and not 4)
        ...
        if (isdx != VTSS_ISDX_NONE) {
            inj_hdr[1] |= VTSS_ENCODE_BITFIELD64(1, 32 - 0,
                                                 1); // Use ISDX for ES0 lookups
        }

        if (info->cos >= 8) {
            // Inject super priority by setting ACL_HIT (IFH[31]) to 0 (done by
            // VTSS_MEMSET() above) and ACL_ID[4] (IFH[41]) to 1
            inj_hdr[1] |= VTSS_ENCODE_BITFIELD64(1, 41, 1); // acl_id[4].
        }

        inj_hdr[1] |=
            VTSS_ENCODE_BITFIELD64(info->dp, 30 - 0, 1); // Drop Precedence
        inj_hdr[1] |= VTSS_ENCODE_BITFIELD64(pop_cnt, 28 - 0, 2); // POP_CNT
        inj_hdr[1] |= VTSS_ENCODE_BITFIELD64(info->cos >= 8 ? 7 : info->cos,
                                             17 - 0, 3); // QOS Class
        inj_hdr[1] |= VTSS_ENCODE_BITFIELD64(info->tag.tpid == 0 ||
                              ...
                              const vtss_debug_info_t *const info)
{
    ...
    for (slot = 0; slot < VTSS_ARRSZ(vtss_state->afi.slots); slot++) {
        ...
        pr("%4u 0x%04x %4u %5u %6u\n", slot,
           ...
           timer == 0 ? 7 : timer - 1 /* H/W uses one-based counters (SIC!), S/W
                                         uses zero-based */
           ,
...
static vtss_rc srvl_packet_init(vtss_state_t *vtss_state)
{
    ...
    /* Setup the CPU port as VLAN aware to support switching frames based on
     * tags */
    ...
    SRVL_WR(VTSS_ANA_PORT_PORT_CFG(VTSS_CHIP_PORT_CPU),
            VTSS_F_ANA_PORT_PORT_CFG_RECV_ENA);

    /* Set-up default packet Rx endianness, position of status word, and who
     * will be extracting. */
    for (i = 0; i < VTSS_PACKET_RX_GRP_CNT; i++) {
        /* status word before last data */
        SRVL_WRM_CLR(VTSS_DEVCPU_QS_XTR_XTR_GRP_CFG(i),
...
#else
        /* Little-endian */
        SRVL_WRM_SET(VTSS_DEVCPU_QS_XTR_XTR_GRP_CFG(i),
                     VTSS_F_DEVCPU_QS_XTR_XTR_GRP_CFG_BYTE_SWAP);
#endif

        if (!vtss_state->sys_config.using_vrap) {
            /* If not using VRAP, default to do register-based extraction. The
            ...
            if (!vtss_state->init_conf.using_ufdma || i != 0) {
                ...
                SRVL_WRM(VTSS_DEVCPU_QS_XTR_XTR_GRP_CFG(i),
                         VTSS_F_DEVCPU_QS_XTR_XTR_GRP_CFG_MODE(1),
    ...
    for (i = 0; i < VTSS_PACKET_TX_GRP_CNT; i++) {
#ifdef VTSS_OS_BIG_ENDIAN
        /* Big-endian */
        SRVL_WRM_CLR(VTSS_DEVCPU_QS_INJ_INJ_GRP_CFG(i),
                     VTSS_F_DEVCPU_QS_INJ_INJ_GRP_CFG_BYTE_SWAP);
#else
        /* Little-endian */
        SRVL_WRM_SET(VTSS_DEVCPU_QS_INJ_INJ_GRP_CFG(i),
                     VTSS_F_DEVCPU_QS_INJ_INJ_GRP_CFG_BYTE_SWAP);
#endif
        /* According to datasheet, we must insert a small delay after every
         * end-of-frame when injecting to qs. */
        SRVL_WR(VTSS_DEVCPU_QS_INJ_INJ_CTRL(i),
                VTSS_F_DEVCPU_QS_INJ_INJ_CTRL_GAP_SIZE(1));
        ...
        if (!vtss_state->sys_config.using_vrap) {
            ...
            if (!vtss_state->init_conf.using_ufdma || i != 0) {
                ...
                // uFDMA, which may be loaded before the switch application,
                // because it may be part of a Linux kernel.
                SRVL_WRM(VTSS_DEVCPU_QS_INJ_INJ_GRP_CFG(i),
                         VTSS_F_DEVCPU_QS_INJ_INJ_GRP_CFG_MODE(1),
    ...
    for (i = VTSS_CHIP_PORT_CPU_0;
        ...
        SRVL_WRM_SET(VTSS_SYS_SYSTEM_PORT_MODE(i),
                     VTSS_F_SYS_SYSTEM_PORT_MODE_INCL_XTR_HDR(1));
    ...
    /* Enable CPU port */
    SRVL_WRM_SET(VTSS_QSYS_SYSTEM_SWITCH_PORT_MODE(VTSS_CHIP_PORT_CPU),
    ...
    SRVL_WR(VTSS_ANA_PORT_QOS_CFG(VTSS_CHIP_PORT_CPU),
            VTSS_F_ANA_PORT_QOS_CFG_QOS_PCP_ENA);

    /* Disable aging of Rx CPU queues to allow the frames to stay there longer
     * than on normal front ports. */
    SRVL_WRM_SET(VTSS_REW_PORT_PORT_CFG(VTSS_CHIP_PORT_CPU_0),
                 VTSS_F_REW_PORT_PORT_CFG_AGE_DIS);
    SRVL_WRM_SET(VTSS_REW_PORT_PORT_CFG(VTSS_CHIP_PORT_CPU_1),
                 VTSS_F_REW_PORT_PORT_CFG_AGE_DIS);

    /* Set-up the one-to-one mapping */
    for (pcp = 0; pcp < VTSS_PCP_END - VTSS_PCP_START; pcp++) {
        for (dei = 0; dei < VTSS_DEI_END - VTSS_DEI_START; dei++) {
            SRVL_WR(VTSS_ANA_PORT_QOS_PCP_DEI_MAP_CFG(VTSS_CHIP_PORT_CPU,
                                                      (8 * dei + pcp)),
