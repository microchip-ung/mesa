                                     ...
                                     BOOL          member[VTSS_PORT_ARRAY_SIZE])
{
    ...
    return lan966x_pgid_mask_write(vtss_state,
                                   lan966x_chip_pgid(vtss_state, pgid), member,
                                   pgid_entry->cpu_copy, pgid_entry->cpu_queue);
}

static vtss_rc lan966x_pgid_update(vtss_state_t *vtss_state,
                                   u32           pgid,
                                   BOOL          member[])
{
    vtss_port_no_t     port_no;
...
{
    return lan966x_pgid_mask_write(vtss_state,
                                   PGID_SRC + VTSS_CHIP_PORT(port_no), member,
                                   0, 0);
}
...
vtss_rc vtss_cil_l2_aggr_mode_set(vtss_state_t *vtss_state)
{
    ...
    REG_WR(ANA_AGGR_CFG,
           (mode->sip_dip_enable ? ANA_AGGR_CFG_AC_IP4_SIPDIP_ENA_M : 0) |
               (mode->sport_dport_enable ? ANA_AGGR_CFG_AC_IP4_TCPUDP_ENA_M
                                         : 0) |
                                    ...
                                    const BOOL    member[VTSS_PORT_ARRAY_SIZE])
{
    ...
    for (port_no = 0; port_no < vtss_state->port_count; port_no++) {
        REG_WRM_CTL(ANA_PORT_CFG(VTSS_CHIP_PORT(port_no)), member[port_no],
                    ANA_PORT_CFG_LEARN_ENA_M);
    }
    return VTSS_RC_OK;
}
                                  ...
                                  u32                                 pgid)
{
    ...
    if (pgid == VTSS_PGID_NONE) {
        ...
        mask = vtss_lan966x_port_mask(vtss_state,
        ...
        if (entry->index_table) {
            REG_WR(ANA_PMACTIDX,
                   ANA_PMACTIDX_PMAC_INDEX(vtss_state->l2.mac_index_table
                                               .idx_add));
    ...
    REG_WR(ANA_MACACCESS,
           (copy_to_cpu ? ANA_MACACCESS_MAC_CPU_COPY_M : 0) |
               ...
               (aged ? ANA_MACACCESS_AGED_FLAG_M : 0) | ANA_MACACCESS_VALID_M |
               ANA_MACACCESS_ENTRYTYPE(type) | ANA_MACACCESS_DEST_IDX(idx) |
...
static u32 lan966x_mac_type(const vtss_vid_mac_t *vid_mac)
{
    return (VTSS_MAC_IPV4_MC(vid_mac->mac.addr)   ? MAC_TYPE_IPV4_MC
            : VTSS_MAC_IPV6_MC(vid_mac->mac.addr) ? MAC_TYPE_IPV6_MC
                                                  : MAC_TYPE_NORMAL);
}

static vtss_rc lan966x_mac_table_cmd(vtss_state_t               *vtss_state,
                                     ...
                                     u32                         cmd)
{
    ...
    REG_WR(ANA_MACHDATA, mach);
    REG_WR(ANA_MACLDATA, macl);
    REG_WR(ANA_MACACCESS, ANA_MACACCESS_VALID_M |
                                  ...
                                  const vtss_vid_mac_t *const vid_mac)
{
    ...
    VTSS_RC(lan966x_mac_table_cmd(vtss_state, vid_mac, type,
                                  MACACCESS_CMD_FORGET));
    if (type != MAC_TYPE_NORMAL) {
        /* IPMC entries may be encoded as NORMAL */
        VTSS_RC(lan966x_mac_table_cmd(vtss_state, vid_mac, MAC_TYPE_NORMAL,
                                      MACACCESS_CMD_FORGET));
    }
                                  ...
                                  u32                          *pgid)
{
    ...
    if (entry->index_table) {
        ...
        REG_WR(ANA_PMACTIDX,
               ANA_PMACTIDX_PMAC_INDEX(vtss_state->l2.mac_index_table.idx_get));
        REG_WR(ANA_PMACACCESS,
               ANA_PMACACCESS_PMAC_TBL_CMD(PMACACCESS_CMD_READ));
        ...
        entry->locked = 1;
        entry->aged = 0;
        *pgid = lan966x_vtss_pgid(vtss_state,
                                  ANA_PMACACCESS_PMAC_DEST_IDX_X(value));
        return VTSS_RC_OK;
    }
    VTSS_RC(lan966x_mac_table_cmd(vtss_state, &entry->vid_mac, type,
                                  MACACCESS_CMD_READ));
    rc = lan966x_mac_table_result(vtss_state, entry, pgid);
    if (type != MAC_TYPE_NORMAL && rc != VTSS_RC_OK) {
        /* IPMC entries may be encoded as NORMAL */
        VTSS_RC(lan966x_mac_table_cmd(vtss_state, &entry->vid_mac,
                                      MAC_TYPE_NORMAL, MACACCESS_CMD_READ));
        rc = lan966x_mac_table_result(vtss_state, entry, pgid);
    }
    return rc;
}
...
vtss_rc vtss_cil_l2_mac_table_get_next(vtss_state_t                 *vtss_state,
                                       ...
                                       u32                          *pgid)
{
    VTSS_RC(lan966x_mac_table_cmd(vtss_state, &entry->vid_mac, MAC_TYPE_NORMAL,
                                  MACACCESS_CMD_GET_NEXT));
                                  ...
                                  const vtss_vid_t vid)
{
    ...
    /* Do the aging */
    REG_WR(ANA_MACACCESS, ANA_MACACCESS_MAC_TABLE_CMD(MACACCESS_CMD_AGE));
                                         ...
                                         vtss_mac_table_status_t *status)
{
    ...
    REG_WR(ANA_ANEVENTS,
           value & (ANA_ANEVENTS_AUTO_MOVED_M | ANA_ANEVENTS_AUTO_LEARNED_M |
                    ANA_ANEVENTS_LEARN_REMOVE_M | ANA_ANEVENTS_AGED_ENTRY_M));
...
vtss_rc vtss_cil_l2_mac_index_update(vtss_state_t *vtss_state)
{
    ...
    REG_WR(ANA_PMAC_CFG, ANA_PMAC_CFG_PMAC_ENA(t->cnt ? 1 : 0) |
                                        ...
                                        const vtss_port_no_t port_no)
{
    ...
    REG_WRM(ANA_PORT_CFG(port),
            (mode->discard ? ANA_PORT_CFG_LEARNDROP_M : 0) |
                (mode->automatic ? ANA_PORT_CFG_LEARNAUTO_M
                                 : ANA_PORT_CFG_LOCKED_PORTMOVE_CPU_M) |
                (mode->cpu ? ANA_PORT_CFG_LEARNCPU_M : 0) |
                ANA_PORT_CFG_LOCKED_PORTMOVE_DROP_M,
            ANA_PORT_CFG_LEARNDROP_M | ANA_PORT_CFG_LEARNAUTO_M |
                ANA_PORT_CFG_LOCKED_PORTMOVE_CPU_M | ANA_PORT_CFG_LEARNCPU_M |
                ANA_PORT_CFG_LOCKED_PORTMOVE_DROP_M);
...
#define TAG_TPID_CFG_PTPID_NC                                                  \
    3 /* As above, but unless ingress tag was a C-tag */
                                            ...
                                            vtss_vlan_port_conf_t *conf)
{
    ...
    REG_WRM(ANA_VLAN_CFG(port),
            ...
            ANA_VLAN_CFG_VLAN_VID_M | ANA_VLAN_CFG_VLAN_AWARE_ENA_M |
    ...
    value =
        (REW_TAG_CFG_TAG_TPID_CFG(tpid) |
         REW_TAG_CFG_TAG_CFG(uvid == VTSS_VID_ALL    ? TAG_CFG_DISABLE
                             : uvid == VTSS_VID_NULL ? TAG_CFG_ALL_NNUL
                                                     : TAG_CFG_ALL_NPV_NNUL));
    REG_WRM(REW_TAG_CFG(port), value,
            REW_TAG_CFG_TAG_TPID_CFG_M | REW_TAG_CFG_TAG_CFG_M);
    REG_WRM(REW_PORT_VLAN_CFG(port),
            REW_PORT_VLAN_CFG_PORT_TPID(conf->s_etype ? conf->s_etype : etype) |
                REW_PORT_VLAN_CFG_PORT_VID(uvid),
            REW_PORT_VLAN_CFG_PORT_TPID_M | REW_PORT_VLAN_CFG_PORT_VID_M);
                                          ...
                                          vtss_vlan_port_conf_t *conf)
{
    ...
    return lan966x_vlan_port_conf_apply(vtss_state, VTSS_CHIP_PORT(port_no),
                                        conf);
}
                                     ...
                                     BOOL          member[VTSS_PORT_ARRAY_SIZE])
{
    ...
    REG_WR(ANA_VLANTIDX,
           ANA_VLANTIDX_V_INDEX(vid) |
               ANA_VLANTIDX_VLAN_FLOOD_DIS(e->flags & VLAN_FLAGS_FLOOD ? 0
                                                                       : 1) |
               ANA_VLANTIDX_VLAN_PRIV_VLAN(e->flags & VLAN_FLAGS_ISOLATED ? 1
                                                                          : 0) |
               ANA_VLANTIDX_VLAN_LEARN_DISABLED(e->flags & VLAN_FLAGS_LEARN
                                                    ? 0
                                                    : 1) |
    ...
    /* VLAN mask */
    mask |= ANA_VLAN_PORT_MASK_VLAN_PORT_MASK(vtss_lan966x_port_mask(vtss_state,
                                                                     member));
    ...
    REG_WR(ANA_FID_MAP(vid),
           ANA_FID_MAP_FID_VAL(fid > LAN966X_FID_MAX ? 0 : fid));
...
vtss_rc vtss_cil_l2_flood_conf_set(vtss_state_t *vtss_state)
{
    ...
    /* IPv4 flood mask */
    VTSS_RC(lan966x_pgid_update(vtss_state, PGID_MCIPV4,
                                vtss_state->l2.ipv4_mc_flood));

    /* IPv6 flood mask */
    VTSS_RC(lan966x_pgid_update(vtss_state, PGID_MCIPV6,
                                vtss_state->l2.ipv6_mc_flood));

    /* IPv6 flood scope */
    REG_WRM(ANA_FLOODING_IPMC, ANA_FLOODING_IPMC_FLD_MC6_CTRL(pgid),
                                 ...
                                 vtss_ipmc_cmd_t   cmd)
{
    ...
    vtss_vcap_key_size_t key_size =
        (ipmc->ipv6 ? VTSS_VCAP_KEY_SIZE_FULL : VTSS_VCAP_KEY_SIZE_HALF);
    vtss_res_chg_t         res;
...
vtss_rc vtss_cil_l2_mirror_conf_set(vtss_state_t *vtss_state)
{
    ...
    for (port_no = 0; port_no < vtss_state->port_count; port_no++) {
        ...
        REG_WRM_CTL(ANA_PORT_CFG(port), vtss_state->l2.mirror_ingress[port_no],
                    ANA_PORT_CFG_SRC_MIRROR_ENA_M);
    }
    REG_WR(ANA_MIRRORPORTS, vtss_lan966x_port_mask(vtss_state, member));
    REG_WR(ANA_EMIRRORPORTS,
           vtss_lan966x_port_mask(vtss_state, vtss_state->l2.mirror_egress));
    REG_WRM_CTL(ANA_PORT_CFG(VTSS_CHIP_PORT_CPU),
                vtss_state->l2.mirror_cpu_ingress,
                ANA_PORT_CFG_SRC_MIRROR_ENA_M);
    REG_WRM_CTL(ANA_AGENCTRL, vtss_state->l2.mirror_cpu_egress,
                ANA_AGENCTRL_MIRROR_CPU_M);
    return VTSS_RC_OK;
}

#if defined(VTSS_FEATURE_VCAP)
vtss_rc vtss_cil_l2_vcap_port_conf_set(vtss_state_t        *vtss_state,
                                       const vtss_port_no_t port_no)
{
    ...
    return vtss_lan966x_vcap_port_key_addr_set(vtss_state, port_no, 1, key_new,
...
vtss_rc vtss_cil_l2_isdx_update(vtss_state_t *vtss_state, vtss_sdx_entry_t *sdx)
{
    ...
    REG_WR(QSYS_FRER_FIRST(sdx->sdx),
           QSYS_FRER_FIRST_FRER_FIRST_MEMBER(sdx->ms_idx));
    act.isdx = sdx->sdx;
    vtss_lan966x_is1_action_update(vtss_state, sdx, &act);
    VTSS_I("sdx: %u, dlb: %u/%u, filter: %u/%u, gate: %u/%u", act.isdx,
           act.dlb_enable, act.dlb, act.sfid_enable, act.sfid, act.sgid_enable,
           act.sgid);
                                   ...
                                   const vtss_iflow_id_t id)
{
    ...
    REG_WR(ANA_STREAMTIDX,
           ANA_STREAMTIDX_S_INDEX(sdx->sdx) | ANA_STREAMTIDX_STREAM_SPLIT(0));
                                    ...
                                    BOOL             clr)
{
    ...
    if (idx != 0) {
        ...
        base = 640;
        VTSS_RC(vtss_lan966x_counter_update(vtss_state, p, &c->rx_green.bytes,
                                            clr));
        VTSS_RC(vtss_lan966x_counter_update(vtss_state, p, &c->rx_green.frames,
                                            clr));
        VTSS_RC(vtss_lan966x_counter_update(vtss_state, p, &c->rx_yellow.bytes,
                                            clr));
        VTSS_RC(vtss_lan966x_counter_update(vtss_state, p, &c->rx_yellow.frames,
                                            clr));
        VTSS_RC(vtss_lan966x_counter_update(vtss_state, p, &c->rx_red.bytes,
                                            clr));
        VTSS_RC(vtss_lan966x_counter_update(vtss_state, p, &c->rx_red.frames,
                                            clr));
        VTSS_RC(vtss_lan966x_counter_update(vtss_state, p, &c->rx_discard.bytes,
                                            clr)); // Green drops
        VTSS_RC(vtss_lan966x_counter_update(vtss_state, p,
                                            &c->rx_discard.frames,
                                            clr)); // Green drops
        VTSS_RC(vtss_lan966x_counter_update(vtss_state, p, &c->tx_discard.bytes,
                                            clr)); // Yellow drops
        VTSS_RC(vtss_lan966x_counter_update(vtss_state, p,
                                            &c->tx_discard.frames,
                                            clr)); // Yellow drops
        ...
        // PSFP counters
        if ((sdx = vtss_iflow_lookup(vtss_state, idx)) != NULL &&
            sdx->conf.psfp.filter_enable) {
            REG_WR(SYS_STAT_CFG,
                   SYS_STAT_CFG_STAT_VIEW(sdx->conf.psfp.filter_id));
            base = 512;
            VTSS_RC(vtss_lan966x_counter_update(vtss_state, p, &c->rx_match,
                                                clr));
            VTSS_RC(vtss_lan966x_counter_update(vtss_state, p,
                                                &c->rx_gate_discard, clr));
            VTSS_RC(vtss_lan966x_counter_update(vtss_state, p,
                                                &c->rx_sdu_discard, clr));
        }
    }
    ...
    if (idx != 0) {
        ...
        REG_WR(SYS_STAT_CFG, SYS_STAT_CFG_STAT_VIEW(idx));
        base = 768;
        VTSS_RC(vtss_lan966x_counter_update(vtss_state, p, &c->tx_green.bytes,
                                            clr));
        VTSS_RC(vtss_lan966x_counter_update(vtss_state, p, &c->tx_green.frames,
                                            clr));
        VTSS_RC(vtss_lan966x_counter_update(vtss_state, p, &c->tx_yellow.bytes,
                                            clr));
        VTSS_RC(vtss_lan966x_counter_update(vtss_state, p, &c->tx_yellow.frames,
                                            clr));
    }
    return VTSS_RC_OK;
}
                             ...
                             vtss_ingress_counters_t *counters)
{
    ...
    if (counters != NULL) {
        ...
        counters->rx_discard.frames =
            (c->rx_discard.frames.value +
             c->tx_discard.frames.value); // Green and yellow
        counters->rx_discard.bytes =
            (c->rx_discard.bytes.value +
             c->tx_discard.bytes.value); // Green and yellow
...
vtss_rc vtss_cil_l2_policer_update(vtss_state_t *vtss_state, u16 idx)
{
    ...
    return vtss_lan966x_qos_policer_conf_set(vtss_state,
                                             LAN966X_POLICER_DLB + idx,
                                             &pol_conf);
}
                                     ...
                                     const vtss_frer_cstream_id_t id)
{
    ...
    REG_WR(QSYS_FRER_CFG_CMP(id),
           QSYS_FRER_CFG_CMP_TAKE_NO_SEQUENCE(conf->take_no_seq ? 1 : 0) |
               ...
               QSYS_FRER_CFG_CMP_HISTORY_LENGTH(conf->hlen < 2
                                                    ? 1
                                                    : (conf->hlen - 1)) |
               QSYS_FRER_CFG_CMP_RESET_TICKS(conf->reset_time ? conf->reset_time
                                                              : 1) |
...
vtss_rc vtss_cil_l2_mstream_conf_set(vtss_state_t *vtss_state, const u16 idx)
{
    ...
    REG_WR(QSYS_FRER_CFG_MBM(idx),
           QSYS_FRER_CFG_MBM_TAKE_NO_SEQUENCE(conf->take_no_seq ? 1 : 0) |
               ...
               QSYS_FRER_CFG_MBM_HISTORY_LENGTH(conf->hlen < 2
                                                    ? 1
                                                    : (conf->hlen - 1)) |
               QSYS_FRER_CFG_MBM_RESET_TICKS(conf->reset_time ? conf->reset_time
                                                              : 1) |
...
vtss_rc vtss_cil_l2_cstream_cnt_get(vtss_state_t                *vtss_state,
                                    ...
                                    vtss_frer_counters_t        *counters)
{
    return lan966x_cstream_cnt_update(vtss_state, id, counters,
                                      counters == NULL);
}
...
vtss_rc vtss_cil_l2_mstream_cnt_get(vtss_state_t         *vtss_state,
                                    ...
                                    vtss_frer_counters_t *counters)
{
    return lan966x_mstream_cnt_update(vtss_state, idx, counters,
                                      counters == NULL);
}
                                       ...
                                       const vtss_psfp_gate_id_t id)
{
    ...
    REG_WR(ANA_SG_CFG_3,
           ANA_SG_CFG_3_BASE_TIME_SEC_MSB(gcl_conf->base_time.sec_msb) |
               ...
               ANA_SG_CFG_3_INVALID_RX_ENA(conf->close_invalid_rx.enable ? 1
                                                                         : 0) |
               ANA_SG_CFG_3_INVALID_RX(conf->close_invalid_rx.value ? 1 : 0) |
               ANA_SG_CFG_3_OCTETS_EXCEEDED_ENA(conf->close_octets_exceeded.enable
                                                    ? 1
                                                    : 0) |
               ANA_SG_CFG_3_OCTETS_EXCEEDED(conf->close_octets_exceeded.value
                                                ? 1
                                                : 0));
                                         ...
                                         vtss_psfp_gate_status_t *const status)
{
    ...
    status->config_change_time.sec_msb =
        ANA_SG_STS_3_CFG_CHG_TIME_SEC_MSB_X(value);
...
#define LAN966X_SFLOW_MIN_SAMPLE_RATE                                          \
    0 /**< Minimum allowable sampling rate for sFlow */
#define LAN966X_SFLOW_MAX_SAMPLE_RATE                                          \
...
static u32 lan966x_sflow_hw_rate(const u32  desired_sw_rate,
                                 u32 *const realizable_sw_rate)
{
    u32 hw_rate =
        desired_sw_rate
            ? VTSS_ROUNDING_DIVISION(LAN966X_SFLOW_MAX_SAMPLE_RATE + 1,
                                     desired_sw_rate)
            : 0;
    hw_rate = (hw_rate > LAN966X_SFLOW_MIN_SAMPLE_RATE ? hw_rate - 1 : hw_rate);
    *realizable_sw_rate =
        VTSS_ROUNDING_DIVISION(LAN966X_SFLOW_MAX_SAMPLE_RATE + 1, hw_rate + 1);
    return hw_rate;
}
                                                ...
                                                u32 *const rate_out)
{
    ...
    if (sflow_max_power_of_two == 0) {
        sflow_max_power_of_two =
            next_power_of_two(LAN966X_SFLOW_MAX_SAMPLE_RATE);
                                            ...
                                            *const new_conf)
{
    ...
    vtss_sflow_type_t       type =
        (new_conf->sampling_rate == 0 ? VTSS_SFLOW_TYPE_NONE : new_conf->type);

    *cur_conf = *new_conf;
    hw_rate = lan966x_sflow_hw_rate(new_conf->sampling_rate,
                                    &cur_conf->sampling_rate);
    REG_WR(ANA_SFLOW_CFG(VTSS_CHIP_PORT(port_no)),
           ANA_SFLOW_CFG_SF_RATE(hw_rate) |
               (type == VTSS_SFLOW_TYPE_ALL || type == VTSS_SFLOW_TYPE_RX
                    ? ANA_SFLOW_CFG_SF_SAMPLE_RX_M
                    : 0) |
               (type == VTSS_SFLOW_TYPE_ALL || type == VTSS_SFLOW_TYPE_TX
                    ? ANA_SFLOW_CFG_SF_SAMPLE_TX_M
                    : 0));
    return VTSS_RC_OK;
}
                                       ...
                                       const vtss_debug_info_t *const info)
{
    ...
    vtss_lan966x_debug_reg(vtss_state, ss, REG_ADDR(ANA_PMAC_CFG), "PMAC_CFG");
    for (i = 0; i < VTSS_MAC_INDEX_VID_CNT; i++) {
        vtss_lan966x_debug_reg_inst(vtss_state, ss,
                                    REG_ADDR(ANA_PMAC_VLAN_CFG(i)), i,
                                    "PMAC_VLAN_CFG");
    }
    pr("\n");
    ...
    vtss_debug_print_sticky(ss, "FLOOD_DISCARD", value,
                            ANA_ANEVENTS_FLOOD_DISCARD_M);
    vtss_debug_print_sticky(ss, "MSTI_DROP", value, ANA_ANEVENTS_MSTI_DROP_M);
    vtss_debug_print_sticky(ss, "ACLKILL", value, ANA_ANEVENTS_ACLKILL_M);
    vtss_debug_print_sticky(ss, "ACLUSED", value, ANA_ANEVENTS_ACLUSED_M);
    vtss_debug_print_sticky(ss, "AUTOAGE", value, ANA_ANEVENTS_AUTOAGE_M);
    vtss_debug_print_sticky(ss, "LEARN_DROP", value, ANA_ANEVENTS_LEARN_DROP_M);
    vtss_debug_print_sticky(ss, "AGED_ENTRY", value, ANA_ANEVENTS_AGED_ENTRY_M);
    vtss_debug_print_sticky(ss, "CPU_LEARN_FAILED", value,
                            ANA_ANEVENTS_CPU_LEARN_FAILED_M);
    vtss_debug_print_sticky(ss, "AUTO_LEARN_FAILED", value,
                            ANA_ANEVENTS_AUTO_LEARN_FAILED_M);
    ...
    vtss_debug_print_sticky(ss, "AUTO_MOVED", value, ANA_ANEVENTS_AUTO_MOVED_M);
    vtss_debug_print_sticky(ss, "CLASSIFIED_DROP", value,
                            ANA_ANEVENTS_CLASSIFIED_DROP_M);
    vtss_debug_print_sticky(ss, "CLASSIFIED_COPY", value,
                            ANA_ANEVENTS_CLASSIFIED_COPY_M);
    vtss_debug_print_sticky(ss, "VLAN_DISCARD", value,
                            ANA_ANEVENTS_VLAN_DISCARD_M);
    vtss_debug_print_sticky(ss, "FWD_DISCARD", value,
                            ANA_ANEVENTS_FWD_DISCARD_M);
    vtss_debug_print_sticky(ss, "MULTICAST_FLOOD", value,
                            ANA_ANEVENTS_MULTICAST_FLOOD_M);
    vtss_debug_print_sticky(ss, "UNICAST_FLOOD", value,
                            ANA_ANEVENTS_UNICAST_FLOOD_M);
    vtss_debug_print_sticky(ss, "DEST_KNOWN", value, ANA_ANEVENTS_DEST_KNOWN_M);
    vtss_debug_print_sticky(ss, "BUCKET3_MATCH", value,
                            ANA_ANEVENTS_BUCKET3_MATCH_M);
    vtss_debug_print_sticky(ss, "BUCKET2_MATCH", value,
                            ANA_ANEVENTS_BUCKET2_MATCH_M);
    vtss_debug_print_sticky(ss, "BUCKET1_MATCH", value,
                            ANA_ANEVENTS_BUCKET1_MATCH_M);
    vtss_debug_print_sticky(ss, "BUCKET0_MATCH", value,
                            ANA_ANEVENTS_BUCKET0_MATCH_M);
    vtss_debug_print_sticky(ss, "CPU_OPERATION", value,
                            ANA_ANEVENTS_CPU_OPERATION_M);
    vtss_debug_print_sticky(ss, "DMAC_LOOKUP", value,
                            ANA_ANEVENTS_DMAC_LOOKUP_M);
    vtss_debug_print_sticky(ss, "SMAC_LOOKUP", value,
                            ANA_ANEVENTS_SMAC_LOOKUP_M);
    vtss_debug_print_sticky(ss, "SEQ_GEN_ERR_0", value,
                            ANA_ANEVENTS_SEQ_GEN_ERR_0_M);
    vtss_debug_print_sticky(ss, "SEQ_GEN_ERR_1", value,
                            ANA_ANEVENTS_SEQ_GEN_ERR_1_M);
    pr("\n");
                                       ...
                                       const vtss_debug_info_t *const info)
{
    ...
    vtss_lan966x_debug_print_port_header(vtss_state, ss, "Port  ");
    for (port = 0; port <= VTSS_CHIP_PORTS; port++) {
        REG_RD(ANA_PGID(PGID_SRC + port), &mask);
        mask = ANA_PGID_PGID_X(mask);
        pr("%-4u  ", port);
        vtss_lan966x_debug_print_mask(ss, mask);
    }
    pr("\n");
                                  ...
                                  const vtss_debug_info_t *const info)
{
    ...
    VTSS_RC(lan966x_debug_src_table(vtss_state, ss, info));

    vtss_debug_print_header(ss, "Aggregation Masks");
    vtss_lan966x_debug_print_port_header(vtss_state, ss, "AC    ");
    ...
    vtss_lan966x_debug_print_port_header(vtss_state, ss, "PGID  CPU  Queue  ");
    for (i = 0; i < VTSS_PGIDS; i++) {
        REG_RD(ANA_PGID(i), &value);
        mask = ANA_PGID_PGID_X(value);
        REG_RD(ANA_PGID_CFG(i), &value);
        pr("%-4u  %-3u  %-5u  ", i, mask & VTSS_BIT(VTSS_CHIP_PORT_CPU) ? 1 : 0,
    ...
    vtss_lan966x_debug_reg_header(ss, "Aggr. Mode");
    vtss_lan966x_debug_reg(vtss_state, ss, REG_ADDR(ANA_AGGR_CFG), "AGGR_CFG");
    pr("\n");
                                 ...
                                 const vtss_debug_info_t *const info)
{
    ...
    for (port = 0; port < VTSS_CHIP_PORTS; port++) {
        ...
        header = 0;
        vtss_lan966x_debug_reg_inst(vtss_state, ss,
                                    REG_ADDR(ANA_PORT_CFG(port)), port,
                                    "PORT_CFG");
    }
    if (!header)
        pr("\n");
                                  ...
                                  const vtss_debug_info_t *const info)
{
    ...
    for (sdx = vtss_state->l2.sdx_info.iflow; sdx != NULL; sdx = sdx->next) {
        if (sdx->conf.frer.mstream_enable == 0 &&
            sdx->conf.frer.generation == 0) {
            continue;
        }
        if (header) {
            header = 0;
        ...
        REG_WR(ANA_STREAMACCESS,
               ANA_STREAMACCESS_STREAM_TBL_CMD(LAN966X_TBL_CMD_READ));
        REG_RD(ANA_STREAMACCESS, &val);
        pr("%-6u%-5u%-5u0x%04x  ", sdx->sdx,
        ...
        for (i = 0; i < 4; i++) {
            ...
            pr("%x%s", QSYS_FRER_PORT_FRER_EGR_PORT_X(val), i < 3 ? "-" : "\n");
        }
    }
    if (!header) {
        pr("\n");
        ...
        for (port_no = 0; port_no < vtss_state->port_count; port_no++) {
            if (VTSS_PORT_BF_GET(ms->port_list, port_no)) {
                ...
                vtss_lan966x_debug_reg_inst(vtss_state, ss,
                                            ...
                                            idx, "FRER_CFG_MBM");
                REG_WRM(QSYS_FRER_CFG, QSYS_FRER_CFG_ADDR(idx),
                        QSYS_FRER_CFG_ADDR_M);
                vtss_lan966x_debug_reg_inst(vtss_state, ss,
                                            REG_ADDR(QSYS_FRER_STA_MBM), idx,
                                            "FRER_STA_MBM");
                vtss_lan966x_debug_reg_inst(vtss_state, ss,
                                            REG_ADDR(QSYS_FRER_HST_MBM), idx,
                                            "FRER_HST_MBM");
                idx++;
                pr("\n");
            }
    ...
    for (i = 0; i < VTSS_CSTREAM_CNT; i++) {
        ...
        vtss_lan966x_debug_reg_inst(vtss_state, ss, REG_ADDR(QSYS_FRER_STA_CMP),
                                    i, "QSYS:FRER_STA_CMP");
        vtss_lan966x_debug_reg_inst(vtss_state, ss, REG_ADDR(QSYS_FRER_HST_CMP),
                                    i, "QSYS:FRER_HST_CMP");
        pr("\n");
    }
    return VTSS_RC_OK;
                                  ...
                                  const vtss_debug_info_t *const info)
{
    ...
    for (i = 0; i < VTSS_PSFP_FILTER_CNT; i++) {
        ...
        if (info->full || conf->gate_enable || conf->max_sdu ||
            ...
            if (first) {
                first = FALSE;
                vtss_lan966x_debug_reg_header(ss, "PSFP Filters");
            }
            REG_WR(ANA_SFIDTIDX, ANA_SFIDTIDX_SFID_INDEX(i));
            REG_WR(ANA_SFIDACCESS,
                   ANA_SFIDACCESS_SFID_TBL_CMD(LAN966X_TBL_CMD_READ));
            vtss_lan966x_debug_reg_inst(vtss_state, ss, REG_ADDR(ANA_SFIDTIDX),
                                        i, "SFIDTIDX");
            vtss_lan966x_debug_reg_inst(vtss_state, ss,
                                        REG_ADDR(ANA_SFIDACCESS), i,
                                        "SFIDACCESS");
        }
    }
    if (!first) {
    ...
    for (i = 0; i < VTSS_PSFP_GATE_CNT; i++) {
        if (info->full || vtss_state->l2.psfp.gate[i].enable) {
            ...
            vtss_lan966x_debug_reg(vtss_state, ss, REG_ADDR(ANA_SG_CFG_1),
                                   "SG_CFG_1");
            vtss_lan966x_debug_reg(vtss_state, ss, REG_ADDR(ANA_SG_CFG_2),
                                   "SG_CFG_2");
            vtss_lan966x_debug_reg(vtss_state, ss, REG_ADDR(ANA_SG_CFG_3),
                                   "SG_CFG_3");
            vtss_lan966x_debug_reg(vtss_state, ss, REG_ADDR(ANA_SG_CFG_4),
                                   "SG_CFG_4");
            vtss_lan966x_debug_reg(vtss_state, ss, REG_ADDR(ANA_SG_CFG_5),
                                   "SG_CFG_5");
            for (j = 0; j < 4; j++) {
                vtss_lan966x_debug_reg_inst(vtss_state, ss,
                                            REG_ADDR(ANA_SG_GCL_GS_CFG(j)), j,
                                            "GS_CFG");
                vtss_lan966x_debug_reg_inst(vtss_state, ss,
                                            REG_ADDR(ANA_SG_GCL_TI_CFG(j)), j,
                                            "TI_CFG");
                vtss_lan966x_debug_reg_inst(vtss_state, ss,
                                            REG_ADDR(ANA_SG_GCL_OCT_CFG(j)), j,
                                            "OCT_CFG");
            }
            vtss_lan966x_debug_reg(vtss_state, ss, REG_ADDR(ANA_SG_STS_1),
                                   "SG_STS_1");
            vtss_lan966x_debug_reg(vtss_state, ss, REG_ADDR(ANA_SG_STS_2),
                                   "SG_STS_2");
            vtss_lan966x_debug_reg(vtss_state, ss, REG_ADDR(ANA_SG_STS_3),
                                   "SG_STS_3");
            pr("\n");
        }
    }
                                  ...
                                  const vtss_debug_info_t *const info)
{
    ...
    for (port = 0; port < (VTSS_CHIP_PORTS + 2); port++) {
        ...
        if (port != VTSS_CHIP_PORT_CPU_1) {
            vtss_lan966x_debug_reg_inst(vtss_state, ss,
                                        ...
                                        "ANA:VLAN_CFG");
            vtss_lan966x_debug_reg_inst(vtss_state, ss,
                                        REG_ADDR(ANA_DROP_CFG(port)), port,
                                        "ANA:DROP_CFG");
        }
        vtss_lan966x_debug_reg_inst(vtss_state, ss,
                                    REG_ADDR(REW_PORT_VLAN_CFG(port)), port,
                                    "REW:VLAN_CFG");
        vtss_lan966x_debug_reg_inst(vtss_state, ss, REG_ADDR(REW_TAG_CFG(port)),
                                    port, "REW:TAG_CFG");
        pr("\n");
    }
    vtss_lan966x_debug_reg(vtss_state, ss, REG_ADDR(ANA_VLAN_ETYPE_CFG),
                           "ETYPE_CFG");
    vtss_lan966x_debug_reg(vtss_state, ss, REG_ADDR(ANA_ADVLEARN), "ADVLEARN");
    vtss_lan966x_debug_reg(vtss_state, ss, REG_ADDR(ANA_VLANMASK), "VLANMASK");
    pr("\n");
    ...
    for (vid = VTSS_VID_NULL; vid < VTSS_VIDS; vid++) {
        ...
        REG_WR(ANA_VLANACCESS,
               ANA_VLANACCESS_VLAN_TBL_CMD(VLANACCESS_CMD_READ));
        ...
        if (header)
            vtss_lan966x_debug_print_port_header(
                vtss_state, ss, "VID   FID  Lrn  Fld  Mir  Flt  Prv  ");
        header = 0;
...
{
    vtss_lan966x_debug_reg_header(ss, "ANA");
    vtss_lan966x_debug_reg(vtss_state, ss, REG_ADDR(ANA_ISOLATED_PORTS),
                           "ISOLATED_PORTS");
    pr("\n");

    return lan966x_debug_src_table(vtss_state, ss, info);
}
                                    ...
                                    const vtss_debug_info_t *const info)
{
    ...
    vtss_lan966x_debug_print_port_header(vtss_state, ss, "Mirror   ");
    pr("Ingress  ");
    vtss_lan966x_debug_print_mask(ss, mask);
...
vtss_rc vtss_lan966x_l2_debug_print(vtss_state_t                  *vtss_state,
                                    ...
                                    const vtss_debug_info_t *const info)
{
    VTSS_RC(vtss_debug_print_group(VTSS_DEBUG_GROUP_VLAN, lan966x_debug_vlan,
                                   vtss_state, ss, info));
    VTSS_RC(vtss_debug_print_group(VTSS_DEBUG_GROUP_PVLAN, lan966x_debug_pvlan,
                                   vtss_state, ss, info));
    VTSS_RC(vtss_debug_print_group(VTSS_DEBUG_GROUP_MAC_TABLE,
                                   lan966x_debug_mac_table, vtss_state, ss,
                                   info));
#if defined(VTSS_FEATURE_VCAP)
    VTSS_RC(vtss_debug_print_group(VTSS_DEBUG_GROUP_VXLAT, lan966x_debug_vxlat,
                                   vtss_state, ss, info));
#endif
    VTSS_RC(vtss_debug_print_group(VTSS_DEBUG_GROUP_AGGR, lan966x_debug_aggr,
                                   vtss_state, ss, info));
    VTSS_RC(vtss_debug_print_group(VTSS_DEBUG_GROUP_STP, lan966x_debug_stp,
                                   vtss_state, ss, info));
    VTSS_RC(vtss_debug_print_group(VTSS_DEBUG_GROUP_MIRROR,
                                   lan966x_debug_mirror, vtss_state, ss, info));
    return VTSS_RC_OK;
}
...
static vtss_rc lan966x_l2_port_map_set(vtss_state_t *vtss_state)
{
    ...
    vtss_state->l2.pgid_count =
        (VTSS_PGIDS - VTSS_CHIP_PORTS + vtss_state->port_count);
    ...
    for (i = 0; i < VTSS_PRIOS; i++) {
        REG_WR(ANA_FLOODING(i), ANA_FLOODING_FLD_UNICAST(PGID_UC) |
                                    ANA_FLOODING_FLD_BROADCAST(PGID_MC) |
                                    ANA_FLOODING_FLD_MULTICAST(PGID_MC));
    }
    REG_WR(ANA_FLOODING_IPMC, ANA_FLOODING_IPMC_FLD_MC4_CTRL(PGID_MC) |
...
static vtss_rc lan966x_l2_init(vtss_state_t *vtss_state)
{
    ...
    for (port = 0; port < VTSS_CHIP_PORTS; port++) {
        ...
        REG_WRM(ANA_PORT_MODE(port), ANA_PORT_MODE_REDTAG_PARSE_CFG(1),
    ...
    REG_WR(SYS_FRM_AGING, SYS_FRM_AGING_AGE_TX_ENA_M |
...
vtss_rc vtss_lan966x_l2_init(vtss_state_t *vtss_state, vtss_init_cmd_t cmd)
{
    ...
    switch (cmd) {
    case VTSS_INIT_CMD_CREATE: state->ac_count = LAN966X_ACS;
#if defined(VTSS_SDX_CNT)
        state->sdx_info.max_count = VTSS_SDX_CNT;
#endif
        break;

    case VTSS_INIT_CMD_INIT: VTSS_RC(lan966x_l2_init(vtss_state)); break;

    case VTSS_INIT_CMD_PORT_MAP:
        VTSS_RC(lan966x_l2_port_map_set(vtss_state));
        break;

    case VTSS_INIT_CMD_POLL: VTSS_RC(lan966x_l2_poll(vtss_state)); break;
