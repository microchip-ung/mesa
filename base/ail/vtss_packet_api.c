                                       ...
                                       BOOL                      tx_filter)
{
    ...
    for (port_tx = VTSS_PORT_NO_START; port_tx < state->port_count; port_tx++) {
        ...
        if (port_rx != VTSS_PORT_NO_NONE) {
            ...
            /* If Tx filtering is not required, return result for the first port
             * only */
            if (!tx_filter) {
...
vtss_rc vtss_packet_inst_create(vtss_state_t *vtss_state)
{
    ...
    /* Learn-all frames should only be enabled on JR-stacking and JR-48
     * platforms. */
 ...
 *
 * On successful exit:
 *   #chip_port_mask is set to the physical port mask corresponding to the
 * logical port mask we were called with. #chip_no        is set to the chip
 * number for which #chip_port_mask applies. #stack_port_no  is set to the
 * logical stack port number if #logical_port_mask points out a stack port,
 * VTSS_PORT_NO_NONE if not. #port_cnt       is set to the number of bits set in
 * #logical_port_mask. #port_no        is set to the last logical port number
 * investigated in #logical_port_mask. Usually only useful when #port_cnt is
 * exactly 1.
 */
                                           ...
                                           vtss_port_no_t           *port_no)
{
    ...
    /* Lint cannot see that state->port_count < VTSS_PORT_ARRAY_SIZE, so it
     * thinks that port_map[p] can happen to exceed the array size.
     */
                                             ...
                                             const vtss_phys_port_no_t chip_port)
{
    ...
    for (port_no = VTSS_PORT_NO_START; port_no < state->port_count; port_no++) {
        if ((u32)VTSS_CHIP_PORT_FROM_STATE(state, port_no) == chip_port &&
            VTSS_CHIP_NO_FROM_STATE(state, port_no) == chip_no) {
            return port_no;
        }
    }
