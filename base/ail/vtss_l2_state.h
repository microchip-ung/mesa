...
#define VTSS_PORT_RX_FORWARDING(fwd_state)                                     \
    (fwd_state == VTSS_PORT_FORWARD_ENABLED ||                                 \
             fwd_state == VTSS_PORT_FORWARD_INGRESS                            \
         ? 1                                                                   \
         : 0)
#define VTSS_PORT_TX_FORWARDING(fwd_state)                                     \
    (fwd_state == VTSS_PORT_FORWARD_ENABLED ||                                 \
             fwd_state == VTSS_PORT_FORWARD_EGRESS                             \
         ? 1                                                                   \
         : 0)
...
#define VTSS_MAC_FLAG_ADDED                                                    \
    0x01 /* Entry added flag used for warm start synchronization */
...
#define VSI_USED_FOR_OAM                                                       \
    127 /* This VSI is reserved for OAM in order to fix problem with Port      \
           ...
           VLAN */
#define VTSS_VSI_CNT                                                           \
    126 /* This is the number of VSI that can be allocated (1-VTSS_VSI_CNT) */
#else
#define VSI_USED_FOR_OAM                                                       \
    1023 /* This VSI is reserved for OAM in order to fix problem with Port     \
            ...
            VLAN */
#define VTSS_VSI_CNT                                                           \
    1022 /* This is the number of VSI that can be allocated (1-VTSS_VSI_CNT) */
#endif
...
typedef struct {
    ...
    vtss_erps_counter_t
        erps_discard_cnt[VTSS_PORT_ARRAY_SIZE]; /* ERPS discard counter */
#endif
...
typedef struct {
    ...
    vtss_vlan_trans_grp2vlan_entry_t  trans_list
        [VTSS_VLAN_TRANS_MAX_CNT]; /* Actual storage for list members   */
...
typedef struct {
    ...
    vtss_vlan_trans_port2grp_entry_t
        port_list[VTSS_VLAN_TRANS_GROUP_MAX_CNT]; /* Actual storage for list
                                                     members   */
...
typedef struct {
    vtss_chip_counter_pair_t
        rx_unicast; /**< Unicast packets and bytes counted on this VLAN */
    vtss_chip_counter_pair_t
        rx_multicast; /**< Multicast packets and bytes counted on this VLAN */
    vtss_chip_counter_pair_t
        rx_broadcast; /**< Broadcast packets and bytes counted on this VLAN */
...
typedef struct {
    ...
    vtss_vlan_chip_counters_t
        counters[VTSS_VIDS]; /* Counters for all the VLANs */
...
#define VTSS_IPMC_DST_MAX                                                      \
    (4 * 4 * 256) /* JR2: Quarter IS2 rules in VCAP_SUPER */
#else
#define VTSS_IPMC_DST_MAX                                                      \
    (4 * 5 * 256) /* FA: Quarter IS2 rules in VCAP_SUPER */
#endif
...
typedef struct {
    ...
    vtss_rc (*move)(struct vtss_state_s *vtss_state,
                    u16                  idx_old,
                    u16                  idx_new,
                    u16                  count);
...
#else
    vtss_sdx_counters_t
        sdx_table[VTSS_SDX_CNT +
                  1]; /* Allow 1-based indexing (index zero is unused) */
#endif
...
#endif
#if defined(VTSS_FEATURE_RCL)
vtss_rc vtss_cil_l2_rcl_vid_conf_set(struct vtss_state_s *vtss_state,
                                     const u8             idx);
#endif
#if defined(VTSS_FEATURE_REDBOX)
...
typedef struct {
    ...
    vtss_dlb_policer_conf_t
        pol_conf[VTSS_EVC_POL_CNT]; /* Policer configuration */
#if defined(VTSS_FEATURE_PSFP)
...
vtss_rc vtss_cmn_vlan_members_get(struct vtss_state_s *state,
                                  const vtss_vid_t     vid,
                                  BOOL member[VTSS_PORT_ARRAY_SIZE]);
vtss_rc vtss_cmn_vlan_members_set(struct vtss_state_s *vtss_state,
                                  const vtss_vid_t     vid);
vtss_rc vtss_cmn_vlan_port_conf_set(struct vtss_state_s *vtss_state,
                                    const vtss_port_no_t port_no);
...
vtss_rc vtss_cmn_erps_port_state_set(struct vtss_state_s *vtss_state,
                                     ...
                                     const vtss_port_no_t port_no);
#endif
vtss_rc vtss_cmn_eps_port_set(struct vtss_state_s *vtss_state,
...
#endif
vtss_rc vtss_cmn_vcl_port_conf_set(struct vtss_state_s *vtss_state,
                                   const vtss_port_no_t port_no);
vtss_rc vtss_cmn_vce_add(struct vtss_state_s    *vtss_state,
                         const vtss_vce_id_t     vce_id,
                         const vtss_vce_t *const vce);
vtss_rc vtss_cmn_vce_del(struct vtss_state_s *vtss_state,
                         const vtss_vce_id_t  vce_id);
...
vtss_sdx_entry_t *vtss_cmn_sdx_alloc(struct vtss_state_s *vtss_state,
                                     ...
                                     u16                  id,
                                     BOOL                 isdx);
void              vtss_cmn_sdx_free(struct vtss_state_s *vtss_state,
                                    vtss_sdx_entry_t    *sdx,
                                    BOOL                 isdx);
vtss_rc           vtss_cmn_policer_alloc(struct vtss_state_s *vtss_state,
                                         u8                   count,
                                         u16                 *idx);
vtss_rc vtss_cmn_policer_free(struct vtss_state_s *vtss_state, u16 *idx);
vtss_rc vtss_cmn_istat_alloc(struct vtss_state_s *vtss_state,
                             u8                   count,
                             u16                 *idx);
vtss_rc vtss_cmn_istat_free(struct vtss_state_s *vtss_state, u16 *idx);
vtss_rc vtss_cmn_estat_alloc(struct vtss_state_s *vtss_state,
                             u8                   count,
                             u16                 *idx);
vtss_rc vtss_cmn_estat_free(struct vtss_state_s *vtss_state, u16 *idx);
vtss_sdx_entry_t   *vtss_iflow_lookup(struct vtss_state_s *vtss_state,
                                      vtss_iflow_id_t      id);
vtss_eflow_entry_t *vtss_eflow_lookup(struct vtss_state_s *vtss_state,
                                      vtss_eflow_id_t      id);
#endif
#if defined(VTSS_FEATURE_XSTAT)
