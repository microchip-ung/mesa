...
{
    switch (sel) {
    case FWD_NOP:          return "NOP";
    case FWD_COPY_CPU:     return "COPY_CPU";
    case FWD_REDIRECT_CPU: return "REDIRECT_CPU";
    case FWD_DISCARD:      return "DISCARD";
...
{
    switch (ring_role) {
    case VTSS_MRP_RING_ROLE_DISABLED: return "Disabled";
    case VTSS_MRP_RING_ROLE_CLIENT:   return "Client";
    case VTSS_MRP_RING_ROLE_MANAGER:  return "Manager";
    default:                          return "ERROR!";
    }
}
...
{
    switch (state) {
    case VTSS_MRP_PORT_STATE_DISABLED:   return "Disabled";
    case VTSS_MRP_PORT_STATE_BLOCKED:    return "Blocked";
    case VTSS_MRP_PORT_STATE_FORWARDING: return "Forwarding";
    default:                             return "ERROR!";
    }
}
...
{
    /*
     * These macros will aid in reading and optionally clearing counter
     * registers. The clear operation will be triggered by matching the
...
static vtss_rc mrp_uninit_port(vtss_state_t *vtss_state, vtss_voe_idx_t voe_idx)
{
    ...
    REG_WRM(VTSS_VOP_MRP_MRP_CTRL(voe_idx), VTSS_F_VOP_MRP_MRP_CTRL_MRP_ENA(0),
            VTSS_M_VOP_MRP_MRP_CTRL_MRP_ENA);
...
static vtss_rc mrp_port_update_best_mrm(vtss_state_t              *vtss_state,
                                        vtss_voe_idx_t             voe_idx,
                                      ...
                                      vtss_mrp_data_t *mrp)
{
    ...
    for (p = 0; p < 3; p++) {
        ...
        REG_WRM(VTSS_VOP_MRP_MRP_FWD_CTRL(p == 0   ? p_voe_idx
                                          : p == 1 ? s_voe_idx
                                                   : i_voe_idx),
    ...
    for (p = 0; p < 3; p++) {
        ...
        REG_WR(VTSS_VOP_MRP_RING_MASK_CFG(p == 0   ? p_voe_idx
                                          : p == 1 ? s_voe_idx
                                                   : i_voe_idx),
               ring_mask);
        REG_WR(VTSS_VOP_MRP_ICON_MASK_CFG(p == 0   ? p_voe_idx
                                          : p == 1 ? s_voe_idx
                                                   : i_voe_idx),
               icon_mask);
    }
                                   ...
                                   vtss_mrp_data_t *mrp)
{
    ...
    for (p = 0; p < 3; p++) {
        ...
        REG_WRM(VTSS_VOP_MRP_MRP_TX_CFG(p == 0   ? p_voe_idx
                                        : p == 1 ? s_voe_idx
                                                 : i_voe_idx,
                                        CONFIG_INTEST),
                                    ...
                                    vtss_mrp_data_t *mrp)
{
    ...
    if (mrp->itst_loc_idx != LOC_PERIOD_CNT) {
        VTSS_RC(loc_period_configure(vtss_state, mrp->itst_loc_idx,
                                     mrp->tst_loc_conf.itst_interval));
    }
                                     ...
                                     vtss_mrp_data_t *mrp)
{
    ...
    // (VTSS_VOP_MRP_TST_CFG_SAMPLE_HI_PRIO_ENA) into MEP_TST_PRIO_CFG_TEST_PRIO
    // whenever we check against the current best MRM. This is safe, because it
    // only overwrites it if it's coming from the MRM currently marked as the
    // best MAC and its priority is higher than our own. Its value is not used
    // by the chip itself for anything.
                               ...
                               const vtss_mrp_conf_t *const conf)
{
    ...
    if (conf->ring_role != VTSS_MRP_RING_ROLE_MANAGER &&
        conf->ring_role != VTSS_MRP_RING_ROLE_CLIENT) {
    ...
    if ((conf->p_port == conf->s_port) || (conf->p_port == conf->i_port) ||
        (conf->s_port == conf->i_port)) {
    ...
    for (i = 0; i < VTSS_MRP_CNT; ++i) {
        ...
        if ((conf->p_port == mrp_array[i].conf.p_port) ||
            (conf->p_port == mrp_array[i].conf.s_port) ||
            (conf->p_port == mrp_array[i].conf.i_port)) {
            VTSS_E("Primary port already in use");
            return VTSS_RC_ERROR;
        }
        if ((conf->s_port == mrp_array[i].conf.p_port) ||
            (conf->s_port == mrp_array[i].conf.s_port) ||
            (conf->s_port == mrp_array[i].conf.i_port)) {
            VTSS_E("Secondary port already in use");
            return VTSS_RC_ERROR;
        }
        if ((conf->in_ring_role != VTSS_MRP_RING_ROLE_DISABLED) &&
            ((conf->i_port == mrp_array[i].conf.p_port) ||
             (conf->i_port == mrp_array[i].conf.s_port) ||
             (conf->i_port == mrp_array[i].conf.i_port))) {
                                         ...
                                         const vtss_mrp_ring_role_t role)
{
    ...
    if (mrp_data->tst_loc_idx == LOC_PERIOD_CNT) {
        VTSS_E(
            "Internal error: We're active and configured as MRA, but no TST LOC timer index is allocated");
        return VTSS_RC_ERROR;
    }
                                          ...
                                          const vtss_mrp_port_state_t state)
{
    ...
    if (port != mrp_data->conf.p_port && port != mrp_data->conf.s_port &&
        port != mrp_data->conf.i_port) {
                                           ...
                                           const vtss_mrp_idx_t mrp_idx)
{
    ...
    for (p = 0; p < 3; p++) {
        REG_WRM(VTSS_VOP_MRP_ITST_NXT_LOC_HMO(p == 0   ? mrp_data->p_voe_idx
                                              : p == 1 ? mrp_data->s_voe_idx
                                                       : mrp_data->i_voe_idx),
                                                 ...
                                                 *const copy)
{
    ...
    if (mrp_data->conf.i_port < VTSS_PORTS &&
        mrp_data->conf.in_ring_role == VTSS_MRP_RING_ROLE_MANAGER) {
        i_voe_idx = mrp_data->i_voe_idx;
                                      ...
                                      vtss_mrp_status_t *const status)
{
    ...
    for (p = 0; p < 3; p++) {
        ...
        port_status->tst_loc = VTSS_X_VOP_MRP_TST_CFG_LOC_PERIOD(value) != 0 &&
                               VTSS_X_VOP_MRP_TST_STAT_MISS_CNT(value1) ==
        ...
        port_status->dmac_err_seen =
            VTSS_X_VOP_MRP_MRP_STICKY_DMAC_ERR_STICKY(value) ? TRUE : FALSE;
        port_status->vers_err_seen =
            VTSS_X_VOP_MRP_MRP_STICKY_VERSION_ERR_STICKY(value) ? TRUE : FALSE;

        /* Clear the sticky bits that has been detected */
        value = value & (VTSS_M_VOP_MRP_MRP_STICKY_MRP_RX_STICKY |
                         VTSS_M_VOP_MRP_MRP_STICKY_MRP_RX_PROC_STICKY |
                         VTSS_M_VOP_MRP_MRP_STICKY_DMAC_ERR_STICKY |
                         VTSS_M_VOP_MRP_MRP_STICKY_VERSION_ERR_STICKY);
                                     ...
                                     vtss_mrp_event_t *const events)
{
    ...
    for (p = 0; p < 3; p++) {
        ...
        *mask = (VTSS_X_VOP_MRP_MRP_STICKY_TST_LOC_STICKY(sticky_mask) != 0
                     ? VTSS_MRP_EVENT_MASK_TST_LOC
                     : 0) |
                (VTSS_X_VOP_MRP_MRP_STICKY_ITST_LOC_STICKY(sticky_mask) != 0
                                 ...
                                 const vtss_debug_info_t *const info)
{
    ...
    if (!info->has_action || mrp) { /* MRP configuration must be printed */
        ...
        vtss_fa_debug_reg_header(pr, buf);
        vtss_fa_debug_reg_inst(vtss_state, pr, REG_ADDR(VTSS_VOP_TICK_CFG), 0,
                               "VOP_TICK_CFG");
        vtss_fa_debug_reg_inst(vtss_state, pr, REG_ADDR(VTSS_VOP_MRP_TS_CFG), 0,
                               "VOP_MRP_TS_CFG");
        vtss_fa_debug_reg_inst(vtss_state, pr, REG_ADDR(VTSS_VOP_LOC_CTRL), 0,
                               "VOP_LOC_CTRL");
        vtss_fa_debug_reg_inst(vtss_state, pr, REG_ADDR(VTSS_VOP_VOP_CTRL), 0,
                               "VOP_VOP_CTRL");
        vtss_fa_debug_reg_inst(vtss_state, pr, REG_ADDR(VTSS_VOP_CPU_EXTR_MRP),
                               0, "VOP_CPU_EXTR_MRP");
        pr("\n");
    }
    pr("\n");
...
vtss_rc vtss_lan969x_mrp_init(vtss_state_t *vtss_state, vtss_init_cmd_t cmd)
{
    ...
    case VTSS_INIT_CMD_POLL: VTSS_RC(lan969x_mrp_poll_1sec(vtss_state)); break;
