...
{
    if (voi_idx < RT_DOWN_VOI_CNT) {
        return (voi_idx);
    } else {
        return ((voi_idx - RT_DOWN_VOI_CNT) + 1); /* Up MIP instance 0 is not used as it cannot be addressed by ES0 action. */
    }
}
...
#define UPDATE16(v, cnt, ctype, cdir)   { vtss_cmn_counter_16_update(v, cnt, ((clear_mask & (ctype)) != 0) && ((clear_mask & (cdir)) != 0)); }
    ...
    UPDATE16(VTSS_X_VOP_CCM_RX_FRM_CNT_CCM_RX_VLD_FC_CNT(v),    &chipcnt->ccm.rx_valid_counter,    VTSS_OAM_CNT_CCM, VTSS_OAM_CNT_DIR_RX);
    UPDATE16(VTSS_X_VOP_CCM_RX_FRM_CNT_CCM_RX_INVLD_FC_CNT(v),  &chipcnt->ccm.rx_invalid_counter,  VTSS_OAM_CNT_CCM, VTSS_OAM_CNT_DIR_RX);
    CHIPREAD(VTSS_VOP_CCM_TX_SEQ_CFG(voe_idx),                  &chipcnt->ccm.tx_counter,          VTSS_OAM_CNT_CCM, VTSS_OAM_CNT_DIR_TX);

    REG_RD(VTSS_VOP_CCM_RX_WARNING(voe_idx), &v);
    UPDATE16(VTSS_X_VOP_CCM_RX_WARNING_CCM_RX_SEQNO_ERR_CNT(v),  &chipcnt->ccm.rx_oo_counter,      VTSS_OAM_CNT_CCM, VTSS_OAM_CNT_DIR_RX);

    /* LB counter update */
    VTSS_RC(doing_calculate(vtss_state, voe_idx, &doing_lb, &doing_tst));
    if (doing_lb || (clear_mask & VTSS_OAM_CNT_LB)) {
        CHIPREAD(VTSS_VOP_LBR_RX_FRM_CNT(voe_idx),         &chipcnt->lb.rx_lbr_counter,      VTSS_OAM_CNT_LB, VTSS_OAM_CNT_DIR_RX);
        CHIPREAD(VTSS_VOP_LBM_TX_TRANSID_CFG(voe_idx),     &chipcnt->lb.tx_lbm_counter,      VTSS_OAM_CNT_LB, VTSS_OAM_CNT_DIR_TX);
        CHIPREAD(VTSS_VOP_LBR_RX_TRANSID_ERR_CNT(voe_idx), &chipcnt->lb.rx_lbr_oo_counter,   VTSS_OAM_CNT_LB, VTSS_OAM_CNT_DIR_RX);
        CHIPREAD(VTSS_VOP_LBR_CRC_ERR_CNT(voe_idx),        &chipcnt->lb.rx_lbr_crc_counter,  VTSS_OAM_CNT_LB, VTSS_OAM_CNT_DIR_RX);
        CHIPREAD(VTSS_VOP_LBR_TX_FRM_CNT(voe_idx),         &chipcnt->lb.tx_lbr_counter,      VTSS_OAM_CNT_LB, VTSS_OAM_CNT_DIR_TX);
    }

    /* VOE counter update */
    CHIPREAD(VTSS_VOP_RX_SEL_OAM_CNT(voe_idx), &chipcnt->voe.rx_selected_counter,  VTSS_OAM_CNT_VOE, VTSS_OAM_CNT_DIR_RX);
    CHIPREAD(VTSS_VOP_TX_SEL_OAM_CNT(voe_idx), &chipcnt->voe.tx_selected_counter,  VTSS_OAM_CNT_VOE, VTSS_OAM_CNT_DIR_TX);
    CHIPREAD(VTSS_VOP_RX_OAM_FRM_CNT(voe_idx), &chipcnt->voe.rx_counter,           VTSS_OAM_CNT_VOE, VTSS_OAM_CNT_DIR_RX);
    CHIPREAD(VTSS_VOP_TX_OAM_FRM_CNT(voe_idx), &chipcnt->voe.tx_counter,           VTSS_OAM_CNT_VOE, VTSS_OAM_CNT_DIR_TX);
    CHIPREAD(VTSS_VOP_TX_OAM_DISCARD(voe_idx), &chipcnt->voe.tx_discard_counter,   VTSS_OAM_CNT_VOE, VTSS_OAM_CNT_DIR_TX);
    CHIPREAD(VTSS_VOP_RX_OAM_DISCARD(voe_idx), &chipcnt->voe.rx_discard_counter,   VTSS_OAM_CNT_VOE, VTSS_OAM_CNT_DIR_RX);
...
{
    REG_WRM(VTSS_VOP_MASTER_INTR_CTRL, VTSS_F_VOP_MASTER_INTR_CTRL_OAM_MEP_INTR_ENA(enable ? 1 : 0), VTSS_M_VOP_MASTER_INTR_CTRL_OAM_MEP_INTR_ENA);
...
static vtss_rc oam_vop_int_update(vtss_state_t *vtss_state)
{
    ...
    for (i = 0, must_enable = 0; !must_enable && i < RT_VOE_CNT; ++i) {
                               ...
                               const vtss_vop_conf_t   *const conf)
{
    ...
    REG_WR(VTSS_VOP_COMMON_MEP_MC_MAC_MSB, VTSS_F_VOP_COMMON_MEP_MC_MAC_MSB_MEP_MC_MAC_MSB(value));
    ...
    REG_WR(VTSS_VOP_COMMON_MEP_MC_MAC_LSB, VTSS_F_VOP_COMMON_MEP_MC_MAC_LSB_MEP_MC_MAC_LSB(value));
    ...
    value = VTSS_F_VOP_OAM_GENERIC_CFG_GENERIC_OPCODE_CPU_QU(conf->voe_queue_aps) |
    ...
    REG_WR(VTSS_VOP_HMO_PERIOD_CFG(1), (((u64)conf->auto_copy_period[1]) * 1000) / loc_base / (8 / 2));
    REG_WR(VTSS_VOP_HMO_PERIOD_CFG(0), (((u64)conf->auto_copy_period[0]) * 1000) / loc_base / (8 / 2));

    REG_WR(VTSS_VOP_HMO_TIMER_CFG, (VTSS_F_VOP_HMO_TIMER_CFG_HMO_RX_CCM_NXT_TIMER(conf->auto_copy_ccm_valid ? 1 : 0)       |
                                    VTSS_F_VOP_HMO_TIMER_CFG_HMO_CCM_RX_TLV_NON_ZERO_TIMER(conf->auto_copy_ccm_tlv ? 1 :0) |
                                    VTSS_F_VOP_HMO_TIMER_CFG_HMO_CCM_RX_BAD_NXT_TIMER(conf->auto_copy_ccm_err ? 1 : 0)     |
                                    VTSS_F_VOP_HMO_TIMER_CFG_HMO_RX_TEST_FRM_NXT_TIMER(conf->auto_copy_lbr ? 1 : 0)));

    /* Enable VOP */
    value = VTSS_F_VOP_VOP_CTRL_VOP_ENA(1) | VTSS_F_VOP_VOP_CTRL_LOC_SCAN_ENA(1) | VTSS_F_VOP_VOP_CTRL_CCM_RX_SRC_PORT_DETECT_CNT(3);
                                       ...
                                       u32            *const active)
{
    ...
    /* just scan all VOEs: */
    for (i = 0; (i < active_size) && (i < RT_EVENT_MASK_ARRAY_SIZE); ++i) {
                                     ...
                                     const BOOL            enable)
{
    ...
    reg_mask = (((mask & VTSS_VOE_EVENT_MASK_CCM_SRC_PORT_MOVE)   ? VTSS_M_VOP_INTR_ENA_CCM_RX_SRC_PORT_DETECT_INTR_ENA : 0) |
                ((mask & VTSS_VOE_EVENT_MASK_CCM_TLV_PORT_STATUS) ? VTSS_M_VOP_INTR_ENA_TLV_PORT_STATUS_INTR_ENA : 0) |
                ((mask & VTSS_VOE_EVENT_MASK_CCM_TLV_IF_STATUS)   ? VTSS_M_VOP_INTR_ENA_TLV_INTERFACE_STATUS_INTR_ENA : 0) |
                ((mask & VTSS_VOE_EVENT_MASK_CCM_PERIOD)          ? VTSS_M_VOP_INTR_ENA_CCM_UNEXP_PERIOD_INTR_ENA : 0) |
                ((mask & VTSS_VOE_EVENT_MASK_CCM_PRIORITY)        ? VTSS_M_VOP_INTR_ENA_CCM_UNEXP_PRIO_INTR_ENA : 0) |
                ((mask & VTSS_VOE_EVENT_MASK_CCM_ZERO_PERIOD)     ? VTSS_M_VOP_INTR_ENA_CCM_ZERO_PERIOD_INTR_ENA : 0) |
                ((mask & VTSS_VOE_EVENT_MASK_CCM_RX_RDI)          ? VTSS_M_VOP_INTR_ENA_CCM_RX_RDI_INTR_ENA : 0) |
                ((mask & VTSS_VOE_EVENT_MASK_CCM_LOC)             ? VTSS_M_VOP_INTR_ENA_CCM_LOC_INTR_ENA : 0) |
                ((mask & VTSS_VOE_EVENT_MASK_CCM_MEP_ID)          ? VTSS_M_VOP_INTR_ENA_CCM_UNEXP_MEPID_INTR_ENA : 0) |
                ((mask & VTSS_VOE_EVENT_MASK_CCM_MEG_ID)          ? VTSS_M_VOP_INTR_ENA_CCM_UNEXP_MEGID_INTR_ENA : 0) |
                ((mask & VTSS_VOE_EVENT_MASK_CCM_MEG_LEVEL)       ? VTSS_M_VOP_INTR_ENA_CCM_RX_MEL_LOW_INTR_ENA : 0));
...
VTSS_D("enable_mask %X", enable_mask);
    ...
    (VTSS_F_VOP_INTR_STICKY_CCM_RX_SRC_PORT_DETECT_STICKY((enable_mask & VTSS_M_VOP_INTR_ENA_CCM_RX_SRC_PORT_DETECT_INTR_ENA)   ? 1 : 0) |
     ...
     VTSS_F_VOP_INTR_STICKY_CCM_UNEXP_PERIOD_STICKY((enable_mask       & VTSS_M_VOP_INTR_ENA_CCM_UNEXP_PERIOD_INTR_ENA)         ? 1 : 0) |
     VTSS_F_VOP_INTR_STICKY_CCM_UNEXP_PRIO_STICKY((enable_mask         & VTSS_M_VOP_INTR_ENA_CCM_UNEXP_PRIO_INTR_ENA)           ? 1 : 0) |
     VTSS_F_VOP_INTR_STICKY_CCM_ZERO_PERIOD_RX_ERR_STICKY((enable_mask & VTSS_M_VOP_INTR_ENA_CCM_ZERO_PERIOD_INTR_ENA)          ? 1 : 0) |
     VTSS_F_VOP_INTR_STICKY_CCM_RX_RDI_STICKY((enable_mask             & VTSS_M_VOP_INTR_ENA_CCM_RX_RDI_INTR_ENA)               ? 1 : 0) |
     VTSS_F_VOP_INTR_STICKY_CCM_LOC_STICKY((enable_mask                & VTSS_M_VOP_INTR_ENA_CCM_LOC_INTR_ENA)                  ? 1 : 0) |
     VTSS_F_VOP_INTR_STICKY_CCM_UNEXP_MEPID_STICKY((enable_mask        & VTSS_M_VOP_INTR_ENA_CCM_UNEXP_MEPID_INTR_ENA)          ? 1 : 0) |
     VTSS_F_VOP_INTR_STICKY_CCM_UNEXP_MEGID_STICKY((enable_mask        & VTSS_M_VOP_INTR_ENA_CCM_UNEXP_MEGID_INTR_ENA)          ? 1 : 0) |
     VTSS_F_VOP_INTR_STICKY_CCM_RX_MEL_LOW_STICKY((enable_mask         & VTSS_M_VOP_INTR_ENA_CCM_RX_MEL_LOW_INTR_ENA)           ? 1 : 0));
VTSS_D("enable_mask %X", enable_mask);
...
VTSS_D("sticky_mask %X", sticky_mask);
    REG_WR(VTSS_VOP_INTR_STICKY(voe_idx), sticky_mask); /* Sticky bits cleared by writing 1 to them */

    /* Translate sticky mask to returned event mask */
    *mask = ((VTSS_X_VOP_INTR_STICKY_CCM_RX_SRC_PORT_DETECT_STICKY(sticky_mask) ? VTSS_VOE_EVENT_MASK_CCM_SRC_PORT_MOVE   : 0) |
             (VTSS_X_VOP_INTR_STICKY_TLV_PORT_STATUS_STICKY(sticky_mask)        ? VTSS_VOE_EVENT_MASK_CCM_TLV_PORT_STATUS : 0) |
             (VTSS_X_VOP_INTR_STICKY_TLV_INTERFACE_STATUS_STICKY(sticky_mask)   ? VTSS_VOE_EVENT_MASK_CCM_TLV_IF_STATUS   : 0) |
             (VTSS_X_VOP_INTR_STICKY_CCM_UNEXP_PERIOD_STICKY(sticky_mask)       ? VTSS_VOE_EVENT_MASK_CCM_PERIOD          : 0) |
             (VTSS_X_VOP_INTR_STICKY_CCM_UNEXP_PRIO_STICKY(sticky_mask)         ? VTSS_VOE_EVENT_MASK_CCM_PRIORITY        : 0) |
             (VTSS_X_VOP_INTR_STICKY_CCM_ZERO_PERIOD_RX_ERR_STICKY(sticky_mask) ? VTSS_VOE_EVENT_MASK_CCM_ZERO_PERIOD     : 0) |
             (VTSS_X_VOP_INTR_STICKY_CCM_RX_RDI_STICKY(sticky_mask)             ? VTSS_VOE_EVENT_MASK_CCM_RX_RDI          : 0) |
             (VTSS_X_VOP_INTR_STICKY_CCM_LOC_STICKY(sticky_mask)                ? VTSS_VOE_EVENT_MASK_CCM_LOC             : 0) |
             (VTSS_X_VOP_INTR_STICKY_CCM_UNEXP_MEPID_STICKY(sticky_mask)        ? VTSS_VOE_EVENT_MASK_CCM_MEP_ID          : 0) |
             (VTSS_X_VOP_INTR_STICKY_CCM_UNEXP_MEGID_STICKY(sticky_mask)        ? VTSS_VOE_EVENT_MASK_CCM_MEG_ID          : 0) |
             (VTSS_X_VOP_INTR_STICKY_CCM_RX_MEL_LOW_STICKY(sticky_mask)         ? VTSS_VOE_EVENT_MASK_CCM_MEG_LEVEL       : 0));
                                         ...
                                         vtss_voe_function_t  function)
{
    ...
    for (i = 0; (i < RT_PATH_SERVICE_VOE_CNT) && vtss_state->oam.voe_alloc_data[service_voe_alloc_idx].allocated; ++i) {
        service_voe_alloc_idx = (service_voe_alloc_idx + 1) % RT_PATH_SERVICE_VOE_CNT;
    }
    if (i >= RT_PATH_SERVICE_VOE_CNT) {
        VTSS_E("No free Service VOE to allocate");
        return RT_PATH_SERVICE_VOE_CNT;
    }
                             ...
                             vtss_voe_idx_t              *const voe_idx)
{
    ...
    if (type == VTSS_VOE_TYPE_SERVICE) {
        ...
        i = VTSS_CHIP_PORT(port) + RT_PORT_VOE_BASE_IDX;
        if ((i >= RT_VOE_CNT)  ||  vtss_state->oam.voe_alloc_data[i].allocated) {
    ...
    *voe_idx = i;
    ...
    /* Disable VOE */
    REG_WRM(VTSS_VOP_VOE_MISC_CONFIG(*voe_idx), 0, VTSS_M_VOP_VOE_MISC_CONFIG_VOE_ENA);
    ...
    case VTSS_VOE_TYPE_PORT:
        offset = (*voe_idx - RT_PORT_VOE_BASE_IDX) * VTSS_PRIO_ARRAY_SIZE;
        for (i = 0; i < VTSS_PRIO_ARRAY_SIZE; ++i) {
        ...
        REG_WR(VTSS_VOP_PORT_RX_COSID_MAP(*voe_idx - RT_PORT_VOE_BASE_IDX), 0);
        REG_WR(VTSS_VOP_PORT_TX_COSID_MAP(*voe_idx - RT_PORT_VOE_BASE_IDX), 0);
        /* Default PORT_RX_COSID_MAP + PORT_TX_COSID_MAP, DEI=1: */
        REG_WR(VTSS_VOP_PORT_RX_COSID_MAP1(*voe_idx - RT_PORT_VOE_BASE_IDX), 0);
        REG_WR(VTSS_VOP_PORT_TX_COSID_MAP1(*voe_idx - RT_PORT_VOE_BASE_IDX), 0);
        break;
    }
                                ...
                                const vtss_voe_conf_t  *const conf)
{
    ...
    if (((voe_idx < RT_PORT_VOE_BASE_IDX) && (alloc_data->type == VTSS_VOE_TYPE_PORT)) ||
        ((voe_idx >= RT_PORT_VOE_BASE_IDX) && (alloc_data->type == VTSS_VOE_TYPE_SERVICE))) {
        VTSS_E("voe_idx %u  type %u.  VOE index and type mismatch", voe_idx, alloc_data->type);
        return VTSS_RC_ERROR;
    }

    /* Configure the unicast MAC */
    value = VTSS_F_VOP_MEP_UC_MAC_MSB_MEP_UC_MAC_MSB((conf->unicast_mac.addr[0] << 8) | conf->unicast_mac.addr[1]);
    ...
    REG_WRM(VTSS_VOP_VOE_COMMON_CFG(voe_idx), VTSS_F_VOP_VOE_COMMON_CFG_UPMEP_ENA(alloc_data->direction == VTSS_OAM_DIRECTION_UP ? 1 : 0), VTSS_M_VOP_VOE_COMMON_CFG_UPMEP_ENA);

    /* Configure VOE looped frames to hit ES0 using ISDX in case IFLOW is not none */
    value = VTSS_F_VOP_LOOPBACK_CFG_LB_ES0_ISDX_ENA((conf->loop_iflow_id == VTSS_IFLOW_ID_NONE) ? 0 : 1) |
    ...
    if (alloc_data->type == VTSS_VOE_TYPE_PORT) {
        ...
        REG_WR(VTSS_VOP_PORT_RX_COSID_MAP(voe_idx - RT_PORT_VOE_BASE_IDX), value);
        REG_WR(VTSS_VOP_PORT_TX_COSID_MAP(voe_idx - RT_PORT_VOE_BASE_IDX), value);
        /* PORT_RX_COSID_MAP + PORT_TX_COSID_MAP, DEI=1: */
        for (i = 0, value = 0x88888888; i < 8; ++i) { /* set all "CountEnable=1" */
            value |= (i & 7) << (4 * i);
        }
        REG_WR(VTSS_VOP_PORT_RX_COSID_MAP1(voe_idx - RT_PORT_VOE_BASE_IDX), value);
        REG_WR(VTSS_VOP_PORT_TX_COSID_MAP1(voe_idx - RT_PORT_VOE_BASE_IDX), value);
    }
    ...
    REG_WRM(VTSS_VOP_AUTO_HIT_ME_ONCE(voe_idx), VTSS_F_VOP_AUTO_HIT_ME_ONCE_HMO_SLOT(voe_idx & 7), VTSS_M_VOP_AUTO_HIT_ME_ONCE_HMO_SLOT);    /* always using all 8 HMO slots */

    if (conf->enable) {
        REG_RD(VTSS_VOP_VOE_MISC_CONFIG(voe_idx), &value);
        if (VTSS_X_VOP_VOE_MISC_CONFIG_VOE_ENA(value) == 0) {  /* VOE changed from disabled to enabled. */
            /* Clear the logical counters */
            VTSS_RC(voe_counter_update(vtss_state, voe_idx, VTSS_OAM_CNT_ALL | VTSS_OAM_CNT_DIR_BOTH));

            /* Disable CC */
            REG_WRM(VTSS_VOP_OAM_HW_CTRL(voe_idx), VTSS_F_VOP_OAM_HW_CTRL_CCM_ENA(0), VTSS_M_VOP_OAM_HW_CTRL_CCM_ENA);
        }
    }
    ...
    REG_WRM(VTSS_VOP_OAM_CPU_COPY_CTRL(voe_idx), VTSS_F_VOP_OAM_CPU_COPY_CTRL_PDU_ERR_EXTRACT_CCM_ONLY(0), VTSS_M_VOP_OAM_CPU_COPY_CTRL_PDU_ERR_EXTRACT_CCM_ONLY);

    /* Enable/Disable VOE */
    REG_WRM(VTSS_VOP_VOE_MISC_CONFIG(voe_idx), VTSS_F_VOP_VOE_MISC_CONFIG_VOE_ENA(conf->enable ? 1 : 0), VTSS_M_VOP_VOE_MISC_CONFIG_VOE_ENA);
                                   ...
                                   const vtss_voe_cc_conf_t  *const conf)
{
    ...
    mask = VTSS_M_VOP_AUTO_HIT_ME_ONCE_HMO_CCM_RX_CCM_NXT      |   /* mask for VTSS_VOP_AUTO_HIT_ME_ONCE register */
    ...
    case VTSS_OAM_CPU_COPY_ALL:     /* Configure the CPU copy ALL */
        REG_WRM(VTSS_VOP_OAM_CPU_COPY_CTRL(voe_idx), VTSS_F_VOP_OAM_CPU_COPY_CTRL_CCM_CPU_COPY_ENA(conf->enable ? 1 : 0), VTSS_M_VOP_OAM_CPU_COPY_CTRL_CCM_CPU_COPY_ENA);
        REG_WRM(VTSS_VOP_AUTO_HIT_ME_ONCE(voe_idx), 0, mask);
        break;
    case VTSS_OAM_CPU_COPY_AUTO:    /* Configure the CPU copy AUTO */
        REG_WRM(VTSS_VOP_OAM_CPU_COPY_CTRL(voe_idx), VTSS_F_VOP_OAM_CPU_COPY_CTRL_CCM_CPU_COPY_ENA(0), VTSS_M_VOP_OAM_CPU_COPY_CTRL_CCM_CPU_COPY_ENA);
        value = conf->enable ? (VTSS_F_VOP_AUTO_HIT_ME_ONCE_HMO_CCM_RX_CCM_NXT(1)     |
                                VTSS_F_VOP_AUTO_HIT_ME_ONCE_HMO_CCM_RX_TLV_NON_ZERO(1) |
                                VTSS_F_VOP_AUTO_HIT_ME_ONCE_HMO_CCM_RX_BAD_NXT(1)) : 0;
    ...
    if (conf->enable && (VTSS_X_VOP_OAM_HW_CTRL_CCM_ENA(value) == 0)) {    /* CC is just enabled so clear */
        REG_WR(VTSS_VOP_CCM_TX_SEQ_CFG(voe_idx), 0);
        REG_WR(VTSS_VOP_CCM_RX_SEQ_CFG(voe_idx), 0);
    ...
    value = (VTSS_F_VOP_CCM_CFG_CCM_SEQ_UPD_ENA((vtss_state->oam.voe_rdi_conf[voe_idx] || conf->seq_no_update) ? 1 : 0)  |
    ...
    value = VTSS_F_VOP_OAM_CNT_OAM_CTRL_CCM_OAM_CNT_ENA((conf->enable && conf->count_as_selected) ? 1 : 0);
    ...
    REG_WR(VTSS_VOP_PEER_MEPID_CFG(voe_idx), VTSS_F_VOP_PEER_MEPID_CFG_PEER_MEPID(conf->expected_peer_mepid));

    /* Enable/Disable CCM handling */
    REG_WRM(VTSS_VOP_OAM_HW_CTRL(voe_idx), VTSS_F_VOP_OAM_HW_CTRL_CCM_ENA(conf->enable ? 1 : 0), VTSS_M_VOP_OAM_HW_CTRL_CCM_ENA);
                                  ...
                                  const BOOL            rdi)
{
    ...
    value = VTSS_F_VOP_CCM_CFG_CCM_SEQ_UPD_ENA((rdi || vtss_state->oam.voe_cc_conf[voe_idx].seq_no_update) ? 1 : 0);
                                   ...
                                   const vtss_voe_lt_conf_t   *const conf)
{
    ...
    value = VTSS_F_VOP_OAM_CPU_COPY_CTRL_LTM_CPU_COPY_ENA((conf->enable && conf->ltm_cpu_copy) ? 1 : 0) |
            VTSS_F_VOP_OAM_CPU_COPY_CTRL_LTR_CPU_COPY_ENA((conf->enable && conf->ltr_cpu_copy) ? 1 : 0);
    mask = VTSS_M_VOP_OAM_CPU_COPY_CTRL_LTM_CPU_COPY_ENA | VTSS_M_VOP_OAM_CPU_COPY_CTRL_LTR_CPU_COPY_ENA;
    REG_WRM(VTSS_VOP_OAM_CPU_COPY_CTRL(voe_idx), value, mask);

    /* Configure count_as_selected */
    value = VTSS_F_VOP_OAM_CNT_OAM_CTRL_LTM_OAM_CNT_ENA((conf->enable && conf->count_as_selected) ? 1 : 0) |
            VTSS_F_VOP_OAM_CNT_OAM_CTRL_LTR_OAM_CNT_ENA((conf->enable && conf->count_as_selected) ? 1 : 0);
    mask = VTSS_M_VOP_OAM_CNT_OAM_CTRL_LTM_OAM_CNT_ENA | VTSS_M_VOP_OAM_CNT_OAM_CTRL_LTR_OAM_CNT_ENA;
    REG_WRM(VTSS_VOP_OAM_CNT_OAM_CTRL(voe_idx), value, mask);
                                   ...
                                   const vtss_voe_lb_conf_t   *const conf)
{
    ...
    /* Configure copy to CPU */
    value = VTSS_F_VOP_OAM_CPU_COPY_CTRL_LBM_CPU_COPY_ENA((conf->enable && conf->lbm_cpu_copy) ? 1 : 0) |
            VTSS_F_VOP_OAM_CPU_COPY_CTRL_LBR_CPU_COPY_ENA((conf->enable && conf->lbr_cpu_copy) ? 1 : 0);
    mask = VTSS_M_VOP_OAM_CPU_COPY_CTRL_LBM_CPU_COPY_ENA | VTSS_M_VOP_OAM_CPU_COPY_CTRL_LBR_CPU_COPY_ENA;
    REG_WRM(VTSS_VOP_OAM_CPU_COPY_CTRL(voe_idx), value, mask);

    /* Configure count_as_selected */
    value = VTSS_F_VOP_OAM_CNT_OAM_CTRL_LBM_OAM_CNT_ENA(conf->count_as_selected ? 1 : 0) |
            VTSS_F_VOP_OAM_CNT_OAM_CTRL_LBR_OAM_CNT_ENA(conf->count_as_selected ? 1 : 0);
    mask = VTSS_M_VOP_OAM_CNT_OAM_CTRL_LBM_OAM_CNT_ENA | VTSS_M_VOP_OAM_CNT_OAM_CTRL_LBR_OAM_CNT_ENA;
    REG_WRM(VTSS_VOP_OAM_CNT_OAM_CTRL(voe_idx), value, mask);

    /* Configure transaction id */
    value = VTSS_F_VOP_TX_TRANSID_UPDATE_TST_UPDATE_ENA(conf->enable ? 1 : 0) | VTSS_F_VOP_TX_TRANSID_UPDATE_LBM_UPDATE_ENA(conf->enable ? 1 : 0);
                                     ...
                                     const vtss_voe_laps_conf_t   *const conf)
{
    ...
    /* Configure CPU copy */
    /* Generic [GENERIC_OFFSET_LAPS] is used for LAPS */
    value = VTSS_F_VOP_OAM_CPU_COPY_CTRL_2_GENERIC_COPY_MASK((conf->enable && conf->cpu_copy) ? GENERIC_MASK_LAPS : 0);
    mask = VTSS_M_VOP_OAM_CPU_COPY_CTRL_2_GENERIC_COPY_MASK;
    REG_WRM(VTSS_VOP_OAM_CPU_COPY_CTRL_2(voe_idx), value, mask);

    /* Configure count_as_selected */
    /* Generic [GENERIC_OFFSET_LAPS] is used for LAPS */
    value = VTSS_F_VOP_OAM_CNT_OAM_CTRL_GENERIC_OAM_CNT_MASK((conf->enable && conf->count_as_selected) ? GENERIC_MASK_LAPS : 0);
                                  ...
                                  vtss_voe_status_t     *const status)
{
    ...
    status->opcode_unexp_seen  = (value & VTSS_M_VOP_OAM_RX_STICKY_UNK_OPCODE_RX_STICKY) != 0;
    status->dmac_unexp_seen    = (value & VTSS_M_VOP_OAM_RX_STICKY_DMAC_RX_ERR_STICKY) != 0;
    status->version_unexp_seen = (value & VTSS_M_VOP_OAM_RX_STICKY_PDU_VERSION_RX_ERR_STICKY) != 0;
    status->rx_level_low_seen  = (value & VTSS_M_VOP_OAM_RX_STICKY_RX_MEL_LOW_BLOCK_STICKY) != 0;
    status->rx_level_high_seen = (value & VTSS_M_VOP_OAM_RX_STICKY_RX_MEL_HIGH_BLOCK_STICKY) != 0;

    /* Clear the RX sticky bits that has been detected */
    value = value &
            (VTSS_M_VOP_OAM_RX_STICKY_UNK_OPCODE_RX_STICKY       |
    ...
    status->tx_level_low_seen  = (value & VTSS_M_VOP_OAM_TX_STICKY_TX_BLOCK_ERR_STICKY) != 0;
                                     ...
                                     vtss_voe_cc_status_t  *status)
{
    ...
    status->zero_period     = VTSS_X_VOP_CCM_RX_LAST_CCM_ZERO_PERIOD_ERR(value)     != 0;
    status->rdi             = VTSS_X_VOP_CCM_RX_LAST_CCM_RX_RDI(value)              != 0;
    status->loc             = VTSS_X_VOP_CCM_RX_LAST_CCM_LOC_DEFECT(value)          != 0;
    status->period_unexp    = VTSS_X_VOP_CCM_RX_LAST_CCM_UNEXP_PERIOD_DEFECT(value) != 0;
    status->priority_unexp  = VTSS_X_VOP_CCM_RX_LAST_CCM_UNEXP_PRIO_DEFECT(value)   != 0;
    status->mep_id_unexp    = VTSS_X_VOP_CCM_RX_LAST_CCM_UNEXP_MEPID_DEFECT(value)  != 0;
    status->meg_id_unexp    = VTSS_X_VOP_CCM_RX_LAST_CCM_UNEXP_MEGID_DEFECT(value)  != 0;
    status->mel_unexp       = VTSS_X_VOP_CCM_RX_LAST_CCM_RX_MEL_LOW_DEFECT(value)   != 0;
    status->rx_port         = vtss_api_port(vtss_state, VTSS_X_VOP_CCM_RX_LAST_CCM_RX_SRC_PORT(value));
    ...
    status->tlv_seen       = (value & VTSS_M_VOP_OAM_RX_STICKY_CCM_RX_TLV_NON_ZERO_STICKY) != 0;
    status->seq_unexp_seen = (value & VTSS_M_VOP_OAM_RX_STICKY_CCM_RX_SEQ_ERR_STICKY) != 0;
                                     ...
                                     vtss_voe_lb_status_t   *status)
{
    ...
    status->trans_unexp_seen  = (value & VTSS_M_VOP_OAM_RX_STICKY_LBR_TRANSID_ERR_STICKY) != 0;
                                       ...
                                       vtss_voe_laps_status_t   *status)
{
    ...
    status->seen  = (VTSS_X_VOP_OAM_RX_STICKY2_GENERIC_RX_STICKY_MASK(value) & GENERIC_MASK_LAPS) != 0;

    /* Clear the sticky bits that has been detected */
    value = value &
            (VTSS_M_VOP_OAM_RX_STICKY2_GENERIC_RX_STICKY_MASK);
                             ...
                             vtss_voi_idx_t              *const voi_idx)
{
    ...
    if (direction == VTSS_OAM_DIRECTION_DOWN) {
        ...
        count = RT_DOWN_VOI_CNT;
        alloc_offset = &down_voi_alloc_offset;
    } else {
        start = RT_DOWN_VOI_CNT;      /* voi_idx == RT_DOWN_VOI_CNT is the first Up-VOI */
        count = RT_UP_VOI_CNT;
                                ...
                                const vtss_voi_conf_t   *const conf)
{
    ...
    if (alloc_data->direction == VTSS_OAM_DIRECTION_DOWN) {
        value = conf->enable ? (VTSS_F_ANA_CL_MIP_CFG_MEL_VAL(conf->meg_level)                           |
                                VTSS_F_ANA_CL_MIP_CFG_LBM_REDIR_ENA(conf->lbm_cpu_redir ? 1 : 0)         |
                                VTSS_F_ANA_CL_MIP_CFG_LTM_REDIR_ENA(conf->ltm_cpu_redir ? 1 : 0)         |
                                VTSS_F_ANA_CL_MIP_CFG_RAPS_CFG(raps_handling_value(conf->raps_handling)) |
                                VTSS_F_ANA_CL_MIP_CFG_PIPELINE_PT(0)) : 0;      /* Currently only inner MIP is supported */
        ...
        value = VTSS_F_ANA_CL_LBM_MAC_HIGH_LBM_MAC_HIGH((conf->unicast_mac.addr[0] << 8) | conf->unicast_mac.addr[1]);
        ...
        value = conf->enable ? (VTSS_F_REW_MIP_CFG_MEL_VAL(conf->meg_level)                           |
                                VTSS_F_REW_MIP_CFG_LBM_REDIR_ENA(conf->lbm_cpu_redir ? 1 : 0)         |
                                VTSS_F_REW_MIP_CFG_LTM_REDIR_ENA(conf->ltm_cpu_redir ? 1 : 0)         |
                                VTSS_F_REW_MIP_CFG_RAPS_CFG(raps_handling_value(conf->raps_handling)) |
                                VTSS_F_REW_MIP_CFG_PIPELINE_PT(0)) : 0;      /* Currently only inner MIP is supported */
        mask = VTSS_M_REW_MIP_CFG_MEL_VAL        |
               VTSS_M_REW_MIP_CFG_LBM_REDIR_ENA  |
               VTSS_M_REW_MIP_CFG_LTM_REDIR_ENA  |
                             ...
                             const vtss_debug_info_t    *const info)
{
            ...
            if ((v & VTSS_M_VOP_VOE_MISC_CONFIG_VOE_ENA) != 0) {  /* Only print for active VOEs */
                ...
                for (k = 0; k < VTSS_PRIO_ARRAY_SIZE; ++k) { /* Print RX and TX count for each COS */
                    ...
                    if ((w & VTSS_M_VOP_VOE_COMMON_CFG_UPMEP_ENA) != 0) { /* This an Up-MEP (Service instance) */
                        ...
                        if (i >= RT_PORT_VOE_BASE_IDX) {  /* This is port Down-MEP */
                            tx_counter = REG_ADDR(VTSS_REW_SRV_LM_CNT_LSB(((i - RT_PORT_VOE_BASE_IDX) * VTSS_PRIO_ARRAY_SIZE) + k));
                            rx_counter = REG_ADDR(VTSS_ANA_AC_OAM_MOD_SRV_LM_CNT_LSB(((i - RT_PORT_VOE_BASE_IDX) * VTSS_PRIO_ARRAY_SIZE) + k));
    ...
    if (!info->has_action || resources) { /* Used resources must be printed */
        ...
        for (i = 0; i < RT_DOWN_VOI_CNT; ++i) {
            REG_RD(VTSS_ANA_CL_MIP_CFG(i), &v);
            if ((v & VTSS_M_ANA_CL_MIP_CFG_LBM_REDIR_ENA) != 0) {
                dmip_cnt++;
            }
        }
        for (i = 1; i < (RT_UP_VOI_CNT + 1); ++i) {   /* Up VOI instance 0 is not used as it cannot be addressed by ES0 action - 0 has the meaning of no MIB */
    ...
    if (!info->has_action || u_mip) { /* Up-MIP must be printed */
        for (i = 1; i < (RT_UP_VOI_CNT + 1); ++i) {   /* Up MIP instance 0 is not used as it cannot be addressed by ES0 action - 0 has the meaning of no MIB */
                               ...
                               const vtss_voe_idx_t  voe_idx)
{
    ...
    if ((rc = fa_voe_event_mask_set(vtss_state, voe_idx, VTSS_VOE_EVENT_MASK_ALL, FALSE)) != VTSS_RC_OK) {
        ret_rc = rc;
    }
    if ((rc = fa_voe_cc_rdi_set(vtss_state, voe_idx, FALSE)) != VTSS_RC_OK) {
        ret_rc = rc;
    }
    if ((rc = fa_voe_cc_conf_set(vtss_state, voe_idx, &vtss_state->oam.voe_cc_conf[voe_idx])) != VTSS_RC_OK) {
        ret_rc = rc;
    }
    if ((rc = fa_voe_lt_conf_set(vtss_state, voe_idx, &vtss_state->oam.voe_lt_conf[voe_idx])) != VTSS_RC_OK) {
        ret_rc = rc;
    }
    if ((rc = fa_voe_lb_conf_set(vtss_state, voe_idx, &vtss_state->oam.voe_lb_conf[voe_idx])) != VTSS_RC_OK) {
        ret_rc = rc;
    }
    if ((rc = fa_voe_laps_conf_set(vtss_state, voe_idx, &vtss_state->oam.voe_laps_conf[voe_idx])) != VTSS_RC_OK) {
        ret_rc = rc;
    }
    ...
    if ((rc = fa_voe_conf_set(vtss_state, voe_idx, &vtss_state->oam.voe_conf[voe_idx])) != VTSS_RC_OK) {
        ret_rc = rc;
    }
...
static vtss_rc fa_init(vtss_state_t *vtss_state)
{
    ...
    switch (vtss_state->init_conf.core_clock.freq) {
        ...
        /* system clock is 1,6 ns (625 MHz) and LOC_BASE_TICK_CNT is default 50, i.e. 80 ns */
            loc_base = 80; /* ns */
            break;
        case VTSS_CORE_CLOCK_500MHZ:
        /* system clock is 2 ns (500 MHz) and LOC_BASE_TICK_CNT is default 50, i.e. 100 ns */
            loc_base = 100; /* ns */
            break;
    ...
    REG_WR(VTSS_VOP_LOC_PERIOD_CFG(cc_loc_period_index(VTSS_VOE_CCM_PERIOD_10_MS)),  (    10000ULL * 1000) / loc_base);
    REG_WR(VTSS_VOP_LOC_PERIOD_CFG(cc_loc_period_index(VTSS_VOE_CCM_PERIOD_100_MS)), (   100000ULL * 1000) / loc_base);
    REG_WR(VTSS_VOP_LOC_PERIOD_CFG(cc_loc_period_index(VTSS_VOE_CCM_PERIOD_1_SEC)),  (  1000000ULL * 1000) / loc_base);
    REG_WR(VTSS_VOP_LOC_PERIOD_CFG(cc_loc_period_index(VTSS_VOE_CCM_PERIOD_10_SEC)), ( 10000000ULL * 1000) / loc_base);
    ...
    REG_WR(VTSS_VOP_LOC_PERIOD_CFG(lm_loc_period_index(VTSS_VOE_CCM_PERIOD_100_MS)), ( 2 * 99900ULL * 1000) / loc_base); /* 2 x 100 ms */
    REG_WR(VTSS_VOP_LOC_PERIOD_CFG(lm_loc_period_index(VTSS_VOE_CCM_PERIOD_1_SEC)),  (2 * 999900ULL * 1000) / loc_base); /* 2 x 1 sec */
    ...
    vtss_state->oam.path_service_voe_cnt = RT_PATH_SERVICE_VOE_CNT;
    vtss_state->oam.port_voe_base_idx = RT_PORT_VOE_BASE_IDX;
    vtss_state->oam.port_voe_cnt = RT_PORT_VOE_CNT;
    vtss_state->oam.down_voi_cnt = RT_DOWN_VOI_CNT;
    vtss_state->oam.up_voi_cnt = RT_UP_VOI_CNT;
    vtss_state->oam.voe_cnt = RT_VOE_CNT;
    vtss_state->oam.voi_cnt = RT_VOI_CNT;
    vtss_state->oam.event_mask_array_size = RT_EVENT_MASK_ARRAY_SIZE;
