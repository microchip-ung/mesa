...
vtss_rc  vtss_ant_sd10g28_cmu_reg_cfg(vtss_state_t *vtss_state, u32 cmu_mask) {
    ...
    for (cmu_num = 0; cmu_num < FA_SERDES_CMU; cmu_num++) {
        ...
        if (FA_TGT) {
            cmu_cfg_tgt = VTSS_TO_SD_CMU_CFG(cmu_num);
        } else {
            cmu_cfg_tgt = VTSS_TO_SD_CMU_CFG(0);
        }
...
static vtss_rc  vtss_laguna_sd10g28_reg_cfg(vtss_state_t *vtss_state, vtss_sd10g28_setup_struct_t *const res_struct, vtss_port_no_t port_no) {
    vtss_rc rc = VTSS_RC_OK;
    u32 value;
    u32 sd_lane_tgt;
    u32 sd_tgt;
...
/* Note: SerDes SD10G_LANE_0 is configured in 10G_LAN mode */
    REG_WRM(VTSS_SD_LANE_TARGET_SD_LANE_CFG(sd_lane_tgt),
                VTSS_F_SD_LANE_TARGET_SD_LANE_CFG_EXT_CFG_RST(1),
                VTSS_M_SD_LANE_TARGET_SD_LANE_CFG_EXT_CFG_RST);
