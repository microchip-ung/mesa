...
static void l26_acl_bytes(int off, int bytes, u8 *val, u8 *msk, void *ent_val, void *ent_msk)
{
    while (bytes) {
...
static vtss_rc l26_vcap_cache2action(vtss_state_t *vtss_state, const tcam_props_t *tcam, u32 *entry)
{
    int i;
    for (i = 0; i < tcam->action_width; i++)
...
static u32 l26_u8_to_u32(u8 *p) { return ((p[0] << 24) | (p[1] << 16) | (p[2] << 8) | p[3]); }
...
static vtss_rc l26_is1_prepare_action(vtss_state_t       *vtss_state,
                                      ...
                                      u32                 counter)
{
    u32                entry[VTSS_TCAM_ENTRY_WIDTH], type;
    ...
    if (is1->entry->key.type == VTSS_IS1_TYPE_SMAC_SIP) {
        type = IS1_ACTION_TYPE_SMAC_SIP4;
    } else if (is1->entry->key.type == VTSS_IS1_TYPE_IPV6_SMAC_SIP) {
        type = IS1_ACTION_TYPE_SMAC_SIP6;
    } else {
        type = IS1_ACTION_TYPE_NORMAL;
    }
    vtss_bs_set(entry, 0, 2, type);

    if (type != IS1_ACTION_TYPE_NORMAL) {
        /* SIP/SMAC action */
                                   ...
                                   vtss_is1_data_t    *is1)
{
    ...
    u32             port_mask, etype_len, ip_snap, ip4 = 0, tcp_udp = 0, tcp = 0;
    u32             l4_used = 0, msk;
    vtss_vcap_ip_t  sip;
    ...
    if (key->type == VTSS_IS1_TYPE_IPV6_SMAC_SIP) {
        L26_ACL_BITSET(0, 1, entry, mask); /* IS1_TYPE */
        /* IPv6 SMAC/SIP entry */
        for (i = 0; i < 6; i++) {
            smac.value[i] = key->frame.ipv6_smac_sip.smac.addr[i];
            smac.mask[i] = 0xff;
        }
        L26_ACE_MAC(6, smac, entry, mask); /* L2_SMAC */
        for (i = 0; i < 4; i++) {
            sip.value = l26_u8_to_u32(&key->frame.ipv6_smac_sip.sip.addr[i * 4]);
            sip.mask = ACL_MASK_ONES;
            L26_ACL_CFIELD(54 + i * 32, 32, sip.value, sip.mask, entry, mask);
        }
                                 ...
                                 u32               counter)
{
    ...
    return l26_vcap_index_command(vtss_state, tcam, idx->row, VTSS_TCAM_CMD_WRITE,
                                  VTSS_TCAM_SEL_ALL);
}
...
static vtss_rc l26_is1_entry_move(vtss_state_t *vtss_state, vtss_vcap_idx_t *idx, u32 count, BOOL up)
{
                                   ...
                                   vtss_is2_data_t    *is2)
{
    ...
    case ACL_TYPE_ETYPE:
        ...
        /* ETYPE */
        l26_acl_bytes(155, 2, ace->frame.etype.etype.value, ace->frame.etype.etype.mask, entry,
                      mask);
        /* L2_PAYLOAD */
    ...
    case ACL_TYPE_SNAP:
        ...
        /* L2_SNAP bytes 1-4 */
        l26_acl_bytes(163, 4, ace->frame.snap.snap.value + 1, ace->frame.snap.snap.mask + 1, entry,
                      mask);
        break;
    case ACL_TYPE_ARP:
...
static vtss_rc l26_is2_entry_move(vtss_state_t *vtss_state, vtss_vcap_idx_t *idx, u32 count, BOOL up)
{
    VTSS_I("row: %u, count: %u, up: %u", idx->row, count, up);
                                       ...
                                       vtss_is2_data_t *is2)
{
    ...
    VTSS_I("row: %u", idx->row);
    VTSS_RC(l26_vcap_index_command(vtss_state, tcam, idx->row, VTSS_TCAM_CMD_READ,
    ...
    VTSS_RC(l26_vcap_index_command(vtss_state, tcam, idx->row, VTSS_TCAM_CMD_WRITE,
                                   VTSS_TCAM_SEL_ACTION));
    return VTSS_RC_OK;
}
                                   ...
                                   vtss_es0_key_t     *key)
{
    ...
    L26_ACL_FIELD(10, 12, key->data.vid.vid, key->vid_any ? 0 : ACL_MASK_ONES, entry,
                  mask); /* VID */
...
static vtss_rc l26_es0_entry_move(vtss_state_t *vtss_state, vtss_vcap_idx_t *idx, u32 count, BOOL up)
{
    VTSS_I("row: %u, count: %u, up: %u", idx->row, count, up);
                                      ...
                                      const vtss_acl_policer_no_t policer_no)
{
    ...
    return vtss_l26_policer_conf_set(vtss_state, VTSS_POLICER_USER_ACL, pol_alloc->policer, 1,
                                     &cfg);
}
                                      ...
                                      u16                 policer_new)
{
    ...
    for (cur = obj->used_list, i = 0; cur != NULL; cur = cur->next, i++) {
        ...
        if (cur->user == VTSS_IS2_USER_ACL && is2->policer_type != VTSS_L26_POLICER_NONE &&
            ...
            VTSS_RC(l26_vcap_index_command(vtss_state, tcam, i, VTSS_TCAM_CMD_READ,
                                           VTSS_TCAM_SEL_ACTION));
            VTSS_RC(l26_vcap_cache2action(vtss_state, tcam, entry));
            vtss_bs_set(entry, 10, 8, policer_new);
            VTSS_RC(l26_vcap_action2cache(vtss_state, tcam, entry, 0));
            VTSS_RC(l26_vcap_index_command(vtss_state, tcam, i, VTSS_TCAM_CMD_WRITE,
                                           VTSS_TCAM_SEL_ACTION));
        }
    }
...
vtss_rc vtss_cil_vcap_acl_port_conf_set(vtss_state_t *vtss_state, const vtss_port_no_t port_no)
{
    ...
    L26_WRM(VTSS_ANA_PORT_VCAP_CFG(port),
            (enable ? VTSS_F_ANA_PORT_VCAP_CFG_S2_ENA : 0) |
                VTSS_F_ANA_PORT_VCAP_CFG_S2_ARP_DIS(conf->key.arp == VTSS_ACL_KEY_ETYPE ? lookup
                                                                                        : 0) |
                VTSS_F_ANA_PORT_VCAP_CFG_S2_IP_TCPUDP_DIS(conf->key.ipv4 == VTSS_ACL_KEY_ETYPE
                                                              ? lookup
                                                              : 0) |
                VTSS_F_ANA_PORT_VCAP_CFG_S2_IP_OTHER_DIS(conf->key.ipv4 == VTSS_ACL_KEY_ETYPE
                                                             ? lookup
                                                             : 0) |
                VTSS_F_ANA_PORT_VCAP_CFG_S2_IP6_STD_DIS(conf->key.ipv6 == VTSS_ACL_KEY_EXT
                                                            ? 0
                                                            : lookup) |
                VTSS_F_ANA_PORT_VCAP_CFG_S2_IP6_TCPUDP_OTHER_DIS(conf->key.ipv6 ==
                                                                         VTSS_ACL_KEY_ETYPE
                                                                     ? lookup
                                                                     : 0) |
                VTSS_F_ANA_PORT_VCAP_CFG_PAG_VAL(enable ? conf->policy_no : 0),
            VTSS_F_ANA_PORT_VCAP_CFG_S2_ENA | VTSS_F_ANA_PORT_VCAP_CFG_S2_ARP_DIS(lookup) |
                              ...
                              const vtss_ace_t *const ace)
{
    ...
    vtss_ace_frame_ipv4_t      *ipv4 = &ace_copy->frame.ipv4;
    vtss_ace_frame_ipv6_t      *ipv6 = &ace_copy->frame.ipv6;
    ...
    /* Initialize entry data and copy ACE */
    ...
    *ace_copy = *ace;
    ...
    if (vtss_vcap_lookup(vtss_state, is2_obj, VTSS_IS2_USER_ACL_PTP, ace->id, NULL, NULL) ==
        VTSS_RC_OK)
        old_ptp = 1;
    ...
    if ((ace->type == VTSS_ACE_TYPE_ETYPE && ace->frame.etype.ptp.enable) ||
        (ace->type == VTSS_ACE_TYPE_IPV4 && ipv4->ptp.enable) ||
        (ace->type == VTSS_ACE_TYPE_IPV6 && ipv6->ptp.enable))
        new_ptp = 1;

    /* Check if extra IP entry must be added */
    if ((ace->type == VTSS_ACE_TYPE_IPV4 && ipv4->proto.mask != 0xff) ||
        (ace->type == VTSS_ACE_TYPE_IPV6 && ipv6->proto.mask != 0xff))
        new_ip = 1;
    ...
    if (ace->type == VTSS_ACE_TYPE_IPV4 && vtss_vcap_udp_tcp_rule(&ipv4->proto)) {
        sport = &ipv4->sport;
        dport = &ipv4->dport;
    }
    if (ace->type == VTSS_ACE_TYPE_IPV6 && vtss_vcap_udp_tcp_rule(&ipv6->proto)) {
        sport = &ipv6->sport;
        dport = &ipv6->dport;
    }
    ...
    id = (ace->id + VTSS_ACE_ID_IP);
    if (new_ip) {
        vtss_ace_u8_t *proto = (ace->type == VTSS_ACE_TYPE_IPV4 ? &ipv4->proto : &ipv6->proto);
        entry.first = 1;
        entry.udp_tcp_any = 1;
        proto->mask = 0xff;
        proto->value = 6;
    ...
    if (entry.host_match) {
        ...
        if (ace->type == VTSS_ACE_TYPE_IPV4) {
            ...
            is1_entry.key.frame.smac_sip.smac = ipv4->sip_smac.smac;
            is1_entry.key.frame.smac_sip.sip = ipv4->sip_smac.sip;
        } else {
            is1_entry.key.type = VTSS_IS1_TYPE_IPV6_SMAC_SIP;
            is1_entry.key.frame.ipv6_smac_sip.smac = ipv6->sip_smac.smac;
            is1_entry.key.frame.ipv6_smac_sip.sip = ipv6->sip_smac.sip;
        }
                           ...
                           BOOL                      clear)
{
    ...
    if (vtss_vcap_lookup(vtss_state, obj, VTSS_IS2_USER_ACL, ace_id + VTSS_ACE_ID_IP, NULL, &idx) ==
        VTSS_RC_OK) {
                           ...
                           u32         len)
{
    ...
    for (i = 0; i < len; i++) {
        ...
        if (i != 0 && (j % 8) == 7)
            pr(".");
        j += offset;
        pr("%c", mask == NULL || vtss_bs_bit_get(mask, j) ? (vtss_bs_bit_get(entry, j) ? '1' : '0')
                                                          : 'X');
    }
    pr(len > 23 ? "\n" : " ");
}

static void l26_debug_bit(lmu_ss_t *ss, const char *name, u32 *entry, u32 *mask, u32 offset)
{
    l26_debug_bits(ss, name, entry, mask, offset, 1);
}

static void l26_debug_mac_bits(lmu_ss_t *ss, const char *name, u32 *entry, u32 *mask, u32 offset)
{
...
static void l26_debug_fld(lmu_ss_t *ss, const char *name, u32 *entry, u32 offs, u32 len)
{
                             ...
                             lmu_ss_t     *ss)
{
    if (mask == NULL) {
        ...
        if (vtss_bs_get(entry, 0, 2) == IS1_ACTION_TYPE_NORMAL) {
            /* Normal IS1 action */
            l26_debug_action(ss, "dscp", entry, 2, 3, 6);   /* DSCP_ENA/DSCP_VAL */
            ...
            l26_debug_action(ss, "vid", entry, 24, 25, 12); /* VID_ENA/VID_VAL */
            l26_debug_action(ss, "pcp", entry, 51, 52, 3);  /* PCP_DEI_ENA/PCP_VAL */
            ...
            l26_debug_action(ss, "fid", entry, 37, 39, 12); /* FID_SEL & FID_VAL */
        } else {
            /* SMAC/SIP action */
                             ...
                             lmu_ss_t     *ss)
{
    ...
    if (mask == NULL) {
        ...
        l26_debug_action(ss, "cpu", entry, 1, 2, 3);     /* CPU_COPY_ENA/CPU_QU_NUM */
        l26_debug_action(ss, "mir", entry, 7, 0, 0);     /* MIRROR_ENA */
        l26_debug_action(ss, "lrn", entry, 8, 0, 0);     /* LRN_DIS */
        l26_debug_action(ss, "pol", entry, 9, 10, 8);    /* POLICER_ENA/POLICE_IDX */
    ...
    case ACL_TYPE_SNAP:
        ...
        case ACL_TYPE_ETYPE:
            ...
            pr("\n");
            break;
        case ACL_TYPE_LLC: l26_debug_bits(ss, "l2_llc", entry, mask, 155, 32); break;
        case ACL_TYPE_SNAP:
...
#define L26_DEBUG_VCAP(ss, name, tgt)                                                              \
    vtss_l26_debug_reg(vtss_state, ss, VTSS_VCAP_CORE_VCAP_CONST_##name(tgt), #name)
                                             ...
                                             lmu_ss_t     *ss))
{
    ...
    for (i = (tcam->actions - 1); i >= 0; i--) {
        ...
        if (is_entry &&
            l26_vcap_command(vtss_state, tcam, i, VTSS_TCAM_CMD_READ, VTSS_TCAM_SEL_ENTRY) ==
                VTSS_RC_OK &&
            ...
            if (info->full) {
                ...
                for (j = 0; j < tcam->entry_width; j++) {
                    vtss_l26_debug_reg_inst(
                        vtss_state, ss,
                        VTSS_VCAP_CORE_VCAP_CORE_CACHE_VCAP_ENTRY_DAT(tcam->target, j), j,
                        "ENTRY_DAT");
...
vtss_rc vtss_l26_debug_vcap_is1(vtss_state_t                  *vtss_state,
                                ...
                                const vtss_debug_info_t *const info)
{
    return l26_debug_vcap(vtss_state, VTSS_TCAM_S1, "IS1", ss, info, l26_debug_is1);
}
...
vtss_rc vtss_l26_debug_vcap_es0(vtss_state_t                  *vtss_state,
                                ...
                                const vtss_debug_info_t *const info)
{
    return l26_debug_vcap(vtss_state, VTSS_TCAM_ES0, "ES0", ss, info, l26_debug_es0);
}
                                      ...
                                      const vtss_debug_info_t *const info)
{
    ...
    for (i = 0; i < VTSS_VCAP_RANGE_CHK_CNT; i++) {
        vtss_l26_debug_reg_inst(vtss_state, ss, VTSS_ANA_COMMON_VCAP_RNG_TYPE_CFG(i), i,
                                "RNG_TYPE_CFG");
        vtss_l26_debug_reg_inst(vtss_state, ss, VTSS_ANA_COMMON_VCAP_RNG_VAL_CFG(i), i,
                                "RNG_VAL_CFG");
    }
    pr("\n");
    return VTSS_RC_OK;
                                 ...
                                 const vtss_debug_info_t *const info)
{
    ...
    for (port = 0; port < VTSS_CHIP_PORTS; port++) {
        vtss_l26_debug_reg_inst(vtss_state, ss, VTSS_ANA_PORT_VCAP_CFG(port), port, "VCAP_CFG");
    }
    pr("\n");
    return VTSS_RC_OK;
}
...
vtss_rc vtss_l26_vcap_debug_print(vtss_state_t                  *vtss_state,
                                  ...
                                  const vtss_debug_info_t *const info)
{
    return vtss_debug_print_group(VTSS_DEBUG_GROUP_ACL, l26_debug_acl, vtss_state, ss, info);
}
...
static vtss_rc l26_vcap_init(vtss_state_t *vtss_state)
{
    ...
    for (port = 0; port < VTSS_CHIP_PORTS; port++) {
        ...
        L26_WR(VTSS_ANA_PORT_VCAP_CFG(port),
               VTSS_F_ANA_PORT_VCAP_CFG_S1_ENA | VTSS_F_ANA_PORT_VCAP_CFG_S2_UDP_PAYLOAD_ENA(2) |
                   VTSS_F_ANA_PORT_VCAP_CFG_S2_ETYPE_PAYLOAD_ENA(2) |
                   VTSS_F_ANA_PORT_VCAP_CFG_S2_ENA | VTSS_F_ANA_PORT_VCAP_CFG_S2_IP6_STD_DIS(3));
