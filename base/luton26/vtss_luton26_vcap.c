...
#define L26_ACL_CBITSET(off, bval, ent_bits, msk_bits)                         \
    {                                                                          \
        if (bval != ((int)VTSS_ACE_BIT_ANY))                                   \
            L26_ACL_BITSET(off, bval == ((int)VTSS_ACE_BIT_1), ent_bits,       \
                           msk_bits);                                          \
    }
...
#define L26_ACL_CBITSET_INV(off, bval, ent_bits, msk_bits)                     \
    {                                                                          \
        if (bval != ((int)VTSS_ACE_BIT_ANY))                                   \
            L26_ACL_BITSET(off, bval == ((int)VTSS_ACE_BIT_0), ent_bits,       \
                           msk_bits);                                          \
    }

static u32 l26_get_bytes(u8 *data, int nbytes)
{
    u32 val = 0;
    int i;
    for (i = 0; i < nbytes; i++)
...
static void l26_acl_bytes(int   off,
                          int   bytes,
                          u8   *val,
                          u8   *msk,
                          void *ent_val,
                                ...
                                int                 sel)
{
    ...
    do {
        L26_RD(VTSS_VCAP_CORE_VCAP_CORE_CFG_VCAP_UPDATE_CTRL(tcam->target),
               &value);
    } while (value &
             VTSS_F_VCAP_CORE_VCAP_CORE_CFG_VCAP_UPDATE_CTRL_UPDATE_SHOT);
                                    ...
                                    u32                *mask)
{
    ...
    for (i = 0; i < tcam->entry_width; i++) {
        L26_WR(VTSS_VCAP_CORE_VCAP_CORE_CACHE_VCAP_ENTRY_DAT(tcam->target, i),
               entry ? entry[i] : 0);
        L26_WR(VTSS_VCAP_CORE_VCAP_CORE_CACHE_VCAP_MASK_DAT(tcam->target, i),
               mask ? ~mask[i] : ~0);
    }
    return VTSS_RC_OK;
}
                                    ...
                                    u32                *mask)
{
    ...
    for (i = 0; i < tcam->entry_width; i++) {
        u32 m;
        L26_RD(VTSS_VCAP_CORE_VCAP_CORE_CACHE_VCAP_ENTRY_DAT(tcam->target, i),
               &entry[i]);
        L26_RD(VTSS_VCAP_CORE_VCAP_CORE_CACHE_VCAP_MASK_DAT(tcam->target, i),
               &m);
                                     ...
                                     u32                 counter)
{
    ...
    for (i = 0; i < tcam->action_width; i++)
        L26_WR(VTSS_VCAP_CORE_VCAP_CORE_CACHE_VCAP_ACTION_DAT(tcam->target, i),
               entry ? entry[i] : 0);
    L26_WR(VTSS_VCAP_CORE_VCAP_CORE_CACHE_VCAP_CNT_DAT(tcam->target, 0),
           counter);
    return VTSS_RC_OK;
}
                                     ...
                                     u32                *entry)
{
    int i;
    for (i = 0; i < tcam->action_width; i++)
        L26_RD(VTSS_VCAP_CORE_VCAP_CORE_CACHE_VCAP_ACTION_DAT(tcam->target, i),
               &entry[i]);
    return VTSS_RC_OK;
}

static vtss_rc l26_vcap_initialize(vtss_state_t       *vtss_state,
                                   const tcam_props_t *tcam)
{
    /* First write entries */
    ...
    VTSS_RC(l26_vcap_command(vtss_state, tcam, 0, VTSS_TCAM_CMD_INITIALIZE,
                             VTSS_TCAM_SEL_ENTRY));
                                  ...
                                  BOOL          clear)
{
    ...
    VTSS_RC(l26_vcap_index_command(vtss_state, tcam, ix, VTSS_TCAM_CMD_READ,
                                   VTSS_TCAM_SEL_COUNTER));
    L26_RD(VTSS_VCAP_CORE_VCAP_CORE_CACHE_VCAP_CNT_DAT(tcam->target, 0),
           counter);
    if (!clear)
        return VTSS_RC_OK;
    ...
    return l26_vcap_index_command(vtss_state, tcam, ix, VTSS_TCAM_CMD_WRITE,
                                  VTSS_TCAM_SEL_COUNTER);
}
                                 ...
                                 BOOL          clear)
{
    ...
    /* Read counter at index */
    VTSS_RC(l26_vcap_port_command(vtss_state, tcam, port, VTSS_TCAM_CMD_READ,
                                  VTSS_TCAM_SEL_COUNTER));
    L26_RD(VTSS_VCAP_CORE_VCAP_CORE_CACHE_VCAP_CNT_DAT(tcam->target, 0),
           counter);
    if (!clear)
    ...
    /* Clear counter at index */
    L26_WR(VTSS_VCAP_CORE_VCAP_CORE_CACHE_VCAP_CNT_DAT(tcam->target, 0), 0);
    return l26_vcap_port_command(vtss_state, tcam, port, VTSS_TCAM_CMD_WRITE,
                                 VTSS_TCAM_SEL_COUNTER);
}
...
static vtss_rc l26_vcap_entry_move(vtss_state_t *vtss_state,
                                   int           bank,
                                   u32           ix,
                                   u32           count,
                                   BOOL          up)
{
    ...
    if (up) {
        /* Moving up corresponds to move down command and must be done one entry
         * at a time */
        ...
        for (i = 0; i < count; i++) {
            VTSS_RC(l26_vcap_index_command(vtss_state, tcam, ix + i, cmd,
                                           VTSS_TCAM_SEL_ALL));
        }
    } else {
        ...
        L26_WR(VTSS_VCAP_CORE_VCAP_CORE_CFG_VCAP_MV_CFG(tcam->target),
               VTSS_F_VCAP_CORE_VCAP_CORE_CFG_VCAP_MV_CFG_MV_NUM_POS(1) |
                   VTSS_F_VCAP_CORE_VCAP_CORE_CFG_VCAP_MV_CFG_MV_SIZE(count -
                                                                      1));
        VTSS_RC(l26_vcap_index_command(vtss_state, tcam, ix + count - 1, cmd,
                                       VTSS_TCAM_SEL_ALL));
    }
                                   ...
                                   vtss_is1_data_t    *is1)
{
    ...
    if (key->type == VTSS_IS1_TYPE_SMAC_SIP) {
        ...
        if (key->frame.smac_sip.port_no != VTSS_PORT_NO_NONE) {
            ...
            L26_ACL_FIELD(0, 5, port, ACL_MASK_ONES, entry,
                          mask); /* IGR_PORT */
        }
        for (i = 0; i < 6; i++) {
        ...
        L26_ACL_FIELD(53, 32, key->frame.smac_sip.sip, ACL_MASK_ONES, entry,
                      mask); /* L3_IP4_SIP */
        return l26_vcap_entry2cache(vtss_state, tcam, VCAP_TG_VAL_IS1_IP4,
                                    entry, mask);
    }
    ...
    L26_ACL_CBITSET(30, key->inner_tag.tagged, entry,
                    mask);                        /* VLAN_DBL_TAGGED */
    L26_ACL_CBITSET(31, tag->s_tag, entry, mask); /* TPID */
    if (key->tag.vid.type == VTSS_VCAP_VR_TYPE_VALUE_MASK)
        L26_ACL_CFIELD(32, 12, tag->vid.vr.v.value, tag->vid.vr.v.mask, entry,
                       mask); /* VID */
    ...
    case VTSS_IS1_TYPE_ANY:
        ...
        L26_ACL_CFIELD(176, 8, range_value, range_mask, entry,
                       mask); /* L4_RNG */
        return l26_vcap_entry2cache(vtss_state, tcam, VCAP_TG_VAL_IS1_IS1,
                                    entry, mask);
    ...
    case VTSS_IS1_TYPE_IPV6:
        ...
        break;
    default: VTSS_E("illegal type: %d", key->type); return VTSS_RC_ERROR;
    }
    ...
    if (dscp != NULL) {
        if (dscp->type == VTSS_VCAP_VR_TYPE_VALUE_MASK)
            L26_ACL_CFIELD(120, 6, dscp->vr.v.value, dscp->vr.v.mask, entry,
                           mask); /* L3_DSCP */
    ...
    if (tcp_udp) {
        ...
        if (sport != NULL && sport->type == VTSS_VCAP_VR_TYPE_VALUE_MASK) {
            msk = sport->vr.v.mask;
            L26_ACL_CFIELD(160, 16, sport->vr.v.value, msk, entry,
                           mask); /* L4_SPORT */
            if (msk)
                l4_used = 1;
        }
        if (is1->sport_range != VTSS_VCAP_RANGE_CHK_NONE) {
            ...
            if (sport != NULL &&
                sport->type == VTSS_VCAP_VR_TYPE_RANGE_INCLUSIVE)
        ...
        if (is1->dport_range != VTSS_VCAP_RANGE_CHK_NONE) {
            ...
            if (dport != NULL &&
                dport->type == VTSS_VCAP_VR_TYPE_RANGE_INCLUSIVE)
    ...
    if ((key->type == VTSS_IS1_TYPE_IPV4) ||
        ...
        if (ip4) {
            /* If L4 filtering is used, IPv4 frame must not be fragment or have
             * options */
    ...
    return l26_vcap_entry2cache(vtss_state, tcam, VCAP_TG_VAL_IS1_IS1, entry,
                                mask);
}
                                 ...
                                 BOOL             clear)
{
    ...
    return l26_vcap_entry_get(vtss_state, VTSS_TCAM_S1, idx->row, counter,
                              clear);
}
                                 ...
                                 u32               counter)
{
    ...
    return l26_vcap_index_command(vtss_state, tcam, idx->row,
                                  VTSS_TCAM_CMD_WRITE, VTSS_TCAM_SEL_ALL);
}
                                      ...
                                      u32                 counter)
{
    ...
    if (action->cpu_once || action->cpu) /* CPU_QU_NUM */
        entry[0] |=
            VTSS_ENCODE_BITFIELD(action->cpu_queue - VTSS_PACKET_RX_QUEUE_START,
                                 2, 3);

    /* MASK_MODE */
    switch (action->port_action) {
    case VTSS_ACL_PORT_ACTION_NONE:   mode = 0; break;
    case VTSS_ACL_PORT_ACTION_FILTER: mode = 1; break;
    case VTSS_ACL_PORT_ACTION_REDIR:  mode = 3; break;
    default:                          VTSS_E("unknown port_action"); return VTSS_RC_ERROR;
    }
    ...
    if (mode != 0 && mask == 0 && action->cpu_once == 0 && action->cpu == 0) {
        /* Forwarding and CPU copy disabled, discard using policer to avoid CPU
         * copy */
        policer_ena = 1;
        ...
        policer_ena = 1;
        policer =
            vtss_state->qos.evc_policer_alloc[action->evc_policer_id].policer;
#endif /* VTSS_FEATURE_QOS_POLICER_DLB */
    } else if (action->police) {
        policer_ena = 1;
        policer =
            vtss_state->vcap.acl_policer_alloc[action->policer_no].policer;
    }
    ...
    if (mode) {
        if (action->port_action == VTSS_ACL_PORT_ACTION_REDIR) {
            mask &=
                vtss_l26_port_mask(vtss_state, vtss_state->l2.tx_forward_aggr);
        }
        vtss_bs_set(entry, 18, 26, mask);
    }
    /* PTP_ENA */
    switch (action->ptp_action) {
    case VTSS_ACL_PTP_ACTION_NONE:             mode = 0; break;
    case VTSS_ACL_PTP_ACTION_ONE_STEP:         mode = 1; break;
    case VTSS_ACL_PTP_ACTION_TWO_STEP:         mode = 2; break;
    case VTSS_ACL_PTP_ACTION_ONE_AND_TWO_STEP: mode = 3; break;
    default:                                   VTSS_E("unknown ptp_action"); return VTSS_RC_ERROR;
    }
                                   ...
                                   vtss_is2_data_t    *is2)
{
    ...
    vtss_ace_u8_t      *proto;
    vtss_ace_bit_t     *ttl, *tcp_fin, *tcp_syn, *tcp_rst, *tcp_psh, *tcp_ack,
        *tcp_urg;
    ...
    switch (ace->type) {
    case VTSS_ACE_TYPE_ANY:   type = ACL_TYPE_ANY; break;
    case VTSS_ACE_TYPE_ETYPE: type = ACL_TYPE_ETYPE; break;
    case VTSS_ACE_TYPE_LLC:   type = ACL_TYPE_LLC; break;
    case VTSS_ACE_TYPE_SNAP:  type = ACL_TYPE_SNAP; break;
    case VTSS_ACE_TYPE_ARP:   type = ACL_TYPE_ARP; break;
    ...
    case VTSS_ACE_TYPE_IPV6:
        ...
        break;
    default: VTSS_E("illegal type: %d", ace->type); return VTSS_RC_ERROR;
    }

    /* Common Fields */
    L26_ACL_BITSET(3, is2->entry->first, entry, mask); /* FIRST */

    L26_ACL_FIELD(4, 8, ace->policy.value, ace->policy.mask, entry,
                  mask); /* PAG */
    ...
    /* VID */
    L26_ACL_CFIELD(41, 12, ace->vlan.vid.value, ace->vlan.vid.mask, entry,
                   mask);
    /* DEI (formerly CFI) */
    L26_ACL_CBITSET(53, ace->vlan.cfi, entry, mask);
    /* PCP (formerly UPRIO) */
    L26_ACL_CFIELD(54, 3, ace->vlan.usr_prio.value, ace->vlan.usr_prio.mask,
                   entry, mask);
    switch (type) {
    case ACL_TYPE_ANY:
        type = ACL_TYPE_ETYPE;
        type_mask = 0;
        mac_valid = (ace->dmac_mc != VTSS_ACE_BIT_ANY ||
        ...
        if (mac_valid) {
            /* Match ETYPE/LLC/SNAP frames with DMAC/SMAC filtering. ARP frames
             * must be mapped to ETYPE */
            type_mask = 0x4;
    ...
    case ACL_TYPE_ETYPE:
        ...
        /* ETYPE */
        l26_acl_bytes(155, 2, ace->frame.etype.etype.value,
                      ace->frame.etype.etype.mask, entry, mask);
        /* L2_PAYLOAD */
    ...
    case ACL_TYPE_LLC:
        ...
        /* L2_LLC */
        l26_acl_bytes(155, 4, ace->frame.llc.llc.value, ace->frame.llc.llc.mask,
                      entry, mask);
        break;
    case ACL_TYPE_SNAP:
        mac_valid = 1;
        ...
        /* L2_SNAP byte 0 */
        l26_acl_bytes(155, 1, ace->frame.snap.snap.value,
                      ace->frame.snap.snap.mask, entry, mask);
        /* L2_SNAP bytes 1-4 */
        l26_acl_bytes(163, 4, ace->frame.snap.snap.value + 1,
                      ace->frame.snap.snap.mask + 1, entry, mask);
        break;
    case ACL_TYPE_ARP:
        ...
        L26_ACL_CBITSET(107, ace->frame.arp.ethernet, entry,
                        mask); /* ARP_ADDR_SPACE_OK */
        L26_ACL_CBITSET(108, ace->frame.arp.ip, entry,
                        mask); /* ARP_PROTO_SPACE_OK */
        L26_ACL_CBITSET(109, ace->frame.arp.length, entry,
                        mask); /* ARP_LEN_OK */
        L26_ACL_CBITSET(110, ace->frame.arp.dmac_match, entry,
                        mask); /* ARP_TARGET_MATCH */
        L26_ACL_CBITSET(111, ace->frame.arp.smac_match, entry,
                        mask); /* ARP_SENDER_MATCH */
        L26_ACL_CBITSET(112, ace->frame.arp.unknown, entry,
                        mask); /* ARP_UNKNOWN_OPCODE */
        L26_ACL_CBITSET_INV(113, ace->frame.arp.req, entry,
                            mask); /* ARP_OPCODE */
        L26_ACL_CBITSET_INV(114, ace->frame.arp.arp, entry,
                            mask); /* ARP_OPCODE */
        L26_ACL_CFIELD(115, 32, ace->frame.arp.dip.value,
                       ace->frame.arp.dip.mask, entry, mask);
        L26_ACL_CFIELD(147, 32, ace->frame.arp.sip.value,
                       ace->frame.arp.sip.mask, entry, mask);
    ...
    case ACL_TYPE_IPV4:
        ...
        if (is2->entry->first && !ipv4 && ace->type_ext) {
            ...
            for (i = 0; i < 4; i++) {
                if (sipv6 != NULL) {
                    ...
                    L26_ACL_CFIELD(67 + i * 32, 32, sip.value, sip.mask, entry,
                                   mask);
                }
            }
            break;
        ...
        if (ipv4) {
            L26_ACL_CBITSET(60, ace->frame.ipv4.fragment, entry,
                            mask); /* L3_FRAGMENT */
            /* L3_FRAG_OFS_GT0 (bit 61): Not supported */
            L26_ACL_CBITSET(62, ace->frame.ipv4.options, entry,
                            mask); /* L3_OPTIONS */
        }
        ...
        if (type == ACL_TYPE_UDP_TCP) {
            ...
            if (ptp->enable && !is2->entry->first) {
                ...
                l26_acl_bytes(113, 4, ptp->header.value, ptp->header.mask,
                              entry, mask);
            } else {
            ...
            L26_ACL_CBITSET(186, *sport_eq_dport, entry,
                            mask);                        /* SPORT_EQ_DPORT */
        ...
        break;
    default: VTSS_E("illegal type: %d", type); return VTSS_RC_ERROR;
    }
                                 ...
                                 BOOL             clear)
{
    ...
    return l26_vcap_entry_get(vtss_state, VTSS_TCAM_S2, idx->row, counter,
                              clear);
}
                                 ...
                                 u32               counter)
{
    ...
    VTSS_RC(l26_is2_prepare_action(vtss_state, tcam,
                                   &data->u.is2.entry->ace.action, counter));
    return l26_vcap_index_command(vtss_state, tcam, idx->row,
                                  VTSS_TCAM_CMD_WRITE, VTSS_TCAM_SEL_ALL);
}
                                    ...
                                    vtss_is2_data_t *is2)
{
    ...
    VTSS_I("row: %u", idx->row);
    VTSS_RC(l26_vcap_index_command(vtss_state, tcam, idx->row,
                                   VTSS_TCAM_CMD_READ, VTSS_TCAM_SEL_ACTION));
    ...
    VTSS_RC(l26_vcap_index_command(vtss_state, tcam, idx->row,
                                   VTSS_TCAM_CMD_WRITE, VTSS_TCAM_SEL_ACTION));
    return VTSS_RC_OK;
}

static vtss_rc l26_is2_port_get(vtss_state_t *vtss_state,
                                u32           port,
                                u32          *counter,
                                BOOL          clear)
{
                                   ...
                                   vtss_es0_key_t     *key)
{
    ...
    L26_ACL_FIELD(10, 12, key->data.vid.vid, key->vid_any ? 0 : ACL_MASK_ONES,
                  entry, mask); /* VID */
                                 ...
                                 BOOL             clear)
{
    ...
    return l26_vcap_entry_get(vtss_state, VTSS_TCAM_ES0, idx->row, counter,
                              clear);
}
                                   ...
                                   const vtss_acl_policer_no_t policer_no)
{
    ...
    vtss_policer_alloc_t    *pol_alloc =
        &vtss_state->vcap.acl_policer_alloc[policer];
    ...
    return vtss_l26_policer_conf_set(vtss_state, VTSS_POLICER_USER_ACL,
                                     pol_alloc->policer, 1, &cfg);
}

static vtss_rc l26_is2_port_action_update(vtss_state_t        *vtss_state,
...
static vtss_rc l26_acl_policer_free(vtss_state_t      *vtss_state,
                                    vtss_acl_action_t *action)
{
    vtss_policer_alloc_t *pol_alloc;
    vtss_policer_user_t   user;
    ...
    pol_alloc->count--;
    if (pol_alloc->count == 0) {
        VTSS_RC(vtss_l26_policer_conf_set(vtss_state, user, pol_alloc->policer,
                                          0, NULL));
    }
    return VTSS_RC_OK;
}
...
static vtss_rc l26_acl_policer_alloc(vtss_state_t            *vtss_state,
                                     const vtss_acl_action_t *action)
{
    vtss_policer_alloc_t *pol_alloc;
    vtss_policer_user_t   user;
    ...
    if (vtss_l26_policer_free_get(vtss_state, &pol_alloc->policer) ==
        VTSS_RC_OK) {
        pol_alloc->count++;
        ...
        if (user == VTSS_POLICER_USER_EVC)
            return vtss_l26_evc_policer_conf_set(vtss_state,
...
vtss_rc vtss_l26_acl_policer_move(vtss_state_t *vtss_state, u32 policer)
{
    ...
    for (policer_id = 0; policer_id < VTSS_ACL_POLICERS; policer_id++) {
        ...
        if (pol_alloc->count && pol_alloc->policer == policer) {
            ...
            VTSS_RC(vtss_l26_acl_evc_policer_move(vtss_state,
                                                  VTSS_POLICER_USER_ACL,
                                                  policer_id, policer,
                                                  pol_alloc->policer));
            break;
        }
...
static vtss_rc l26_acl_port_conf_set(vtss_state_t        *vtss_state,
                                     const vtss_port_no_t port_no)
{
    vtss_acl_port_conf_t *conf = &vtss_state->vcap.acl_port_conf[port_no];
    ...
    VTSS_RC(l26_acl_policer_free(vtss_state,
                                 &vtss_state->vcap.acl_old_port_conf.action));
    VTSS_RC(l26_acl_policer_alloc(vtss_state, &conf->action));

    /* Enable/disable S2, set policy # */
    L26_WRM(
        VTSS_ANA_PORT_VCAP_CFG(port),
        (enable ? VTSS_F_ANA_PORT_VCAP_CFG_S2_ENA : 0) |
            VTSS_F_ANA_PORT_VCAP_CFG_S2_ARP_DIS(conf->key.arp ==
                                                        VTSS_ACL_KEY_ETYPE
                                                    ? lookup
                                                    : 0) |
            VTSS_F_ANA_PORT_VCAP_CFG_S2_IP_TCPUDP_DIS(conf->key.ipv4 ==
                                                              VTSS_ACL_KEY_ETYPE
                                                          ? lookup
                                                          : 0) |
            VTSS_F_ANA_PORT_VCAP_CFG_S2_IP_OTHER_DIS(conf->key.ipv4 ==
                                                             VTSS_ACL_KEY_ETYPE
                                                         ? lookup
                                                         : 0) |
            VTSS_F_ANA_PORT_VCAP_CFG_S2_IP6_STD_DIS(conf->key.ipv6 ==
                                                            VTSS_ACL_KEY_EXT
                                                        ? 0
                                                        : lookup) |
            VTSS_F_ANA_PORT_VCAP_CFG_S2_IP6_TCPUDP_OTHER_DIS(
                conf->key.ipv6 == VTSS_ACL_KEY_ETYPE ? lookup : 0) |
...
static vtss_rc l26_acl_port_counter_clear(vtss_state_t        *vtss_state,
                                          const vtss_port_no_t port_no)
{
    u32 counter;
...
static vtss_rc l26_is2_policer_free(vtss_state_t    *vtss_state,
                                    vtss_is2_data_t *is2)
{
    vtss_acl_action_t action;
                           ...
                           const vtss_ace_t *const ace)
{
    ...
    if (vtss_vcap_lookup(vtss_state, is2_obj, is2_user, ace->id, &data, NULL) ==
        VTSS_RC_OK) {
        is2->entry = &entry; /* NOTE: Restore entry pointer which was
    ...
    if (vtss_vcap_lookup(vtss_state, is2_obj, is2_user, id, NULL, NULL) ==
        VTSS_RC_OK)
        old_ip = 1;
    ...
    if ((is2_obj->count + new_ptp + new_ip) >=
        (is2_obj->max_count + old + old_ptp + old_ip)) {
        VTSS_I("IS2 is full");
        return VTSS_RC_ERROR;
    }
    ...
    id_next = (vtss_vcap_lookup(vtss_state, is2_obj, is2_user, id, NULL,
                                NULL) == VTSS_RC_OK
                   ? id
                   : ace_id);
    VTSS_RC(vtss_vcap_add(vtss_state, is2_obj, is2_user, ace->id, id_next, NULL,
                          0));

    /*** Step 3: Check if IS1 entry can be added */
    if (ace->type == VTSS_ACE_TYPE_IPV4 && ace->frame.ipv4.sip_smac.enable) {
        entry.host_match = 1;
        VTSS_RC(vtss_vcap_add(vtss_state, is1_obj, is1_user, ace->id,
                              VTSS_ACE_ID_LAST, NULL, 0));
    }
    ...
    if (ace->type == VTSS_ACE_TYPE_IPV4 &&
        vtss_vcap_udp_tcp_rule(&ace->frame.ipv4.proto)) {
    ...
    *ace_copy = *ace;
    ...
    VTSS_RC(vtss_vcap_add(vtss_state, is2_obj, is2_user, ace->id, id_next,
                          &data, 0));

    /* Check if PTP entry must be added */
    if (new_ptp) {
        /* Find next ID value */
        VTSS_RC(vtss_vcap_get_next_id(is2_obj, is2_user, VTSS_IS2_USER_ACL_PTP,
                                      ace->id, &id));
        ...
        *ace_copy = *ace;
        entry.first = 0;
        is2->action.redir =
            (ace->action.port_action == VTSS_ACL_PORT_ACTION_REDIR);
        is2->policer_type = policer_type;
        VTSS_RC(vtss_vcap_add(vtss_state, is2_obj, VTSS_IS2_USER_ACL_PTP,
                              ace->id, id, &data, 0));
    } else if (old_ptp) {
        /* Delete old PTP entry */
        VTSS_RC(vtss_vcap_del(vtss_state, is2_obj, VTSS_IS2_USER_ACL_PTP,
                              ace->id));
    }
    ...
    if (new_ip) {
        vtss_ace_u8_t *proto =
            (ace->type == VTSS_ACE_TYPE_IPV4 ? &ace_copy->frame.ipv4.proto
                                             : &ace_copy->frame.ipv6.proto);
        entry.first = 1;
        entry.udp_tcp_any = 1;
        proto->mask = 0xff;
        proto->value = 6;
        VTSS_RC(vtss_vcap_add(vtss_state, is2_obj, is2_user, id, ace->id, &data,
                              0));
    } else if (old_ip) {
        /* Delete old IP entry */
...
static vtss_rc l26_ace_del(vtss_state_t *vtss_state, const vtss_ace_id_t ace_id)
{
    ...
    VTSS_RC(vtss_vcap_lookup(vtss_state, obj, VTSS_IS2_USER_ACL, ace_id, &data,
                             NULL));
    ...
    VTSS_RC(vtss_vcap_del(vtss_state, obj, VTSS_IS2_USER_ACL,
                          ace_id + VTSS_ACE_ID_IP));

    /* Delete SMAC/SIP entry in IS1 */
    VTSS_RC(vtss_vcap_del(vtss_state, &vtss_state->vcap.is1.obj,
                          VTSS_IS1_USER_ACL, ace_id));
                           ...
                           BOOL                      clear)
{
    ...
    if (vtss_vcap_lookup(vtss_state, obj, VTSS_IS2_USER_ACL,
                         ace_id + VTSS_ACE_ID_IP, NULL, &idx) == VTSS_RC_OK) {
...
/* Clear ACE counter */
static vtss_rc l26_ace_counter_clear(vtss_state_t       *vtss_state,
                                     const vtss_ace_id_t ace_id)
{
    ...
    for (i = 0; i < VTSS_VCAP_RANGE_CHK_CNT; i++) {
        ...
        L26_WR(VTSS_ANA_COMMON_VCAP_RNG_VAL_CFG(i),
               VTSS_F_ANA_COMMON_VCAP_RNG_VAL_CFG_VCAP_RNG_MIN_VAL(entry->min) |
                   VTSS_F_ANA_COMMON_VCAP_RNG_VAL_CFG_VCAP_RNG_MAX_VAL(entry
                                                                           ->max))
    }
    return VTSS_RC_OK;
}
                                  ...
                                  vtss_ace_status_t *const status)
{
    ...
    for (cur = vtss_state->vcap.is2.obj.used; cur != NULL;
         cur = cur->next, idx++) {
        if (cur->id == id) {
    ...
    status->idx[0] =
        (idx_0 == VTSS_ACE_IDX_NONE ? idx_0 : (VTSS_L26_IS2_CNT - idx_0));
    status->idx[1] =
        (idx_1 == VTSS_ACE_IDX_NONE ? idx_1 : (VTSS_L26_IS2_CNT - idx_1));
    return VTSS_RC_OK;
}
                             ...
                             lmu_ss_t     *ss)
{
    ...
    if (mask == NULL) {
        ...
        pr("tag_es0:%u (%s) ", val,
           ...
           : val == VTSS_ES0_TAG_ES0  ? "es0"
           : val == VTSS_ES0_TAG_PORT ? "port"
                                      : "both");
        val = vtss_bs_get(entry, 3, 2); /* TAG_TPID_SEL */
        pr("tag_tpid_sel:%u (%s) ", val,
           val == VTSS_ES0_TPID_C      ? "c"
                                             ...
                                             lmu_ss_t     *ss))
{
    ...
    for (i = (tcam->actions - 1); i >= 0; i--) {
        ...
        if (is_entry &&
            l26_vcap_command(vtss_state, tcam, i, VTSS_TCAM_CMD_READ,
                             VTSS_TCAM_SEL_ENTRY) == VTSS_RC_OK &&
            l26_vcap_cache2entry(vtss_state, tcam, entry, mask) == VTSS_RC_OK) {
            L26_RD(VTSS_VCAP_CORE_VCAP_CORE_CACHE_VCAP_TG_DAT(tcam->target),
                   &typegroup);
        ...
        if (l26_vcap_command(vtss_state, tcam, i, VTSS_TCAM_CMD_READ, sel) !=
                VTSS_RC_OK ||
        ...
        if (info->full) {
            L26_WR(VTSS_VCAP_CORE_VCAP_CORE_CACHE_VCAP_CNT_DAT(tcam->target, 0),
                   0);
            if (l26_vcap_command(vtss_state, tcam, i, VTSS_TCAM_CMD_WRITE,
                                 sel) != VTSS_RC_OK)
                continue;
        }
...
static vtss_rc l26_vcap_init(vtss_state_t *vtss_state)
{
    ...
    for (port = 0; port < VTSS_CHIP_PORTS; port++) {
        /* Enable S1 and S2, PTP in S2 second lookup and treat IPv6 as IPv4 in
         * both S2 lookups */
