...
static const u8 ptp_gpio[VTSS_TS_IO_ARRAY_SIZE] = {7};
/*
 * Note: The values i have added to the table below ensures the correct path
 * delay, byt the asymmetry may not be correct.
 */
...
static vtss_rc l26_ts_hw_timeofday_read(vtss_state_t *vtss_state,
                                        u32          *tod_secs,
                                        u32          *tod_nanosecs,
                                        u64          *tc,
                                        BOOL          reset)
{
    ...
    *tc = (u64)((u64)tc_32 << 16);
    ...
    if (*tod_nanosecs == 0) {
        ++*tod_secs; /* HW bug: the seconds counter is incremented one clock
                        cycle after the ns counter wraps */
    }
    return VTSS_RC_OK;
}
                                          ...
                                          u64              *tc)
{
    ...
    deltat = (ts->nanoseconds > saved) ? ts->nanoseconds - saved
                                       : ts->nanoseconds - saved + HW_NS_PR_SEC;
 ...
 * The NS counter in hw (TOD_NANOSECS) has to be aligned with the
 * ts.nanoseconds, this is done by using the PTP_UPPER_LIMIT_1_TIME_ADJ: This
 * register can be used to change the one sec. period once, the period can be
 * set to [0..268.435.455] periods of 4 ns = [0..1.073.741.820] ns. alignment
 * algorithm: (TOD_NANOSECS - ts.nanoseconds) > 50 ms =>
 * PTP_UPPER_LIMIT_1_TIME_ADJ = TOD_NANOSECS - ts.nanoseconds (TBD if this
 * works, because the ADJ is < actual TOD_NANOSECS ??) 50 ms > (TOD_NANOSECS -
 * ts.nanoseconds) > -950 ms => PTP_UPPER_LIMIT_1_TIME_ADJ = TOD_NANOSECS -
 * ts.nanoseconds + 1 sec -950 ms > (TOD_NANOSECS - ts.nanoseconds) =>
 *    PTP_UPPER_LIMIT_1_TIME_ADJ = TOD_NANOSECS - ts.nanoseconds + 2 sec
 * The second offset is adjusted accordingly.
 *
 */
...
static vtss_rc l26_ts_adjtimer_set(vtss_state_t *vtss_state)
{
    ...
    if (clk_adj > HW_CLK_CNT_PR_SEC)
        clk_adj = HW_CLK_CNT_PR_SEC;
    ...
    //     L26_WR(VTSS_DEVCPU_GCB_PTP_STAT_PTP_EVT_STAT,
