                                           ...
                                           u64              *tc)
{
    ...
    VTSS_RC(jr2_ts_timeofday_read(vtss_state, domain, ts, tc));
#endif
    VTSS_D("ts->sec_msb: %u, ts->seconds: %u, ts->nanoseconds: %u, domain: %u", ts->sec_msb,
           ts->seconds, ts->nanoseconds, domain);
    return VTSS_RC_OK;
}
...
#endif
    VTSS_D("ts->sec_msb: %u, ts->seconds: %u, ts->nanoseconds: %u, domain %d", ts->sec_msb,
           ts->seconds, ts->nanoseconds, domain);
...
static vtss_rc jr2_ts_domain_timeofday_offset_set(vtss_state_t *vtss_state, u32 domain, i32 offset)
{
    ...
    VTSS_RC(jr2_ts_timeofday_sample(vtss_state));
#endif
    VTSS_D("Time after offset_set: %d, %d, %d", vtss_state->ts.conf.sample_ts[domain].sec_msb,
           vtss_state->ts.conf.sample_ts[domain].seconds,
           vtss_state->ts.conf.sample_ts[domain].nanoseconds);
...
static vtss_rc jr2_ts_external_clock_mode_set(vtss_state_t *vtss_state)
{
    ...
    if (ext_clock_mode->enable) {
        ...
        JR2_PTP_PIN_ACTION(EXT_CLK_PIN, PTP_PIN_ACTION_CLOCK, PTP_PIN_ACTION_NOSYNC,
                           ext_clock_mode->domain);
        ...
        JR2_PTP_PIN_ACTION(EXT_CLK_PIN, PTP_PIN_ACTION_CLOCK, PTP_PIN_ACTION_SYNC,
                           ext_clock_mode->domain);
    } else {
        /* This is for 'one_pps_mode'  TS_EXT_CLOCK_MODE_ONE_PPS_INPUT -
         * TS_EXT_CLOCK_MODE_ONE_PPS_OUTPUT_INPUT - TS_EXT_CLOCK_MODE_MAX */
    ...
    if (ext_clock_mode->one_pps_mode != TS_EXT_CLOCK_MODE_ONE_PPS_DISABLE) {
        ...
        JR2_PTP_PIN_ACTION(EXT_PPS_PIN, PTP_PIN_ACTION_CLOCK, PTP_PIN_ACTION_SYNC,
                           ext_clock_mode->domain);
    } else {
                                         ...
                                         BOOL           mode_domain_config)
{
    ...
    JR2_WRM(VTSS_REW_PORT_PTP_MODE_CFG(port, 0),
            ...
            VTSS_M_REW_PORT_PTP_MODE_CFG_PTP_MODE_VAL | VTSS_M_REW_PORT_PTP_MODE_CFG_PTP_DOM_VAL);
    ...
    JR2_WRM(VTSS_ANA_ACL_PORT_PTP_CFG(port),
            ...
            VTSS_M_ANA_ACL_PORT_PTP_CFG_PTP_PORT_NUM | VTSS_M_ANA_ACL_PORT_PTP_CFG_PTP_DOMAIN);
    // disable central counters in DEVCPU
    JR2_WRM(VTSS_DEVCPU_PTP_PTP_CFG_PTP_MISC_CFG, VTSS_F_DEVCPU_PTP_PTP_CFG_PTP_MISC_CFG_PTP_ENA(0),
...
static vtss_rc jr2_ts_status_change(vtss_state_t *vtss_state, const vtss_port_no_t port_no)
{
    ...
    case VTSS_PORT_INTERFACE_XAUI:
        ...
        rx_delay +=
            32 *
            (VTSS_X_DEV10G_PCS_XAUI_STATUS_PCS_XAUI_DESKEW_STATUS_LANE_DESKEW_DELAY(value) % 16);
        JR2_RD(VTSS_DEV10G_PCS_XAUI_STATUS_PCS_XAUI_CGALIGN_STATUS(VTSS_TO_DEV(port)), &value);
        rx_delay +=
            32 *
            (VTSS_X_DEV10G_PCS_XAUI_STATUS_PCS_XAUI_CGALIGN_STATUS_LANE_CGALIGN_DELAY(value) % 32) /
            10;
        break;
    case VTSS_PORT_INTERFACE_RXAUI:
...
static vtss_rc jr2_ts_smac_set(vtss_state_t *vtss_state, vtss_port_no_t port_no)
{
    ...
    JR2_WR(VTSS_REW_PORT_PTP_SMAC_LOW(port), VTSS_F_REW_PORT_PTP_SMAC_LOW_PTP_SMAC_LOW(smac_low));
...
static vtss_rc jr2_ts_seq_cnt_get(vtss_state_t *vtss_state, u32 sec_cntr, u16 *const cnt_val)
{
                            ...
                            const vtss_debug_info_t *const info)
{
    ...
    for (port = 0; port <= VTSS_CHIP_PORTS; port++) {
        ...
        vtss_jr2_debug_reg(vtss_state, ss, VTSS_REW_PORT_PTP_MODE_CFG(port, 0), "PTP_MODE_CFG[0]");
        vtss_jr2_debug_reg(vtss_state, ss, VTSS_REW_PORT_PTP_MODE_CFG(port, 1), "PTP_MODE_CFG[1]");
        vtss_jr2_debug_reg(vtss_state, ss, VTSS_REW_PORT_PTP_EDLY_CFG(port), "PTP_EDLY_CFG");
        vtss_jr2_debug_reg(vtss_state, ss, VTSS_REW_PORT_PTP_IDLY1_CFG(port), "PTP_IDLY1_CFG");
        vtss_jr2_debug_reg(vtss_state, ss, VTSS_REW_PORT_PTP_IDLY2_CFG(port), "PTP_IDLY2_CFG");
    }

    /* REW:PTP_CTRL */
    vtss_jr2_debug_reg_header(ss, "REW:PTP_CTRL");
    vtss_jr2_debug_reg(vtss_state, ss, VTSS_REW_PTP_CTRL_PTP_TWOSTEP_CTRL, "PTP_TWOSTEP_CTRL");
    vtss_jr2_debug_reg(vtss_state, ss, VTSS_REW_PTP_CTRL_PTP_TWOSTEP_STAMP, "PTP_TWOSTEP_STAMP");

    /* DEVCPU_PTP:PTP_CFG */
    vtss_jr2_debug_reg_header(ss, "DEVCPU_PTP:PTP_CFG");
    vtss_jr2_debug_reg(vtss_state, ss, VTSS_DEVCPU_PTP_PTP_CFG_PTP_MISC_CFG, "PTP_MISC_CFG");

    /* ANA_ACL::PTP_MISC_CTRL */
    vtss_jr2_debug_reg_header(ss, "ANA_ACL::PTP_MISC_CTRL");
    vtss_jr2_debug_reg(vtss_state, ss, VTSS_ANA_ACL_VCAP_S2_PTP_MISC_CTRL, "PTP_MISC_CTRL");

    for (idx = 0; idx <= 2; idx++) {
        VTSS_FMT(buf, "CLK_ADJ_CFG[%u]", idx);
        vtss_jr2_debug_reg(vtss_state, ss, VTSS_DEVCPU_PTP_PTP_CFG_CLK_ADJ_CFG(idx), buf.s);
    }
    for (idx = 0; idx <= 2; idx++) {
        VTSS_FMT(buf, "CLK_ADJ_FRQ[%u]", idx);
        vtss_jr2_debug_reg(vtss_state, ss, VTSS_DEVCPU_PTP_PTP_CFG_CLK_ADJ_FRQ(idx), buf.s);
    }
    for (idx = 0; idx <= 2; idx++) {
        VTSS_FMT(buf, "DOM_PTP_CLOCK_ID_MSB[%u]", idx);
        vtss_jr2_debug_reg(vtss_state, ss, VTSS_ANA_ACL_PTP_DOM_PTP_CLOCK_ID_MSB(idx), buf.s);
        VTSS_FMT(buf, "DOM_PTP_CLOCK_ID_LSB[%u]", idx);
        vtss_jr2_debug_reg(vtss_state, ss, VTSS_ANA_ACL_PTP_DOM_PTP_CLOCK_ID_LSB(idx), buf.s);
        VTSS_FMT(buf, "DOM_PTP_SRC_PORT_CFG[%u]", idx);
        vtss_jr2_debug_reg(vtss_state, ss, VTSS_ANA_ACL_PTP_DOM_PTP_SRC_PORT_CFG(idx), buf.s);
        VTSS_FMT(buf, "PTP_DOM_PTP_MISC_CFG[%u]", idx);
        vtss_jr2_debug_reg(vtss_state, ss, VTSS_ANA_ACL_PTP_DOM_PTP_MISC_CFG(idx), buf.s);
    }

    vtss_jr2_debug_reg(vtss_state, ss, VTSS_DEVCPU_PTP_PTP_CFG_PTP_PIN_INTR, "PTP_PIN_INTR");
    ...
    for (idx = 0; idx <= 3; idx++) {
        ...
        vtss_jr2_debug_reg_header(ss, buf.s);
        vtss_jr2_debug_reg(vtss_state, ss, VTSS_DEVCPU_PTP_PTP_PINS_PTP_PIN_CFG(idx),
                           "PTP_PIN_CFG");
        vtss_jr2_debug_reg(vtss_state, ss, VTSS_DEVCPU_PTP_PTP_PINS_PTP_TOD_SEC_MSB(idx),
                           "PTP_TOD_SEC_MSB");
        vtss_jr2_debug_reg(vtss_state, ss, VTSS_DEVCPU_PTP_PTP_PINS_PTP_TOD_SEC_LSB(idx),
                           "PTP_TOD_SEC_LSB");
        vtss_jr2_debug_reg(vtss_state, ss, VTSS_DEVCPU_PTP_PTP_PINS_PTP_TOD_NSEC(idx),
                           "PTP_TOD_NSEC");
        vtss_jr2_debug_reg(vtss_state, ss, VTSS_DEVCPU_PTP_PTP_PINS_PTP_NSF(idx), "PTP_NSF");
        vtss_jr2_debug_reg(vtss_state, ss, VTSS_DEVCPU_PTP_PTP_PINS_PIN_WF_HIGH_PERIOD(idx),
                           "PIN_WF_HIGH_PERIOD");
        vtss_jr2_debug_reg(vtss_state, ss, VTSS_DEVCPU_PTP_PTP_PINS_PIN_WF_LOW_PERIOD(idx),
                           "PIN_WF_LOW_PERIOD");
    }
    ...
    for (port_no = VTSS_PORT_NO_START; port_no < vtss_state->port_count; port_no++) {
        ...
        case VTSS_PORT_INTERFACE_QSGMII:
            ...
            vtss_jr2_debug_reg(vtss_state, ss,
                               VTSS_DEV1G_DEV_CFG_STATUS_PTP_CFG(VTSS_TO_DEV1G(port)), "PTP_CFG");
            break;
        case VTSS_PORT_INTERFACE_SFI:
        case VTSS_PORT_INTERFACE_XAUI:
        case VTSS_PORT_INTERFACE_RXAUI:
...
vtss_rc vtss_jr2_ts_debug_print(vtss_state_t                  *vtss_state,
                                ...
                                const vtss_debug_info_t *const info)
{
    return vtss_debug_print_group(VTSS_DEBUG_GROUP_TS, jr2_debug_ts, vtss_state, ss, info);
}
