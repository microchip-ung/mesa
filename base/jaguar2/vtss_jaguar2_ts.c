...
static vtss_rc jr2_ts_io_pin_timeofday_get(vtss_state_t     *vtss_state,
                                           u32               io,
                                           vtss_timestamp_t *ts,
                                           u64              *tc)
{
    ...
    ts->sec_msb =
        VTSS_X_DEVCPU_PTP_PTP_PINS_PTP_TOD_SEC_MSB_PTP_TOD_SEC_MSB(value);
    JR2_RD(VTSS_DEVCPU_PTP_PTP_PINS_PTP_TOD_SEC_LSB(io), &ts->seconds);
    JR2_RD(VTSS_DEVCPU_PTP_PTP_PINS_PTP_TOD_NSEC(io), &value);
    ts->nanoseconds =
        VTSS_X_DEVCPU_PTP_PTP_PINS_PTP_TOD_NSEC_PTP_TOD_NSEC(value);
...
static vtss_rc jr2_ts_timeofday_read(vtss_state_t     *vtss_state,
                                     u32               domain,
                                     vtss_timestamp_t *ts,
                                     u64              *tc)
{
    JR2_PTP_PIN_ACTION(TOD_ACC_PIN, PTP_PIN_ACTION_SAVE, PTP_PIN_ACTION_NOSYNC,
                       domain);
...
static vtss_rc jr2_ts_timeofday_sample(vtss_state_t *vtss_state)
{
    ...
    vtss_ts_ext_clock_one_pps_mode_t one_pps_mode =
        vtss_state->ts.conf.ext_clock_mode.one_pps_mode;
    u32 domain;
    if ((one_pps_mode == TS_EXT_CLOCK_MODE_ONE_PPS_INPUT) ||
        (one_pps_mode == TS_EXT_CLOCK_MODE_ONE_PPS_OUTPUT_INPUT) ||
        (one_pps_mode == TS_EXT_CLOCK_MODE_MAX)) {
        /* This is for 'one_pps_mode'  TS_EXT_CLOCK_MODE_ONE_PPS_INPUT -
         * TS_EXT_CLOCK_MODE_ONE_PPS_OUTPUT_INPUT - TS_EXT_CLOCK_MODE_MAX */
        for (domain = 0; domain < VTSS_TS_DOMAIN_ARRAY_SIZE; domain++) {
            VTSS_RC(jr2_ts_timeofday_read(vtss_state, domain,
                                          &vtss_state->ts.conf.sample_ts[domain],
                                          &vtss_state->ts.conf
                                               .sample_tc[domain]));
            VTSS_I("domain %u, sample_ts: %u-%u:%u, tc %" PRIu64 "", domain,
                   vtss_state->ts.conf.sample_ts[domain].sec_msb,
                   vtss_state->ts.conf.sample_ts[domain].seconds,
                   vtss_state->ts.conf.sample_ts[domain].nanoseconds,
                   vtss_state->ts.conf.sample_tc[domain]);
        }
        JR2_PTP_PIN_ACTION(TOD_ACC_PIN, PTP_PIN_ACTION_SAVE,
                           PTP_PIN_ACTION_SYNC, 0);
    } else if (one_pps_mode == TS_EXT_CLOCK_MODE_ONE_PPS_OUTPUT) {
        (void)vtss_jr2_gpio_mode(vtss_state, 0, ptp_gpio[EXT_PPS_PIN],
                                 VTSS_GPIO_OUT);
        for (domain = 0; domain < VTSS_TS_DOMAIN_ARRAY_SIZE; domain++) {
            VTSS_RC(jr2_ts_timeofday_read(vtss_state, domain,
                                          &vtss_state->ts.conf.sample_ts[domain],
        ...
        JR2_PTP_PIN_ACTION(EXT_PPS_PIN, PTP_PIN_ACTION_CLOCK,
                           PTP_PIN_ACTION_SYNC, 0);
        (void)vtss_jr2_gpio_mode(vtss_state, 0, ptp_gpio[EXT_PPS_PIN],
                                 VTSS_GPIO_ALT_0);
    }
...
static vtss_rc jr2_ts_domain_timeofday_get(vtss_state_t     *vtss_state,
                                           u32               domain,
                                           vtss_timestamp_t *ts,
                                           u64              *tc)
{
#if defined(TOD_ACC_EXT_PPS_PIN_SHARED)
    if (vtss_state->ts.conf.ext_clock_mode.one_pps_mode ==
    ...
    VTSS_RC(jr2_ts_timeofday_read(vtss_state, domain, ts, tc));
#endif
    VTSS_D("ts->sec_msb: %u, ts->seconds: %u, ts->nanoseconds: %u, domain: %u",
           ts->sec_msb, ts->seconds, ts->nanoseconds, domain);
    return VTSS_RC_OK;
}

static vtss_rc jr2_ts_timeofday_get(vtss_state_t     *vtss_state,
                                    vtss_timestamp_t *ts,
                                    u64              *tc)
{
...
static vtss_rc jr2_ts_domain_timeofday_next_pps_get(vtss_state_t *vtss_state,
                                                    u32           domain,
                                                    vtss_timestamp_t *ts)
{
...
static vtss_rc jr2_ts_timeofday_next_pps_get(vtss_state_t     *vtss_state,
                                             vtss_timestamp_t *ts)
{
    return jr2_ts_domain_timeofday_next_pps_get(vtss_state, 0, ts);
}

static vtss_rc jr2_ts_timeofday_prev_pps_get(vtss_state_t     *vtss_state,
                                             vtss_timestamp_t *ts)
{
    return jr2_ts_domain_timeofday_prev_pps_get(vtss_state, 0, ts);
}

static vtss_rc jr2_ts_multi_domain_timeofday_get(vtss_state_t  *vtss_state,
                                                 ...
                                                 vtss_timestamp_t *const ts)
{
    ...
    JR2_WRM(VTSS_DEVCPU_PTP_PTP_CFG_PTP_MISC_CFG,
            VTSS_F_DEVCPU_PTP_PTP_CFG_PTP_MISC_CFG_PTP_TOD_FREEZE(7),
            VTSS_M_DEVCPU_PTP_PTP_CFG_PTP_MISC_CFG_PTP_TOD_FREEZE);
    for (domain = 0; domain < domain_cnt; domain++) {
        t_stamp = &ts[domain];
        JR2_RD(VTSS_DEVCPU_PTP_PTP_STATUS_PTP_CUR_SEC_MSB(domain), &value);
        t_stamp->sec_msb =
            VTSS_X_DEVCPU_PTP_PTP_STATUS_PTP_CUR_SEC_MSB_PTP_CUR_SEC_MSB(value);
        JR2_RD(VTSS_DEVCPU_PTP_PTP_STATUS_PTP_CUR_SEC_LSB(domain),
               &t_stamp->seconds);
        JR2_RD(VTSS_DEVCPU_PTP_PTP_STATUS_PTP_CUR_NSEC(domain), &value);
        t_stamp->nanoseconds =
            VTSS_X_DEVCPU_PTP_PTP_STATUS_PTP_CUR_NSEC_PTP_CUR_NSEC(value);
    ...
    JR2_WRM(VTSS_DEVCPU_PTP_PTP_CFG_PTP_MISC_CFG,
            VTSS_F_DEVCPU_PTP_PTP_CFG_PTP_MISC_CFG_PTP_TOD_FREEZE(0),
            VTSS_M_DEVCPU_PTP_PTP_CFG_PTP_MISC_CFG_PTP_TOD_FREEZE);
    return VTSS_RC_OK;
#else
...
static vtss_rc jr2_ts_domain_timeofday_set(vtss_state_t           *vtss_state,
                                           u32                     domain,
                                           const vtss_timestamp_t *ts)
{
    // truncate nanosec value
    u32 ns = (ts->nanoseconds / vtss_state->ts.conf.clk_set_trunc) *
             vtss_state->ts.conf.clk_set_trunc;
    /* must be in IDLE mode before the time can be loaded */
    JR2_PTP_PIN_ACTION(TOD_ACC_PIN, PTP_PIN_ACTION_IDLE, PTP_PIN_ACTION_NOSYNC,
                       domain);
    JR2_WR(VTSS_DEVCPU_PTP_PTP_PINS_PTP_TOD_SEC_MSB(TOD_ACC_PIN),
           VTSS_F_DEVCPU_PTP_PTP_PINS_PTP_TOD_SEC_MSB_PTP_TOD_SEC_MSB(
               ts->sec_msb));
    ...
    JR2_PTP_PIN_ACTION(TOD_ACC_PIN, PTP_PIN_ACTION_LOAD, PTP_PIN_ACTION_NOSYNC,
                       domain);
#if defined(TOD_ACC_EXT_PPS_PIN_SHARED)
    VTSS_RC(jr2_ts_timeofday_sample(vtss_state));
#endif
    VTSS_D("ts->sec_msb: %u, ts->seconds: %u, ts->nanoseconds: %u, domain %d",
           ts->sec_msb, ts->seconds, ts->nanoseconds, domain);
    VTSS_D("truncated ns: %u truncate value %d", ns,
           vtss_state->ts.conf.clk_set_trunc);
    return VTSS_RC_OK;
}

static vtss_rc jr2_ts_timeofday_set(vtss_state_t           *vtss_state,
                                    const vtss_timestamp_t *ts)
{
    return jr2_ts_domain_timeofday_set(vtss_state, 0, ts);
}
static vtss_rc jr2_ts_domain_timeofday_offset_set(vtss_state_t *vtss_state,
                                                  u32           domain,
                                                  i32           offset)
{
    VTSS_D("offset before: %d", offset);
    // truncate nanosec value
    offset = (offset / vtss_state->ts.conf.clk_set_trunc) *
             vtss_state->ts.conf.clk_set_trunc;
    /* must be in IDLE mode before the time can be loaded */
    JR2_PTP_PIN_ACTION(TOD_ACC_PIN, PTP_PIN_ACTION_IDLE, PTP_PIN_ACTION_NOSYNC,
                       domain);
    ...
    JR2_PTP_PIN_ACTION(TOD_ACC_PIN, PTP_PIN_ACTION_DELTA, PTP_PIN_ACTION_NOSYNC,
                       domain);

#if defined(TOD_ACC_EXT_PPS_PIN_SHARED)
    VTSS_RC(jr2_ts_timeofday_sample(vtss_state));
#endif
    VTSS_D("Time after offset_set: %d, %d, %d",
           vtss_state->ts.conf.sample_ts[domain].sec_msb,
           vtss_state->ts.conf.sample_ts[domain].seconds,
           vtss_state->ts.conf.sample_ts[domain].nanoseconds);
...
static vtss_rc jr2_ts_domain_timeofday_set_delta(vtss_state_t *vtss_state,
                                                 u32           domain,
                                                 const vtss_timestamp_t *ts,
                                                 BOOL negative)
{
    if (ts->seconds > 0 || ts->sec_msb > 0 ||
        ts->nanoseconds > HW_NS_PR_SEC / 2) {
        ...
        return jr2_ts_domain_timeofday_offset_set(
            vtss_state, domain,
            negative ? (i32)ts->nanoseconds
                     : -ts->nanoseconds); /* Function
                                             jr2_ts_domain_timeofday_offset_set
                                             is subtracting nanoseconds from
                                             TOD, so 'negative' must be positive
                                             and visa versa */
    }
}

static vtss_rc jr2_ts_timeofday_set_delta(vtss_state_t           *vtss_state,
                                          const vtss_timestamp_t *ts,
                                          BOOL                    negative)
{
...
static vtss_rc jr2_ts_domain_adjtimer_set(vtss_state_t *vtss_state, u32 domain)
{
    ...
    if (domain >= VTSS_TS_DOMAIN_ARRAY_SIZE) {
        ...
        if (adj != 0) {
            ...
            u64 adj_val =
                VTSS_DIV64((u64)HW_PS_PR_SEC * (u64)ADJ_UNITS_PR_NS, adj_abs);
            ...
            if (adj_val >=
                (1L << 30)) { /* if it is still too big then rate is 0 */
                /* The adjustment value is too large to be in ns */
                adj = 0;
            } else {
                /* Configure and enable adjustment */
                JR2_WR(
                    VTSS_DEVCPU_PTP_PTP_CFG_CLK_ADJ_FRQ(domain),
...
static vtss_rc jr2_ts_external_clock_mode_set(vtss_state_t *vtss_state)
{
    ...
    JR2_PTP_PIN_ACTION(EXT_CLK_PIN, PTP_PIN_ACTION_IDLE, PTP_PIN_ACTION_NOSYNC,
                       ext_clock_mode->domain);
    if (ext_clock_mode->enable) {
        /* When external sync pulse is enabled the 'one_pps_mode' is not
         * considered */
        ...
        (void)vtss_jr2_gpio_mode(vtss_state, 0, ptp_gpio[EXT_CLK_PIN],
                                 VTSS_GPIO_ALT_0);
        JR2_PTP_PIN_ACTION(EXT_CLK_PIN, PTP_PIN_ACTION_CLOCK,
...
static vtss_rc jr2_ts_alt_clock_saved_get(vtss_state_t *vtss_state,
                                          u64 *const    saved)
{
    u32 nsec;
...
static vtss_rc jr2_ts_alt_clock_mode_set(vtss_state_t *vtss_state)
{
    ...
    JR2_PTP_PIN_ACTION(ALT_PPS_PIN, PTP_PIN_ACTION_IDLE, PTP_PIN_ACTION_NOSYNC,
                       0);
    ...
    if (alt_clock_mode->one_pps_out) {
        ...
        (void)vtss_jr2_gpio_mode(vtss_state, 0, ptp_gpio[ALT_PPS_PIN],
                                 VTSS_GPIO_ALT_0);
        JR2_PTP_PIN_ACTION(ALT_PPS_PIN, PTP_PIN_ACTION_CLOCK,
                           PTP_PIN_ACTION_SYNC, 0);
    } else {
        (void)vtss_jr2_gpio_mode(vtss_state, 0, ptp_gpio[ALT_PPS_PIN],
                                 VTSS_GPIO_IN);
    ...
    JR2_PTP_PIN_ACTION(ALT_LDST_PIN, PTP_PIN_ACTION_IDLE, PTP_PIN_ACTION_NOSYNC,
                       0);
    if (alt_clock_mode->one_pps_in) {
        if (alt_clock_mode->save && alt_clock_mode->load) {
            ...
            (void)vtss_jr2_gpio_mode(vtss_state, 0, ptp_gpio[ALT_LDST_PIN],
                                     VTSS_GPIO_ALT_0);
            JR2_PTP_PIN_ACTION(ALT_LDST_PIN, PTP_PIN_ACTION_SAVE,
                               PTP_PIN_ACTION_SYNC, 0);
        } else if (alt_clock_mode->load) {
            (void)vtss_jr2_gpio_mode(vtss_state, 0, ptp_gpio[ALT_LDST_PIN],
                                     VTSS_GPIO_ALT_0);
            JR2_PTP_PIN_ACTION(ALT_LDST_PIN, PTP_PIN_ACTION_LOAD,
                               PTP_PIN_ACTION_SYNC, 0);
        }
    } else {
        (void)vtss_jr2_gpio_mode(vtss_state, 0, ptp_gpio[ALT_LDST_PIN],
...
static vtss_rc jr2_ts_timeofday_next_pps_set(vtss_state_t *vtss_state,
                                             const vtss_timestamp_t *const ts)
{
    if (vtss_state->ts.conf.alt_clock_mode.one_pps_in) {
        JR2_WR(VTSS_DEVCPU_PTP_PTP_PINS_PTP_TOD_SEC_MSB(ALT_LDST_PIN),
               VTSS_F_DEVCPU_PTP_PTP_PINS_PTP_TOD_SEC_MSB_PTP_TOD_SEC_MSB(
                   ts->sec_msb));
        JR2_WR(VTSS_DEVCPU_PTP_PTP_PINS_PTP_TOD_SEC_LSB(ALT_LDST_PIN),
               ts->seconds - 1);
        JR2_WR(VTSS_DEVCPU_PTP_PTP_PINS_PTP_TOD_NSEC(ALT_LDST_PIN),
               VTSS_F_DEVCPU_PTP_PTP_PINS_PTP_TOD_NSEC_PTP_TOD_NSEC(
                   ts->nanoseconds));
    }
...
static vtss_rc jr2_ts_ingress_latency_set(vtss_state_t  *vtss_state,
                                          vtss_port_no_t port_no)
{
    ...
    case VTSS_PORT_INTERFACE_RXAUI:
        ...
        break;
    case VTSS_PORT_INTERFACE_NO_CONNECTION: break;
    default:
...
static vtss_rc jr2_ts_p2p_delay_set(vtss_state_t  *vtss_state,
                                    vtss_port_no_t port_no)
{
    vtss_ts_port_conf_t *conf = &vtss_state->ts.port_conf[port_no];
    i32                  p2p_delay = VTSS_INTERVAL_NS(conf->p2p_delay);
    JR2_WR(VTSS_REW_PORT_PTP_IDLY2_CFG(VTSS_CHIP_PORT(port_no)),
           p2p_delay + VTSS_INTERVAL_NS(conf->delay_asymmetry));
    return VTSS_RC_OK;
}

static vtss_rc jr2_ts_egress_latency_set(vtss_state_t  *vtss_state,
                                         vtss_port_no_t port_no)
{
    vtss_port_interface_t interface = vtss_state->port.conf[port_no].if_type;
    u32                   port = VTSS_CHIP_PORT(port_no);
...
static vtss_rc jr2_ts_delay_asymmetry_set(vtss_state_t  *vtss_state,
                                          vtss_port_no_t port_no)
{
    vtss_ts_port_conf_t *conf = &vtss_state->ts.port_conf[port_no];
...
static vtss_rc jr2_ts_operation_mode_set(vtss_state_t  *vtss_state,
                                         vtss_port_no_t port_no,
                                         BOOL           mode_domain_config)
{
    ...
    if (mode == TS_MODE_INTERNAL) {
        ...
        case TS_INTERNAL_FMT_RESERVED_LEN_32BIT:     mode_val = 2; break;
        case TS_INTERNAL_FMT_SUB_ADD_LEN_44BIT_CF62: mode_val = 3; break;
        case TS_INTERNAL_FMT_RESERVED_LEN_48BIT_CF:  mode_val = 4; break;
        default:
...
static vtss_rc jr2_ts_timestamp_get(vtss_state_t *vtss_state)
{
    ...
    while (VTSS_X_REW_PTP_CTRL_PTP_TWOSTEP_CTRL_PTP_VLD(value)) {
        ...
        if (!VTSS_X_REW_PTP_CTRL_PTP_TWOSTEP_CTRL_STAMP_TX(value)) {
            ...
            JR2_WR(VTSS_REW_PTP_CTRL_PTP_TWOSTEP_CTRL,
        ...
        tx_port =
            vtss_api_port(vtss_state,
                          VTSS_X_REW_PTP_CTRL_PTP_TWOSTEP_CTRL_STAMP_PORT(value));
        ...
        VTSS_D("value %x, delay %u, tx_port %u, mess_id %u", value, delay,
               tx_port, mess_id);
        JR2_WR(VTSS_REW_PTP_CTRL_PTP_TWOSTEP_CTRL,
               VTSS_F_REW_PTP_CTRL_PTP_TWOSTEP_CTRL_PTP_NXT(1));
        JR2_RD(VTSS_REW_PTP_CTRL_PTP_TWOSTEP_CTRL, &value);
 ...
 */
static vtss_rc jr2_ts_status_change(vtss_state_t        *vtss_state,
                                    const vtss_port_no_t port_no)
{
    ...
    case VTSS_PORT_INTERFACE_SERDES:
        ...
        JR2_RD(VTSS_DEV1G_PCS1G_CFG_STATUS_PCS1G_LINK_STATUS(VTSS_TO_DEV1G(port)),
               &value);
        rx_delay +=
            8 *
            VTSS_X_DEV1G_PCS1G_CFG_STATUS_PCS1G_LINK_STATUS_DELAY_VAR(value);
        break;
    case VTSS_PORT_INTERFACE_VAUI:
    ...
    case VTSS_PORT_INTERFACE_QSGMII:
        ...
        rx_delay +=
            2 * VTSS_X_HSIO_HW_CFGSTAT_HW_QSGMII_STAT_DELAY_VAR_X200PS(value);
        break;
    case VTSS_PORT_INTERFACE_XAUI:
        ...
        JR2_RD(VTSS_DEV10G_PCS_XAUI_STATUS_PCS_XAUI_DESKEW_STATUS(
                   VTSS_TO_DEV(port)),
               &value);
        rx_delay +=
            32 *
            (VTSS_X_DEV10G_PCS_XAUI_STATUS_PCS_XAUI_DESKEW_STATUS_LANE_DESKEW_DELAY(
                 value) %
             16);
        JR2_RD(VTSS_DEV10G_PCS_XAUI_STATUS_PCS_XAUI_CGALIGN_STATUS(
                   VTSS_TO_DEV(port)),
               &value);
        rx_delay +=
            32 *
            (VTSS_X_DEV10G_PCS_XAUI_STATUS_PCS_XAUI_CGALIGN_STATUS_LANE_CGALIGN_DELAY(
                 value) %
             32) /
            10;
        break;
    ...
    case VTSS_PORT_INTERFACE_SGMII:
        ...
        case VTSS_SPEED_10M:
            JR2_WR(
                VTSS_DEV1G_DEV_CFG_STATUS_PTP_PREEDICT_CFG(VTSS_TO_DEV1G(port)),
                VTSS_F_DEV1G_DEV_CFG_STATUS_PTP_PREEDICT_CFG_PTP_PHASE_PREDICT_CFG(
                    3) |
                    VTSS_F_DEV1G_DEV_CFG_STATUS_PTP_PREEDICT_CFG_PTP_PHY_PREDICT_CFG(
                        0x30));
            break;
    ...
    case VTSS_PORT_INTERFACE_SFI:
        ...
        JR2_WR(VTSS_DEV10G_DEV_CFG_STATUS_PTP_PREEDICT_CFG(VTSS_TO_DEV(port)),
               VTSS_F_DEV10G_DEV_CFG_STATUS_PTP_PREEDICT_CFG_PTP_PHASE_PREDICT_CFG(
                   4));
        break;
    case VTSS_PORT_INTERFACE_NO_CONNECTION: break;
    default:
    ...
    case VTSS_PORT_INTERFACE_SGMII:
        ...
        switch (vtss_state->port.conf[port_no].speed) {
        case VTSS_SPEED_10M:
            //                rx_delay += 46290; // these big numbers are not
            //                supported tx_delay += 58410; // these big numbers
            //                are not supported
            break;
        case VTSS_SPEED_100M:
            //                rx_delay += 3300;
    ...
    VTSS_D("port_no %d, igr_latency %d, egr_latency %d", port_no, rx_delay,
           tx_delay);
...
static vtss_rc jr2_ts_timestamp_id_release(vtss_state_t *vtss_state, u32 ts_id)
{
    return VTSS_RC_OK;
}
...
static vtss_rc jr2_ts_external_io_mode_set(vtss_state_t *vtss_state, u32 io)
{
    ...
    JR2_PTP_PIN_ACTION(io, PTP_PIN_ACTION_IDLE, PTP_PIN_ACTION_NOSYNC,
                       ext_io_mode->domain);
    /* Set gpio mode */
    ...
    if (ext_io_mode->pin == TS_EXT_IO_MODE_WAVEFORM_OUTPUT) {
        ...
        JR2_WR(VTSS_DEVCPU_PTP_PTP_PINS_PIN_WF_LOW_PERIOD(io),
               VTSS_F_DEVCPU_PTP_PTP_PINS_PIN_WF_LOW_PERIOD_PIN_WFL(low_div));
        JR2_PTP_PIN_ACTION(io, PTP_PIN_ACTION_CLOCK, PTP_PIN_ACTION_NOSYNC,
                           ext_io_mode->domain);
    } else if (ext_io_mode->pin == TS_EXT_IO_MODE_ONE_PPS_OUTPUT) {
        JR2_WR(VTSS_DEVCPU_PTP_PTP_PINS_PIN_WF_HIGH_PERIOD(io),
               VTSS_F_DEVCPU_PTP_PTP_PINS_PIN_WF_HIGH_PERIOD_PIN_WFH(PPS_WIDTH));
        JR2_WR(VTSS_DEVCPU_PTP_PTP_PINS_PIN_WF_LOW_PERIOD(io), 0);
        JR2_PTP_PIN_ACTION(io, PTP_PIN_ACTION_CLOCK, PTP_PIN_ACTION_SYNC,
                           ext_io_mode->domain);
    } else if (ext_io_mode->pin == TS_EXT_IO_MODE_ONE_PPS_LOAD) {
        JR2_PTP_PIN_ACTION(io, PTP_PIN_ACTION_LOAD, PTP_PIN_ACTION_SYNC,
                           ext_io_mode->domain);
    } else if (ext_io_mode->pin == TS_EXT_IO_MODE_ONE_PPS_SAVE) {
        JR2_PTP_PIN_ACTION(io, PTP_PIN_ACTION_SAVE, PTP_PIN_ACTION_SYNC,
                           ext_io_mode->domain);
    } else if (ext_io_mode->pin == TS_EXT_IO_MODE_ONE_PPS_LOAD) {
        JR2_PTP_PIN_ACTION(io, PTP_PIN_ACTION_LOAD, PTP_PIN_ACTION_SYNC,
                           ext_io_mode->domain);
...
static vtss_rc jr2_ts_saved_timeofday_get(vtss_state_t     *vtss_state,
                                          u32               io,
                                          vtss_timestamp_t *ts,
                                          u64              *tc)
{
...
static vtss_rc jr2_autoresp_dom_cfg_set(vtss_state_t *vtss_state, u8 domain)
{
    ...
    if (domain < VTSS_TS_DOMAIN_ARRAY_SIZE) {
        ...
        u32 msb = (cfg->clock_identity[0] << 24) +
                  (cfg->clock_identity[1] << 16) +
                  (cfg->clock_identity[2] << 8) + (cfg->clock_identity[3]);
        u32 lsb = (cfg->clock_identity[4] << 24) +
                  (cfg->clock_identity[5] << 16) +
        ...
        JR2_WR(VTSS_ANA_ACL_PTP_DOM_PTP_SRC_PORT_CFG(domain),
               VTSS_F_ANA_ACL_PTP_DOM_PTP_SRC_PORT_CFG_PORT_NUM_SEL(
                   cfg->ptp_port_individual) |
                   VTSS_F_ANA_ACL_PTP_DOM_PTP_SRC_PORT_CFG_PORT_NUM(
                       cfg->ptp_port_msb));
        JR2_WR(VTSS_ANA_ACL_PTP_DOM_PTP_MISC_CFG(domain),
               VTSS_F_ANA_ACL_PTP_DOM_PTP_MISC_CFG_FLAG_FIELD_MASK(
                   cfg->flag_field_update.mask) |
                   VTSS_F_ANA_ACL_PTP_DOM_PTP_MISC_CFG_FLAG_FIELD(
                       cfg->flag_field_update.value));
        return VTSS_RC_OK;
    } else {
        return VTSS_RC_ERROR;
    }
...
static vtss_rc jr2_ts_smac_set(vtss_state_t *vtss_state, vtss_port_no_t port_no)
{
    ...
    u32        smac_low = (smac.addr[2] << 24) + (smac.addr[3] << 16) +
                   (smac.addr[4] << 8) + (smac.addr[5]);
...
vtss_rc vtss_cil_ts_conf_set(vtss_state_t               *vtss_state,
                            ...
                            const vtss_debug_info_t *const info)
{
    ...
    for (idx = 0; idx <= 2; idx++) {
        VTSS_FMT(buf, "CLK_ADJ_CFG[%u]", idx);
        vtss_jr2_debug_reg(vtss_state, ss,
                           VTSS_DEVCPU_PTP_PTP_CFG_CLK_ADJ_CFG(idx), buf.s);
    }
    for (idx = 0; idx <= 2; idx++) {
        VTSS_FMT(buf, "CLK_ADJ_FRQ[%u]", idx);
        vtss_jr2_debug_reg(vtss_state, ss,
                           VTSS_DEVCPU_PTP_PTP_CFG_CLK_ADJ_FRQ(idx), buf.s);
    }
    for (idx = 0; idx <= 2; idx++) {
        VTSS_FMT(buf, "DOM_PTP_CLOCK_ID_MSB[%u]", idx);
        vtss_jr2_debug_reg(vtss_state, ss,
                           VTSS_ANA_ACL_PTP_DOM_PTP_CLOCK_ID_MSB(idx), buf.s);
        VTSS_FMT(buf, "DOM_PTP_CLOCK_ID_LSB[%u]", idx);
        vtss_jr2_debug_reg(vtss_state, ss,
                           VTSS_ANA_ACL_PTP_DOM_PTP_CLOCK_ID_LSB(idx), buf.s);
        VTSS_FMT(buf, "DOM_PTP_SRC_PORT_CFG[%u]", idx);
        vtss_jr2_debug_reg(vtss_state, ss,
                           VTSS_ANA_ACL_PTP_DOM_PTP_SRC_PORT_CFG(idx), buf.s);
        VTSS_FMT(buf, "PTP_DOM_PTP_MISC_CFG[%u]", idx);
        vtss_jr2_debug_reg(vtss_state, ss,
                           VTSS_ANA_ACL_PTP_DOM_PTP_MISC_CFG(idx), buf.s);
    }
    ...
    for (port_no = VTSS_PORT_NO_START; port_no < vtss_state->port_count;
        ...
        case VTSS_PORT_INTERFACE_100FX:
        case VTSS_PORT_INTERFACE_QSGMII:
            VTSS_FMT(buf, "DEV1G (port_no %u):DEV_CFG_STATUS", port_no);
            vtss_jr2_debug_reg_header(ss, buf.s);
            vtss_jr2_debug_reg(
                vtss_state, ss,
...
static vtss_rc jr2_ts_init(vtss_state_t *vtss_state)
{
    ...
    JR2_WR(VTSS_DEVCPU_PTP_PTP_CFG_PTP_MISC_CFG,
           VTSS_F_DEVCPU_PTP_PTP_CFG_PTP_MISC_CFG_PTP_ENA(7));
    /* Enable PTP Delay_Req/resp processing, use default values for other bits */
    JR2_WRM(VTSS_ANA_ACL_VCAP_S2_PTP_MISC_CTRL,
            VTSS_F_ANA_ACL_VCAP_S2_PTP_MISC_CTRL_PTP_ALLOW_ACL_REW_ENA(1) |
                VTSS_F_ANA_ACL_VCAP_S2_PTP_MISC_CTRL_PTP_DELAY_REQ_UDP_LEN52(0),
            VTSS_M_ANA_ACL_VCAP_S2_PTP_MISC_CTRL_PTP_ALLOW_ACL_REW_ENA |
                VTSS_M_ANA_ACL_VCAP_S2_PTP_MISC_CTRL_PTP_DELAY_REQ_UDP_LEN52);
    JR2_RD(VTSS_DEVCPU_PTP_PTP_CFG_PTP_SYS_CLK_CFG, &clk_cfg);
    // the clock period is read back from the chip, and the clk_set_trunc is
    // calculated as a multiplum of the clock period multiplied with the lowest
    // number that results in an integer number of ns.
    clk_pr_ns =
        VTSS_X_DEVCPU_PTP_PTP_CFG_PTP_SYS_CLK_CFG_PTP_SYS_CLK_PER_NS(clk_cfg);
    clk_pr_100ps =
        VTSS_X_DEVCPU_PTP_PTP_CFG_PTP_SYS_CLK_CFG_PTP_SYS_CLK_PER_PS100(clk_cfg);
    ...
    VTSS_D("PTP clock period %u.%u => clk_set_trunc %u", clk_pr_ns,
           clk_pr_100ps, vtss_state->ts.conf.clk_set_trunc);
    return VTSS_RC_OK;
}
...
vtss_rc vtss_jr2_ts_init(vtss_state_t *vtss_state, vtss_init_cmd_t cmd)
{
    ...
    case VTSS_INIT_CMD_CREATE:
        ...
        state->domain_timeofday_next_pps_get =
            jr2_ts_domain_timeofday_next_pps_get;
        ...
        break;
    case VTSS_INIT_CMD_INIT: VTSS_RC(jr2_ts_init(vtss_state)); break;
    case VTSS_INIT_CMD_PORT_MAP:
        /* Initialize the PTP Port ID port number. This is done anyway in the
         * jr2_ts_operation_mode_set() function but this is only called from AIL
         * in case of changes in mode or domain. */
        for (port_no = VTSS_PORT_NO_START; port_no < vtss_state->port_count;
             port_no++) {
            port = VTSS_CHIP_PORT(port_no);
            JR2_WRM(VTSS_ANA_ACL_PORT_PTP_CFG(port),
