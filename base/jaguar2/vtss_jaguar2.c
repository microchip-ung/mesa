...
static void jr2_evc_counter_update(u32                       frames,
                                   u32                       lsb,
                                   u32                       msb,
...
static jr2_cal_speed_t jr2_cal_speed_get(vtss_state_t  *vtss_state,
                                         vtss_port_no_t port_no,
                                         i32           *port)
{
    ...
    case VTSS_BW_1G: return JR2_CAL_SPEED_1G;

    case VTSS_BW_2G5: return JR2_CAL_SPEED_2G5;

    case VTSS_BW_10G: return JR2_CAL_SPEED_10G;
...
static vtss_rc jr2_calendar_auto(vtss_state_t *vtss_state)
{
    ...
    JR2_WRM(VTSS_QSYS_CALCFG_CAL_CTRL,
            VTSS_F_QSYS_CALCFG_CAL_CTRL_CAL_AUTO_GRANT_RATE(671), /* 672->671,
                                                                     BZ19678 */
    ...
    JR2_WRM(VTSS_QSYS_CALCFG_CAL_CTRL, VTSS_F_QSYS_CALCFG_CAL_CTRL_CAL_MODE(8),
...
static vtss_rc jr2_calendar_do_set(vtss_state_t *vtss_state,
                                   u8           *cal,
                                   u32           length,
                                   BOOL          grant_idle_to_vd1,
                                   BOOL          dynamic)
{
    u32 i;

    // This function is used when applying a pre-computed calendar on JR2B, when
    // the calendar calculation function fails and a pre-computed calendar for
    // JR2C is needed, or when applying a on-target-calculated calendar.
    ...
    /* Halt the calendar */
    JR2_WRM(VTSS_QSYS_CALCFG_CAL_CTRL, VTSS_F_QSYS_CALCFG_CAL_CTRL_CAL_MODE(10),
            VTSS_M_QSYS_CALCFG_CAL_CTRL_CAL_MODE);
    ...
    /* Press start */
    JR2_WRM(VTSS_QSYS_CALCFG_CAL_CTRL, VTSS_F_QSYS_CALCFG_CAL_CTRL_CAL_MODE(9),
            VTSS_M_QSYS_CALCFG_CAL_CTRL_CAL_MODE);
...
#if defined(VTSS_ARCH_JAGUAR_2_B)
// Auto calendar does not supports rates close to 80Gb.  Those must be manually
// configured.
...
#define FFL_DSM_BUF_SIZE_10G 576
    ...
    rtl_cfg->dsm_bytes_below_wm_mul_100[CBC_PORT_TYPE_ETH][CBC_SPEED_1000] =
        4 * 100 * FFL_DSM_BUF_WM_1G;
    rtl_cfg->dsm_bytes_below_wm_mul_100[CBC_PORT_TYPE_ETH][CBC_SPEED_2500] =
    ...
    // This is scaled by a factor of 100.
    rtl_cfg->dsm_cells_above_wm_mul_100[CBC_PORT_TYPE_ETH][CBC_SPEED_1000] =
        (4 * 100 * (FFL_DSM_BUF_SIZE_1G - FFL_DSM_BUF_WM_1G)) /
        208; // (4 * (288 - 117)) / 208 = 1.75
    rtl_cfg->dsm_cells_above_wm_mul_100[CBC_PORT_TYPE_ETH][CBC_SPEED_2500] =
        (4 * 100 * (FFL_DSM_BUF_SIZE_2G5 - FFL_DSM_BUF_WM_2G5)) /
        208; // (4 * (288 - 117)) / 208 = 1.75
                                        ...
                                        cbc_rtl_cfg_t *rtl_cfg)
{
    ...
    rtl_cfg->dsm_bytes_below_wm_mul_100[CBC_PORT_TYPE_ETH][CBC_SPEED_1000] =
        29867; // 14 * 512 / (8 * 3) = 298.6667
    rtl_cfg->dsm_bytes_below_wm_mul_100[CBC_PORT_TYPE_ETH][CBC_SPEED_2500] =
        29867; // 14 * 512 / (8 * 3) = 298.6667
    rtl_cfg->dsm_bytes_below_wm_mul_100[CBC_PORT_TYPE_ETH][CBC_SPEED_10000] =
        49067; // 23 * 512 / (8 * 3) = 490.6667
    rtl_cfg->dsm_bytes_below_wm_mul_100[CBC_PORT_TYPE_VD][CBC_SPEED_NA] =
        34133; // 16 *  64 / (1 * 3) = 341.3333
    rtl_cfg->dsm_bytes_below_wm_mul_100[CBC_PORT_TYPE_CPU][CBC_SPEED_NA] =
        29867; // 14 * 512 / (8 * 3) = 298.6667

    // This is scaled by a factor of 100.
    rtl_cfg->dsm_cells_above_wm_mul_100[CBC_PORT_TYPE_ETH][CBC_SPEED_1000] =
        287; // 14 * 512 * 2 / (8 * 208 * 3) =  2.8718
    rtl_cfg->dsm_cells_above_wm_mul_100[CBC_PORT_TYPE_ETH][CBC_SPEED_2500] =
        287; // 14 * 512 * 2 / (8 * 208 * 3) =  2.8718
    rtl_cfg->dsm_cells_above_wm_mul_100[CBC_PORT_TYPE_ETH][CBC_SPEED_10000] =
        472; // 23 * 512 * 2 / (8 * 208 * 3) =  4.7179
    rtl_cfg->dsm_cells_above_wm_mul_100[CBC_PORT_TYPE_VD][CBC_SPEED_NA] =
        1067; // 16       * 2 / (          3) = 10.6667
    rtl_cfg->dsm_cells_above_wm_mul_100[CBC_PORT_TYPE_CPU][CBC_SPEED_NA] =
        1067; // 16       * 2 / (          3) = 10.6667
                                         ...
                                         cbc_req_cfg_t *req_cfg)
{
    ...
    // When it comes to VD0 (multicast), we don't assign anything in the
    // calendar, because that could lead to failure to calculate a new calendar.
    // Instead, we assign the idle periods of the calendar to VD0. We do that
    // when the calendar is successfully applied.
...
#if defined(VTSS_CALENDAR_CALC)
static vtss_rc jr2_calendar_req_cfg_get(vtss_state_t  *vtss_state,
                                        cbc_rtl_cfg_t *rtl_cfg,
                                        cbc_req_cfg_t *req_cfg)
{
    vtss_port_no_t port_no;
    ...
    for (port_no = 0; port_no < VTSS_PORTS; port_no++) {
        ...
        req_cfg->port_cfg[chip_port].bw = spd == JR2_CAL_SPEED_1G    ? 1000
                                          : spd == JR2_CAL_SPEED_2G5 ? 2500
                                                                     : 10000;
    }
...
#if defined(VTSS_CALENDAR_CALC)
static vtss_rc jr2_cbc_dist(cbc_rtl_cfg_t      *rtl_cfg,
                            vtss_calendar_t    *cbc,
...
#if defined(VTSS_CALENDAR_CALC)
static vtss_rc jr2_cbc_slot_acceptable(cbc_rtl_cfg_t      *rtl_cfg,
                                       cbc_req_port_cfg_t *port_cfg,
                                       vtss_calendar_t    *cbc,
                                       u32                 pos,
                                       BOOL               *acceptable)
{
    ...
    // Taxi check
    if (cbc->cbc[pos_prv] != CBC_SLOT_FREE &&
        rtl_cfg->port_cfg[cbc->cbc[pos_prv]].taxi_num == taxi_num) {
        // Taxi number conflict with previous slot
        *acceptable = FALSE;
        return VTSS_RC_OK;
    }

    if (cbc->cbc[pos_nxt] != CBC_SLOT_FREE &&
        rtl_cfg->port_cfg[cbc->cbc[pos_nxt]].taxi_num == taxi_num) {
...
#if defined(VTSS_CALENDAR_CALC)
static vtss_rc jr2_cbc_find_slot(cbc_rtl_cfg_t      *rtl_cfg,
                                 cbc_req_port_cfg_t *port_cfg,
                                 vtss_calendar_t    *cbc,
                                 u32                 pos_ideal,
                                 u32                 pos_prv,
                                 u32                *pos_found)
{
    ...
    for (dist_from_ideal = 1; dist_from_ideal <= dist_from_ideal_max;
        ...
        // dist_from_ideal and pos_ideal + dist_from_ideal
        for (i = 0; i < 3; i++) {
            u32 pos;
    ...
    // If we get here, no position was found, but caller wants us to return
    // VTSS_RC_OK because we #accept_failure.
    *pos_found = -1;
    return VTSS_RC_OK;
}
#endif /* defined(VTSS_CALENDAR_CALC) */
...
#if defined(VTSS_CALENDAR_CALC)
static vtss_rc jr2_cbc_free_port_slots(cbc_rtl_cfg_t      *rtl_cfg,
                                       cbc_req_port_cfg_t *port_cfg,
                                       vtss_calendar_t    *cbc)
{
    int pos;
...
#if defined(VTSS_CALENDAR_CALC)
static vtss_rc jr2_cbc_assign_slots(cbc_rtl_cfg_t      *rtl_cfg,
                                    cbc_req_port_cfg_t *port_cfg,
                                    vtss_calendar_t    *cbc)
{
                                         ...
                                         u32             line_speed)
{
    ...
    switch (line_speed) {
    case 1000: return CBC_SPEED_1000;

    case 2500: return CBC_SPEED_2500;
...
#if defined(VTSS_CALENDAR_CALC)
static vtss_rc jr2_calendar_do_calc(cbc_rtl_cfg_t   *rtl_cfg,
                                    cbc_req_cfg_t   *req_cfg,
                                    vtss_calendar_t *cbc)
{
    typedef struct {
    ...
    cbc_grp_t           cbc_grps[CBC_IDLE + 1],
        *cbc_grp; // We can in principle have CBC_IDLE different B/Ws.
    int i, j;
    ...
    for (chip_port = 0; chip_port <= CBC_IDLE; chip_port++) {
        ...
        if (((bw * rtl_cfg->cbc_len) % rtl_cfg->max_bw) != 0) {
            VTSS_E(
                "Either CBC_LEN (%u) is not set to match MAX_BW (%u) or specified BW (%u) for port %u is not in steps of 250 Mbps",
                rtl_cfg->cbc_len, rtl_cfg->max_bw, bw, chip_port);
            return VTSS_RC_ERROR;
        }
        ...
        case CBC_PORT_TYPE_ETH:
            ...
            case 1000: port_cnt_1g++; break;

            case 2500: port_cnt_2g5++; break;
        ...
        case CBC_PORT_TYPE_VD: port_cnt_vd++; break;

        case CBC_PORT_TYPE_CPU: port_cnt_cpu++; break;
    ...
    for (i = 0; i < VTSS_ARRSZ(cbc_grps); i++) {
        ...
        // Within group, further group ports according to which Taxi ring they
        // belong to
        p = cbc_grp->port_list;
        while (p) {
            // Just insert by taxi-num, so that we get the same behavior as the
            // offline tool
...
#if defined(VTSS_CALENDAR_CALC)
static vtss_rc jr2_calendar_check(cbc_rtl_cfg_t   *rtl_cfg,
                                  cbc_req_cfg_t   *req_cfg,
                                  vtss_calendar_t *cbc)
{
    ...
    for (chip_port = 0; chip_port < CBC_IDLE /* exclude IDLE */; chip_port++) {
        ...
        // u32 or u64? #bytes_mul_100 is at most 100000, so #bits <= 800000.
...
static vtss_rc jr2_calendar_calc_and_apply(vtss_state_t *vtss_state)
{
    ...
    // in the calculations, because doing so could cause the calendar
...
#define INIT_CNT (sizeof(ram_init_list) / sizeof(ram_init_list[0]))
    ...
    if (!vtss_state->init_conf.skip_switch_reset) {
        ...
        JR2_WR(VTSS_ICPU_CFG_CPU_SYSTEM_CTRL_RESET,
               VTSS_M_ICPU_CFG_CPU_SYSTEM_CTRL_RESET_CORE_RST_PROTECT);
    ...
    if (!(value & VTSS_M_QSYS_SYSTEM_RESET_CFG_CORE_ENA)) {
        for (i = 0; i < 10; i++) {
            ...
            for (j = 0; j < INIT_CNT; j++) {
                r = ram_init_list[j];
                if (r.gazwrap)
                    r.init_val = VTSS_M_QSYS_RAM_CTRL_RAM_INIT_RAM_INIT |
                                 VTSS_M_QSYS_RAM_CTRL_RAM_INIT_RAM_ENA;
