...
static BOOL check_tupe_value(vtss_state_t *vtss_state, vtss_tupe_val_t val)
{
    u32 i = 0;
    ...
    if (cmd == VTSS_TUPE_CMD_QUERY) { // Query if TUPE is ready
        ...
        if (use_bits == 0) {
            ...
            if (use_comb) {
                ...
                if (mask) {
                    // must use portmask bit, look through
                          ...
                          vtss_tupe_val_t      val)
{
    ...
    if (type == VTSS_TUPE_TYPE_BITS) {
        if (check_bit(val / TUPE_VALS_MAX, &i) && i < TUPE_BITS_MAX &&
            ...
            JR2_WR(VTSS_ANA_L3_VLAN_TUPE_CTRL(addr),
                   VTSS_F_ANA_L3_VLAN_TUPE_CTRL_TUPE_CTRL(val & 0xffff));
            VTSS_D("Exit ok");
            return VTSS_RC_OK;
        }
    } else if (type == VTSS_TUPE_TYPE_VALUE) {
                          ...
                          vtss_tupe_val_t      *val)
{
    ...
    if (v < TUPE_VALS_MAX) {
        ...
        *type = VTSS_TUPE_TYPE_BITS;
        ...
        if (v) {
            if (check_bit(v / TUPE_VALS_MAX, &i) && i < TUPE_BITS_MAX) {
                *val = v;
                VTSS_D("Exit ok");
                return VTSS_RC_OK;
            }
                          ...
                          u32           addr) // VLAN/VSI address
{
    ...
    JR2_WR(VTSS_ANA_L3_VLAN_TUPE_CTRL(addr),
           0); // 0 is reserved as 'no match value'
    VTSS_D("Exit ok");
    return VTSS_RC_OK;
}
...
exit_revert:
    ...
    for (i = 0; i <= max_addr; ++i) {
        ...
        if (tupe_type == VTSS_TUPE_TYPE_VALUE) {
            ...
            if (check_bit(tupe_val / TUPE_VALS_MAX, &k) && k < TUPE_BITS_MAX) {
...
vtss_rc jr2_tupe_init(vtss_state_t *vtss_state, u8 tupe_linear_prot_bits)
{
    ...
    // accesses and normal TTI processing. JR2_WR(VTSS_AFI_TTI_MISC_TTI_CTRL2,
    // VTSS_F_AFI_TTI_MISC_TTI_CTRL2_TTI_TUPE_RSV(10));
    VTSS_D("Exit ok");
    return VTSS_RC_OK;
}
...
vtss_rc jr2_tupe_test(vtss_state_t *vtss_state)
{
    ...
    for (loop = 0; loop < 10 && ok; ++loop) {
        ...
        for (i = 0; i < max_addr; ++i) {
            if ((rand() % 1024) < 700) {
                if (jr2_tupe_vlan_set(vtss_state, i,
                                      v < TUPE_VALS_MAX ? VTSS_TUPE_TYPE_VALUE
                                                        : VTSS_TUPE_TYPE_BITS,
                                      v) != VTSS_RC_OK) {
...
vtss_rc jr2_tupe_realloc_test(vtss_state_t *vtss_state)
{
    ...
    for (loop = 0; loop < 10 && ok; ++loop) {
        ...
        for (i = 0; i < ((TUPE_VALS_MAX / 2) - 1); ++i) {
            vtss_state->tupe.tupe_vals_next =
                rand() % TUPE_VALS_MAX; // trick to alloc random values
            if (vtss_state->tupe.tupe_vals_next == 0) {
                vtss_state->tupe.tupe_vals_next = 1; // 0 is reserved
            }
            if (jr2_tupe_alloc(vtss_state, VTSS_TUPE_TYPE_VALUE, &tupe_vals[i]) != VTSS_RC_OK) {
