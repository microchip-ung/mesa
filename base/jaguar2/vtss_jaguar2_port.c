                                                ...
                                                *const control)
{
    ...
    control->enable =
        VTSS_BOOL(VTSS_X_DEV1G_PCS1G_CFG_STATUS_PCS1G_ANEG_CFG_ANEG_ENA(value));
    value =
        VTSS_X_DEV1G_PCS1G_CFG_STATUS_PCS1G_ANEG_CFG_ADV_ABILITY(control->enable
                                                                     ? value
                                                                     : 0);
                                            ...
                                            const vtss_port_no_t port_no)
{
    ...
    VTSS_RC(vtss_cmn_port_clause_37_adv_set(&value, &control->advertisement,
                                            control->enable));
    ...
    if (!control->enable) {
        /* Disable Aneg */
        JR2_WR(
            VTSS_DEV1G_PCS1G_CFG_STATUS_PCS1G_ANEG_CFG(tgt),
...

{
    ...
    status->link =
        (JR2_BF(DEV1G_PCS1G_CFG_STATUS_PCS1G_STICKY_LINK_DOWN_STICKY, value)
             ? 0
             : 1);
    if (status->link == 0) {
        ...
        status->link =
            JR2_BF(DEV1G_PCS1G_CFG_STATUS_PCS1G_LINK_STATUS_LINK_STATUS,
                   value) &&
        ...
        if (status->autoneg.complete) {
            if (((value >> 21) & 0x1) == 0) {
                ...
                JR2_WRM_SET(VTSS_DEV1G_PCS1G_CFG_STATUS_PCS1G_CFG(tgt),
                            VTSS_M_DEV1G_PCS1G_CFG_STATUS_PCS1G_CFG_PCS_ENA);
                (void)vtss_cil_port_clause_37_control_set(vtss_state,
                                                          port_no); /* Restart
                                                                       Aneg */
                VTSS_MSLEEP(50);
                JR2_RD(VTSS_DEV1G_PCS1G_CFG_STATUS_PCS1G_ANEG_STATUS(tgt),
                       &value);
                status->autoneg.complete =
                    JR2_BF(DEV1G_PCS1G_CFG_STATUS_PCS1G_ANEG_STATUS_ANEG_COMPLETE,
                           value);
            }
        }
    }
    ...
    value =
        VTSS_X_DEV1G_PCS1G_CFG_STATUS_PCS1G_ANEG_STATUS_LP_ADV_ABILITY(value);

    if (vtss_state->port.conf[port_no].if_type ==
        VTSS_PORT_INTERFACE_SGMII_CISCO) {
        ...
        VTSS_RC(vtss_cmn_port_clause_37_adv_get(value,
                                                &status->autoneg.partner.cl37));
    }
...
static vtss_rc jr2_port_inst_get(vtss_state_t  *vtss_state,
                                 ...
                                 u32           *serdes_type)
{
    ...
    if ((serdes_inst == NULL) && (serdes_type == NULL) &&
    ...
    if (vtss_state->port.conf[port_no].if_type == VTSS_PORT_INTERFACE_QSGMII ||
        ...
        if (port < 8) {
            ...
            if (vtss_state->port.conf[port_no].if_type ==
                ...
                t1 = VTSS_TO_DEV10G_1;
                t = (port - VTSS_PORT_10G_START);
            } else if (vtss_state->port.conf[port_no].if_type ==
                           VTSS_PORT_INTERFACE_XAUI ||
                       vtss_state->port.conf[port_no].if_type ==
                           VTSS_PORT_INTERFACE_RXAUI) {
                // XAUI/RXAUI
                st = JR2_SERDES_TYPE_6G;
                si = (port == 49)   ? 0xF0000
                     : (port == 50) ? 0xF00000
...
#define RCVRD_CLK_GPIO_NO                                                      \
    33 // on servalt the 4 recovered clock outputs are GPIO 33-36
vtss_rc vtss_cil_synce_clock_out_set(vtss_state_t               *vtss_state,
                                     const vtss_synce_clk_port_t clk_port_par)
{
    ...
    VTSS_D("clk_port %X  enable %u\n", clk_port,
           vtss_state->synce.out_conf[clk_port].enable);
    ...
    rcvrd_clk_src_idx =
        clk_port + 4; // In serval T with internal DPLL, recovered clock 4-7 are
                      // used as clock inputs to the internal EEC Controller(s)
#else
    rcvrd_clk_src_idx =
        clk_port + 1; // In serval T with external DPLL, recovered clock 1-3 are
                      // used as clock inputs to the external DPLL
#endif
    VTSS_D("divider %d, div_mask %d, rcvrd_clk_src_idx %d\n",
           vtss_state->synce.out_conf[clk_port].divider, div_mask,
           rcvrd_clk_src_idx);
...
#if !defined(VTSS_FEATURE_CLOCK)
    if (VTSS_RC_OK != vtss_jr2_gpio_mode(vtss_state, 0,
                                         RCVRD_CLK_GPIO_NO + rcvrd_clk_src_idx,
                                         VTSS_GPIO_ALT_0)) {
    ...
    JR2_WRM(VTSS_HSIO_SYNC_ETH_CFG_SYNC_ETH_CFG(clk_port),
            VTSS_F_HSIO_SYNC_ETH_CFG_SYNC_ETH_CFG_SEL_RECO_CLK_DIV(div_mask) |
                VTSS_F_HSIO_SYNC_ETH_CFG_SYNC_ETH_CFG_RECO_CLK_ENA(
                    vtss_state->synce.out_conf[clk_port].enable),
...
vtss_rc vtss_cil_synce_clock_in_set(vtss_state_t               *vtss_state,
                                    const vtss_synce_clk_port_t clk_port_par)
{
    vtss_synce_clk_port_t  clk_port = clk_port_par;
    ...
    if (port_type == VTSS_SYNCE_CLOCK_INTERFACE) {
        if (new_chip_port <= 1) {
            ...
            VTSS_E("SyncE not supported for port_no %u, chip port %u",
                   conf->port_no, new_chip_port);
            return VTSS_RC_ERROR;
        }
    ...
    if (!ena) {
        // This is a work around because in ServalT the
        // SYNC_ETH_CFG_RCVRD_CLK_ENA bit does not work, i.e  it is not possible
        // to tristate the recovered clock output.
        clk_src = 23; // port is currently not used, use this to force signal
...
#endif // VTSS_ARCH_SERVAL_T
    VTSS_D(
        "clk_port %X, port_no %u, chip port %u enable %u, squelsh %u, clk_src %d\n",
        clk_port, conf->port_no, new_chip_port, ena, conf->squelsh, clk_src);
#if defined(VTSS_ARCH_SERVAL_T)
    /* In Serval-T recovered clock 4-7 are used as clock inputs to the internal
     * EEC Controller(s) */
    JR2_WRM(VTSS_HSIO_SYNC_ETH_CFG_SYNC_ETH_CFG(rcvrd_clk_src_idx),
    ...
    if (ena) {
         ...
         * the same) input port */
        if (VTSS_RC_OK == jr2_port_inst_get(vtss_state, conf->port_no, &tgt,
                                            &serdes_instance, &serdes_type)) {
            ...
            if (serdes_type == JR2_SERDES_TYPE_1G) {
                VTSS_RC(jr2_sd1g_read(vtss_state,
                                      mask)); /* Readback the 1G common config
                                                 register */
                JR2_WRM(
                    VTSS_HSIO_SERDES1G_ANA_CFG_SERDES1G_COMMON_CFG,
                    VTSS_F_HSIO_SERDES1G_ANA_CFG_SERDES1G_COMMON_CFG_SE_AUTO_SQUELCH_ENA(
                        conf->squelsh),
                    VTSS_M_HSIO_SERDES1G_ANA_CFG_SERDES1G_COMMON_CFG_SE_AUTO_SQUELCH_ENA);
                VTSS_RC(jr2_sd1g_write(vtss_state, mask)); /* transfer 1G common
                                                              config register */
            }
            if (serdes_type == JR2_SERDES_TYPE_6G) {
                VTSS_RC(jr2_sd6g_read(vtss_state,
                                      mask)); /* Readback the 1G common config
                                                 register */
                JR2_WRM(
                    VTSS_HSIO_SERDES6G_ANA_CFG_SERDES6G_COMMON_CFG,
                    VTSS_F_HSIO_SERDES6G_ANA_CFG_SERDES6G_COMMON_CFG_SE_AUTO_SQUELCH_ENA(
                        conf->squelsh),
                    VTSS_M_HSIO_SERDES6G_ANA_CFG_SERDES6G_COMMON_CFG_SE_AUTO_SQUELCH_ENA);
                VTSS_RC(jr2_sd6g_write(vtss_state, mask)); /* transfer 6G common
                                                              config register */
            }
            if (serdes_type == JR2_SERDES_TYPE_10G) {
#if defined(VTSS_ARCH_SERVAL_T)
                if (vtss_state->port.conf[conf->port_no].speed ==
                    VTSS_SPEED_1G) {
                    JR2_WRM(
                        VTSS_HSIO_SYNC_ETH_CFG_SYNC_ETH_SD10G_CFG(
                            serdes_instance),
                        VTSS_F_HSIO_SYNC_ETH_CFG_SYNC_ETH_SD10G_CFG_SD10G_RCVRD_CLK_DIV(
                            0) |
                            VTSS_F_HSIO_SYNC_ETH_CFG_SYNC_ETH_SD10G_CFG_SD10G_AUTO_SQUELCH_ENA(
                                conf->squelsh),
                ...
                if (vtss_state->port.conf[conf->port_no].speed ==
                    ...
                    JR2_WRM(
                        VTSS_HSIO_SYNC_ETH_CFG_SYNC_ETH_SD10G_CFG(
                            serdes_instance),
                        VTSS_F_HSIO_SYNC_ETH_CFG_SYNC_ETH_SD10G_CFG_SD10G_RECO_CLK_DIV(
                            0) |
                            VTSS_F_HSIO_SYNC_ETH_CFG_SYNC_ETH_SD10G_CFG_SD10G_AUTO_SQUELCH_ENA(
                                conf->squelsh),
...
#if defined(VTSS_ARCH_SERVAL_T)
    JR2_WRM(VTSS_HSIO_SYNC_ETH_CFG_SYNC_ETH_CFG(clk_port),
            VTSS_F_HSIO_SYNC_ETH_CFG_SYNC_ETH_CFG_SEL_RCVRD_CLK_SRC(
                17 + vtss_state->synce.station_clk_out_conf[clk_port]
                         .dpll_out_no) | // Select SD10G[dpll_out_no] TX clock
                VTSS_F_HSIO_SYNC_ETH_CFG_SYNC_ETH_CFG_SEL_RCVRD_CLK_DIV(div_mask) |
                VTSS_F_HSIO_SYNC_ETH_CFG_SYNC_ETH_CFG_RCVRD_CLK_ENA(
                    vtss_state->synce.station_clk_out_conf[clk_port].enable),
            VTSS_M_HSIO_SYNC_ETH_CFG_SYNC_ETH_CFG_SEL_RCVRD_CLK_SRC |
                VTSS_M_HSIO_SYNC_ETH_CFG_SYNC_ETH_CFG_SEL_RCVRD_CLK_DIV |
                VTSS_M_HSIO_SYNC_ETH_CFG_SYNC_ETH_CFG_RCVRD_CLK_ENA);
#else
    JR2_WRM(VTSS_HSIO_SYNC_ETH_CFG_SYNC_ETH_CFG(clk_port),
            VTSS_F_HSIO_SYNC_ETH_CFG_SYNC_ETH_CFG_SEL_RECO_CLK_SRC(
                17 +
                vtss_state->synce.station_clk_out_conf[clk_port].dpll_out_no) |
                VTSS_F_HSIO_SYNC_ETH_CFG_SYNC_ETH_CFG_SEL_RECO_CLK_DIV(div_mask) |
                VTSS_F_HSIO_SYNC_ETH_CFG_SYNC_ETH_CFG_RECO_CLK_ENA(
                    vtss_state->synce.station_clk_out_conf[clk_port].enable),
            VTSS_M_HSIO_SYNC_ETH_CFG_SYNC_ETH_CFG_SEL_RECO_CLK_SRC |
...
static vtss_rc vtss_jr2_vaui_lane_alignement(vtss_state_t *vtss_state,
                                             u32           p,
                                             BOOL          ena)
{
    ...
    if (VTSS_PORT_IS_10G(p)) {
        ...
        JR2_WRM(
            VTSS_VAUI_CHANNEL_VAUI_CHANNEL_CFG_VAUI_CHANNEL_CFG(
                p >= 51 ? VTSS_TO_VAUI0 : VTSS_TO_VAUI1, !(p % 2)),
            VTSS_F_VAUI_CHANNEL_VAUI_CHANNEL_CFG_VAUI_CHANNEL_CFG_LANE_SYNC_ENA(
                ena ? 0xf : 0),
...
static BOOL port_is_in_10g_mode(vtss_state_t  *vtss_state,
                                vtss_port_no_t port_no)
{
    if (vtss_state->port.conf[port_no].if_type == VTSS_PORT_INTERFACE_SFI ||
                                   ...
                                   vtss_port_no_t port_no)
{
    ...
    vtss_vlan_port_type_t vlan_type =
        vtss_state->l2.vlan_port_conf[port_no].port_type;
    u32 etype, port = VTSS_CHIP_PORT(port_no);

    /* S-ports and VLAN unaware ports both support 0x88a8 (in addition to
     * 0x8100) */
    etype = (vlan_type == VTSS_VLAN_PORT_TYPE_S_CUSTOM
    ...
    if (VTSS_PORT_IS_10G(port)) {
        ...
        JR2_WRM(VTSS_DEV10G_MAC_CFG_STATUS_MAC_TAGS_CFG(VTSS_TO_DEV(port), 0),
                VTSS_F_DEV10G_MAC_CFG_STATUS_MAC_TAGS_CFG_TAG_ID(etype) |
                    VTSS_F_DEV10G_MAC_CFG_STATUS_MAC_TAGS_CFG_TAG_ENA(
                        max_tags == VTSS_PORT_MAX_TAGS_NONE ? 0 : 1),
                VTSS_M_DEV10G_MAC_CFG_STATUS_MAC_TAGS_CFG_TAG_ID |
                    VTSS_M_DEV10G_MAC_CFG_STATUS_MAC_TAGS_CFG_TAG_ENA);

        JR2_WRM(VTSS_DEV10G_MAC_CFG_STATUS_MAC_NUM_TAGS_CFG(
                    VTSS_TO_DEV(port)), // Triple tags not currently supported
                VTSS_F_DEV10G_MAC_CFG_STATUS_MAC_NUM_TAGS_CFG_NUM_TAGS(
                    max_tags == VTSS_PORT_MAX_TAGS_ONE   ? 1
    ...
    JR2_WR(VTSS_DEV1G_MAC_CFG_STATUS_MAC_TAGS_CFG(VTSS_TO_DEV1G(port)),
           VTSS_F_DEV1G_MAC_CFG_STATUS_MAC_TAGS_CFG_TAG_ID(etype) |
               VTSS_F_DEV1G_MAC_CFG_STATUS_MAC_TAGS_CFG_PB_ENA(
                   max_tags == VTSS_PORT_MAX_TAGS_TWO
                       ? 1
                       : 0) | // Triple tags not currently supported
               VTSS_F_DEV1G_MAC_CFG_STATUS_MAC_TAGS_CFG_VLAN_AWR_ENA(
                   max_tags == VTSS_PORT_MAX_TAGS_NONE ? 0 : 1) |
               VTSS_F_DEV1G_MAC_CFG_STATUS_MAC_TAGS_CFG_VLAN_LEN_AWR_ENA(
                   max_tags == VTSS_PORT_MAX_TAGS_NONE ? 0 : 1));
                            ...
                            BOOL                   report_errors)
{
    ...
    for (n = 0;; n++) {
        ...
        if ((value &
             (VTSS_M_DEVCPU_GCB_MIIM_MII_STATUS_MIIM_STAT_PENDING_RD |
...
mmd_error:
    if (report_errors) {
        VTSS_E(
            "miim failed, cmd: %s, miim_addr: %u, addr: %u, miim_controller:%u, sof:%u",
...
vtss_rc vtss_cil_miim_read(vtss_state_t          *vtss_state,
                           ...
                           BOOL                   report_errors)
{
    return jr2_miim_cmd(vtss_state, PHY_CMD_READ, 1, miim_controller, miim_addr,
                        addr, value, report_errors);
...
vtss_rc vtss_cil_miim_write(vtss_state_t          *vtss_state,
                            ...
                            BOOL                   report_errors)
{
    return jr2_miim_cmd(vtss_state, PHY_CMD_WRITE, 1, miim_controller,
                        miim_addr, addr, &value, report_errors);
                          ...
                          BOOL                   report_errors)
{
    ...
    VTSS_RC(jr2_miim_cmd(vtss_state, PHY_CMD_ADDRESS, 0, miim_controller,
                         miim_addr, mmd, &addr, report_errors));
    if (jr2_miim_cmd(vtss_state, PHY_CMD_READ, 0, miim_controller, miim_addr,
                     mmd, value, 0) != VTSS_RC_OK) {
        if (report_errors) {
            VTSS_E(
                "jr2_mmd_read failed, miim_controller:%d miim_addr:%x, mmd:%x, addr:%x",
                              ...
                              BOOL                   report_errors)
{
    ...
    VTSS_RC(jr2_miim_cmd(vtss_state, PHY_CMD_ADDRESS, 0, miim_controller,
                         miim_addr, mmd, &addr, report_errors));
    while (count > 1) {
        VTSS_RC(jr2_miim_cmd(vtss_state, PHY_CMD_READ_INC, 0, miim_controller,
                             miim_addr, mmd, buf, report_errors));
        buf++;
        count--;
    }
    VTSS_RC(jr2_miim_cmd(vtss_state, PHY_CMD_READ, 0, miim_controller,
                         miim_addr, mmd, buf, report_errors));
    return VTSS_RC_OK;
}
                           ...
                           BOOL                   report_errors)
{
    VTSS_RC(jr2_miim_cmd(vtss_state, PHY_CMD_ADDRESS, 0, miim_controller,
                         miim_addr, mmd, &addr, report_errors));
    return jr2_miim_cmd(vtss_state, PHY_CMD_WRITE, 0, miim_controller,
                        miim_addr, mmd, &data, report_errors);
}

static BOOL vrfy_spd_iface(vtss_port_no_t        port_no,
                           ...
                           BOOL                 *port_10g)
{
    *port_10g = 0;
    switch (if_type) {
    case VTSS_PORT_INTERFACE_NO_CONNECTION: return TRUE;

    case VTSS_PORT_INTERFACE_SERDES:
        if (speed != VTSS_SPEED_1G && speed != VTSS_SPEED_2500M) {
            VTSS_E(
                "Serdes port interface only supports 1000M speed (port:%u speed:%d)",
                port_no, speed);
            return 0;
        }
        break;
    ...
    case VTSS_PORT_INTERFACE_SGMII_CISCO:
        if (speed != VTSS_SPEED_1G && speed != VTSS_SPEED_100M &&
            speed != VTSS_SPEED_10M) {
            VTSS_E(
                "SGMII/QSGMII port interface only supports 10/100/1000M speeds (port:%u)",
                port_no);
            return 0;
        }
        break;
    case VTSS_PORT_INTERFACE_SGMII_2G5:
        if (speed != VTSS_SPEED_1G && speed != VTSS_SPEED_100M &&
            speed != VTSS_SPEED_10M && speed != VTSS_SPEED_2500M) {
            VTSS_E(
                "SGMII/QSGMII port interface only supports 10/100/1000M/2.5G speeds (port:%u)",
                port_no);
            return 0;
        }
        break;
    case VTSS_PORT_INTERFACE_100FX:
        if (speed != VTSS_SPEED_100M) {
            VTSS_E("100FX port interface only supports 100M speed (port:%u)",
                   port_no);
            return 0;
        }
        break;
    ...
    case VTSS_PORT_INTERFACE_XAUI:
        if (speed != VTSS_SPEED_10G) {
            VTSS_E(
                "SFI/XAUI/RXAUI port interface only supports 10G speeds (port:%u)",
                port_no);
            return 0;
        }
        *port_10g = 1;
    ...
    case VTSS_PORT_INTERFACE_VAUI:
        if (speed != VTSS_SPEED_2500M) {
            VTSS_E("VAUI port interface only supports 2.5G speed (port:%u)",
                   port_no);
            return 0;
        }
        break;
    default: VTSS_E("Illegal interface type (%d)", if_type); return 0;
    }
    return 1;
}
...
static u16 wm_enc(u16 value)
{
    if (value > JR2_BUFFER_REFERENCE) {
        return VTSS_M_QRES_RES_CTRL_RES_CFG_WM_HIGH; /* Encode as all ones in
                                                        relevant bit width */
    }
...
{
    u32 wm_high;
    JR2_RD(VTSS_QRES_RES_CTRL_RES_CFG((queue + 496 + 2048)),
           &wm_high); /* Shared egress high watermark for queue - common for all
                         dpls */
    return wm_dec(wm_high) *
           JR2_BUFFER_CELL_SZ; /* Convert from 176 byte chunks to bytes */
}

static vtss_rc jr2_port_pfc(vtss_state_t     *vtss_state,
                            u32               port,
...
static vtss_rc jr2_port_fc_setup(vtss_state_t     *vtss_state,
                                 u32               port,
                                 vtss_port_conf_t *conf)
{
    u8 *smac = &conf->flow_control.smac.addr[0];
    u32 q, pause_start = 0x77F00, pause_stop = 0x77F00, rsrv_raw = 0x77F00,
           atop_tot = 0x77F00, sum_port, sum_cpu, val;
    BOOL pfc = 0, fc_gen = conf->flow_control.generate,
         fc_obey = conf->flow_control.obey;
    vtss_port_no_t port_no;
    ...
    for (q = 0; q < VTSS_PRIOS; q++) {
        if (conf->flow_control.pfc[q]) {
            ...
            if (fc_gen || fc_obey) {
                VTSS_E(
                    "802.3X FC and 802.1Qbb PFC cannot both be enabled, chip port %u",
                    port);
                return VTSS_RC_ERROR;
            }
        }
    ...
    if (pfc || fc_gen || fc_obey) {
        ...
        // Per port:
        JR2_RD(VTSS_QRES_RES_CTRL_RES_CFG(0 /* ingress */ + 512 /* per port */ +
                                          port),
               &sum_port);
        JR2_RD(VTSS_QRES_RES_CTRL_RES_CFG(2048 /* egress  */ +
                                          512 /* per port */ +
        ...
        // Per prio:
        for (q = 0; q < VTSS_PRIOS; q++) {
            JR2_RD(VTSS_QRES_RES_CTRL_RES_CFG(0 /* ingress */ +
                                              port * VTSS_PRIOS /* per prio */ +
                                              q),
                   &val);
            sum_port += val;

            JR2_RD(VTSS_QRES_RES_CTRL_RES_CFG(
                       2048 /* egress  */ +
                       VTSS_CHIP_PORT_CPU_0 * VTSS_PRIOS /* per prio */ + q),
                   &val);
            sum_cpu += val;
        }
        ...
        if (conf->max_frame_length > VTSS_MAX_FRAME_LENGTH_STANDARD) {
            rsrv_raw = pause_start + (3 * VTSS_MAX_FRAME_LENGTH_STANDARD) /
                                         JR2_BUFFER_CELL_SZ;
        } else {
    ...
    JR2_WR(VTSS_QSYS_PAUSE_CFG_PAUSE_CFG(port),
           VTSS_F_QSYS_PAUSE_CFG_PAUSE_CFG_PAUSE_START(wm_enc(pause_start)) |
               ...
               VTSS_F_QSYS_PAUSE_CFG_PAUSE_CFG_PAUSE_ENA(0)); // enabled after
                                                              // reset

    /* Set SMAC of Pause frame */
    JR2_WR(VTSS_DSM_CFG_MAC_ADDR_BASE_HIGH_CFG(port),
           (smac[0] << 16) | (smac[1] << 8) | smac[2]);
    JR2_WR(VTSS_DSM_CFG_MAC_ADDR_BASE_LOW_CFG(port),
           (smac[3] << 16) | (smac[4] << 8) | smac[5]);
...
static vtss_rc jr2_sd10g_xfi_mode(vtss_state_t      *vtss_state,
                                  ...
                                  vtss_port_no_t     port_no)
{
    ...
    if (mode == VTSS_SERDES_MODE_100FX || mode == VTSS_SERDES_MODE_SGMII ||
        mode == VTSS_SERDES_MODE_1000BaseX || mode == VTSS_SERDES_MODE_2G5) {
    ...
    JR2_WRM(VTSS_XFI_SHELL_XFI_CONTROL_XFI_MODE(tgt),
            VTSS_F_XFI_SHELL_XFI_CONTROL_XFI_MODE_TX_INVERT(conf->serdes
                                                                .tx_invert),
            VTSS_M_XFI_SHELL_XFI_CONTROL_XFI_MODE_TX_INVERT);

    JR2_WRM(VTSS_XFI_SHELL_XFI_CONTROL_XFI_MODE(tgt),
            VTSS_F_XFI_SHELL_XFI_CONTROL_XFI_MODE_RX_INVERT(conf->serdes
                                                                .rx_invert),
                              ...
                              vtss_serdes_mode_t   mode)
{
    ...
    VTSS_RC(jr2_port_inst_get(vtss_state, port_no, &tgt, &serdes_inst,
                              &serdes_type));

    VTSS_D(
        "Configure Serdes chip port %u, serdes mode:%d serdes inst:%d, serdes type:%d)",
        port, mode, serdes_inst, serdes_type);
    ...
    case JR2_SERDES_TYPE_6G:
        if ((mode != VTSS_SERDES_MODE_QSGMII) ||
            (mode == VTSS_SERDES_MODE_QSGMII && ((port % 4) == 0))) {
    ...
    case JR2_SERDES_TYPE_10G:
        ...
        if (mode == VTSS_SERDES_MODE_100FX) {
            JR2_WRM(
                VTSS_XFI_SHELL_XFI_CONTROL_XFI_MODE(VTSS_TO_10G_XFI_TGT(port)),
...
#if defined(VTSS_FEATURE_CLOCK)
        // whenever the serdes setup is changed, the tx clock
        // synchronizationmust be refreshed
        VTSS_RC(vtss_omega_clock_output_filter_bw_refresh(vtss_state,
                                                          serdes_inst));
...
static vtss_rc jr2_port_flush_poll(vtss_state_t       *vtss_state,
                                   vtss_phys_port_no_t port)
{
...
static vtss_rc jr2_port_flush(vtss_state_t        *vtss_state,
                              const vtss_port_no_t port_no,
                              BOOL                 is_10g)
{
    u32 port = VTSS_CHIP_PORT(port_no);
    ...
    JR2_WRM_CLR(VTSS_QSYS_PAUSE_CFG_PAUSE_CFG(port),
                VTSS_M_QSYS_PAUSE_CFG_PAUSE_CFG_PAUSE_ENA);
    ...
    if (is_10g) {
        ...
        /* Disable SFI PCS */
        JR2_WRM_CLR(
            VTSS_PCS_10GBASE_R_PCS_10GBR_CFG_PCS_CFG(VTSS_TO_10G_PCS_TGT(port)),
...
static vtss_rc srvlt_phy_config(vtss_state_t *vtss_state, u32 port, BOOL enable)
{
    ...
    JR2_WRM(VTSS_DEVCPU_GCB_PHY_PHY_CFG, enable ? VTSS_BIT(port) : 0,
            VTSS_BIT(port));

    // Change serdes to phy interrupt or vice versa in the ICPU DEV interrupt
    // vector
    JR2_RD(VTSS_ICPU_CFG_INTR_DEV_INTR_ENA, &intr);
    if (enable) { // Serdes to phy interrupt
        if (intr & VTSS_BIT(port)) {
            JR2_WRM(VTSS_ICPU_CFG_INTR_DEV_INTR_ENA, VTSS_BIT(port + 11),
                    VTSS_BIT(port) | VTSS_BIT(port + 11));
        }
    } else { // Phy to phy interrupt
        if (intr & VTSS_BIT(port + 11)) {
            JR2_WRM(VTSS_ICPU_CFG_INTR_DEV_INTR_ENA, VTSS_BIT(port),
                    VTSS_BIT(port) | VTSS_BIT(port + 11));
        }
    }
    return VTSS_RC_OK;
                                ...
                                vtss_port_kr_status_t *const status)
{
    ...
    JR2_RD(VTSS_PCS_10GBASE_R_PCS_10GBR_STATUS_PCS_STATUS(
               VTSS_TO_10G_PCS_TGT(port)),
           &pcs);
    ...
    if (an_sm == AN_GOOD ||           // State: AN_GOOD
        ...
        // Aneg state machine is waiting, store and clear the speed requests to
        // let aneg finish
        JR2_RD(VTSS_XFI_SHELL_XFI_CONTROL_KR_CONTROL(xfi), &sticky);
        JR2_WR(
            VTSS_XFI_SHELL_XFI_CONTROL_KR_CONTROL(xfi),
    ...
    if (an_sm == AN_ABIL_DET || (an_sm == AN_GOOD && !status->aneg.complete)) {
        ...
        if ((hist & 0xFF7B) ==
            0) { // mask out parallel detect and ability check
            // Restart aneg
            VTSS_I("Port:%d. Stuck in %s.  Restart Aneg", port_no,
                   an_sm == AN_ABIL_DET ? "ABIL_DET" : "AN_GOOD");
            JR2_WRM(VTSS_KR_DEV7_KR_7X0000_KR_7X0000(dev7),
                    VTSS_F_KR_DEV7_KR_7X0000_KR_7X0000_AN_RESTART(1),
                    VTSS_M_KR_DEV7_KR_7X0000_KR_7X0000_AN_RESTART);
            // Clear stickys
    ...
    if (status->fec.r_fec_enable) {
        ...
        JR2_RD(VTSS_PCS_10GBASE_R_KR_FEC_HA_STATUS_KR_FEC_UNCORRECTED(br),
               &fec);
    ...
    // FEC request
    status->aneg.request_fec_change =
        VTSS_X_XFI_SHELL_XFI_CONTROL_KR_CONTROL_KR_ANEG_FEC_STICKY(val);
    status->aneg.r_fec_enable =
        VTSS_X_XFI_SHELL_XFI_CONTROL_KR_CONTROL_KR_FEC_ENABLE(val);
    ...
    if (status->aneg.speed_req == VTSS_SPEED_UNDEFINED ||
        ...
        if (VTSS_X_XFI_SHELL_XFI_CONTROL_KR_CONTROL_KR_ANEG_TX1G_STICKY(val) ||
            ...
            JR2_WR(
                VTSS_XFI_SHELL_XFI_CONTROL_KR_CONTROL(xfi),
                VTSS_F_XFI_SHELL_XFI_CONTROL_KR_CONTROL_KR_ANEG_TX1G_STICKY(1) |
                    VTSS_F_XFI_SHELL_XFI_CONTROL_KR_CONTROL_KR_ANEG_TX1G_STICKY(
                        1));
        }
    }
    ...
    if (status->train.complete || (an_sm == AN_TRAIN)) {
        ...
        if (an_sm == AN_TRAIN) {
            VTSS_I("Port:%d. Stuck in training  Restart Aneg. AN sm:%d",
                   port_no, an_sm);
    ...
    if (!aneg_is_restarted && vtss_state->port.kr_fec[port_no].r_fec &&
        vtss_state->port.kr_conf[port_no].aneg.enable &&
        !VTSS_X_PCS_10GBASE_R_PCS_10GBR_STATUS_PCS_STATUS_RX_BLOCK_LOCK(pcs) &&
        status->aneg.lp_aneg_able) {
        // Workaround for FEC enable request
        VTSS_I("Port:%d. Lost block lock. Restart Aneg. AN sm:%d", port_no,
               an_sm);
        JR2_WRM(VTSS_KR_DEV7_KR_7X0000_KR_7X0000(dev7),
                VTSS_F_KR_DEV7_KR_7X0000_KR_7X0000_AN_RESTART(1),
                VTSS_M_KR_DEV7_KR_7X0000_KR_7X0000_AN_RESTART);
                                  ...
                                  const vtss_port_no_t port_no)
{
    ...
    if (aneg->aneg.enable) {
        ...
        JR2_WRM(VTSS_KR_DEV1_TR_CFG1_TR_CFG1(dev1), VTSS_BIT(10), VTSS_BIT(10));

        // Freeze page detect timer (disable parallel detect)
        JR2_WRM(VTSS_KR_DEV1_TR_CFG1_TR_CFG1(dev1),
                aneg->aneg.no_pd ? VTSS_BIT(6) : 0, VTSS_BIT(6));
    }
    ...
    JR2_WRM(VTSS_KR_DEV1_TR_CFG1_TR_CFG1(dev1), 0, VTSS_BIT(10));
...
static vtss_rc jr2_port_conf_1g_set(vtss_state_t        *vtss_state,
                                    const vtss_port_no_t port_no)
{
    vtss_port_conf_t  *conf = &vtss_state->port.conf[port_no];
    u32                port = VTSS_CHIP_PORT(port_no);
    ...
    // Find serdes mode
    switch (conf->if_type) {
    case VTSS_PORT_INTERFACE_NO_CONNECTION: disable = 1; break;
    ...
    case VTSS_PORT_INTERFACE_SGMII:
        ...
        break;
    case VTSS_PORT_INTERFACE_SGMII_2G5:
        serdes_mode = (speed == VTSS_SPEED_2500M ? VTSS_SERDES_MODE_2G5
    ...
    case VTSS_PORT_INTERFACE_QSGMII:
        ...
        sgmii = 1;
        JR2_WRM(VTSS_HSIO_HW_CFGSTAT_HW_CFG, VTSS_BIT(port / 4),
                VTSS_BIT(port / 4));
        if ((port % 4) == 0) {
            // BZ23738
            for (u32 p = port + 1; p <= port + 3; ++p)
                JR2_WRM_CLR(
                    VTSS_DEV1G_DEV_CFG_STATUS_DEV_RST_CTRL(VTSS_TO_DEV(p)),
    ...
    case VTSS_PORT_INTERFACE_SGMII_CISCO:
        if (vtss_state->port.serdes_mode[port_no] == VTSS_SERDES_MODE_QSGMII) {
            serdes_mode =
                VTSS_SERDES_MODE_QSGMII; // Do not change the Serdes mode
        } else {
    ...
    if (port < 2 && !fdx) {
        ...
        VTSS_E("Half-duplex not supported on internal ports (port %u)",
               port_no);
        return VTSS_RC_ERROR;
    }
    ...
    if (VTSS_PORT_IS_10G(port)) {
        ...
        JR2_WRM(VTSS_HSIO_HW_CFGSTAT_HW_CFG, VTSS_ENCODE_BITFIELD(3, bt_fld, 2),
    ...
    if (disable) {
#if defined(VTSS_ARCH_SERVAL_T)
        // In order to keep the Tx clock running, so that frames don't buffer up
        // on a disabled port, make sure to move the mux to pointing to Serdes
        // (which always has the Tx clock enabled) rather than the internal PHY
        // (which will get its Tx clock disabled during administrative
        // shutdown). Having the queue system operable during shutdown will
        // allow for Up-MEP counting and for allowing the AFI to remove frames
        // from a port that already has reached its AFI::FRM_OUT_MAX.
        ...
        /* The port is not powered down in CE builds, as frames still needs to
         * be forwarded to an up-mep */
        /* Instead the PCS is disabled, which forces link down and discards
         * frame forwarding */
        JR2_WR(
            VTSS_DEV1G_PCS1G_CFG_STATUS_PCS1G_ANEG_CFG(tgt),
            VTSS_F_DEV1G_PCS1G_CFG_STATUS_PCS1G_ANEG_CFG_ANEG_ENA(1) |
    ...
    JR2_WRM_CTL(VTSS_HSCH_HSCH_MISC_PORT_MODE(port), !fdx,
                VTSS_M_HSCH_HSCH_MISC_PORT_MODE_AGE_DIS);
    ...
    JR2_WRM(VTSS_DEV1G_MAC_CFG_STATUS_MAC_HDX_CFG(tgt),
            VTSS_F_DEV1G_MAC_CFG_STATUS_MAC_HDX_CFG_LATE_COL_POS(67) |
                VTSS_F_DEV1G_MAC_CFG_STATUS_MAC_HDX_CFG_RETRY_AFTER_EXC_COL_ENA(
                    conf->exc_col_cont),
    ...
    if (if_100fx) {
        ...
        /* 100FX PCS */
        JR2_WRM(VTSS_DEV1G_PCS_FX100_CONFIGURATION_PCS_FX100_CFG(tgt),
                VTSS_F_DEV1G_PCS_FX100_CONFIGURATION_PCS_FX100_CFG_PCS_ENA(
                    !disable) |
                    VTSS_F_DEV1G_PCS_FX100_CONFIGURATION_PCS_FX100_CFG_SD_SEL(
                        !conf->sd_internal) |
                    VTSS_F_DEV1G_PCS_FX100_CONFIGURATION_PCS_FX100_CFG_SD_POL(
                        conf->sd_active_high) |
        ...
        JR2_WR(
            VTSS_DEV1G_PCS1G_CFG_STATUS_PCS1G_MODE_CFG(tgt),
        ...
        JR2_WR(VTSS_DEV1G_PCS1G_CFG_STATUS_PCS1G_SD_CFG(tgt),
               VTSS_F_DEV1G_PCS1G_CFG_STATUS_PCS1G_SD_CFG_SD_POL(
                   conf->sd_active_high) |
                   VTSS_F_DEV1G_PCS1G_CFG_STATUS_PCS1G_SD_CFG_SD_ENA(
                       conf->sd_enable) |
                   VTSS_F_DEV1G_PCS1G_CFG_STATUS_PCS1G_SD_CFG_SD_SEL(
                       !conf->sd_internal));
    ...
    JR2_WRM(VTSS_DEV1G_MAC_CFG_STATUS_MAC_MAXLEN_CFG(tgt),
            VTSS_F_DEV1G_MAC_CFG_STATUS_MAC_MAXLEN_CFG_MAX_LEN(
    ...
    JR2_WRM(VTSS_DEV1G_MAC_CFG_STATUS_MAC_ADV_CHK_CFG(tgt),
            VTSS_F_DEV1G_MAC_CFG_STATUS_MAC_ADV_CHK_CFG_LEN_DROP_ENA(
                conf->frame_length_chk),
    ...
    JR2_WRM(VTSS_DSM_CFG_DEV_TX_STOP_WM_CFG(port),
            VTSS_F_DSM_CFG_DEV_TX_STOP_WM_CFG_DEV_TX_STOP_WM(
                (speed == VTSS_SPEED_10M || speed == VTSS_SPEED_100M) ? 1 : 0),
    ...
    if (!disable) {
         ...
         * reset */
        JR2_WR(VTSS_DEV1G_DEV_CFG_STATUS_DEV_RST_CTRL(tgt),
               VTSS_F_DEV1G_DEV_CFG_STATUS_DEV_RST_CTRL_SPEED_SEL(
                   speed == VTSS_SPEED_10M    ? 0
                   : speed == VTSS_SPEED_100M ? 1
                                              : 2));

        /* Core: This is a 1G/2G5 port (fwd_speed) and enable port for frame
         * transfer */
#if defined(VTSS_ARCH_JAGUAR_2_B)
        /* The arbiter must know the port speed */
        JR2_WRM(VTSS_QFWD_SYSTEM_SWITCH_PORT_MODE(port),
                VTSS_F_QFWD_SYSTEM_SWITCH_PORT_MODE_PORT_ENA(1) |
                    VTSS_F_QFWD_SYSTEM_SWITCH_PORT_MODE_FWD_SPEED(
                        speed == VTSS_SPEED_2500M ? 1 : 0),
                VTSS_M_QFWD_SYSTEM_SWITCH_PORT_MODE_PORT_ENA |
                    VTSS_M_QFWD_SYSTEM_SWITCH_PORT_MODE_FWD_SPEED);
#else
        JR2_WRM(VTSS_QFWD_SYSTEM_SWITCH_PORT_MODE(port),
                VTSS_F_QFWD_SYSTEM_SWITCH_PORT_MODE_PORT_ENA(1) |
                    VTSS_F_QFWD_SYSTEM_SWITCH_PORT_MODE_FWD_URGENCY(
                        speed == VTSS_SPEED_2500M
                            ? 41
                            : 104), //( 270000/6400 - 1) : (672000/6400 - 1)
                VTSS_M_QFWD_SYSTEM_SWITCH_PORT_MODE_PORT_ENA |
        ...
        if (conf->flow_control.generate) {
            JR2_WRM_SET(VTSS_QSYS_PAUSE_CFG_PAUSE_CFG(port),
                        VTSS_M_QSYS_PAUSE_CFG_PAUSE_CFG_PAUSE_ENA);
        }
    }
...
static vtss_serdes_mode_t map_media_to_serdes(vtss_sd10g_media_type_t media)
{
    ...
    default:                       return VTSS_SERDES_MODE_SFI_PR_NONE;
    }
}

static vtss_rc jr2_port_conf_10g_set(vtss_state_t        *vtss_state,
                                     const vtss_port_no_t port_no)
{
    vtss_port_conf_t *conf = &vtss_state->port.conf[port_no];
    u32               port = VTSS_CHIP_PORT(port_no);
    ...
    /* Verify speed */
    switch (speed) {
    case VTSS_SPEED_10G: break;
    default:             VTSS_E("Illegal speed: %d", speed); return VTSS_RC_ERROR;
    }
    ...
    case VTSS_PORT_INTERFACE_LOOPBACK:
        ...
        mode_10g = 3;
        break;
    default: VTSS_E("Illegal interface type"); return VTSS_RC_ERROR;
    }
    ...
    if (conf->power_down) {
        /* The port is not powered down in CE builds, as frames still needs to
         * be forwarded to an up-mep */
        /* Instead the Serdes is configured to send out idles which simulates
         * port down state */
    ...
    if (conf->if_type == VTSS_PORT_INTERFACE_XAUI) {
        ...
        JR2_WR(VTSS_DEV10G_PCS_XAUI_CONFIGURATION_PCS_XAUI_SD_CFG(tgt),
               VTSS_F_DEV10G_PCS_XAUI_CONFIGURATION_PCS_XAUI_SD_CFG_SD_POL(
                   conf->sd_active_high) |
                   VTSS_F_DEV10G_PCS_XAUI_CONFIGURATION_PCS_XAUI_SD_CFG_SD_SEL(
                       !conf->sd_internal) |
                   VTSS_F_DEV10G_PCS_XAUI_CONFIGURATION_PCS_XAUI_SD_CFG_SD_ENA(
                       conf->sd_enable));
        ...
        /* XAUI Lane flipping  */
        JR2_WRM(
            VTSS_DEV10G_PCS_XAUI_CONFIGURATION_PCS_XAUI_EXT_CFG(tgt),
            VTSS_F_DEV10G_PCS_XAUI_CONFIGURATION_PCS_XAUI_EXT_CFG_RX_FLIP_HMBUS(
                rx_flip) |
                VTSS_F_DEV10G_PCS_XAUI_CONFIGURATION_PCS_XAUI_EXT_CFG_TX_FLIP_HMBUS(
                    tx_flip),
        ...
        JR2_WRM(VTSS_DEV10G_PCS2X6G_CONFIGURATION_PCS2X6G_CFG(tgt),
                VTSS_F_DEV10G_PCS2X6G_CONFIGURATION_PCS2X6G_CFG_PCS_ENA(1) |
                    VTSS_F_DEV10G_PCS2X6G_CONFIGURATION_PCS2X6G_CFG_SD_POL(
                        conf->sd_active_high) |
                    VTSS_F_DEV10G_PCS2X6G_CONFIGURATION_PCS2X6G_CFG_SD_SEL(
                        !conf->sd_internal) |
                    VTSS_F_DEV10G_PCS2X6G_CONFIGURATION_PCS2X6G_CFG_SD_ENA(
                        conf->sd_enable),
                VTSS_M_DEV10G_PCS2X6G_CONFIGURATION_PCS2X6G_CFG_PCS_ENA |
        ...
        JR2_WR(VTSS_PCS_10GBASE_R_PCS_10GBR_CFG_PCS_SD_CFG(
                   VTSS_TO_10G_PCS_TGT(port)),
               VTSS_F_PCS_10GBASE_R_PCS_10GBR_CFG_PCS_SD_CFG_SD_POL(
                   conf->sd_active_high) |
                   VTSS_F_PCS_10GBASE_R_PCS_10GBR_CFG_PCS_SD_CFG_SD_SEL(
                       !conf->sd_internal) |
                   VTSS_F_PCS_10GBASE_R_PCS_10GBR_CFG_PCS_SD_CFG_SD_ENA(
                       conf->sd_enable));
    ...
    JR2_WRM(VTSS_DEV10G_MAC_CFG_STATUS_MAC_ADV_CHK_CFG(tgt),
            VTSS_F_DEV10G_MAC_CFG_STATUS_MAC_ADV_CHK_CFG_INR_ERR_ENA(
                conf->frame_length_chk),
    ...
    if (!conf->power_down) {
        ...
        JR2_WR(
            VTSS_DEV10G_DEV_CFG_STATUS_PFC_PAUSE_MODE_CTRL(tgt),
            VTSS_F_DEV10G_DEV_CFG_STATUS_PFC_PAUSE_MODE_CTRL_PFC_PAUSE_MODE_SELECT(
                val));
        ...
        JR2_WRM(
            VTSS_DEV10G_DEV_CFG_STATUS_DEV_RST_CTRL(tgt),
        ...
        /* Core: This is a 10G port (fwd_speed) and enable port for frame
         * transfer */
#if defined(VTSS_ARCH_JAGUAR_2_B)
        JR2_WRM(VTSS_QFWD_SYSTEM_SWITCH_PORT_MODE(port),
    ...
    vtss_state->port.current_mt[port_no] =
...
vtss_rc vtss_cil_port_conf_set(vtss_state_t        *vtss_state,
                               const vtss_port_no_t port_no)
{
    vtss_port_conf_t *conf = &vtss_state->port.conf[port_no];
    BOOL              port_10g;
    ...
    VTSS_I("port_no:%d (port:%d), shutdown:%d", port_no, port,
           conf->power_down);
    ...
    if (VTSS_PORT_IS_10G(port) && !conf->power_down) {
        ...
        if ((mode == 3) && port_10g) {
            VTSS_RC(jr2_port_flush(vtss_state, port_no,
                                   FALSE)); // Shutdown the 2G5 device
            (void)vtss_cil_port_counters_clear(vtss_state,
                                               port_no); // Clear counters
                                                         // before DEV change
        } else if ((mode != 3) && !port_10g) {
            VTSS_RC(jr2_port_flush(vtss_state, port_no,
                                   TRUE)); // Shutdown the 10G device
            (void)vtss_cil_port_counters_clear(vtss_state,
                                               port_no); // Clear counters
                                                         // before DEV change
        }
    ...
    VTSS_D("port_no:%d (port:%d, dev%s) if:%s, spd:%s/%s, shutdown:%d", port_no,
           port, VTSS_PORT_IS_10G(port) ? "10G" : "1G",
           vtss_port_if_txt(conf->if_type), vtss_port_spd_txt(conf->speed),
           conf->fdx ? "FDX" : "HDX", conf->power_down);
...
/* Status for SERDES and SGMII ports is handled elsewhere (through
 * autonegotiation) */
                                 ...
                                 vtss_port_status_t *const status)
{
    ...
    case VTSS_PORT_INTERFACE_100FX:
        ...
        status->link_down =
            VTSS_X_DEV1G_PCS_FX100_STATUS_PCS_FX100_STATUS_SYNC_LOST_STICKY(
                value) ||
            VTSS_X_DEV1G_PCS_FX100_STATUS_PCS_FX100_STATUS_FEF_FOUND_STICKY(
                value) ||
            VTSS_X_DEV1G_PCS_FX100_STATUS_PCS_FX100_STATUS_PCS_ERROR_STICKY(
                value) ||
            VTSS_X_DEV1G_PCS_FX100_STATUS_PCS_FX100_STATUS_FEF_STATUS(value);

        /* BZ#22236 - One of 10G port link with 100FX SFP goes down in J2-24 and
         * J2-48 after traffic is started When polling for link status several
         * sticky bits are checked.  If they are set then the link is considered
         * down and the port is removed from the forwarding mask. The
         * SSD_ERROR_STICKY (Stream Start Delimiter Error from FX100-PCS) was
         * getting asserted on a 100FX/10G port, but no frames are lost. The
         * logic owner (Alexander Kock) have looked at this but cannot explain
         * this, the frames should be dropped if the sticky is asserted.  As a
         * workaround we decided to remove this bit from the checking.
         * changeset: 23d0a0aefa70bf5a99f2cc071d6a6a25efb1bb85
         *
         * And then we found the workaround bring the new issue.
         * BZ#23095: Ports are blinking continuously when SFPs are inserted,
         * without any cable connected Therefore, update the workaround on 10G
         * port only.
         */
        if (!VTSS_PORT_IS_10G(VTSS_CHIP_PORT(port_no))) {
            status->link_down |=
                VTSS_X_DEV1G_PCS_FX100_STATUS_PCS_FX100_STATUS_SSD_ERROR_STICKY(
                    value)
                    ? 1
                    : 0;
        }
        ...
        status->link =
            VTSS_X_DEV1G_PCS_FX100_STATUS_PCS_FX100_STATUS_SYNC_STATUS(value) &&
            (!VTSS_X_DEV1G_PCS_FX100_STATUS_PCS_FX100_STATUS_SYNC_LOST_STICKY(
                 value) &&
             !VTSS_X_DEV1G_PCS_FX100_STATUS_PCS_FX100_STATUS_FEF_FOUND_STICKY(
                 value) &&
        ...
        if (!VTSS_PORT_IS_10G(VTSS_CHIP_PORT(port_no))) {
            status->link &=
                (VTSS_X_DEV1G_PCS_FX100_STATUS_PCS_FX100_STATUS_SYNC_STATUS(
                     value) &&
                 !VTSS_X_DEV1G_PCS_FX100_STATUS_PCS_FX100_STATUS_SSD_ERROR_STICKY(
                     value))
                    ? 1
    ...
    case VTSS_PORT_INTERFACE_VAUI:
        ...
        if (status->link_down) {
            ...
            JR2_WRM_SET(
                VTSS_DEV1G_PCS1G_CFG_STATUS_PCS1G_STICKY(tgt),
                VTSS_M_DEV1G_PCS1G_CFG_STATUS_PCS1G_STICKY_LINK_DOWN_STICKY);
        }
    ...
    case VTSS_PORT_INTERFACE_SFI:
        ...
        JR2_RD(VTSS_SD10G65_SD10G65_IB_SD10G65_IB_CFG10(
                   VTSS_TO_10G_SRD_TGT(VTSS_CHIP_PORT(port_no))),
               &sd);
        if (value != 2) {
            /* The link is or has been down. Clear the sticky bit */
            status->link_down = 1;
            JR2_WR(VTSS_DEV10G_MAC_CFG_STATUS_MAC_TX_MONITOR_STICKY(tgt),
                   0xFFFFFFFF);
            JR2_RD(VTSS_DEV10G_MAC_CFG_STATUS_MAC_TX_MONITOR_STICKY(tgt),
                   &value);
        }
        if (vtss_state->port.conf[port_no].if_type == VTSS_PORT_INTERFACE_SFI) {
                                      ...
                                      BOOL                        clr)
{
    ...
    if (port_no >= vtss_state->port_count) {
        ...
        JR2_CNT_1G(RX_OUT_OF_RANGE_LEN_ERR, i, &c->rx_out_of_range_len_err,
                   clr);
        ...
        JR2_CNT_10G(32BIT_RX_OUT_OF_RANGE_LEN_ERR, i,
                    &c->rx_out_of_range_len_err, clr);
    ...
    JR2_WR(VTSS_XQS_SYSTEM_STAT_CFG,
           VTSS_F_XQS_SYSTEM_STAT_CFG_STAT_VIEW(port));
#else
    JR2_WR(VTSS_QSYS_SYSTEM_STAT_CFG,
           VTSS_F_QSYS_SYSTEM_STAT_CFG_STAT_VIEW(port));
#endif /* VTSS_ARCH_JAGUAR_2_C */
#if defined(VTSS_ARCH_JAGUAR_2_B)
    addr = 0;
#else
    ...
    for (i = 0; i < VTSS_PRIOS; i++) {
        VTSS_RC(vtss_jr2_qsys_counter_update(vtss_state, &addr,
                                             &c->tx_green_drops[i], clr));
    }
    for (i = 0; i < VTSS_PRIOS; i++) {
        VTSS_RC(vtss_jr2_qsys_counter_update(vtss_state, &addr,
                                             &c->tx_yellow_drops[i], clr));
    }
    addr = 256;
    for (i = 0; i < VTSS_PRIOS; i++) {
        VTSS_RC(vtss_jr2_qsys_counter_update(vtss_state, &addr,
                                             &c->tx_green_class[i], clr));
    }
    for (i = 0; i < VTSS_PRIOS; i++) {
        VTSS_RC(vtss_jr2_qsys_counter_update(vtss_state, &addr,
                                             &c->tx_yellow_class[i], clr));
    }
    VTSS_RC(vtss_jr2_qsys_counter_update(vtss_state, &addr, &c->tx_queue_drops,
                                         clr));

    /* ANA_AC counters */
    JR2_CNT_ANA_AC(PORT_STAT_LSB_CNT(port, JR2_CNT_ANA_AC_PORT_FILTER),
                   &c->rx_local_drops, clr);
    JR2_CNT_ANA_AC(PORT_STAT_LSB_CNT(port, JR2_CNT_ANA_AC_PORT_POLICER_DROPS),
                   &c->rx_policer_drops, clr);
    for (i = 0; i < VTSS_PRIOS; i++) {
        JR2_CNT_ANA_AC(QUEUE_STAT_LSB_CNT(port * 8 + i,
                                          JR2_CNT_ANA_AC_QUEUE_PRIO),
                       &c->rx_class[i], clr);
    }
    ...
    for (i = 0; i < VTSS_PRIOS; i++) {
        ...
        prop->tx_prio[i] =
            (c->tx_yellow_class[i].value + c->tx_green_class[i].value);
    }
    ...
    rx_errors =
        (c->rx_crc_err.value + c->rx_undersize.value + c->rx_oversize.value +
         c->rx_symbol_err.value + c->rx_jabbers.value + c->rx_fragments.value);
    if (vtss_state->port.conf[port_no].frame_length_chk) {
        rx_errors +=
            (c->rx_in_range_len_err.value + c->rx_out_of_range_len_err.value);
    }
    ...
    for (i = 0; i < VTSS_PRIOS; i++) {
        rmon->tx_etherStatsDropEvents +=
            (c->tx_green_drops[i].value + c->tx_yellow_drops[i].value);
    }
    ...
    rmon->tx_etherStatsCollisions =
        (c->tx_multi_coll.value + c->tx_backoff1.value + c->tx_late_coll.value +
         c->tx_xcoll.value);
    ...
    if_group->ifOutErrors =
        (c->tx_late_coll.value + c->tx_csense.value + c->tx_xcoll.value);
...
static vtss_rc jr2_port_counters(vtss_state_t               *vtss_state,
                                 ...
                                 BOOL                        clear)
{
    return jr2_port_counters_chip(vtss_state, port_no,
                                  &vtss_state->port.counters[port_no]
                                       .counter.jr2,
                                  counters, clear);
}
...
vtss_rc vtss_cil_port_forward_set(vtss_state_t        *vtss_state,
                                  const vtss_port_no_t port_no)
{
    return VTSS_RC_OK;
}

vtss_rc vtss_cil_port_test_conf_set(vtss_state_t        *vtss_state,
                                    const vtss_port_no_t port_no)
{
    ...
    if (lb == VTSS_PORT_LB_DISABLED) {
        JR2_WRM(
            VTSS_DEV1G_PCS1G_CFG_STATUS_PCS1G_SD_CFG(tgt),
...
vtss_rc vtss_cil_port_ifh_set(vtss_state_t        *vtss_state,
                              const vtss_port_no_t port_no)
{
    u32              port = VTSS_CHIP_PORT(port_no);
    ...
    JR2_WRM(VTSS_ASM_CFG_PORT_CFG(port),
            VTSS_F_ASM_CFG_PORT_CFG_INJ_FORMAT_CFG(ifh->ena_inj_header ? 2
                                                                       : 0) |
...
vtss_rc vtss_jr2_wm_update(vtss_state_t *vtss_state)
{
    ...
    /* Update BUF_PRIO_SHR_I sharing watermarks, to disallow ingress side in
     * determining sharing state. WRED operates on egress calculations only */
    for (queue = VTSS_QUEUE_START; queue < VTSS_QUEUE_END; queue++) {
        if (enabled[queue]) {
            JR2_WR(VTSS_QRES_RES_CTRL_RES_CFG((queue + 496 + 0)),
                   0); /* WRED is enabled somewhere for this QoS class - set
                          watermark to zero */
        } else {
            JR2_WR(VTSS_QRES_RES_CTRL_RES_CFG((queue + 496 + 0)),
                   vtss_state->port
                       .buf_prio_shr[queue]); /* Restore initial value */
        }
    }
    return VTSS_RC_OK;
}
...
static vtss_rc jr2_port_buf_conf_set(vtss_state_t *vtss_state)
{
    int q;
    u32 port_no, port, dp;
    u32 buf_q_rsrv_i, buf_q_rsrv_e, ref_q_rsrv_i, ref_q_rsrv_e,
        buf_prio_shr_i[8], buf_prio_shr_e[8];
    u32 buf_p_rsrv_i, buf_p_rsrv_e, ref_p_rsrv_i, ref_p_rsrv_e, buf_col_shr_i,
        buf_col_shr_e, ref_col_shr_i, ref_col_shr_e;
    u32 buf_prio_rsrv, guaranteed, q_rsrv_mask, prio_mem;

    /* Mode: Strict priority sharing. Higher priorities have right to use all
     * shared before lower */
    ...
    buf_prio_rsrv =
        6000; /* Distance between priority stops in the shared memory */

    /* Reserved frame reference. Values are NUMBER of FRAMES */
    ref_q_rsrv_e =
        4; /* Number of frames that can be pending at each egress queue   */
    ref_q_rsrv_i =
        4; /* Number of frames that can be pending at each ingress queue  */
    ref_p_rsrv_e = 10; /* Number of frames that can be pending shared between
                          the QoS classes at egress */
    ref_p_rsrv_i = 10; /* Number of frames that can be pending shared between
                          the QoS classes at ingress */
    ...
    /* The memory is oversubsrcribed by BUF_OVERSUBSRIPTION_FACTOR (factor 100 =
     * no oversubsription) */
    ...
    if (((vtss_state->port_count + 1) * guaranteed * 100 /
         BUF_OVERSUBSRIPTION_FACTOR) > JR2_BUFFER_MEMORY) {
        VTSS_E("Memory is oversubscribed");
    }
    /* Find the total amount of shared memory incl. oversubscription */
    prio_mem = JR2_BUFFER_MEMORY - (vtss_state->port_count + 1) * guaranteed *
                                       100 / BUF_OVERSUBSRIPTION_FACTOR;
    ...
    for (port_no = 0; port_no <= vtss_state->port_count; port_no++) {
        ...
        for (q = 0; q < VTSS_PRIOS; q++) {
            if (q_rsrv_mask & (1 << q)) {
                JR2_WR(VTSS_QRES_RES_CTRL_RES_CFG(port * VTSS_PRIOS + q + 0),
                       wm_enc(buf_q_rsrv_i / JR2_BUFFER_CELL_SZ));
                JR2_WR(VTSS_QRES_RES_CTRL_RES_CFG(port * VTSS_PRIOS + q + 1024),
                       wm_enc(ref_q_rsrv_i));
                JR2_WR(VTSS_QRES_RES_CTRL_RES_CFG(port * VTSS_PRIOS + q + 2048),
                       wm_enc(buf_q_rsrv_e / JR2_BUFFER_CELL_SZ));
                JR2_WR(VTSS_QRES_RES_CTRL_RES_CFG(port * VTSS_PRIOS + q + 3072),
                       wm_enc(ref_q_rsrv_e));
            }
        }
        /* Reserved amount for PORT at index PORT + 512 */
        JR2_WR(VTSS_QRES_RES_CTRL_RES_CFG(port + 512 + 0),
               wm_enc(buf_p_rsrv_i / JR2_BUFFER_CELL_SZ));
        JR2_WR(VTSS_QRES_RES_CTRL_RES_CFG(port + 512 + 1024),
               wm_enc(ref_p_rsrv_i));
        JR2_WR(VTSS_QRES_RES_CTRL_RES_CFG(port + 512 + 2048),
               wm_enc(buf_p_rsrv_e / JR2_BUFFER_CELL_SZ));
        JR2_WR(VTSS_QRES_RES_CTRL_RES_CFG(port + 512 + 3072),
               wm_enc(ref_p_rsrv_e));
    }
    ...
    for (q = 0; q < VTSS_PRIOS; q++) {
        ...
        vtss_state->port.buf_prio_shr[q] =
            wm_enc(buf_prio_shr_i[q] / JR2_BUFFER_CELL_SZ);

        JR2_WR(VTSS_QRES_RES_CTRL_RES_CFG((q + 496 + 0)),
               wm_enc(buf_prio_shr_i[q] / JR2_BUFFER_CELL_SZ));
        JR2_WR(VTSS_QRES_RES_CTRL_RES_CFG((q + 496 + 2048)),
               wm_enc(buf_prio_shr_e[q] / JR2_BUFFER_CELL_SZ));
        /* Todo bug#20488:
           JR2_WR(VTSS_QRES_RES_CTRL_RES_CFG((q + 496 + 1024)),
           wm_enc(ref_prio_shr_i[q]); JR2_WR(VTSS_QRES_RES_CTRL_RES_CFG((q + 496
           + 3072)), wm_enc(ref_prio_shr_e[q]);
        */
    }

    /* Index 508: Maximum allowed use of the shared buffer for frames with DP=1
     * (yellow). */
    /* Index 509: Maximum allowed use of the shared buffer for frames with DP=2
     * (yellow). */
    /* Index 510: Maximum allowed use of the shared buffer for frames with DP=3
     * (yellow). */
    /* Index 511: Maximum allowed use of the shared buffer for frames with DP=0
     * (green). */
    for (dp = 0; dp < 4; dp++) {
        JR2_WR(VTSS_QRES_RES_CTRL_RES_CFG(dp + 508 + 0),
               wm_enc(buf_col_shr_i / JR2_BUFFER_CELL_SZ));
        JR2_WR(VTSS_QRES_RES_CTRL_RES_CFG(dp + 508 + 1024),
               wm_enc(ref_col_shr_i));
        JR2_WR(VTSS_QRES_RES_CTRL_RES_CFG(dp + 508 + 2048),
               wm_enc(buf_col_shr_e / JR2_BUFFER_CELL_SZ));
        JR2_WR(VTSS_QRES_RES_CTRL_RES_CFG(dp + 508 + 3072),
               wm_enc(ref_col_shr_e));
        if (dp == 2) {
            // Green watermark should match the highest priority watermark.
            // Otherwise the will be no strict shared space per qos level
...
#define QLIM_WM(total, fraction) (total * fraction / 100 / JR2_BUFFER_CELL_SZ)
...
static vtss_rc jr2_port_buf_qlim_set(vtss_state_t *vtss_state)
{
    ...
    for (res = 0; res < 2; res++) {
        for (dp = 0; dp < 4; dp++) {
            JR2_WR(VTSS_QRES_RES_CTRL_RES_CFG(dp + 508 + res * 1024),
                   VTSS_M_QRES_RES_CTRL_RES_CFG_WM_HIGH);
        }

        for (prio = 0; prio < 8; prio++) {
            JR2_WR(VTSS_QRES_RES_CTRL_RES_CFG(prio + 496 + res * 1024),
                   VTSS_M_QRES_RES_CTRL_RES_CFG_WM_HIGH);
        }
    }
    ...
    for (port = 0; port < VTSS_CHIP_PORTS; port++) {
        JR2_WR(VTSS_XQS_QLIMIT_PORT_QLIMIT_PORT_CFG(port),
               VTSS_M_XQS_QLIMIT_PORT_QLIMIT_PORT_CFG_QLIMIT_MAX_MODE_ENA);
    }
    ...
    for (port = 0; port < VTSS_CHIP_PORTS; port++) {
        JR2_WR(VTSS_XQS_QLIMIT_PORT_QLIMIT_DIS_CFG(port),
               VTSS_F_XQS_QLIMIT_PORT_QLIMIT_DIS_CFG_QLIMIT_IGR_DIS(0) |
                   VTSS_F_XQS_QLIMIT_PORT_QLIMIT_DIS_CFG_QLIMIT_EGR_DIS(
                       port == VTSS_CHIP_PORT_VD1));
    }

    // Set 5,70,90,95,100 % of memory size (or wm1) for qlim,qdiv,ctop,atop,top
    // watermarks Set same for yellow, but take half the memory size (or wm2) as
    // base
    for (col = 0; col < 2; col++) {
        ...
        JR2_WR(VTSS_XQS_QLIMIT_SHR_QLIMIT_SHR_QLIM_CFG(0, col),
               QLIM_WM(avail, 5));
        JR2_WR(VTSS_XQS_QLIMIT_SHR_QLIMIT_SHR_QDIV_CFG(0, col),
               QLIM_WM(avail, 70));
        JR2_WR(VTSS_XQS_QLIMIT_SHR_QLIMIT_SHR_CTOP_CFG(0, col),
               QLIM_WM(avail, 90));
        JR2_WR(VTSS_XQS_QLIMIT_SHR_QLIMIT_SHR_ATOP_CFG(0, col),
               QLIM_WM(avail, 95));
        JR2_WR(VTSS_XQS_QLIMIT_SHR_QLIMIT_SHR_TOP_CFG(0, col),
               QLIM_WM(avail, 100));
    }
                                 ...
                                 const vtss_debug_info_t *const info)
{
    ...
    for (shr_id = 0; shr_id < 1; shr_id++) {
        ...
        for (col = 0; col < 2; col++) {
            ...
            JR2_RD(VTSS_XQS_QLIMIT_SHR_QLIMIT_SHR_ATOP_CFG(shr_id, col),
                   &value);
            pr("Stop for active schedulers(*2).: %5u\n", value);
            JR2_RD(VTSS_XQS_QLIMIT_SHR_QLIMIT_SHR_CTOP_CFG(shr_id, col),
                   &value);
            pr("Stop for active consumers(*3)..: %5u\n", value);
            JR2_RD(VTSS_XQS_QLIMIT_SHR_QLIMIT_SHR_QLIM_CFG(shr_id, col),
                   &value);
            pr("Stop for growing consumers(*4).: %5u\n", value);
            JR2_RD(VTSS_XQS_QLIMIT_SHR_QLIMIT_SHR_QDIV_CFG(shr_id, col),
                   &value);
                                   ...
                                   vtss_port_no_t                 port_no)
{
    ...
    if (VTSS_PORT_IS_10G(port)) {
        ...
        for (p = VTSS_PORT_10G_START; p < VTSS_CHIP_PORTS; p++) {
            JR2_RD(VTSS_SD10G65_DIG_SD10G65_APC_APC_EQZ_CTRL_STATUS(
                       VTSS_TO_10G_APC_TGT(p)),
                   &value);
            u32 eqz_gain =
            ...
            JR2_RD(VTSS_SD10G65_DIG_SD10G65_APC_APC_EQZ_CTRL_STATUS(
                   ...
                   &value);
            u32 ld_lev =
                VTSS_X_SD10G65_DIG_SD10G65_APC_APC_EQZ_CTRL_STATUS_LD_LEV_ACTVAL(
                    value);
            JR2_RD(VTSS_SD10G65_DIG_SD10G65_APC_APC_EQZ_OFFS_CTRL(
                       VTSS_TO_10G_APC_TGT(p)),
                   &value);
            u32 eqz_offs =
                VTSS_X_SD10G65_DIG_SD10G65_APC_APC_EQZ_OFFS_CTRL_EQZ_OFFS_ACTVAL(
                    value);
            JR2_RD(VTSS_SD10G65_DIG_SD10G65_APC_APC_EQZ_AGC_CTRL(
                       VTSS_TO_10G_APC_TGT(p)),
                   &value);
            u32 eqz_agc =
                VTSS_X_SD10G65_DIG_SD10G65_APC_APC_EQZ_AGC_CTRL_EQZ_AGC_ACTVAL(
                    value);
            JR2_RD(VTSS_SD10G65_DIG_SD10G65_APC_APC_EQZ_C_CTRL(
                       VTSS_TO_10G_APC_TGT(p)),
                   &value);
            u32 eqz_c =
                VTSS_X_SD10G65_DIG_SD10G65_APC_APC_EQZ_C_CTRL_EQZ_C_ACTVAL(value);
            JR2_RD(VTSS_SD10G65_DIG_SD10G65_APC_APC_EQZ_L_CTRL(
                       VTSS_TO_10G_APC_TGT(p)),
                   &value);
            u32 eqz_l =
                VTSS_X_SD10G65_DIG_SD10G65_APC_APC_EQZ_L_CTRL_EQZ_L_ACTVAL(value);
            JR2_RD(VTSS_SD10G65_DIG_SD10G65_APC_APC_DFE1_CTRL(
                       VTSS_TO_10G_APC_TGT(p)),
                   &value);
            u32 dfe1 =
                VTSS_X_SD10G65_DIG_SD10G65_APC_APC_DFE1_CTRL_DFE1_ACTVAL(value);
            JR2_RD(VTSS_SD10G65_DIG_SD10G65_APC_APC_DFE2_CTRL(
                       VTSS_TO_10G_APC_TGT(p)),
                   &value);
            u32 dfe2 =
                VTSS_X_SD10G65_DIG_SD10G65_APC_APC_DFE2_CTRL_DFE2_ACTVAL(value);
            JR2_RD(VTSS_SD10G65_DIG_SD10G65_APC_APC_DFE3_CTRL(
                       VTSS_TO_10G_APC_TGT(p)),
                   &value);
            u32 dfe3 =
                VTSS_X_SD10G65_DIG_SD10G65_APC_APC_DFE3_CTRL_DFE3_ACTVAL(value);
            JR2_RD(VTSS_SD10G65_DIG_SD10G65_APC_APC_DFE4_CTRL(
                       VTSS_TO_10G_APC_TGT(p)),
                   &value);
            u32 dfe4 =
                VTSS_X_SD10G65_DIG_SD10G65_APC_APC_DFE4_CTRL_DFE4_ACTVAL(value);
            pr("%-7d %-7d %-7d %-7d %-7d %-7d %-7d %-7d %-7d %-7d %-7d %-7d\n",
               p, eqz_gain, eqz_gain_adj, ld_lev, eqz_offs, eqz_agc, eqz_c,
               eqz_l, dfe1, dfe2, dfe3, dfe4);
        }

        pr("\nLink status (MAC/SD/PCS):\n");
        pr("port          local_fault   remote_fault  idle_state    SD            rx_blk_lock   rx_hi_ber\n");
        for (p = VTSS_PORT_10G_START; p < VTSS_CHIP_PORTS; p++) {
            JR2_RD(VTSS_DEV10G_MAC_CFG_STATUS_MAC_TX_MONITOR_STICKY(
                       VTSS_TO_DEV(p)),
                   &value);
            JR2_RD(VTSS_SD10G65_SD10G65_IB_SD10G65_IB_CFG10(
                       VTSS_TO_10G_SRD_TGT(p)),
                   &sd);
            JR2_RD(VTSS_PCS_10GBASE_R_PCS_10GBR_STATUS_PCS_STATUS(
                       VTSS_TO_10G_PCS_TGT(p)),
                   &pcs);
            pr("%-13d %-13d %-13d %-13d %-13d %-13d %-13d\n", p,
               VTSS_X_DEV10G_MAC_CFG_STATUS_MAC_TX_MONITOR_STICKY_LOCAL_ERR_STATE_STICKY(
                   value),
               VTSS_X_DEV10G_MAC_CFG_STATUS_MAC_TX_MONITOR_STICKY_REMOTE_ERR_STATE_STICKY(
                   value),
               VTSS_X_DEV10G_MAC_CFG_STATUS_MAC_TX_MONITOR_STICKY_IDLE_STATE_STICKY(
                   value),
               VTSS_X_SD10G65_SD10G65_IB_SD10G65_IB_CFG10_IB_IA_SDET(sd),
               VTSS_X_PCS_10GBASE_R_PCS_10GBR_STATUS_PCS_STATUS_RX_BLOCK_LOCK(
                   pcs),
               VTSS_X_PCS_10GBASE_R_PCS_10GBR_STATUS_PCS_STATUS_RX_HI_BER(pcs));
            // Clear the stickies
            JR2_WR(VTSS_PCS_10GBASE_R_PCS_10GBR_STATUS_PCS_STATUS(
                       VTSS_TO_10G_PCS_TGT(p)),
                   0xFFFFFFFF);
        }
    }
...

{
    ...
    if (cal->manual) {
        ...
        pr("\nVD0: %u Mbps, VD1: %u Mbps, CPU0: %u Mbps, CPU1: %u Mbps, IDLE = %u Mbps\n\n",
           250 * port_bw[VTSS_CHIP_PORT_VD0], 250 * port_bw[VTSS_CHIP_PORT_VD1],
           250 * port_bw[VTSS_CHIP_PORT_CPU_0],
           250 * port_bw[VTSS_CHIP_PORT_CPU_1], 250 * port_bw[p_idle]);
    } else {
        pr("Auto calendar\n\n");
    }
                                       ...
                                       vtss_port_no_t                 port_no)
{
    ...
    if (info->full || info->action == 1 || info->action == 3) {
        ...
        for (i = 0; i < VTSS_PRIOS; i++) {
            ...
            vtss_jr2_debug_cnt(ss, rx_buf.s, tx_buf.s, &cnt.rx_class[i],
                               &cnt.tx_green_class[i]);
        }
        for (i = 0; i < VTSS_PRIOS; i++) {
            VTSS_FMT(tx_buf, "yellow_%u", i);
            vtss_jr2_debug_cnt(ss, NULL, tx_buf.s, NULL,
                               &cnt.tx_yellow_class[i]);
        }
        for (i = 0; i < VTSS_PRIOS; i++) {
            VTSS_FMT(tx_buf, "green_drops_%u", i);
            vtss_jr2_debug_cnt(ss, NULL, tx_buf.s, NULL,
                               &cnt.tx_green_drops[i]);
        }
        for (i = 0; i < VTSS_PRIOS; i++) {
            VTSS_FMT(tx_buf, "yellow_drops_%u", i);
            vtss_jr2_debug_cnt(ss, NULL, tx_buf.s, NULL,
                               &cnt.tx_yellow_drops[i]);
        }
    }
    pr("\n");
...
static char *jr2_chip_port_to_str(vtss_state_t       *vtss_state,
                                  ...
                                  char               *str)
{
    ...
    case VTSS_CHIP_PORT_VD0: VTSS_FMT(buf, "VD0"); break;

    case VTSS_CHIP_PORT_VD1: VTSS_FMT(buf, "VD1"); break;
                                 ...
                                 u32                 val)
{
    ...
    pr("%4u %-8s %7s %9d %4u %10u\n", idx, jr2_qsys_resource_to_str(resource),
                                 ...
                                 BOOL          res_stat_cur)
{
    ...
    for (resource = 0; resource < 4; resource++) {
        ...
        for (chip_port = 0; chip_port < VTSS_CHIP_PORTS_ALL; chip_port++) {
            ...
            for (prio = 0; prio < VTSS_PRIOS; prio++) {
                idx = port_base + prio;
                addr = res_stat_cur ? VTSS_QRES_RES_CTRL_RES_STAT_CUR(idx)
                                    : VTSS_QRES_RES_CTRL_RES_STAT(idx);
                JR2_RD(addr, &val);
                if (val) {
                                 ...
                                 vtss_port_no_t                 port_no)
{
    ...
    if (info->has_action && info->action == 5) {
        JR2_WRM(VTSS_KR_DEV1_TR_CFG1_TR_CFG1(dev1), VTSS_BIT(6), VTSS_BIT(6));
    ...
    JR2_RD(VTSS_DEV10G_MAC_CFG_STATUS_MAC_TX_MONITOR_STICKY(VTSS_TO_DEV(port)),
           &value);
    pr("\nLink status: %d:\n", (value == 2));
    pr("port          local_fault   remote_fault  idle_state    SD            rx_blk_lock   rx_hi_ber\n");
    JR2_RD(VTSS_SD10G65_SD10G65_IB_SD10G65_IB_CFG10(VTSS_TO_10G_SRD_TGT(port)),
           &sd);
    JR2_RD(VTSS_PCS_10GBASE_R_PCS_10GBR_STATUS_PCS_STATUS(
               VTSS_TO_10G_PCS_TGT(port)),
           &pcs);
...

{
    ...
    pr("FC Pause TOT_START     : %d bytes\n",
       wm_dec(VTSS_X_QSYS_PAUSE_CFG_PAUSE_TOT_CFG_PAUSE_TOT_START(value)) *
           JR2_BUFFER_CELL_SZ);
    pr("FC Pause TOT_STOP      : %d bytes\n",
       wm_dec(VTSS_X_QSYS_PAUSE_CFG_PAUSE_TOT_CFG_PAUSE_TOT_STOP(value)) *
           JR2_BUFFER_CELL_SZ);
    JR2_RD(VTSS_QSYS_PAUSE_CFG_ATOP_TOT_CFG, &value);
    pr("FC ATOP_TOT            : %d bytes\n",
       wm_dec(VTSS_X_QSYS_PAUSE_CFG_ATOP_TOT_CFG_ATOP_TOT(value)) *
           JR2_BUFFER_CELL_SZ);
    pr("\n");
    ...
    for (port_no = VTSS_PORT_NO_START; port_no < (vtss_state->port_count + 2);
        ...
        pr("FC Start    (Bytes) : %d\n",
           wm_dec(VTSS_X_QSYS_PAUSE_CFG_PAUSE_CFG_PAUSE_START(value)) *
               JR2_BUFFER_CELL_SZ);
        pr("FC Stop     (Bytes) : %d\n",
           wm_dec(VTSS_X_QSYS_PAUSE_CFG_PAUSE_CFG_PAUSE_STOP(value)) *
               JR2_BUFFER_CELL_SZ);
        ...
        /* Reserved amount for (PORT,PRIO) at index 8 * PORT + PRIO */
        for (q = 0; q < VTSS_PRIOS; q++) {
            JR2_RD(VTSS_QRES_RES_CTRL_RES_CFG((port * VTSS_PRIOS + q + 0)),
                   &q_val1[q]);
            JR2_RD(VTSS_QRES_RES_CTRL_RES_CFG((port * VTSS_PRIOS + q + 1024)),
                   &q_val2[q]);
            JR2_RD(VTSS_QRES_RES_CTRL_RES_CFG((port * VTSS_PRIOS + q + 2048)),
                   &q_val3[q]);
            JR2_RD(VTSS_QRES_RES_CTRL_RES_CFG((port * VTSS_PRIOS + q + 3072)),
                   &q_val4[q]);
            JR2_RD(VTSS_QRES_RES_CTRL_RES_CFG((port * VTSS_PRIOS + q + 4096)),
                   &q_val5[q]);
        }
    ...
    if (info->full) {
        ...
        for (group = 0; group < VTSS_WRED_GROUP_CNT; group++) {
            for (queue = VTSS_QUEUE_START; queue < VTSS_QUEUE_END; queue++) {
                u32 wm_high;
                JR2_RD(VTSS_QRES_RES_CTRL_RES_CFG((queue + 496 + 2048)),
                       &wm_high); /* Shared egress high watermark for queue -
                                     common for all dpls */
                for (dpl = 0; dpl < VTSS_WRED_DPL_CNT; dpl++) {
                    JR2_RD(VTSS_QRES_RES_WRED_WRED_PROFILE((24 * group) +
                                                           (8 * dpl) + queue),
                           &value);
                    u32 wm_red_low =
                        VTSS_X_QRES_RES_WRED_WRED_PROFILE_WM_RED_LOW(value);
                    u32 wm_red_high =
                        VTSS_X_QRES_RES_WRED_WRED_PROFILE_WM_RED_HIGH(value);
                    pr("%5d %5d %3d 0x%04x %7u 0x%04x %7u 0x%04x %7u\n", group,
                       queue, dpl + 1, wm_high,
                       wm_dec(wm_high) *
                           JR2_BUFFER_CELL_SZ, /* Convert from 176 byte chunks
                                                  to bytes */
                       wm_red_low,
                       wm_red_low *
                           2816, /* Convert from 2816 byte chunks to bytes */
                       wm_red_high,
                       wm_red_high *
                           2816); /* Convert from 2816 byte chunks to bytes */
                }
    ...
    pr("Burst capacity expected for one port (prio 7): %d bytes\n",
       wm_dec(c_val1[7]) * JR2_BUFFER_CELL_SZ +
           wm_dec(p_val1) * JR2_BUFFER_CELL_SZ +
           wm_dec(p_val3) * JR2_BUFFER_CELL_SZ);
...
#if defined(VTSS_ARCH_JAGUAR_2_C)
        JR2_WR(VTSS_XQS_SYSTEM_FWD_DROP_EVENTS(port),
               0xFFFFFFFF); // Clear stickies
#else
        JR2_WR(VTSS_QFWD_SYSTEM_FWD_DROP_EVENTS(port),
               0xFFFFFFFF); // Clear stickies
#endif /* VTSS_ARCH_JAGUAR_2_C */
    }
...
static vtss_rc jr2_init_ana(vtss_state_t *vtss_state)
{
    ...
    JR2_WRM(VTSS_ANA_AC_POL_POL_ALL_CFG_POL_ALL_CFG,
            VTSS_F_ANA_AC_POL_POL_ALL_CFG_POL_ALL_CFG_STORM_GAP_VALUE(
                20) | /* GAP_VALUE = 20 : Use line-rate measurement */
                VTSS_F_ANA_AC_POL_POL_ALL_CFG_POL_ALL_CFG_DP_TO_COLOR_MAP(
                    0xe) | /* Non-zero DPL means yellow statistics */
    ...
    JR2_WR(
        VTSS_ANA_AC_STAT_GLOBAL_CFG_PORT_STAT_GLOBAL_EVENT_MASK(
            JR2_CNT_ANA_AC_PORT_FILTER),
        VTSS_F_ANA_AC_STAT_GLOBAL_CFG_PORT_STAT_GLOBAL_EVENT_MASK_GLOBAL_EVENT_MASK(
            1 << 0));
    JR2_WR(
        VTSS_ANA_AC_STAT_GLOBAL_CFG_PORT_STAT_GLOBAL_EVENT_MASK(
            JR2_CNT_ANA_AC_PORT_POLICER_DROPS),
        VTSS_F_ANA_AC_STAT_GLOBAL_CFG_PORT_STAT_GLOBAL_EVENT_MASK_GLOBAL_EVENT_MASK(
            mask));
    for (port = 0; port < VTSS_CHIP_PORTS_ALL; port++) {
        JR2_WR(VTSS_ANA_AC_STAT_CNT_CFG_PORT_STAT_CFG(port,
                                                      JR2_CNT_ANA_AC_PORT_FILTER),
               VTSS_F_ANA_AC_STAT_CNT_CFG_PORT_STAT_CFG_CFG_PRIO_MASK(0xff) |
                   VTSS_F_ANA_AC_STAT_CNT_CFG_PORT_STAT_CFG_CFG_CNT_FRM_TYPE(1));
        JR2_WR(VTSS_ANA_AC_STAT_CNT_CFG_PORT_STAT_CFG(
                   port, JR2_CNT_ANA_AC_PORT_POLICER_DROPS),
               VTSS_F_ANA_AC_STAT_CNT_CFG_PORT_STAT_CFG_CFG_PRIO_MASK(0xff) |
                   VTSS_F_ANA_AC_STAT_CNT_CFG_PORT_STAT_CFG_CFG_CNT_FRM_TYPE(1));
    }
    ...
    JR2_WR(
        VTSS_ANA_AC_STAT_GLOBAL_CFG_QUEUE_GLOBAL_CNT_FRM_TYPE_CFG(
            JR2_CNT_ANA_AC_QUEUE_PRIO),
        VTSS_F_ANA_AC_STAT_GLOBAL_CFG_QUEUE_GLOBAL_CNT_FRM_TYPE_CFG_GLOBAL_CFG_CNT_FRM_TYPE(
            0));
    ...
    if (vlan_counters) {
        ...
        JR2_WRM_SET(VTSS_ANA_AC_PS_COMMON_MISC_CTRL,
    ...
    for (i = 0; i < 6; i++) {
        ...
        value = (1 << j);
        JR2_WR(
            VTSS_ANA_AC_STAT_GLOBAL_CFG_ISDX_STAT_GLOBAL_CFG(i),
            VTSS_F_ANA_AC_STAT_GLOBAL_CFG_ISDX_STAT_GLOBAL_CFG_GLOBAL_CFG_CNT_BYTE(
                i < 3 ? 1 : 0));
        JR2_WR(
            VTSS_ANA_AC_STAT_GLOBAL_CFG_ISDX_STAT_GLOBAL_EVENT_MASK(i),
            VTSS_F_ANA_AC_STAT_GLOBAL_CFG_ISDX_STAT_GLOBAL_EVENT_MASK_GLOBAL_EVENT_MASK(
                value));
    }
...
static vtss_rc jr2_port_init(vtss_state_t *vtss_state)
{
    ...
    // Bug in ServalT Rev. A chip. Sometime PLL is not running with correct
    // speed. That can be detected is reading of supervisor takes less than
    // 100ms.
    if (vtss_state->misc.chip_id.revision == 0) {
        ...
        while (1) {
            ...
            if (VTSS_X_DEVCPU_GCB_PHY_PHY_STAT_SUPERVISOR_COMPLETE(val) ||
                (msec > 200)) {
                if (msec < 100) {
                    VTSS_E(
                        "---------------------> PHYs NOT working, please power cycle the board (Waited %d ms for Phy supervisor) <--------------------------\n",
                        msec);
                }
                break;
            }
    ...
    for (port = VTSS_PORT_10G_START; port < VTSS_CHIP_PORTS; port++) {
        //        VTSS_RC(jr2_sd10g_cfg(vtss_state, VTSS_SERDES_MODE_DISABLE,
        //        port));
    }
#if defined(VTSS_ARCH_SERVAL_T)
    VTSS_RC(jr2_sd6g_cfg(vtss_state, VTSS_PORT_NO_START,
                         VTSS_SERDES_MODE_DISABLE,
                         0x3)); // 2 * 6G - From DS1185-Serval-T_port_conf.xlsx
    VTSS_RC(jr2_sd1g_cfg(vtss_state, VTSS_PORT_NO_START,
                         VTSS_SERDES_MODE_DISABLE,
                         0x1F)); // 5 * 1G - From DS1185-Serval-T_port_conf.xlsx
#else
    VTSS_RC(jr2_sd6g_cfg(vtss_state, VTSS_PORT_NO_START,
                         VTSS_SERDES_MODE_DISABLE, 0xFFFFFF));
    VTSS_RC(jr2_sd1g_cfg(vtss_state, VTSS_PORT_NO_START,
                         VTSS_SERDES_MODE_DISABLE, 0xFF));
#endif
...
#if defined(VTSS_ARCH_SERVAL_T)
    JR2_WR(VTSS_QFWD_SYSTEM_STAT_CNT_CFG,
           VTSS_F_QFWD_SYSTEM_STAT_CNT_CFG_DROP_COUNT_EGRESS(1));
#else
    JR2_WR(VTSS_XQS_SYSTEM_STAT_CNT_CFG,
           VTSS_F_XQS_SYSTEM_STAT_CNT_CFG_DROP_COUNT_EGRESS(1));
#endif
...
vtss_rc vtss_jr2_port_init(vtss_state_t *vtss_state, vtss_init_cmd_t cmd)
{
    ...
    case VTSS_INIT_CMD_INIT: VTSS_RC(jr2_port_init(vtss_state)); break;
