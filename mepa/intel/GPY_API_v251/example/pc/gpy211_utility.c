...
#if FCON_MSEC_TEST
static struct e160_macsec_rx_sc *e160_find_rx_sc_rtnl(struct e160_macsec_secy *secy, sci_t sci)
{
...
static struct e160_macsec_rx_sc *e160_del_rx_sc(struct e160_macsec_secy *secy, sci_t sci)
{
	struct e160_macsec_rx_sc *rx_sc, **rx_scp;
...
static void e160_free_rx_sc(struct e160_macsec_rx_sc *rx_sc)
{
	int i;
	struct macsec_ops *sec_ops = gsw_get_macsec_ops(INGRESS_DEVID);
	struct e160_macsec_rx_sa *sa;
	...
	for (i = 0; i < MACSEC_NUM_AN; i++) {
		...
		if (sa) {
			e160_remove_transrec(sec_ops, i);
			e160_remove_flowrec(sec_ops, i);
			sa->active = false;
			free(sa);
		}
	}
...
static struct e160_macsec_rx_sc *e160_create_rx_sc(sci_t sci)
{
	struct e160_macsec_rx_sc *rx_sc;
	struct e160_macsec_secy *secy;
	struct macsec_prv_data *pdata =
		MACSEC_PDATA(gsw_get_macsec_ops(INGRESS_DEVID));

	rx_sc = malloc(sizeof(*rx_sc));
...
void convert_sci_to_mac_addr(sci_t sci, u8 *mac_addr)
{
	...
	val_0 = (sci & 0x00000000FFFFFFFF);
	val_1 = ((sci & 0xFFFFFFFF00000000) >> 32);
#if 0
...
#else
	//printf("\nRaw mac %08x, %08x", val_0, val_1);
	mac_addr[0] = (val_0 & 0xff);
	mac_addr[1] = ((val_0 >> 8) & 0xff);
	mac_addr[2] = ((val_0 >> 16) & 0xff);
	mac_addr[3] = ((val_0 >> 24) & 0xff);
	mac_addr[4] = (val_1 & 0xff);
	mac_addr[5] = ((val_1 >> 8) & 0xff);
	//printf("\nConverted mac %02x:%02x:%02x:%02x:%02x:%02x", mac_addr[0], mac_addr[1], mac_addr[2], mac_addr[3], mac_addr[4], mac_addr[5]);
#endif
}
#endif
...
int gpy2xx_msec_main(int argc, char *argv[])
{
	...
	if (xstrncasecmp(argv[0], "gpy2xx_msec_init_ing_dev", strlen("gpy2xx_msec_init_ing_dev")) == 0) {
		...
		*/
	...
	} else if (xstrncasecmp(argv[0], "gpy2xx_msec_config_ing_sa_nm_expl", strlen("gpy2xx_msec_config_ing_sa_nm_expl")) == 0) {
		struct sa_nonmatch_fca nm_fca;
		memset(&nm_fca, 0, sizeof(struct sa_nonmatch_fca));
		...
		ret = gpy2xx_msec_config_ing_sa_nm_expl(&phy, &nm_fca);

		if (ret < 0) {
			printf("\nERROR: SA Non-Match Expl class action set failed.\n");
			return -1;
		}
	} else if (xstrncasecmp(argv[0], "gpy2xx_msec_config_egr_sa_nm_expl", strlen("gpy2xx_msec_config_egr_sa_nm_expl")) == 0) {
		struct sa_nonmatch_fca nm_fca;
		memset(&nm_fca, 0, sizeof(struct sa_nonmatch_fca));
		...
		ret = gpy2xx_msec_config_egr_sa_nm_expl(&phy, &nm_fca);
...
#endif
		printf("\nCC interrup status (raw) = 0x%08x", cc_stat.int_stat.raw_int_stat);
...
int gpy211_main(int argc, char *argv[])
{
	...
	if (xstrncasecmp(argv[0], "gpy2xx_init", strlen("gpy2xx_init")) == 0) {
		...
		cnt += scanParamArg(argc, argv, "sensingPeriod", sizeof(brightnessCfg.sensing_period), &brightnessCfg.sensing_period);
		cnt += scanParamArg(argc, argv, "dischargeSlots", sizeof(brightnessCfg.discharge_slots), &brightnessCfg.discharge_slots);
		...
		printf("\t%40s:\t0x%x\n", "sensingPeriod", brightnessCfg.sensing_period);
		printf("\t%40s:\t0x%x\n", "dischargeSlots", brightnessCfg.discharge_slots);
#if defined(EN_LED_TOP_CFG) && EN_LED_TOP_CFG
...
#ifdef SMDIO_FOR_PHY
		cnt += scanParamArg(argc, argv, "extIntId", sizeof(extInterrupt.id), &extInterrupt.id);
#endif
		cnt += scanParamArg(argc, argv, "gpioFlags", sizeof(extInterrupt.gpio_flags), &extInterrupt.gpio_flags);
...
#ifdef SMDIO_FOR_PHY
		cnt += scanParamArg(argc, argv, "activeLevel", sizeof(extInterrupt.op_char), &extInterrupt.op_char);
#endif
		cnt += scanParamArg(argc, argv, "imaskIstat", sizeof(extInterrupt.imask_istat), &extInterrupt.imask_istat);
		cnt += scanParamArg(argc, argv, "im2ImaskIstat", sizeof(extInterrupt.im2_imask_istat), &extInterrupt.im2_imask_istat);
		/*
				ret = gpy211_extin_clr(&phy, &extInterrupt);

				if (ret < 0)
					printf("\nERROR: Clear External Interrupt configuration failed.\n");
		*/
...
#ifdef SMDIO_FOR_PHY
		printf("\t%40s:\t0x%x\n", "External Interrupt Id", extInterrupt.id);
#endif
		printf("\t%40s:\t0x%x\n", "GPIO flags sent", extInterrupt.gpio_flags);
#ifdef SMDIO_FOR_PHY
		printf("\t%40s:\t0x%x\n", "Interrupt output characteristic", extInterrupt.op_char);
#endif
		printf("\t%40s:\t0x%x\n", "Interrupt status/mask", extInterrupt.imask_istat);
		printf("\t%40s:\t0x%x\n", "IM2-Interrupt status/mask", extInterrupt.im2_imask_istat);
#if defined(EN_GMAC_DEBUG_ACCESS) && EN_GMAC_DEBUG_ACCESS
...
//#endif
		...
		cnt += scanParamArg(argc, argv, "enTxOstCor", sizeof(data), &data);
		ptpCfg.ts_ostcor_en = data;
		...
		printf("\t%40s:\t0x%x\n", "enTxOstCor", ptpCfg.ptp_ctrl.ts_ostcor_en);
		printf("\t%40s:\t0x%I64x\n", "txOstCorTime", ptpCfg.ptp_ctrl.tx_ost_corr);

		printf("\t%40s:\t0x%x\n", "enAllPkts", ptpCfg.ts_ctrl.en_all_pkts);
		printf("\t%40s:\t0x%x\n", "enEvntMsg", ptpCfg.ts_ctrl.ts_evtmsg_en);
		printf("\t%40s:\t0x%x\n", "enMstrMsg", ptpCfg.ts_ctrl.ts_master_en);
		printf("\t%40s:\t0x%x\n", "snapType", ptpCfg.ts_ctrl.snap_type);
		printf("\t%40s:\t0x%x\n", "tsStatMode", ptpCfg.ts_ctrl.ts_stat_mode);
		printf("\t%40s:\t0x%x\n", "subNsecIncr", ptpCfg.ts_ctrl.snsec_inc);
		printf("\t%40s:\t0x%x\n", "subSecIncr", ptpCfg.ts_ctrl.ssec_inc);
		...
		cnt += scanParamArg(argc, argv, "enAllPkts", sizeof(data), &data);
		tsCfg.en_all_pkts = data;
		...
		cnt += scanParamArg(argc, argv, "tsStatMode", sizeof(data), &data);
		tsCfg.ts_stat_mode = data;
		cnt += scanParamArg(argc, argv, "subNsecIncr", sizeof(data), &data);
		tsCfg.snsec_inc = data;
		cnt += scanParamArg(argc, argv, "subSecIncr", sizeof(data), &data);
		tsCfg.ssec_inc = data;
		...
		cnt += scanParamArg(argc, argv, "syncEClk", sizeof(synceCfg.synce_refclk), &synceCfg.synce_refclk);
		cnt += scanParamArg(argc, argv, "MasterMode", sizeof(synceCfg.master_sel), &synceCfg.master_sel);
		cnt += scanParamArg(argc, argv, "DataRate", sizeof(synceCfg.data_rate), &synceCfg.data_rate);
		cnt += scanParamArg(argc, argv, "PGCSel", sizeof(synceCfg.gpc_sel), &synceCfg.gpc_sel);
		...
		printf("\t%40s:\t0x%x\n", "syncEClk", synceCfg.synce_refclk);
		printf("\t%40s:\t0x%x\n", "MasterMode", synceCfg.master_sel);
