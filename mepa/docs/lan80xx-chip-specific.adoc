...
=== LSC Pin Input config for PPS signal to PHY
   * `lan80xx_phy_ts_pps_input_confset()` -- Configures LSC Pin as PPS input Pin and selects the mode of LTC Save

API Configuration settings of `phy25g_pps_input_conf_t` are listed in following table:
[cols="1,1", options="header"]
|===
| Argument                | Description
| clk_select              | Clock for PPS signal sampling, Asyncronous will sample with internal LTC clock
| pin_inv_pol             | LSC Pin polarity configuration (Active High or Active Low)
| pin_sync_mode           | LTC Save mode (Immediate or one time on active edge or continously on active edge of PPS signal)
| lsc_select              | LSC Pin to be configured with above configurations
|===

The `clk_select` option allows you to specify when the 1PPS signal pin should be sampled to determine its current state.
...
By selecting "clk_sel" to a mode other than Asynchronous, the 1PPS signal will be sampled using the external synchronous clock provided by the user,
thereby improving sub-nanosecond accuracy.

=== LSC Pin Output configuration

   * `lan80xx_phy_ts_pps_ouput_conf_set()` -- Configures selected LSC PIN as output pin either to generate PPS signal or square wave

=== TS FIFO Signature Configuration

The API `lan80xx_phy_ts_fifo_sig_set()` allows to select the fields of the PTP/Header that needs
to be captured in TS FIFO Signature, this API is applicable only for LAN80XX PHYs which can support
28-bytes TS FIFO for PTP Signature

Following are the different fields that can be captured in TS FIFO.
...
The API `lan80xx_phy_ts_fifo_sig_get()` is used to get the signature fields enabled on the port.
