...
meba_poe_parameters_t tPoE_parameters = {
    ...
    .ePoE_Controller_Type_default = SPARX5_POE_PD692x0_CONTROLLER_TYPE_DEFAULT,
...
meba_poe_system_t sparx5_pd_system;
                                                 ...
                                                 meba_poe_init_params_t *tPoe_init_params)
{
    ...
    uint8_t poe_gen6_12c0 = sparx5_gen6_i2c_config[0].i2c_address;
    if (inst->poe_i2c_tags.poe_12c0 != 0) {
        poe_gen6_12c0 = inst->poe_i2c_tags.poe_12c0;
        inst->iface.debug(MEBA_TRACE_LVL_INFO, __FUNCTION__, __LINE__, "poe_gen6_12c0=%d",
                          poe_gen6_12c0);
    }

    uint8_t poe_gen6_12c1 = sparx5_gen6_i2c_config[1].i2c_address;
    if (inst->poe_i2c_tags.poe_12c1 != 0) {
        poe_gen6_12c1 = inst->poe_i2c_tags.poe_12c1;
        inst->iface.debug(MEBA_TRACE_LVL_INFO, __FUNCTION__, __LINE__, "poe_gen6_12c1=%d",
                          poe_gen6_12c1);
    }

    uint8_t poe_gen7_12c = sparx5_gen7_i2c_config[0].i2c_address;
    if (inst->poe_i2c_tags.poe_12c0 != 0) {
        poe_gen7_12c = inst->poe_i2c_tags.poe_12c0;
        inst->iface.debug(MEBA_TRACE_LVL_INFO, __FUNCTION__, __LINE__, "poe_gen7_12c=%d",
                          poe_gen7_12c);
    }

    inst->iface.debug(MEBA_TRACE_LVL_INFO, __FUNCTION__, __LINE__, "controller_count=%d",
                      SPARX5_POE_CONTROLLERS_COUNT + 1);

#if SPARX5_POE_CONTROLLERS_COUNT == MEBA_POE_TWO_CONTROLLERS
    inst->iface.debug(MEBA_TRACE_LVL_INFO, __FUNCTION__, __LINE__, "controller_count is 2");
    sparx5_pd_system.controller_count = 2;
#else
    inst->iface.debug(MEBA_TRACE_LVL_INFO, __FUNCTION__, __LINE__, "controller_count is 1");
    sparx5_pd_system.controller_count = 1;
#endif // SPARX5_POE_CONTROLLERS_COUNT

    inst->iface.debug(MEBA_TRACE_LVL_INFO, __FUNCTION__, __LINE__, "controller_count=%d",
                      sparx5_pd_system.controller_count);

    sparx5_pd_system.controllers =
        malloc(sizeof(meba_poe_ctrl_inst_t) * sparx5_pd_system.controller_count);
    ...
    if (tPoe_init_params->eMeba_poe_firmware_type == MEBA_POE_FIRMWARE_TYPE_GEN6_BT) {
        ...
        sparx5_pd_system.controllers[0].index = 0;
        meba_pd_bt_driver_init(&sparx5_pd_system.controllers[0], "pd69x00bt",
                               meba_pd_i2c_adapter_open(&sparx5_pd_system.controllers[0],
        ...
        sparx5_pd_system.controllers[1].index = 1;
        meba_pd_bt_driver_init(&sparx5_pd_system.controllers[1], "pd69x00bt-2",
                               meba_pd_i2c_adapter_open(&sparx5_pd_system.controllers[1],
        ...
        sparx5_pd_system.controllers[0].index = 0;
        meba_pd_bt_driver_init(&sparx5_pd_system.controllers[0], "pd77010",
                               meba_pd_i2c_adapter_open(&sparx5_pd_system.controllers[0],
        ...
        sparx5_pd_system.controllers[0].index = 0;
        meba_pd69200_driver_init(&sparx5_pd_system.controllers[0], "pd69x00at",
                                 meba_pd_i2c_adapter_open(&sparx5_pd_system.controllers[0],
                                                          sparx5_gen6_i2c_config[0].i2c_device,
                                                          poe_gen6_12c0),
                                 MEBA_POE_CTRL_CAP_POWER_MANAGEMENT |
        ...
        sparx5_pd_system.controllers[1].index = 1;
        meba_pd69200_driver_init(&sparx5_pd_system.controllers[1], "pd69x00at-2",
                                 meba_pd_i2c_adapter_open(&sparx5_pd_system.controllers[1],
                                                          sparx5_gen6_i2c_config[1].i2c_device,
                                                          poe_gen6_12c1),
                                 MEBA_POE_CTRL_CAP_POWER_MANAGEMENT |
                                                   ...
                                                   meba_poe_init_params_t *tPoe_init_params)
{
    ...
    uint8_t poe_gen6_12c0 = lan969_i2c_config[0].i2c_address;
    if (inst->poe_i2c_tags.poe_12c0 != 0) {
        poe_gen6_12c0 = inst->poe_i2c_tags.poe_12c0;
        inst->iface.debug(MEBA_TRACE_LVL_INFO, __FUNCTION__, __LINE__, "poe_gen6_12c0=%d",
                          poe_gen6_12c0);
    ...
    meba_pd_bt_driver_init(&sparx5_pd_system.controllers[0], "pd69x00bt",
                           meba_pd_i2c_adapter_open(&sparx5_pd_system.controllers[0],
                                                    lan969_i2c_config[0].i2c_device, poe_gen6_12c0),
                           MEBA_POE_CTRL_CAP_POWER_MANAGEMENT | MEBA_POE_CTRL_INTERRUPTIBLE_POWER |
                               MEBA_POE_CTRL_PD_AUTO_CLASS_REQUEST |
                               MEBA_POE_CTRL_LEGACY_PD_CLASS_MODE,
                           sparx5_pd69200_4pairs_port_map_1,
                           sizeof(sparx5_pd69200_4pairs_port_map_1) /
...
{
    for (int i = 0; i < sparx5_pd_system.controller_count; ++i) {
        for (int j = 0; j < sparx5_pd_system.controllers[i].port_map_length; ++j) {
            if (sparx5_pd_system.controllers[i].port_map[j].port_no == port_no) {
                *controller = &sparx5_pd_system.controllers[i];
                *handle = sparx5_pd_system.controllers[i].port_map[j].handle;
                return MESA_RC_OK;
            }
        }
    }
...
mesa_rc meba_poe_sparx5_do_detection(meba_inst_t inst)
{
    ...
    for (int i = 0; i < sparx5_pd_system.controller_count; ++i) {
        if (sparx5_pd_system.controllers[i]
                .api->meba_poe_ctrl_do_detection(&sparx5_pd_system.controllers[i]) ==
            MESA_RC_ERROR) {
