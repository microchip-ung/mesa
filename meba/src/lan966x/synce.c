...
/* When a port on phy device 100 is used for clk1, we want the signal to be
   routed to clk1_out on device 100 and from clk1_in to clk1_out on device 200.
   That restriction is not reflected in the above graph. It also allows the
   signal to be routed to clk2_out on device 100 and then from clk2_in to
   clk1_out on device 200. The mux setup is therefore not using the following
   graph, but is being hard coded in the application */
...
static mesa_rc meba_synce_mux_set(meba_inst_t inst, uint32_t dev_id, uint32_t input, uint32_t output)
{
    ...
    if ((dev_id == 200) && input != 0 && input != 1) {
        // Device 200 is configured to route a phy to the clk-out port. Thus
