...
#define DEBUG(_inst, _lvl, _fmt, ...)                                                              \
    do {                                                                                           \
        ((poe_driver_private_t *)((_inst)->private_data))                                          \
            ->debug(_lvl, __FILE__, __LINE__, "(%s) " _fmt, _inst->adapter_name, ##__VA_ARGS__);   \
    } while (0)
...
enum internal_poe_port_status_t {
    ...
    PREBT_0x4A_OFF_RECOVERY_OVL = 74, // # eOTHERFAULT      # During crash a recovery port
                                      // delivering power was disconnected due to OVL.
    PREBT_0x4B_OFF_RECOVERY_SC = 75, // # eOTHERFAULT      # During crash a recovery port delivering
...
typedef struct {
    ...
    uint8_t  port_type_operation_mode[MAX_POE_TYPES][4]; // columne0: pse_type and 0=Legacy,1=BT
                                                         // columne1: 0-BT ,1-legacy org ,2-poh
                                                         // ,3-Ignore-PD-Class
} Prod;

static const uint16_t poe_port_mode_max_power_w[] = {15, 30, 60,
                                                     90}; // System has 4 modes = 15/30/60/90
...
static mesa_rc pd69200_rd(const meba_poe_ctrl_inst_t *const inst, uint8_t *data, uint8_t size)
{
    char buf[size * 3 + 1];
...
static meba_poe_chip_state_t meba_poe_pd69200_get_chipset(const meba_poe_ctrl_inst_t *const inst)
{
...
mesa_rc meba_poe_pd69200_get_individual_mask(const meba_poe_ctrl_inst_t *const inst,
...
static mesa_rc meba_poe_pd69200_set_individual_mask(const meba_poe_ctrl_inst_t *const inst,
                                                    uint8_t                           mask,
                                                    uint8_t                           val)
{
...
static mesa_rc meba_poe_pd69200_set_power_banks(const meba_poe_ctrl_inst_t *const inst,
                                                uint8_t                           bank,
...
// Section 4.7.7
static mesa_rc pd69200_get_power_supply_parameters(const meba_poe_ctrl_inst_t *const inst,
                                                           ...
                                                           uint16_t *tppl)
{
    ...
    *tppl = (buf[4] << 8) + buf[5];
    ...
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG, "[%s] CH=%d ,ppl=%lu ,tppl=%lu", fname, channel, *ppl, *tppl);
                                                             ...
                                                             uint16_t *imain_current_ma)
{
    ...
    *imain_current_ma = (buf[7] << 8) + buf[8];
    ...
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG, "[%s] vmain_voltage_dv=%lu ,imain_current_ma=%lu [W]", fname,
...
static mesa_rc meba_poe_ctrl_pd69200_reset_command(const meba_poe_ctrl_inst_t *const inst)
{
...
static mesa_rc meba_poe_ctrl_pd69200_save_command(const meba_poe_ctrl_inst_t *const inst)
{
...
static mesa_rc meba_poe_pd69200_get_active_matrix(const meba_poe_ctrl_inst_t *const inst,
                                                  meba_poe_port_handle_t            handle,
                                                  uint8_t                          *phys_numb_a,
                                                  uint8_t                          *phys_numb_b)
{
...
static mesa_rc meba_poe_pd69200_set_temporary_matrix(const meba_poe_ctrl_inst_t *const inst,
                                                     meba_poe_port_handle_t            handle,
                                                     uint8_t                           phys_numb_a,
                                                     uint8_t                           phys_numb_b)
{
...
static mesa_rc pd69200_program_global_matrix(const meba_poe_ctrl_inst_t *const inst)
{
    // Transmit the command
    uint8_t buf[PD_BUFFER_SIZE] = {
                                         ...
                                         telemetry_at_boot_up_error_e *peTelemetry_at_boot_up_error)
{
    ...
    while ((iFF_byte_counter < 30) && (i00_byte_counter < 30)) /* 30 bytes with value 0 means that
                                                                  I2C driver has no data to send */
    {
            ...
            bRxMsg[0] == REPORT_KEY) /* first msg byte - telemetry or response */
        {
                 ...
                 (bRxMsg[0] == SYSTEM_STATUS_ECHO_KEY))) /* system status on startup */
            {
                                        ...
                                        // system status on startup or firmware damage...
                {
                    ...
                    return check_for_poe_firmware_errors(inst, rx_data,
                                                         peTelemetry_at_boot_up_error);
                }
 ...
 *succeed
 *---------------------------------------------------------------------*/
static mesa_rc meba_poe_pd69200_bt_event_cause_get(const meba_poe_ctrl_inst_t *const inst,
...
static mesa_rc meba_poe_pd69200_bt_get_bt_system_status(const meba_poe_ctrl_inst_t *const inst,
                                                        bt_system_status_t *ptBT_System_Status)
{
    ...
    mesa_rc rc = pd69200_tx_rx(inst, __FUNCTION__, __LINE__, buf, fname);
    if (rc != MESA_RC_OK) {
        DEBUG(inst, MEBA_TRACE_LVL_DEBUG, "\n\r%s called line %d ,rc=%d", __FUNCTION__, __LINE__,
              rc);
        return rc;
    }
    ...
    ptBT_System_Status->found_devices = (buf[8] & 0xF); // The number of found devices after boot up
                                                        // (Range from 0x0 to 0xB)
    ptBT_System_Status->active_devices_found =
        ((buf[8] >> 4) & 0xF); // The number of active devices (Range from 0x0 to 0xB)
                               //
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          ...
          ptBT_System_Status->user_byte, ptBT_System_Status->found_devices,
          ptBT_System_Status->active_devices_found);

    return MESA_RC_OK;
}

static mesa_rc meba_poe_pd69200_get_software_version(const meba_poe_ctrl_inst_t *const inst,
                                                     software_version_t *ptSoftware_version)
{
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          ...
          fname, ptSoftware_version->hw_version, ptSoftware_version->poe_mcu_type,
          ptSoftware_version->sw_version_major, ptSoftware_version->sw_version_minor,
          ptSoftware_version->param_number, ptSoftware_version->build_number,
          ptSoftware_version->internal_sw_number, ptSoftware_version->asic_patch_number);
...
static mesa_bool_t is_firmware_version_identical(const meba_poe_ctrl_inst_t *const inst,
...
#define MAX_FIRMWARE_HEADER_LEN 300
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO, "detected: %d - %d.%d.%d , from_file: %d - %d.%d.%d",
          ...
          tSoftware_version.sw_version_minor, tSoftware_version.param_number, prod_number_from_file,
    ...
    if ((sw_ver_high_from_file == tSoftware_version.sw_version_major) &&
        ...
        DEBUG(inst, MEBA_TRACE_LVL_INFO,
              ...
              prod_number_from_file, sw_ver_high_from_file, sw_ver_low_from_file, param_from_file);
        return false;
    }
}
                                                         ...
                                                         const char *microsemi_firmware)
{
    ...
    if (chip_state != MEBA_POE_CHIPSET_FOUND) {
        DEBUG(inst, MEBA_TRACE_LVL_WARNING,
              "%s: No PD69200 chip found, don't do anything, chip_state=%d", inst->adapter_name,
              chip_state);
                                                 ...
                                                 const char *microsemi_firmware)
{
        ...
        if (byteArr[1] == '0') { // skip line that starts with "S0"
            ...
            while (*microsemi_firmware != '\n' && *microsemi_firmware < 127 &&
                ...
                DEBUG(inst, MEBA_TRACE_LVL_NOISE,
                      "Skipping S0 lines microsemi_firmware:0x%X, byte_cnt:%u", *microsemi_firmware,
                      byte_cnt);
                microsemi_firmware++;
            }
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO, "POE_BOOL_Is_system_status=%d ,Telemetry_at_Boot_Up_Error=%d",
                                                    ...
                                                    meba_poe_global_cfg_t            *cfg_global)
{
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(inst,
    ...
    if (supports_legacy_detection != cfg_global->legacy_detect) {
        MESA_RC(meba_poe_pd69200_set_individual_mask(inst,
                                                     INDV_MASK_PREBT_SUPPORTS_LEGACY_DETECTION_0x01,
    ...
    MESA_RC(pd69200_get_power_supply_parameters(inst, &power_consumption_w,
                                                &max_shutdown_voltage_dv, &min_shutdown_voltage_dv,
                                                &guard_band, &bank, &power_limit_w));
    ...
    if ((guard_band != 0x2) || (power_limit_w != cfg_global->power_supply_poe_limit_w)) {
        ...
        MESA_RC(meba_poe_pd69200_set_power_banks(inst, bank, power_limit_w, max_shutdown_voltage_dv,
                                          ...
                                          char                             *value)
{
    ...
    snprintf(
        ...
        tSoftware_version.internal_sw_number, tSoftware_version.asic_patch_number);
                               ...
                               meba_poe_indv_mask_prebt_t       *im_prebt)
{
    ...
    // see section 4.5.10
    MESA_RC(meba_poe_pd69200_get_individual_mask(inst, INDV_MASK_PREBT_IGNORE_HIGHER_PRIORITY_0x00,
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(inst, INDV_MASK_PREBT_SUPPORTS_BACKOFF_0x11,
                                                 &im_prebt->prebt_supports_backoff,
                                                 "PREBT- supports_backoff"));
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG, "PREBT- supports_backoff=%d",
          im_prebt->prebt_supports_backoff);

    MESA_RC(meba_poe_pd69200_get_individual_mask(inst, INDV_MASK_PREBT_LED_STREAM_TYPE_0x16,
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(inst, INDV_MASK_PREBT_MESSAGE_READY_NOTIFY_0x1E,
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(inst, INDV_MASK_PREBT_LAYER2_LLDP_ENABLE_0x2E,
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_PREBT_LAYER2_PRIORITY_BY_PD_0x2F, &im_prebt->prebt_layer2_priority_by_PD,
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(inst, INDV_MASK_PREBT_MATRIX_SUPPORT_4P_0x34,
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(inst, INDV_MASK_PREBT_CLASS_0_EQUAL_AF_0x38,
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(inst,
                                                 INDV_MASK_PREBT_SINGLE_DETECTION_FAIL_EVENT_0x46,
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(inst, INDV_MASK_PREBT_INVALID_SIG_COUNT_ONCE_0x55,
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(inst,
                                                 INDV_MASK_PREBT_PSE_POWERING_PSE_CHECKING_0x1F,
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(inst, INDV_MASK_PREBT_ENABLE_ASIC_REFRESH_0x2A,
                                                 &im_prebt->prebt_enable_asic_refresh,
                                                 "PREBT- Enable_ASIC_Refresh"));
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG, "PREBT- Enable_ASIC_Refresh=%d",
          im_prebt->prebt_enable_asic_refresh);

    MESA_RC(meba_poe_pd69200_get_individual_mask(inst, INDV_MASK_PREBT_CLASS_1_2_3_EQUAL_AF_0x39,
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(inst, INDV_MASK_PREBT_LLDP_BEST_EFFORT_0x47,
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(inst,
                                                 INDV_MASK_PREBT_AUTO_ZONE2_PORT_ACTIVATION_0x49,
...
mesa_rc print_indv_masks_bt(const meba_poe_ctrl_inst_t *const inst, meba_poe_indv_mask_bt_t *im_BT)
{
    ...
    // Turn off lowest priority port, when a higher priority has a PD connected.
    MESA_RC(meba_poe_pd69200_get_individual_mask(inst, INDV_MASK_BT_IGNORE_HIGHER_PRIORITY_0x00,
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(inst, INDV_MASK_BT_SUPPORT_HIGH_RES_DETECTION_0x10,
    ...
    // of inactivity.
    MESA_RC(meba_poe_pd69200_get_individual_mask(inst, INDV_MASK_BT_I2C_RESTART_ENABLE_0x1B,
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(inst, INDV_MASK_BT_PSE_POWERING_PSE_CHECKING_0x1F,
    ...
    // Enable port poe led.
    MESA_RC(meba_poe_pd69200_get_individual_mask(inst, INDV_MASK_BT_LED_STREAM_TYPE_0x20,
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(inst, INDV_MASK_BT_HOCPP_0x50, &im_BT->bt_hocpp,
                                                 "BT- hocpp"));
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(inst,
                                                 INDV_MASK_BT_SINGLE_DETECTION_FAIL_EVENT_0x46,
...
static mesa_rc meba_poe_ctrl_pd69200_globals_status_get(const meba_poe_ctrl_inst_t *const inst,
...
static mesa_rc pd69200_active_matrix_verify(const meba_poe_ctrl_inst_t *const inst,
...
mesa_rc meba_poe_ctrl_pd69200_do_detection(const meba_poe_ctrl_inst_t *const inst)
{
    ...
    if (rc == MESA_RC_ERROR) {
        ...
        meba_poe_status_t *current_status =
            &(((poe_driver_private_t *)(inst->private_data))->status.global);
        MESA_RC(meba_poe_ctrl_pd69200_version_get(inst, MEBA_POE_VERSION_STRING_SIZE,
                                                        ...
                                                        int16_t     restart_cause)
{
    ...
    if (supports_backoff != tPoE_parameters.indv_mask_prebt_supports_backoff_default) {
        MESA_RC(meba_poe_pd69200_set_individual_mask(inst, INDV_MASK_PREBT_SUPPORTS_BACKOFF_0x11,
                                                     tPoE_parameters
    ...
    if (enable_LLDP_mode != tPoE_parameters.indv_mask_prebt_layer2_lldp_enable_default) {
        MESA_RC(meba_poe_pd69200_set_individual_mask(
            ...
            tPoE_parameters.indv_mask_prebt_layer2_lldp_enable_default)); // Enable LLDP
                                                                          // mode.
        bChangedFlag = true;
    }
    ...
    if (layer2_priority_by_PD != tPoE_parameters.indv_mask_prebt_layer2_priority_by_PD_default) {
        MESA_RC(meba_poe_pd69200_set_individual_mask(
            ...
            tPoE_parameters.indv_mask_prebt_layer2_priority_by_PD_default)); // Allow the
                                                                             // priority to
                                                                             // be fetched
                                                                             // from lldp
        bChangedFlag = true;
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(inst, INDV_MASK_PREBT_MATRIX_SUPPORT_4P_0x34,
                                                 &matrix_support_4p,
                                                 "PREBT- matrix_support_4p")); // get matrix support
                                                                               // 4p
    if (matrix_support_4p != tPoE_parameters.indv_mask_prebt_matrix_support_4P_default) {
        MESA_RC(meba_poe_pd69200_set_individual_mask(
            ...
            tPoE_parameters.indv_mask_prebt_matrix_support_4P_default)); // Allow matrix to
                                                                         // support 4p
        bChangedFlag = true;
    }
    ...
    if (led_stream_type != tPoE_parameters.indv_mask_prebt_led_stream_type_default) {
        MESA_RC(meba_poe_pd69200_set_individual_mask(inst, INDV_MASK_PREBT_LED_STREAM_TYPE_0x16,
                                                     tPoE_parameters
...
char *get_prebt_title_by_ports_status(uint8_t bt_port_status)
{
    ...
    case PREBT_0x09_OFF_DERATING_TEMPERATURE_HIGH:    return "0x1c: DERATING TEMPERATURE HIGH";
    ...
    case PREBT_0x30_OFF_FORCE_DIS_COMMAND_WAS_REC:    return "0x48: FORCE DIS COMMAND WAS REC";
    ...
    case PREBT_0x40_ON__HIGH_POWER_PORT_IS_ON:        return "0x90: HIGH POWER PORT IS ON";
    case PREBT_0x41_OFF_CHIP_OVER_POWER:              return "0x91: CHIP OVER POWER";
    case PREBT_0x42_OFF_FORCE_PWR_ERR_CHIP_OVR_PWR:   return "0xA0: FORCE PWR ERR CHIP OVR PWR";
                                                    ...
                                                    meba_poe_port_status_t *const     port_status)
{
    ...
    if (bRead_lldp_pse_data) {
        ...
        current_port_status->port_status.pse_data.pse_power_type =
            tPrebt_port_lldp_pse_data.pse_power_type; // : Layer 2 PSE type as specified in the
                                         ...
                                         char                             *msg)
{
    ...
    if (rx_buf[0] != TELEMETRY_KEY) {
        ...
        DEBUG(inst, MEBA_TRACE_LVL_INFO, "%s Failed, Invalid key (%d): %s\n", __FUNCTION__,
              rx_buf[0], print_as_hex_string(rx_buf, PD_BUFFER_SIZE, dbg_txt, sizeof(dbg_txt)));
...
mesa_rc meba_poe_ctrl_pd69200_port_capabilities_get(const meba_poe_ctrl_inst_t *const inst,
                                                    meba_poe_port_handle_t            handle,
...
mesa_rc meba_poe_ctrl_pd69200_firmware_upgrade(const meba_poe_ctrl_inst_t *const inst,
                                               mesa_bool_t                       reset,
                                               uint32_t                          size,
                                               const char                       *data)
{
...
mesa_rc meba_poe_ctrl_pd69200_prepare_firmware_upgrade(const meba_poe_ctrl_inst_t *const inst,
                                                       mesa_bool_t version_check,
                                                       uint32_t    size,
                                                       const char *data)
{
...
static mesa_rc meba_poe_pd69200_bt_set_BT_port_parameters(const meba_poe_ctrl_inst_t *const inst,
                                                          uint8_t                           channel,
                                                          uint8_t                           cfg1,
                                                          uint8_t                           cfg2,
...
static mesa_rc meba_poe_pd69200_bt_get_BT_port_parameters(const meba_poe_ctrl_inst_t *const inst,
                                                          uint8_t                           handle,
                                                          meba_poe_port_cfg_t *cfg_POEMCU)
{
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          ...
          cfg_POEMCU->bPoe_plus_mode, cfg_POEMCU->add_power_for_port_mode_dW, cfg_POEMCU->priority);
...
void Set_BT_ParamsByOperationMode(meba_poe_ctrl_inst_t *inst)
{
    ...
    prod.port_type_operation_mode[0][0] =
        tPoE_parameters.bt_operation_mode_compliant_15w_default; // BT compliant 15w
    prod.port_type_operation_mode[0][1] =
        tPoE_parameters.bt_operation_mode_none_compliant_15w_default; // BT non compliant
                                                                      // 15w legacy
    ...
    prod.port_type_operation_mode[1][0] =
        tPoE_parameters.bt_operation_mode_compliant_30w_default; // BT compliant 30w
    prod.port_type_operation_mode[1][1] =
        tPoE_parameters.bt_operation_mode_none_compliant_30w_default; // BT non compliant
                                                                      // 30w legacy
    ...
    prod.port_type_operation_mode[2][0] =
        tPoE_parameters.bt_operation_mode_compliant_60w_default; // BT compliant 60w
    prod.port_type_operation_mode[2][1] =
        tPoE_parameters.bt_operation_mode_none_compliant_60w_default; // BT non compliant
                                                                      // 60w legacy
    ...
    prod.port_type_operation_mode[2][3] =
        tPoE_parameters.bt_operation_mode_legacy_60W_ignore_pd_class_default; // ignore-pd-class

    // 90W
    prod.port_type_operation_mode[3][0] =
        tPoE_parameters.bt_operation_mode_compliant_90w_default; // BT compliant 90w
    prod.port_type_operation_mode[3][1] =
        tPoE_parameters.bt_operation_mode_none_compliant_90w_default; // BT non compliant
                                                                      // 90w legacy
    ...
    prod.port_type_operation_mode[3][3] =
        tPoE_parameters.bt_operation_mode_legacy_90W_ignore_pd_class_default; // ignore-pd-class
...
static mesa_rc meba_poe_pd69200_bt_get_BT_port_status(const meba_poe_ctrl_inst_t *const inst,
                                                      uint8_t                           channel,
                                                      uint8_t                          *port_status,
                                                      uint8_t                          *enable,
...
static mesa_rc meba_poe_pd69200_bt_get_BT_port_class(const meba_poe_ctrl_inst_t *const inst,
                                                     uint8_t                           channel,
                                                     uint8_t                          *port_status,
...
mesa_rc meba_poe_pd69200_bt_get_BT_port_measurements(const meba_poe_ctrl_inst_t *const inst,
                                                     uint8_t                           channel,
...
mesa_rc meba_poe_ctrl_pd69200_bt_port_cfg_set(const meba_poe_ctrl_inst_t *const inst,
                                              meba_poe_port_handle_t            handle,
                                              meba_poe_port_cfg_t              *req_port_cfg)
{
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          ...
          req_port_cfg->bt_pse_port_type, req_port_cfg->priority, req_bt_port_operation_mode);
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          ...
          port_cfg->bt_port_pm_mode, port_cfg->bPoe_plus_mode, port_cfg->bt_pse_port_type,
          port_cfg->priority,
    ...
    mesa_bool_t bParamChanged = false; // Flag which indicate that we need to update the change into
                                       // the POEMCU. We need to send "Set BT Port Parameters"
        ...
        if ((port_cfg_POEMCU->class_error_selection !=
            ...
            if (port_cfg_POEMCU->class_error_selection !=
                ...
                DEBUG(inst, MEBA_TRACE_LVL_INFO,
                      ...
                      port_cfg_POEMCU->class_error_selection,
                      prod.class_error_selection[port_cfg->bt_en]);
            }
    ...
    if (bParamChanged == true) {
        ...
        MESA_RC(meba_poe_pd69200_bt_set_BT_port_parameters(inst, handle, CFG1, CFG2,
...
static mesa_rc meba_poe_pd69200_bt_get_port_layer2_lldp_pse_data(const meba_poe_ctrl_inst_t
                                                                     *const             inst,
                                                                 meba_poe_port_handle_t channel,
                                                                 bt_lldp_pse_data_t
                                                                     *ptBt_lldp_pse_data)
...
mesa_rc meba_poe_ctrl_pd69200_bt_globals_cfg_set(const meba_poe_ctrl_inst_t *const inst,
                                                 meba_poe_global_cfg_t            *cfg_global)
{
    meba_poe_chip_state_t chip_state = meba_poe_pd69200_get_chipset(inst);
    if (chip_state != MEBA_POE_CHIPSET_FOUND) {
    ...
    MESA_RC(pd69200_get_power_supply_parameters(inst, &power_consumption_w,
                                                &max_shutdown_voltage_dv, &min_shutdown_voltage_dv,
                                                &guard_band, &power_bank, &power_limit_w));

    if (power_limit_w != cfg_global->power_supply_poe_limit_w) {
        power_limit_w = cfg_global->power_supply_poe_limit_w;

        MESA_RC(meba_poe_pd69200_set_power_banks(inst, power_bank, power_limit_w,
                                                 max_shutdown_voltage_dv, min_shutdown_voltage_dv,
                                                 0));
    }
...
static mesa_rc meba_poe_pd69200_bt_get_BT_events_interrupt_mask(const meba_poe_ctrl_inst_t
                                                                    *const inst,
                                                                bt_events_interrupt_mask_t
                                                                    *cfg_events)
{
...
static mesa_rc meba_poe_pd69200_bt_set_BT_events_interrupt_mask(const meba_poe_ctrl_inst_t
                                                                    *const inst,
                                                                bt_events_interrupt_mask_t
                                                                    cfg_events)
{
...
static mesa_rc meba_poe_pd69200_bt_get_bt_port_counters(const meba_poe_ctrl_inst_t *const inst,
                                                        meba_poe_port_handle_t            channel,
...
mesa_rc meba_poe_ctrl_pd69200_bt_port_status_get(const meba_poe_ctrl_inst_t *const inst,
                                                 meba_poe_port_handle_t            handle,
                                                 meba_poe_port_status_t *const     port_status)
{
    if (port_status == NULL) {
    ...
    if (bRead_port_status_and_port_class) {
        ...
        MESA_RC(meba_poe_pd69200_bt_get_BT_port_status(inst, handle, &port_state, &enable,
        ...
        // 'Get BT Class Power' driver
        MESA_RC(meba_poe_pd69200_bt_get_BT_port_class(inst, handle, &port_state, &port_phy_info,
        ...
        if (auto_class_support == 0x2) {
            current_port_status->port_status.measured_autoclass_power_mw =
                auto_class_measurement_dw * 100; // Convert from deciwatt to milliWatts
        } else {
    ...
    if (bRead_port_measurements) {
        ...
        if (current_port_status->port_status.meba_poe_ieee_port_state ==
            MEBA_POE_IEEE_PORT_STATE_DELIVERING_POWER) {
            meba_poe_pd69200_bt_get_BT_port_measurements(inst, handle, &main_voltage_dv,
    ...
    if (bRead_lldp_pse_data) {
        ...
        MESA_RC(meba_poe_pd69200_bt_get_port_layer2_lldp_pse_data(inst, handle,
                                                                  &tBt_lldp_pse_data));
    ...
    if (bCounters_related_event) {
        ...
        // 'Get BT Class Power' driver
        MESA_RC(meba_poe_pd69200_bt_get_bt_port_counters(inst, handle, &tBT_port_counters));
...
mesa_rc meba_poe_ctrl_pd69200_bt_port_prebt_pd_data_set(const meba_poe_ctrl_inst_t *const inst,
                                                        meba_poe_port_handle_t            handle,
                                                        meba_poe_pd_data_t               *pd_data)
{
    meba_poe_chip_state_t chip_state = meba_poe_pd69200_get_chipset(inst);
...
mesa_rc meba_poe_ctrl_pd69200_bt_port_bt_pd_data_set(const meba_poe_ctrl_inst_t *const inst,
                                                     meba_poe_port_handle_t            handle,
                                                     meba_poe_pd_bt_data_t            *pd_data)
{
    meba_poe_chip_state_t chip_state = meba_poe_pd69200_get_chipset(inst);
...
mesa_rc meba_poe_ctrl_pd69200_bt_port_pd_data_clear(const meba_poe_ctrl_inst_t *const inst,
                                                    meba_poe_port_handle_t            handle)
{
    meba_poe_chip_state_t chip_state = meba_poe_pd69200_get_chipset(inst);
    if (chip_state != MEBA_POE_CHIPSET_FOUND) {
...
mesa_rc meba_poe_ctrl_pd69200_bt_chip_initialization(const meba_poe_ctrl_inst_t *const inst,
                                                     mesa_bool_t interruptible_power,
                                                     int16_t     restart_cause)
{
    DEBUG(inst, MEBA_TRACE_LVL_INFO, "%s enter", __FUNCTION__);
    ...
    if (i2c_restart_enable != tPoE_parameters.indv_mask_BT_i2c_restart_enable_default) {
        MESA_RC(meba_poe_pd69200_set_individual_mask(inst, INDV_MASK_BT_I2C_RESTART_ENABLE_0x1B,
                                                     tPoE_parameters
    ...
    if (led_stream_type != tPoE_parameters.indv_mask_BT_led_stream_type_default) {
        MESA_RC(meba_poe_pd69200_set_individual_mask(inst, INDV_MASK_BT_LED_STREAM_TYPE_0x20,
                                                     tPoE_parameters
    ...
    // HOCPP - high_over Current Pulse Protection
    uint8_t hocpp;
    MESA_RC(meba_poe_pd69200_get_individual_mask(inst, INDV_MASK_BT_HOCPP_0x50, &hocpp,
                                                 "BT- hocpp"));
    if (hocpp != tPoE_parameters.indv_mask_BT_HOCPP_default) {
        MESA_RC(meba_poe_pd69200_set_individual_mask(inst, INDV_MASK_BT_HOCPP_0x50,
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO, "%s(%s): syncing poe lldp parameters", __FUNCTION__,
          inst->adapter_name);
    ...
    // cold start and somthing was modified
    if (bMatrixUpdatedFlag ||
        (/*(restart_cause == 0) &&*/ (bChangedFlag == true))) // cold start and somthing was
                                                              // modified
    {
        MESA_RC(meba_poe_ctrl_pd69200_save_command(inst));
        DEBUG(inst, MEBA_TRACE_LVL_INFO, "Save PoE settings");
