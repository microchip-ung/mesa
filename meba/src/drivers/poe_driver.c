...
#define INTERUPTIBLE_POE_OFF_TIME                                              \
    5000 // for how many ms to turn off all poe ports and turn on only the cfg
         // enabled poe ports
...
#define INDV_MASK_BT_PORT_LED_BLINKS_AT_INVALID_SIGNATURE_OR_CONNECTION_CHECK_ERROR_0x30 \
    0x30
...
#define POE_FIRMWARE_SIZE_MAX                                                  \
    2000000 // Maximum size of the PoE firmware in bytes (file mscc_firmware.txt)
...
#define DEBUG(_inst, _lvl, _fmt, ...)                                          \
    do {                                                                       \
        ((poe_driver_private_t *)((_inst)->private_data))                      \
            ->debug(_lvl, __FILE__, __LINE__, "(%s) " _fmt,                    \
...
//  Port Status, Port Event field bit 2)
//      and the correlated invalid signature counter advances on every
//      unsuccessful detection cycle.
...
//  first failure.
//      The correlated invalid signature counter continues to advance on every
//      unsuccessful detection cycle.
...
// class0. (According to IEEE802.3AF). '1'  When the port is set to AF mode,
// class4 will be reported as class4, with class power limit of class0.
...
// configuration (see section ?4.3.5 field 6). '1'  Class 0 will make the port
// behaves as AF, regardless of configured port type.
static u8 prebt_individual_mask__Class_0_equal_AF_default = 0;

// 0x46	Single detection fail event
// '0'  When port detection constantly fails, Detection unsuccessful event (bit
// 2) in the interrupt register will be set on each failure. '1'  When port
// detection constantly fails, Detection unsuccessful event (bit 2) in the
// interrupt register will be set only at the 1st failure.
...
//  error will increment the counter once). '1'  Invalid signature counter
//  counts once when consecutive invalid detection conditions are detected.
...
enum bt_port_status_t {
    ...
    BT_0xA7_OFF_Connection_Check_error =
        0xA7, // normal Ethernet none PD device is connected
    BT_0xA8_OFF_Open_Port_is_not_connected =
        0xA8, // Nopthing is conencted to teh Etehrnet port
...
enum internal_poe_port_status_t {
    ...
    //                                             #  Status          #
    //-------------------------------------------------------------------------
    PREBT_0x00_ON__LEGACY_PD_DET =
        0, // # eDELIVERINGPOWER # Legacy PD was detected
    PREBT_0x01_ON__802_2PAIR_3AF_DET =
        1, // # eDELIVERINGPOWER # Valid PD which comply to 802.3AF spec was
           // detected, or only two pair out of 4 pair are ON (Midspan 95xxG)
    PREBT_0x02_ON__802_4PAIR_DET = 2, // # eDELIVERINGPOWER # 95xxG Midspan
                                      // apply power to both pairs (data+spare)
    PREBT_0x03_ON__802_2PAIR_POWER_SEQUENCE =
        3, // # eDELIVERINGPOWER # PD is powered as ALT_A in CDP mode. ALT_B is
           // waiting for turn on command.
    PREBT_0x04_ON__802_4PAIR_POWER_SEQUENCE =
        4, // # eDELIVERINGPOWER # ALT_A and ALT_B are powered in CDP Mode.#
    PREBT_0x06_OFF_MAIN_PS_TO_HIGH =
        6, // #                  # Main voltage is higher then limit - cause all
           // ports to be shut down
    PREBT_0x07_OFF_MAIN_PS_TO_LOW =
        7, // #                  # Main voltage is lower then limit - cause all
           // ports to be shut down
    PREBT_0x08_OFF_DIS_ALL_PORTS_PIN_ACT =
        8, // # eDISABLED        # HW pin disable all ports
    PREBT_0x09_OFF_DERATING_TEMPERATURE_HIGH =
        9, // # eOTHERFAULT      # 95xxG Temperature is so high, that HC08 shut
           // down all PoE ports
    PREBT_0x0C_OFF_NON_EXIST_PORT_NUM =
        12, // #                  # Port number that doesn't exist
    PREBT_0x11_OFF_PWR_UP_IN_PROCESS =
        17, // # eOTHERFAULT      # Interium state during power-up. State will
            // be changed.
    PREBT_0x12_OFF_INTERNAL_HW_FAULT =
        18, // #                  # Port doesn't responed, or HW failure
    PREBT_0x1A_OFF_USER_SETTING = 26, // # eDISABLED        # Set to off by user
    PREBT_0x1B_OFF_DET_IN_PROCESS =
        27, // # eSEARCHING       # Interium state during line detection. State
            // will be changed.
    PREBT_0x1C_OFF_NON_802_3AF_PD =
        28, // #                  # Non standart PD is connected
    PREBT_0x1D_OFF_SUCCESION_OF_OVL_AND_UDL =
        29, // # eOTHERFAULT      # PD consume more or less then - cause port to
            // be shut down
    PREBT_0x1E_OFF_UNDERLOAD_OR_CAP_TO_SMALL =
        30, // # eFAULT           # underload according to 802.3AF, or capacitor
            // is bellow 22uf
    PREBT_0x1F_OFF_OVERLOAD_OR_CAP_TO_BIG =
        31, // # eFAULT           # Overload according to 802.3AF, or capacitor
            // is above 1000uf
    PREBT_0x20_OFF_PWR_BUDGET_EXCEDDED =
        32, // # eOTHERFAULT      # Power Management shut down the port due to
            // lack of power
    PREBT_0x21_OFF_INTERNAL_HW_ROUTING_ERROR =
        33, // # eOTHERFAULT      # HW problems prevent port operation
    PREBT_0x24_OFF_EXT_VOLT_INJ_CAP_DET_MODE =
        36, // #                  # Port fails cap det due to voltage being
            // applied to the port
    PREBT_0x25_OFF_IMPROPER_CAP_DET_MODE =
        37, // #                  # Port fails Capacitor detection due to
            // out-of-range cap value
    PREBT_0x26_OFF_DISCHARGED_LOAD_CAP_DET_MODE =
        38, // #                  # Port fails Cap Det due to discharged
            // capacitor #                  #
    PREBT_0x2B_ON__FORCE =
        43, // # eTEST            # Port was forced to turn on
    PREBT_0x2C_OFF_FORCE_ERROR =
        44, // # eOTHERFAULT      # Port was forced to ON, and shut down due to
            // other error was shut down
    PREBT_0x2D_OFF_FORCE_PS_TOO_HIGH =
        45, // # eOTHERFAULT      # Port was forced to ON, and detected that
            // power supply is to high
    PREBT_0x2E_OFF_FORCE_PS_TOO_LOW =
        46, // # eOTHERFAULT      # Port was forced to ON, and detected that
            // power supply is to low
    PREBT_0x2F_OFF_FORCE_DIS_PDU_FLAG_WAS_RAISE =
        47, // # eOTHERFAULT      # Port was forced to ON, and Disable PDU flag
            // was raise
    PREBT_0x30_OFF_FORCE_DIS_COMMAND_WAS_REC =
        48, // # eOTHERFAULT      # Port was forced ON, and disable command was
            // rec
    PREBT_0x31_OFF_FORCE_802_3AF_OVERLOAD =
        49, // # eFAULT           # 802.3AF overload condition during force on
    PREBT_0x32_OFF_FORCE_OUT_OF_PWR_BUDGET =
        50, // # eOTHERFAULT      # Power management det out of power. Port is
            // down or denied.
    PREBT_0x33_UNKNOWN_FORCE_COMMUNICATION_ERR =
        51, // # eOTHERFAULT      # Port was forced ON, and comm with ASIC occure
    PREBT_0x34_OFF_SHORT_CONDITION =
        52, // # eFAULT           # Short condition was detected at a port
            // delivering power
    PREBT_0x35_OFF_OVER_TEMP_AT_PORT =
        53, // #                  # Port temperature protection was activated
    PREBT_0x36_OFF_DEVICE_TOO_HOT =
        54, // #                  # DIE temp is above safe operation value
    PREBT_0x37_OFF_UNKNOWN_DEVICE_PORT_STATUS =
        55, // # eOTHERFAULT      # The device returns an unknown port status
            // for the software
    PREBT_0x38_OFF_FORCE_PWR_ERR_SHORT_CIRCUIT =
        56, // # eFAULT           # Short in the port during force On
    PREBT_0x39_OFF_FORCE_PWR_ERR_CH_OVER_TEMP =
        57, // # eOTHERFAULT      # Channel over temperature during Force-ON
    PREBT_0x3A_OFF_FORCE_PWR_ERR_CHIP_OVER_TEMP =
        58, // # eOTHERFAULT      # Device over temperature during force-ON
            // #                  #
    PREBT_0x3C_OFF_PWR_MANAGEMENT_STATIC =
        60, // # eOTHERFAULT      # Calculated power > power limit
    PREBT_0x3D_OFF_PWR_MANAGEMENT_STATIC_OV =
        61, // # eOTHERFAULT      # PD class report > user predefined power value
    PREBT_0x3E_OFF_FORCE_PWR_ERR_MNGMNT_STAT =
        62, // # eOTHERFAULT      # Calculated power > power limit during Force-ON
    PREBT_0x3F_OFF_FORCE_PWR_ERR_MNGMNT_STAT_OV =
        63, // # eOTHERFAULT      # PD class report > user predefined power
            // value during force ON
    PREBT_0x40_ON__HIGH_POWER_PORT_IS_ON =
        64, // # eDELIVERINGPOWER # PD get power from two PSE ports
    PREBT_0x41_OFF_CHIP_OVER_POWER =
        65, // # eOTHERFAULT      # Total Chip power delivery exceed max limit.
    PREBT_0x42_OFF_FORCE_PWR_ERR_CHIP_OVR_PWR =
        66, // # eOTHERFAULT      # Total Chip power delivery exceed max limit.
    PREBT_0x43_OFF_CLASS_ERROR =
        67, // # eOTHERFAULT      # 90xxG Midspan Rotem based - Invalid Class
            // signature, don't power PD.
    PREBT_0x44_OFF_DURING_HOST_CRASH =
        68, // # eOTHERFAULT      # This status appears due to OVL,UDL or Short
            // events during host crash.
    PREBT_0x45_OFF_FROM_DELIVERED_TO_HOST_SHUTDOWN =
        69, // # eOTHERFAULT   # Port is off - The port was delivering power
            // before host crash but was configured to be forced shut when host
            // crashes.
    PREBT_0x46_OFF_FROM_ENABLED_TO_HOST_SHUTDOWN =
        70, // # eOTHERFAULT      # Port is off - The port was enabled and not
            // delivering power before host crash and was configured to be
            // forced shut when host crashes.
    PREBT_0x47_OFF_FORCE_POWER_CRASH_ERROR =
        71, // # eOTHERFAULT      # Port is off - Port is at force power error,
            // according to IEEE test mode error. The port was forced power and
            // host crash occurred.
    PREBT_0x48_OFF_RECOVERY_UDL =
        72, // # eOTHERFAULT      # During crash a recovery port delivering
            // power was disconnected due to UDL.
    PREBT_0x49_OFF_RECOVERY_PG_EVENT =
        73, // # eOTHERFAULT      # During crash a recovery port delivering
            // power was disconnected due to PG event.
    PREBT_0x4A_OFF_RECOVERY_OVL =
        74, // # eOTHERFAULT      # During crash a recovery port delivering
            // power was disconnected due to OVL.
    PREBT_0x4B_OFF_RECOVERY_SC =
        75, // # eOTHERFAULT      # During crash a recovery port delivering
            // power was disconnected due to SC.
    PREBT_0x4C_OFF_RECOVERY_VOLTAGE_INJECTION =
        76, // # eOTHERFAULT      # Voltage was applied to the port from
            // external source, during or before crash.
    PREBT_0x4D_OFF_DVDT_FAIL_DURING_STARTUP =
        77, // # eOTHERFAULT      # DVDT algorithm that checks power up sequence
            // failed to power up the port.

    PREBT_PORT_STATUS_ONKNOWN_0xFF =
        255 //                   # internal NMS soft state which mark that port
            //                   status was never read and updated.
};
...
typedef struct {
    ...
    uint8_t  port_type_operation_mode[MAX_POE_TYPES]
                                    [4]; // columne0: pse_type and 0=Legacy,1=BT
                                         // columne1: 0-BT ,1-legacy org ,2-poh
                                         // ,3-Ignore-PD-Class
} Prod;

static const uint16_t poe_port_mode_max_power_w[] = {
    15, 30, 60, 90}; // System has 4 modes = 15/30/60/90
static uint8_t MAX_ADDED_CLASS_POWER_dW =
    0; // 25;   // Extra power per class in DeciWatt (25=2.5W)
// static const uint8_t  PowerPerClass_W[]        = { 0, 4 , 7,  15, 30, 45, 60,
// 75, 90 };  // Max PoE power per Class

Prod prod; // allow poe to obtain init parameters from application (dynamically)
           // or from configuration file (statically)
 ...
 *    output:   byArrRxBuffer[]              - pointer to recieve data byte
 *array *pePOE_BOOL_Is_system_status - parameter to inform calling function if
 *special system status (with echo 255) message was detected. return: MESA_RC_OK
 *- operation succeed MESA_RC_ERR_POE_RX_BUF_EMPTY - rx buffer empty
 *MESA_RC_ERROR
 *- i2c device error MESA_RC_ERR_POE_FIRM_UPDATE_NEEDED - poe firmware update
 *needed
 *---------------------------------------------------------------------*/
 ...
 *---------------------------------------------------------------------*/
// mesa_rc (*functionPtr_rd_system_status_ok)        (const
// meba_poe_ctrl_inst_t*, mesa_bool_t*, telemetry_at_boot_up_error_e*); mesa_rc
// (*functionPtr_rd_system_status_analyze_ok)(const meba_poe_ctrl_inst_t*,
// uint8_t*    ,telemetry_at_boot_up_error_e*);

static char *print_as_hex_string(uint8_t *in,
                                 int      in_size,
                                 char    *out,
                                 int      out_size)
{
...
static mesa_rc pd69200_rd(const meba_poe_ctrl_inst_t *const inst,
                          uint8_t                          *data,
                          uint8_t                           size)
{
    char buf[size * 3 + 1];
...
static mesa_rc pd69200_wr(const meba_poe_ctrl_inst_t *const inst,
                          uint8_t                          *data,
                          char                              size,
                          char                             *data_description)
{
    ...
    if (cnt == size) {
        ...
        DEBUG(inst, MEBA_TRACE_LVL_DEBUG,
              "%s: %s Wrote(%d/%d. TxErrCnt=%lu) %s ", inst->adapter_name,
              data_description, size, cnt,
              private_data->status.global.i2c_tx_error_counter,
              print_as_hex_string(data, size, buf, sizeof(buf)));
...
// IN/OUT : Data - Pointer to where to put the read data
// IN     : Size - Number of bytes to read.
mesa_rc pd69200_firm_update_rd(const meba_poe_ctrl_inst_t *const inst,
                               uint8_t                          *data,
                               char                              size)
{
...
static void meba_poe_pd69200_set_chipset(const meba_poe_ctrl_inst_t *const inst,
...
static meba_poe_chip_state_t meba_poe_pd69200_get_chipset(
    const meba_poe_ctrl_inst_t *const inst)
{
    if (inst == NULL || inst->private_data == NULL) {
        DEBUG(inst, MEBA_TRACE_LVL_WARNING,
              "%s: no poe chipset was found, chip_state=%d", __FUNCTION__,
...
// in section 7 in the userguide. Instead microSemi has suggested that we simply
// wait more than 10 sec. After 10 sec. the I2C buffer is flushed, and we do not
...
static mesa_rc meba_poe_ctrl_pd69200_sync(const meba_poe_ctrl_inst_t *const inst)
{
    ...
    if (chip_state != MEBA_POE_CHIPSET_FOUND) {
        DEBUG(inst, MEBA_TRACE_LVL_DEBUG,
              "Driver instance %s not found, chip_state=%d", inst->adapter_name,
              chip_state);
        return MESA_RC_OK;
    }

    DEBUG(inst, MEBA_TRACE_LVL_INFO, "Syncing driver instance: %s",
          inst->adapter_name);

    VTSS_MSLEEP(10000); // Wait at least 10 sec in order to reset i2c system.
    ...
    for (timeout = PD_BUFFER_SIZE * 4; timeout > 0; timeout--) {
        ...
        DEBUG(inst, MEBA_TRACE_LVL_DEBUG, "No sync found: %s",
              print_as_hex_string(buf, PD_BUFFER_SIZE, charbuf,
                                  sizeof(charbuf)));
        VTSS_MSLEEP(1000);
    }
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG,
                            ...
                            char                             *data_description)
{
    ...
    return MESA_RC_OK ==
           pd69200_wr(inst, buf, PD_BUFFER_SIZE, data_description);
}
    ...
    if (pd69200_check_sum_ok(&buf[0])) {
        ...
        if (buf[0] != REPORT_KEY) {
            ...
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "%s Failed, report key value error \n", __FUNCTION__);
        ...
        } else if (buf[2] == 0xFF && buf[3] == 0xFF && buf[4] == 0xFF &&
                   buf[5] == 0xFF) {
            report_key_ok_v = false;
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "%s Failed, Command Received/Wrong Checksum \n",
                  __FUNCTION__);
        } else if (buf2_3 >= 0x1 && buf2_3 <= 0x7FFF) {
            report_key_ok_v = false;
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "%s Failed, Failed Execution/Conflict in Subject Bytes \n",
                  __FUNCTION__);
            ...
            report_key_ok_v = false;
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "%s Failed, Failed Execution/Wrong Data Byte Value \n",
                  __FUNCTION__);
        } else if (buf[2] == 0xFF && buf[3] == 0xFF) {
            report_key_ok_v = false;
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "%s Failed, Failed Execution/Undefined Key Value \n",
                  __FUNCTION__);
        } else {
            report_key_ok_v = false;
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "%s Failed, Undefined reply code Key Value \n", __FUNCTION__);
        }
    } else {
        report_key_ok_v = false;
        DEBUG(inst, MEBA_TRACE_LVL_INFO,
              "%s Failed, Rx packet checksum error \n", __FUNCTION__);
    }
...
static mesa_rc get_controller_request_response(const meba_poe_ctrl_inst_t
                                                   *const inst,
                                               uint8_t   *buf_rx,
                                               uint8_t   *buf_tx)
{
    ...
    if (!pd69200_check_sum_ok(buf_rx)) {
        ...
        DEBUG(inst, MEBA_TRACE_LVL_INFO, "%s Failed, Invalid checksum: %s\n",
              __FUNCTION__,
              print_as_hex_string(buf_rx, PD_BUFFER_SIZE, dbg_txt,
                                  sizeof(dbg_txt)));
    ...
    if (buf_rx[0] != TELEMETRY_KEY) {
        char dbg_txt[PD_BUFFER_SIZE * 4];
        DEBUG(inst, MEBA_TRACE_LVL_INFO,
              "%s Failed, Invalid key (%d), TX: %s ,Rx: %s\n", __FUNCTION__,
              buf_rx[0],
              print_as_hex_string(buf_tx, PD_BUFFER_SIZE, dbg_txt,
                                  sizeof(dbg_txt)),
              print_as_hex_string(buf_rx, PD_BUFFER_SIZE, dbg_txt,
                                  sizeof(dbg_txt)));
        return MESA_RC_ERR_POE_COMM_PROT_ERR;
 ...
 *MESA_RC_ERROR
 *- i2c device error MESA_RC_ERR_POE_FIRM_UPDATE_NEEDED - poe firmware update
 *needed MESA_RC_ERR_POE_COMM_PROT_ERR - communication protocol error (checksum
 *or key)
 *---------------------------------------------------------------------*/
static mesa_rc pd69200_tx_rx(const meba_poe_ctrl_inst_t *const inst,
                             const char                       *file,
                             int                               line,
                             uint8_t                          *buf_rx,
                             char                             *data_description)
{
    ...
    rc = get_15_bytes_comm_protocol_reply(inst, buf_rx, buf_tx[1],
                                    ...
                                    meba_poe_serial_number_t         *ptSN)
{
    ...
    unsigned char buf[PD_BUFFER_SIZE] = {
        REQUEST_KEY, DUMMY_SEQ_NUM, GLOBAL_KEY, PRDCTINFO_KEY, DUMMY_BYTE,
    ...
    ptSN->UN =
        ((uint32_t)buf[8] << 16) | ((uint32_t)buf[9] << 8) | (uint32_t)buf[10];

    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "[%s] CH=%d ,Year=%d ,Week=%d ,PN=%d ,UN=%d", fname,
          ptSN->NumOfCh_HC08_Sprt, ptSN->Year, ptSN->Week, ptSN->PN, ptSN->UN);

    return MESA_RC_OK;
}

mesa_rc meba_poe_pd69200_get_individual_mask(const meba_poe_ctrl_inst_t
                                                 *const inst,
                                             ...
                                             char      *indv_desc)
{
    ...
    unsigned char buf[PD_BUFFER_SIZE] = {
        REQUEST_KEY, DUMMY_SEQ_NUM, GLOBAL_KEY, INDIV_MASK_KEY, mask_key_number,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE,     DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE,     DUMMY_BYTE};
    ...
    *indv_mask_value = buf[2];
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "[%s] %s ,mask_key_number=0x%X ,indv_mask_value=%d", fname, indv_desc,
          mask_key_number, *indv_mask_value);

    return MESA_RC_OK;
}

static mesa_rc meba_poe_pd69200_set_individual_mask(const meba_poe_ctrl_inst_t
                                                        *const inst,
                                                    uint8_t    mask,
                                                    uint8_t    val)
{
    uint8_t buf[PD_BUFFER_SIZE] = {
        COMMAND_KEY, DUMMY_SEQ_NUM, GLOBAL_KEY, INDIV_MASK_KEY, mask,
        val,         DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE,     DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE,     DUMMY_BYTE};

    char *fname = "SET INDIVIDUAL MASK";
    MESA_RC(pd69200_tx_rx(inst, __FUNCTION__, __LINE__, buf, fname))

    DEBUG(inst, MEBA_TRACE_LVL_INFO, "[%s] setting mask=0x%X ,val=%d", fname,
          mask, val);

    return MESA_RC_OK;
}

static mesa_rc meba_poe_pd69200_prebt_get_pm_method(const meba_poe_ctrl_inst_t
                                                        *const inst,
                                                    uint8_t   *pm1,
                                                    uint8_t   *pm2,
                                                    uint8_t   *pm3)
{
    ...
    *pm3 = buf[4];
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO, "[%s] pm1=%d ,pm2=%d ,pm3=%d", fname, *pm1,
          *pm2, *pm3);

    return MESA_RC_OK;
}

static mesa_rc meba_poe_pd69200_prebt_set_pm_method(const meba_poe_ctrl_inst_t
                                                        *const inst,
                                                    uint8_t    pm1,
                                                    uint8_t    pm2,
                                                    uint8_t    pm3)
...
static mesa_rc meba_poe_pd69200_set_power_banks(const meba_poe_ctrl_inst_t
                                                    *const inst,
                                                uint8_t    bank,
                                                ...
                                                uint8_t guard_band)
{
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "[%s] bank=%d ,power_limit_w=%lu ,max_shutdown_voltage_dv=%lu ,min_shutdown_voltage_dv=%lu ,guard_band=%d",
          fname, bank, power_limit_w, max_shutdown_voltage_dv,
...
{
    uint8_t buf[PD_BUFFER_SIZE] = {
        REQUEST_KEY, DUMMY_SEQ_NUM, GLOBAL_KEY, SUPPLY_KEY, MAIN_KEY,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE, DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE, DUMMY_BYTE};
    ...
    *power_limit_w = (buf[10] << 8) + buf[11];
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "[%s] pwr_consumption=%lu[w] ,max_shutdown_voltage_dv=%lu ,min_shutdown_voltage_dv=%lu ,guard_band=%d ,bank=%d ,power_limit_w=%lu",
          fname, *power_consumption_w, *max_shutdown_voltage_dv,
          *min_shutdown_voltage_dv, *guard_band, *bank, *power_limit_w);
    return MESA_RC_OK;
}
    ...
    uint16_t                         *tppl)
{
    ...
    uint8_t buf[PD_BUFFER_SIZE] = {
        REQUEST_KEY, DUMMY_SEQ_NUM, CHANNEL_KEY, SUPPLY_KEY, channel,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE,  DUMMY_BYTE, DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE,  DUMMY_BYTE, DUMMY_BYTE};
    ...
    *tppl = (buf[4] << 8) + buf[5];
    ...
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG, "[%s] CH=%d ,ppl=%lu ,tppl=%lu", fname,
          channel, *ppl, *tppl);
    ...
    uint8_t                           port_type)
{
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO, "[%s] CH=%d ,cmd=0x%X ,port_type=%d",
          fname, channel, cmd, port_type);

    return MESA_RC_OK;
}

static mesa_rc meba_poe_pd69200_prebt_set_port_priority(const meba_poe_ctrl_inst_t
                                                            *const inst,
                                                        uint8_t    channel,
                                                        uint8_t    priority)
{
    uint8_t buf[PD_BUFFER_SIZE] = {
        COMMAND_KEY, DUMMY_SEQ_NUM, CHANNEL_KEY, PRIORITY_KEY, channel,
        priority,    DUMMY_BYTE,    DUMMY_BYTE,  DUMMY_BYTE,   DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE,  DUMMY_BYTE,   DUMMY_BYTE};

    char *fname = "SET PORT PRIORITY";
    MESA_RC(pd69200_tx_rx(inst, __FUNCTION__, __LINE__, buf, fname));

    DEBUG(inst, MEBA_TRACE_LVL_INFO, "[%s] CH=%d ,priority=%d", fname, channel,
          priority);

    return MESA_RC_OK;
}

static mesa_rc meba_poe_pd69200_prebt_set_port_layer2_lldp_pd_data(
    const meba_poe_ctrl_inst_t *const inst,
    ...
    uint8_t                           execute_lldp)
{
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "[%s] CH=%d ,layer2_pd_type=%d ,requested_power_dw=%lu ,allocated_power_dw=%lu ,cable_len=%d ,execute_lldp=%d",
          fname, channel, layer2_pd_type, requested_power_dw,
          allocated_power_dw, cable_len, execute_lldp);
    ...
    prebt_port_lldp_pse_data_t       *ptPse_data)
{
    ...
    uint8_t buf[PD_BUFFER_SIZE] = {
        REQUEST_KEY, DUMMY_SEQ_NUM, CHANNEL_KEY, PREBT_LLDP_PSE_KEY,
    ...
    ptPse_data->layer2_request_pending =
        (power_indicator >> 14) &
        1; // Bit 14 - Layer2 request Pending, 0= The layer 2 request was
    ...
    ptPse_data->port_power_consumption =
        power_indicator & 0xFFF; // Port Power Consumption
    ...
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG,
          ...
          fname, channel, ptPse_data->pse_allocated_power_dw,
          ptPse_data->pd_requested_power_dw, ptPse_data->pse_power_type,
          ptPse_data->power_class, ptPse_data->pse_power_pair,
...
{
    uint8_t buf[PD_BUFFER_SIZE] = {
        REQUEST_KEY, DUMMY_SEQ_NUM, CHANNEL_KEY, PREBT_PORT_STATUS_KEY,
        channel,     DUMMY_BYTE,    DUMMY_BYTE,  DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE,  DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE};
    ...
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG,
          ...
          fname, channel, pPrebt_single_port_status->enable,
    ...
    uint16_t                         *imain_current_ma)
{
    ...
    *imain_current_ma = (buf[7] << 8) + buf[8];
    ...
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG,
          "[%s] vmain_voltage_dv=%lu ,imain_current_ma=%lu [W]", fname,
                                         ...
                                         uint16_t   *vmain_voltage_dv)
{
    ...
    // Send request to get status
    unsigned char buf[PD_BUFFER_SIZE] = {
        REQUEST_KEY, DUMMY_SEQ_NUM, GLOBAL_KEY, SUPPLY_KEY, TOTAL_POWER_KEY,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE, DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE, DUMMY_BYTE};
    ...
    *power_bank = buf[10];
    ...
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG,
          ...
          fname, *power_consumption_w, *calculated_power_w, *available_power_w,
          *power_limit_w, *power_bank, *vmain_voltage_dv);
    ...
    uint8_t                          *port_type_prebt_af_at_poh)
{
    ...
    *port_type_prebt_af_at_poh = buf[6];
    ...
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG,
          "[%s] CH=%d ,port_type_prebt_af_at_poh=%d", fname, channel,
    ...
    uint16_t                         *port_voltage)
{
    ...
    // Send request to get status
    unsigned char buf[PD_BUFFER_SIZE] = {
        REQUEST_KEY, DUMMY_SEQ_NUM, CHANNEL_KEY, PREBT_PARAMZ_KEY, channel,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE,  DUMMY_BYTE,       DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE,  DUMMY_BYTE,       DUMMY_BYTE};
    ...
    *port_voltage = (buf[9] << 8) + buf[10];
    ...
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG,
          ...
          fname, channel, *main_voltage, *calculated_current,
          *port_power_consumption, *port_voltage);

    return MESA_RC_OK;
}

static mesa_rc meba_poe_ctrl_pd69200_reset_command(const meba_poe_ctrl_inst_t
                                                       *const inst)
{
    uint8_t buf[PD_BUFFER_SIZE] = {
        COMMAND_KEY, DUMMY_SEQ_NUM, GLOBAL_KEY, RESET_KEY,  0x00,
...
static mesa_rc meba_poe_ctrl_pd69200_save_command(const meba_poe_ctrl_inst_t
                                                      *const inst)
{
    uint8_t buf[PD_BUFFER_SIZE] = {
        PROGRAM_KEY, DUMMY_SEQ_NUM, E2_KEY,     SAVE_CONFIG_KEY, DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE,      DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE,      DUMMY_BYTE};
...
static mesa_rc meba_poe_pd69200_get_active_matrix(const meba_poe_ctrl_inst_t
                                                      *const             inst,
                                                  meba_poe_port_handle_t handle,
                                                  uint8_t *phys_numb_a,
                                                  uint8_t *phys_numb_b)
{
    // Transmit the command
    uint8_t buf[PD_BUFFER_SIZE] = {
        REQUEST_KEY, DUMMY_SEQ_NUM, CHANNEL_KEY, CHANNEL_MATRIX_KEY,
    ...
    *phys_numb_b = buf[3];
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO, "[%s] CH=%d ,phys#_a=%d ,phys#_b=%d",
          fname, handle, *phys_numb_a, *phys_numb_b);

    return MESA_RC_OK;
}

static mesa_rc meba_poe_pd69200_set_temporary_matrix(const meba_poe_ctrl_inst_t
                                                         *const inst,
                                                     meba_poe_port_handle_t
                                                             handle,
                                                     uint8_t phys_numb_a,
                                                     uint8_t phys_numb_b)
{
    // Transmit the command
    uint8_t buf[PD_BUFFER_SIZE] = {
        COMMAND_KEY, DUMMY_SEQ_NUM, CHANNEL_KEY, TMP_MATRIX_KEY, handle,
        phys_numb_a, phys_numb_b,   DUMMY_BYTE,  DUMMY_BYTE,     DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE,  DUMMY_BYTE,     DUMMY_BYTE};
...
static mesa_rc pd69200_program_global_matrix(const meba_poe_ctrl_inst_t
                                                 *const inst)
{
    // Transmit the command
    uint8_t buf[PD_BUFFER_SIZE] = {
        COMMAND_KEY, DUMMY_SEQ_NUM, GLOBAL_KEY, TMP_MATRIX_KEY, DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE,     DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE};
 ...
 *    input :   uint8_t *buf
 *    output:
 *    return:   MESA_RC_ERR_POE_FIRM_UPDATE_NEEDED     - poe firmware update
 *needed MESA_RC_ERROR                - i2c device error MESA_RC_OK - operation
 *succeed
 *---------------------------------------------------------------------*/
mesa_rc check_for_poe_firmware_errors(const meba_poe_ctrl_inst_t *const inst,
                                      uint8_t                          *buf,
                                      telemetry_at_boot_up_error_e
                                          *peTelemetry_at_boot_up_error)
{
        ...
        if ((err_code == 0x4E) && (err_info1 == 0x4E) && (err_info2 == 0x4E) &&
            ...
            if (peTelemetry_at_boot_up_error)
                *peTelemetry_at_boot_up_error =
                    eBoot_error_Application_CRC_error_Download_is_required_for_PD69200;
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "%s: Application CRC error Download is required for PD69200",
                  __FUNCTION__);
        } else if ((err_code == 0x4E) && (err_info1 == 0x4E) &&
                   (err_info2 == 0x4E) && (download_type == 0x33)) {
            if (peTelemetry_at_boot_up_error)
                *peTelemetry_at_boot_up_error =
                    eBoot_error_Application_CRC_error_Download_is_required_for_PD69210_PD69220;
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  ...
                  __FUNCTION__);
        } else if ((err_code == 0x02) && (err_info1 == 0x33) &&
                   (err_info2 == 0x2C) && (download_type == 0x33)) {
            if (peTelemetry_at_boot_up_error)
                *peTelemetry_at_boot_up_error =
                    eBoot_error_hw_error_from_Boot_try_to_program_a_PD69200_firmware_into_PD69210_PD69220_device;
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  ...
                  __FUNCTION__);
        } else if ((err_code == 0x02) && (err_info1 == 0x2C) &&
                   (err_info2 == 0x33) && (download_type == 0x4E)) {
            if (peTelemetry_at_boot_up_error)
                *peTelemetry_at_boot_up_error =
                    eBoot_error_hW_error_from_Boot_try_to_program_a_PD69210_PD69220_firmware_into_PD69200_device;
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  ...
                  __FUNCTION__);
        } else if ((err_code == 0x03) && (err_info1 == 0x40) &&
                   (err_info2 == 0x41) && (download_type == 0x4E)) {
            if (peTelemetry_at_boot_up_error)
                *peTelemetry_at_boot_up_error =
                    eBoot_error_sys_type_error_from_APP_try_to_program_a_PD69220_firmware_into_PD69210_device;
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  ...
                  __FUNCTION__);
        } else if ((err_code == 0x03) && (err_info1 == 0x41) &&
                   (err_info2 == 0x40) && (download_type == 0x4E)) {
            if (peTelemetry_at_boot_up_error)
                *peTelemetry_at_boot_up_error =
                    eBoot_error_sys_type_error_from_APP_try_to_program_a_PD69210_firmware_into_PD69220_device;
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
            ...
            DEBUG(inst, MEBA_TRACE_LVL_INFO, "%s: unknown boot error",
                  __FUNCTION__);
            return MESA_RC_ERROR;
        }
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO, "%s: Telemetry at Boot Up OK",
          __FUNCTION__);

    //  if ((cpu_status2_error_codes != 0)) {
    //	DEBUG(inst, MEBA_TRACE_LVL_INFO, "%s: cpu error=%d", __FUNCTION__,
    // cpu_status2_error_codes); 	return MESA_RC_ERROR;
    //  }
                                             ...
                                             *peTelemetry_at_boot_up_error)
{
    ...
    while ((iFF_byte_counter < 30) &&
           (i00_byte_counter < 30)) /* 30 bytes with value 0 means that I2C
                                       driver has no data to send */
    {
        ...
        if (bRxMsg[0] == TELEMETRY_KEY ||
            bRxMsg[0] ==
                REPORT_KEY) /* first msg byte - telemetry or response */
        {
            ...
            if ((bRxMsg[0] == byTxEcho) || /* original messsage */
                ((rx_data[0] == TELEMETRY_KEY) &&
                 (bRxMsg[0] ==
                  SYSTEM_STATUS_ECHO_KEY))) /* system status on startup */
            {
    ...
    uint16_t                         *pInterrupt_register)
{
    ...
    uint8_t buf[PD_BUFFER_SIZE] = {
        REQUEST_KEY, DUMMY_SEQ_NUM, GLOBAL_KEY, PREBT_SYSTEM_STATUS_KEY,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE};
    ...
    *pInterrupt_register = (buf[11] << 8) + buf[12];
    ...
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG, "[%s] pInterrupt_register=%d", fname,
          *pInterrupt_register);
 ...
 *    return:   void
 *---------------------------------------------------------------------*/
void GetDataPerBit(uint8_t  byArr_Ports[],
                   uint8_t  startIndex,
                   uint16_t ulData,
                   int8_t   iNumberOfBits)
{
 ...
 *succeed
 *---------------------------------------------------------------------*/
static mesa_rc meba_poe_pd69200_bt_event_cause_get(const meba_poe_ctrl_inst_t
                                                       *const inst,
                                                   BT_Event_Cause_t
                                                       *ptBT_Event_Cause)
{
    ...
    uint8_t buf[PD_BUFFER_SIZE] = {
        REQUEST_KEY, DUMMY_SEQ_NUM, GLOBAL_KEY, BT_EVENT_KEY, DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE,   DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE,   DUMMY_BYTE};
    ...
    ptBT_Event_Cause->tSystem_ok_reg.bit0_Vmain_in_range =
        buf[10] &
        1; // 1 = Vmain is in the defined PoE operational voltage range
    ...
    for (int i = 0; i < POE_MAX_PORTS; i++) {
        offset += sprintf(buffer + offset, "[%d]=%d", i,
                          ptBT_Event_Cause->all_ports_event_cause[i]);
        if (i < 47) {
...
typedef struct {
    ...
    uint8_t user_byte; // Saved in nonvolatile memory.  Equals 0xFF at factory
                       // default
    uint8_t found_devices; // The number of found devices after boot up (Range
                           // from 0x0 to 0xB)
    uint8_t active_devices_found; // The number of active devices (Range from
                                  // 0x0 to 0xB)
...
{
    uint8_t buf[PD_BUFFER_SIZE] = {
        REQUEST_KEY, DUMMY_SEQ_NUM, GLOBAL_KEY, BT_SYSTEM_STATUS_KEY,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE};

    DEBUG(inst, MEBA_TRACE_LVL_INFO, "%s called line %d", __FUNCTION__,
          __LINE__);
    ...
    ptBT_System_Status->cpu_status2_err_codes =
        (buf[3] & 3); // 1 = CPU error - must be 0
    ptBT_System_Status->fact_def_param_in_use =
        (buf[4] & 1); // 1 = HC08 use soft default param
    ptBT_System_Status->ram_private_label =
        buf[6]; // Saved in the RAM.  Equals 0x00 after reset
    ptBT_System_Status->user_byte =
        buf[7]; // Saved in nonvolatile memory.  Equals 0xFF at factory default
    ptBT_System_Status->found_devices =
        (buf[8] & 0xF); // The number of found devices after boot up (Range from
                        // 0x0 to 0xB)
    ptBT_System_Status->active_devices_found =
        ((buf[8] >> 4) &
         0xF); // The number of active devices (Range from 0x0 to 0xB)
               //
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "[%s] Boot_up_Error=%d, CPU_Status2_Err_Codes=%d, fact_def_param_in_use=%d, ram_private_label=%d, user_byte=%d, Found_devices=%d, Active_devices_Found=%d",
          fname, ptBT_System_Status->boot_up_error,
          ptBT_System_Status->cpu_status2_err_codes,
          ptBT_System_Status->fact_def_param_in_use,
          ptBT_System_Status->ram_private_label, ptBT_System_Status->user_byte,
          ptBT_System_Status->found_devices,
          ptBT_System_Status->active_devices_found);
...
static mesa_rc meba_poe_pd69200_get_software_version(const meba_poe_ctrl_inst_t
                                                         *const inst,
                                                     software_version_t
                                                         *ptSoftware_version)
{
    ...
    uint8_t buf[PD_BUFFER_SIZE] = {
        REQUEST_KEY, DUMMY_SEQ_NUM, GLOBAL_KEY, VERSIONZ_KEY, SW_VERSION_KEY,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE,   DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE,   DUMMY_BYTE};
    ...
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG, "%s called line %d", __FUNCTION__,
          __LINE__);
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "[%s] HW ver=%d, poe mcu type#=%d, SW ver=%d.%02d, param#=%d, build#=%d, internal SW#=%lu, asic patch#=%lu",
          fname, ptSoftware_version->hw_version,
          ptSoftware_version->poe_mcu_type,
          ptSoftware_version->sw_version_major,
          ptSoftware_version->sw_version_minor,
          ptSoftware_version->param_number, ptSoftware_version->build_number,
...
mesa_rc meba_poe_pd69200_restore_factory_default(const meba_poe_ctrl_inst_t
                                                     *const inst)
{
    // "pd69200_restore_factory_default - Section 4.1.2"

    uint8_t buf[PD_BUFFER_SIZE] = {
        PROGRAM_KEY, DUMMY_SEQ_NUM, RESTORE_FACT_KEY, DUMMY_BYTE, DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE,       DUMMY_BYTE, DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE,       DUMMY_BYTE, DUMMY_BYTE};
...
{
    const uint8_t enter[] = {(uint8_t)'E', (uint8_t)'N', (uint8_t)'T',
                             (uint8_t)'R'};

    // T_IG(VTSS_TRACE_GRP_CUSTOM, "Sending ENTR");

    (void)pd69200_wr(inst, (uint8_t *)&enter[0], 1, "E");
    VTSS_MSLEEP(10); // Section 5.2 - Keep at least 10 ms delay between each
...
static mesa_rc meba_poe_pd69200_bt_get_adc_value(const meba_poe_ctrl_inst_t
                                                     *const inst,
                                                 uint16_t  *sensor_reading)
{
    ...
    *sensor_reading = 0;
    ...
    uint8_t buf[PD_BUFFER_SIZE] = {
        REQUEST_KEY, DUMMY_SEQ_NUM, GLOBAL_KEY, SUPPLY_KEY, BT_ADC_KEY,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE, DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE, DUMMY_BYTE};
...
void meba_poe_io_reset(const meba_poe_ctrl_inst_t *const inst)
{
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "Reseting PoE MCU - clear poe reset io pin - low state");

    // set poe io (pin_level) to '0' - reset mcu poe
    (void)mesa_gpio_write(NULL, 0,
                          private_data->tPoE_parameters.reset_poe_gpio_number,
                          FALSE);
...
// Checking if version of PoE firmware update candidate is identical to the
// current if identical returns true , otherwise returns false
static mesa_bool_t is_firmware_version_identical(const meba_poe_ctrl_inst_t
                                                     *const  inst,
...
#define MAX_FIRMWARE_HEADER_LEN 300
    ...
    if ((private_data->eTelemetry_at_boot_up_error != eBoot_Unknown_error) &&
        ...
        rc = get_15_bytes_comm_protocol_reply(inst, buf_rx, 255,
                                              &ePOE_BOOL_Is_system_status,
                                              &eTelemetry_at_boot_up_error);
    ...
    if (rc == MESA_RC_ERR_POE_FIRM_UPDATE_NEEDED) {
        ...
        default: {
            DEBUG(inst, MEBA_TRACE_LVL_ERROR,
                  "Unable to determine poe firmware.\n");
            break;
        }
        }
        ...
        if (private_data->tPoE_parameters.ePoE_Controller_Type_default !=
            MEBA_POE_PD692X0_CONTROLLER_TYPE_AUTO_DETECTION) {
            DEBUG(inst, MEBA_TRACE_LVL_WARNING,
    ...
    {
        rc = meba_poe_pd69200_get_software_version(inst, &tSoftware_version);

        // check if read firmware version from chip succeed
        if (rc != MESA_RC_OK) {
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "Not able to read firmware version from chip.\n");
            return false;
        }

        private_data->status.global.prod_number_detected =
            tSoftware_version.poe_mcu_type;
        private_data->status.global.sw_version_major_detected =
            tSoftware_version.sw_version_major;
        private_data->status.global.sw_version_minor_detected =
            tSoftware_version.sw_version_minor;
        private_data->status.global.param_number_detected =
            tSoftware_version.param_number;
        private_data->status.global.build_number =
            tSoftware_version.build_number;
        private_data->status.global.internal_sw_number =
            tSoftware_version.internal_sw_number;
        private_data->status.global.asic_patch_number =
        ...
        case ePD69200_PREBT: {
            ...
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "detected poe firmware: pd69200 PREBT firmware.");
            break;
        }
        case ePD69200_BT: {
            ePoE_detected_controller_type = MEBA_POE_PD69200_CONTROLLER_TYPE;
            ...
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "detected poe firmware: pd69200 BT firmware.");
            break;
        }
        case ePD69210_PREBT: {
            ePoE_detected_controller_type = MEBA_POE_PD69210_CONTROLLER_TYPE;
            private_data->status.global.eDetected_poe_firmware_type =
                MEBA_POE_FIRMWARE_TYPE_PREBT;
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "detected poe firmware: pd69210 PREBT firmware.");
            break;
        }
        case ePD69210_BT: {
            ePoE_detected_controller_type = MEBA_POE_PD69210_CONTROLLER_TYPE;
            private_data->status.global.eDetected_poe_firmware_type =
                MEBA_POE_FIRMWARE_TYPE_BT;
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "detected poe firmware: pd69210 BT firmware.");
            break;
        }
        case ePD69220_PREBT: {
            ePoE_detected_controller_type = MEBA_POE_PD69220_CONTROLLER_TYPE;
            private_data->status.global.eDetected_poe_firmware_type =
                MEBA_POE_FIRMWARE_TYPE_PREBT;
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "detected poe firmware: pd69220 PREBT firmware.");
            break;
        }
        case ePD69220_BT: {
            ePoE_detected_controller_type = MEBA_POE_PD69220_CONTROLLER_TYPE;
            private_data->status.global.eDetected_poe_firmware_type =
                MEBA_POE_FIRMWARE_TYPE_BT;
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "detected poe firmware: pd69220 BT firmware.");
            break;
        }
        case ePD69200M_PREBT: {
            private_data->status.global.eDetected_poe_firmware_type =
                MEBA_POE_FIRMWARE_TYPE_PREBT;
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "detected poe firmware: pd69200M PREBT firmware.");
            break;
        }
        case ePD69200M_BT: {
            private_data->status.global.eDetected_poe_firmware_type =
                MEBA_POE_FIRMWARE_TYPE_BT;
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "detected poe firmware: pd69200M BT firmware.");
            break;
        }
        default: {
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "Unknown detected poe firmware: %d .\n",
                  tSoftware_version.poe_mcu_type);
            break;
        }
        }
    ...
    // set poe firmware file depend on controller type (PD69200, PD69210,
    // PD69220) and software required BT/Legacy
    if (private_data->is_bt) {
        if (ePoE_detected_controller_type == MEBA_POE_PD69200_CONTROLLER_TYPE) {
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "loaded poe firmware: pd69200 BT firmware.");
            private_data->builtin_firmware =
                "/etc/mscc/poe/firmware/pd69200_bt_firmware.s19";
            prod_number_from_file = ePD69200_BT;
        } else if (ePoE_detected_controller_type ==
                   MEBA_POE_PD69210_CONTROLLER_TYPE) {
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "loaded poe firmware: pd69210 BT firmware.");
            private_data->builtin_firmware =
                "/etc/mscc/poe/firmware/pd69210_bt_firmware.s19";
            prod_number_from_file = ePD69210_BT;
        } else { // if (ePoE_detected_controller_type == ePD69220_CONTROLLER_TYPE)
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "loaded poe firmware: pd69220 BT firmware.");
            private_data->builtin_firmware =
                "/etc/mscc/poe/firmware/pd69220_bt_firmware.s19";
            prod_number_from_file = ePD69220_BT;
        }
    } else { // PREBT mode               exit meba_poe_supply_limits_get
        if (ePoE_detected_controller_type == MEBA_POE_PD69200_CONTROLLER_TYPE) {
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "loaded poe firmware: pd69200 PREBT firmware.");
            private_data->builtin_firmware =
                "/etc/mscc/poe/firmware/pd69200_at_firmware.s19";
            prod_number_from_file = ePD69200_PREBT;
        } else if (ePoE_detected_controller_type ==
                   MEBA_POE_PD69210_CONTROLLER_TYPE) {
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "loaded poe firmware: pd69210 PREBT firmware.");
            private_data->builtin_firmware =
                "/etc/mscc/poe/firmware/pd69210_at_firmware.s19";
            prod_number_from_file = ePD69210_PREBT;
        } else { // if(ePoE_detected_controller_type ==
                 // MEBA_POE_PD69220_CONTROLLER_TYPE)
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "loaded poe firmware: pd69220 PREBT firmware.");
            private_data->builtin_firmware =
                "/etc/mscc/poe/firmware/pd69220_at_firmware.s19";
    ...
    if (rc != MESA_RC_OK) {
        // DEBUG(inst, MEBA_TRACE_LVL_INFO, "Not able to read firmware version
        // from chip.\n");
        return false;
    }
    ...
    if (!microsemi_firmware) {
        ...
        if (private_data->builtin_firmware == NULL) {
            DEBUG(inst, MEBA_TRACE_LVL_ERROR,
                  "Error: builtin_firmware is null.\n");
            return MESA_RC_ERROR;
        }
        ...
        mapped_memory_size = (((st.st_size - 1) / getpagesize()) + 1) *
                             getpagesize(); // Round up

        if ((mapped_memory = mmap(NULL, mapped_memory_size, PROT_READ,
    ...
    if (swNum_ptr &&
        (swNum_ptr - microsemi_firmware) < MAX_FIRMWARE_HEADER_LEN) {
        ...
        DEBUG(inst, MEBA_TRACE_LVL_INFO,
              "Not able to read %s from upgrade image.\n", swNum);
        if (fd >= 0) {
            if (mapped_memory) {
    ...
    if (param_ptr &&
        (param_ptr - microsemi_firmware) < MAX_FIRMWARE_HEADER_LEN) {
    ...
    private_data->status.global.sw_version_high_from_file =
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "detected: %d - %d.%d.%d , from_file: %d - %d.%d.%d",
          tSoftware_version.poe_mcu_type, tSoftware_version.sw_version_major,
          tSoftware_version.sw_version_minor, tSoftware_version.param_number,
          prod_number_from_file, sw_ver_high_from_file, sw_ver_low_from_file,
          param_from_file);
    ...
    if ((sw_ver_high_from_file == tSoftware_version.sw_version_major) &&
        ...
        DEBUG(inst, MEBA_TRACE_LVL_INFO,
              "Different PoE MCU firmware versions, found %d - %d.%d.%d ,expecting %d - %d.%d.%d",
              tSoftware_version.poe_mcu_type,
              tSoftware_version.sw_version_major,
              tSoftware_version.sw_version_minor,
              tSoftware_version.param_number, prod_number_from_file,
              sw_ver_high_from_file, sw_ver_low_from_file, param_from_file);
        return false;
    }
}
...
static mesa_rc meba_poe_pd69200_prepare_firmware_upgrade(
    const meba_poe_ctrl_inst_t *const inst,
    ...
    const char                       *microsemi_firmware)
{
    meba_poe_chip_state_t chip_state = meba_poe_pd69200_get_chipset(inst);
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG,
          "%s: Enter for instance %s chip_state= %d , version_check= %d",
          __FUNCTION__, inst->adapter_name, chip_state, version_check);
    if (chip_state != MEBA_POE_CHIPSET_FOUND) {
        DEBUG(inst, MEBA_TRACE_LVL_WARNING,
              "%s: No PD69200 chip found, don't do anything, chip_state=%d",
    ...
    if (version_check) {
        if (is_firmware_version_identical(inst, microsemi_firmware)) {
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "%s: Firmware upgrade not needed - versions identical",
                  inst->adapter_name);
...
static mesa_rc meba_poe_pd69200_firmware_upgrade(const meba_poe_ctrl_inst_t
                                                     *const  inst,
                                                 mesa_bool_t reset,
                                                 size_t      firmware_size,
                                                 const char *microsemi_firmware)
{
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO, "%s: Enter %d", __FUNCTION__,
          meba_poe_pd69200_get_chipset(inst));
    ...
    software_version_t tSoftware_version;
    mesa_rc            valid =
        meba_poe_pd69200_get_software_version(inst, &tSoftware_version);

    if (valid == MESA_RC_OK) { // Valid firmware exists
        // Section 5.1
        uint8_t download[] = {0x01,       0x01,       0xFF,       0x99,
                              0x15,       0x16,       0x16,       0x99,
                              DUMMY_BYTE, DUMMY_BYTE, DUMMY_BYTE, DUMMY_BYTE,
                              DUMMY_BYTE, 0x03,       0xFA};
        DEBUG(inst, MEBA_TRACE_LVL_DEBUG, "Valid firmware - start download");
        char *fname = "DOWNLOAD_COMMAND";
        if (MESA_RC_OK !=
            pd69200_tx_rx(inst, __FUNCTION__, __LINE__, download, fname)) {
    ...
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG,
          "Sending 'E' - erasing memory - Section 5.1 - step 3");
    buf[0] = 'E';
    (void)pd69200_wr(inst, buf, 1, "E");
    VTSS_MSLEEP(100);
    pd69200_firm_update_rd(inst, buf,
                           5); // read TOE\r\n - Section 5.1 - step 3;
    ...
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG,
          "Sending 'P' - program memory - Section 5.1 - step 5");
    buf[0] = 'P';
    ...
    while ((byte_cnt < firmware_size) && (*microsemi_firmware <= 127)) {
        ...
        if (progress != prev_progress) {
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "PoE Firmware update progress:%d %%", progress * 10);
        ...
        if (byteArr[0] != 'S') {
            DEBUG(inst, MEBA_TRACE_LVL_ERROR,
                  "Line should start with S byteArr[0]:0x%X", byteArr[0]);
            break;
        }

        if (byteArr[1] == '0') { // skip line that starts with "S0"
            DEBUG(inst, MEBA_TRACE_LVL_NOISE,
                  "Skipping lines starting with S0");
            ...
            DEBUG(inst, MEBA_TRACE_LVL_NOISE, "Writing new line - byte_cnt=%u",
                  byte_cnt);
            line[line_index++] = '\n';
            line[line_index] = 0;

            (void)pd69200_wr(inst, line, line_index,
            ...
            if (buf[0] == 'T' && buf[1] == '*') { // if read T*\r\n
                ...
                DEBUG(inst, MEBA_TRACE_LVL_INFO,
                      "PoE Firmware update progress:100 %%\n");
                break; // end of file
            } else {
            ...
            // Unknown Sx number
            DEBUG(inst, MEBA_TRACE_LVL_ERROR, "Unknown start value S=%d",
                  byteArr[1]);
            break;
        }
    }
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "POE_BOOL_Is_system_status=%d ,Telemetry_at_Boot_Up_Error=%d",
          ePOE_BOOL_Is_system_status, eTelemetry_at_boot_up_error);
    ...
    if (ePOE_BOOL_Is_system_status == true) {
        ...
        if (eTelemetry_at_boot_up_error != eBoot_Unknown_error) {
            ...
            poe_driver_private_t *private_data =
                (poe_driver_private_t *)(inst->private_data);
            private_data->IsBootError = true;
            private_data->eTelemetry_at_boot_up_error =
                eTelemetry_at_boot_up_error;
            memcpy(private_data->buf_rx, buf_rx, PD_BUFFER_SIZE);
        }
    }
    ...
    if (rc == MESA_RC_ERROR) {
        ...
        DEBUG(inst, MEBA_TRACE_LVL_INFO,
              "PD69200 No PoE chipset detected. rc=MESA_RC_ERROR");
        return MESA_RC_ERROR;
    }
...
mesa_rc meba_poe_ctrl_pd69200_prebt_globals_cfg_set(const meba_poe_ctrl_inst_t
                                                        *const inst,
                                                        ...
                                                        *cfg_global)
{
    ...
    if (chip_state != MEBA_POE_CHIPSET_FOUND) {
        DEBUG(inst, MEBA_TRACE_LVL_WARNING,
              "%s(%s): no poe chipset was found, chip_state=%d", __FUNCTION__,
              inst->adapter_name, chip_state);
        return MESA_RC_ERROR;
    }
    ...
    current_global_cfg->global_legacy_pd_class_mode =
        cfg_global->global_legacy_pd_class_mode;

    if ( // cfg_global->power_supply_poe_limit_w       != 0 &&
        current_global_cfg->power_supply_poe_limit_w ==
            cfg_global->power_supply_poe_limit_w &&
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "PREBT Update configuration for controller");
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        ...
        "supports_legacy_detection")); // Supports Legacy detection.
    ...
    MESA_RC(pd69200_get_power_supply_parameters(inst, &power_consumption_w,
    ...
    if ((guard_band != 0x2) ||
        ...
        MESA_RC(meba_poe_pd69200_set_power_banks(inst, bank, power_limit_w,
                                                 max_shutdown_voltage_dv,
                                                 min_shutdown_voltage_dv,
                                                 guard_band));
    }
...
mesa_rc meba_poe_ctrl_pd69200_prebt_port_cfg_set(const meba_poe_ctrl_inst_t
                                                     *const             inst,
                                                 meba_poe_port_handle_t handle,
                                                 meba_poe_port_cfg_t
                                                     *req_port_cfg)
{
    ...
    if (poe_controller[inst->index].save_poe_param_once && (handle == 0) &&
        ...
        DEBUG(inst, MEBA_TRACE_LVL_INFO,
              "%s, port=%d, save synced poe prebt parameters on startup",
              __FUNCTION__, handle);
    ...
    if (handle >= inst->port_map_length) {
        DEBUG(inst, MEBA_TRACE_LVL_ERROR, "%s Invalid handle=%d", __FUNCTION__,
              handle);
        return MESA_RC_ERROR;
    }
    ...
    if (!(inst->port_map[handle].capabilities & MEBA_POE_PORT_CAP_POE)) {
        DEBUG(inst, MEBA_TRACE_LVL_NOISE, "%s Not PoE port=%d", __FUNCTION__,
              handle);
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          ...
          handle, port_cfg->enable, req_port_cfg->enable,
          port_cfg->enable ? "Enabled" : "Disabled",
          req_port_cfg->enable ? "Enabled" : "Disabled", port_cfg->priority,
          req_port_cfg->priority,
          (port_cfg->priority == MEBA_POE_PORT_PD_POWER_PRIORITY_CRITICAL)
              ? "crit"
          : (port_cfg->priority == MEBA_POE_PORT_PD_POWER_PRIORITY_HIGH)
              ? "high"
              : "low",
          (req_port_cfg->priority == MEBA_POE_PORT_PD_POWER_PRIORITY_CRITICAL)
              ? "crit"
          : (req_port_cfg->priority == MEBA_POE_PORT_PD_POWER_PRIORITY_HIGH)
              ? "high"
              : "low",
          port_cfg->bPoe_plus_mode, req_port_cfg->bPoe_plus_mode,
    ...
    if ((current_port_status->port_status.port_type_prebt_af_at_poh !=
         port_type) ||
        ...
        DEBUG(inst, MEBA_TRACE_LVL_INFO,
              ...
              port_type, current_port_status->port_status.enable, enable);
        ...
        MESA_RC(meba_poe_pd69200_prebt_set_enable_disable_channels(
            inst, channel, enable, disable_legacy_support, port_type));
    }
    ...
    MESA_RC(meba_poe_pd69200_prebt_set_port_priority(inst, channel,
                                                     pd69200_priority));
...
mesa_rc meba_poe_ctrl_pd69200_do_reset(const meba_poe_ctrl_inst_t *const inst)
{
    uint8_t                      buf_rx[PD_BUFFER_SIZE];
    ...
    // check for emerging system status
    mesa_rc rc = get_15_bytes_comm_protocol_reply(inst, buf_rx, 255,
                                                  &bIs_poe_system_status,
                                                  &eTelemetry_at_boot_up_error);

    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "POE_BOOL_Is_system_status=%d ,Telemetry_at_Boot_Up_Error=%d",
          bIs_poe_system_status, eTelemetry_at_boot_up_error);
...
mesa_rc meba_poe_ctrl_pd69200_version_get(const meba_poe_ctrl_inst_t *const inst,
                                          uint32_t max_size,
                                          char    *value)
{
    software_version_t tSoftware_version;
    ...
    MESA_RC(meba_poe_pd69200_get_software_version(inst, &tSoftware_version));

    snprintf(
        value, max_size,
        "HW Ver.:%d, poe mcu type:%d, sw ver:%d.%d, param:%d, build=%d, internal sw#=%d, Asic Patch#=%d",
        tSoftware_version.hw_version, tSoftware_version.poe_mcu_type,
        tSoftware_version.sw_version_major, tSoftware_version.sw_version_minor,
        tSoftware_version.param_number, tSoftware_version.build_number,
        tSoftware_version.internal_sw_number,
        tSoftware_version.asic_patch_number);
...
{
    // --------------------- // PREBT individual masks - individual mask
    // configure by user defaults -------------------- //
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG,
          "PREBT individual masks - individual mask configure by user defaults:");
    ...
    // see section 4.5.10
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_PREBT_IGNORE_HIGHER_PRIORITY_0x00,
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_PREBT_SUPPORTS_BACKOFF_0x11,
        &im_prebt->prebt_supports_backoff, "PREBT- supports_backoff"));
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG, "PREBT- supports_backoff=%d",
          im_prebt->prebt_supports_backoff);

    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_PREBT_LED_STREAM_TYPE_0x16,
        &im_prebt->prebt_led_stream_type, "PREBT- LED_stream_type"));
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG, "PREBT- LED_stream_type=%d",
          im_prebt->prebt_led_stream_type);

    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_PREBT_MESSAGE_READY_NOTIFY_0x1E,
        &im_prebt->prebt_message_ready_notify, "PREBT- message_ready_notify"));
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        ...
        &im_prebt->prebt_layer2_lldp_enable, "PREBT- Layer2_LLDP_enable"));
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG, "PREBT- Layer2_LLDP_enable=%d",
          im_prebt->prebt_layer2_lldp_enable);

    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_PREBT_LAYER2_PRIORITY_BY_PD_0x2F,
        &im_prebt->prebt_layer2_priority_by_PD,
        "PREBT- layer2_priority_by_PD")); // get Allow the priority to be
                                          // fetched from lldp
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        ...
        &im_prebt->prebt_matrix_support_4p, "PREBT- matrix_support_4p"));
    ...
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG,
          "PREBT- class4at_AF_reported_as_class4=%d",
          im_prebt->prebt_class4at_af_reported_as_class4);

    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_PREBT_CLASS_0_EQUAL_AF_0x38,
        &im_prebt->prebt_class_0_equal_af, "PREBT- Class_0_equal_AF"));
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_PREBT_INVALID_SIG_COUNT_ONCE_0x55,
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_PREBT_PSE_POWERING_PSE_CHECKING_0x1F,
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_PREBT_ENABLE_ASIC_REFRESH_0x2A,
        &im_prebt->prebt_enable_asic_refresh, "PREBT- Enable_ASIC_Refresh"));
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG, "PREBT- Enable_ASIC_Refresh=%d",
          im_prebt->prebt_enable_asic_refresh);

    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_PREBT_CLASS_1_2_3_EQUAL_AF_0x39,
        &im_prebt->prebt_class_1_2_3_equal_af, "PREBT- Class_1_2_3_equal_AF"));
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG, "PREBT- Class_1_2_3_equal_AF=%d",
          im_prebt->prebt_class_1_2_3_equal_af);

    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_PREBT_LLDP_BEST_EFFORT_0x47,
        &im_prebt->prebt_lldp_best_effort, "PREBT- LLDP_best_effort"));
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG, "PREBT- LLDP_best_effort=%d",
          im_prebt->prebt_lldp_best_effort);

    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_PREBT_AUTO_ZONE2_PORT_ACTIVATION_0x49,
    ...
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG,
          "PREBT- HOCPP_high_over_current_pulse_protection=%d",
          im_prebt->prebt_hocpp_high_over_current_pulse_protection);

    return MESA_RC_OK;
}

mesa_rc print_indv_masks_bt(const meba_poe_ctrl_inst_t *const inst,
                            meba_poe_indv_mask_bt_t          *im_BT)
{
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        ...
        &im_BT->bt_ignore_high_priority, "BT- ignore_high_priority"));
    ...
    // of inactivity.
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_BT_I2C_RESTART_ENABLE_0x1B,
        &im_BT->bt_i2c_restart_enable, "BT- i2c_restart_enable"));
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        ...
        &im_BT->bt_pse_powering_pse_checking, "BT- pse_powering_pse_checking"));
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG, "BT- pse_powering_pse_checking=%d",
          im_BT->bt_pse_powering_pse_checking);

    // Enable port poe led.
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_BT_LED_STREAM_TYPE_0x20, &im_BT->bt_led_stream_type,
        "BT- led_stream_type"));
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG, "BT- led_stream_type=%d",
          im_BT->bt_led_stream_type);
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        ...
        "BT- blinks_at_invalid_signature"));
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(inst, INDV_MASK_BT_HOCPP_0x50,
                                                 &im_BT->bt_hocpp,
                                                 "BT- hocpp"));
 ...
 *succeed
 *---------------------------------------------------------------------*/
static mesa_rc meba_poe_pd69200_prebt_event_cause_get(const meba_poe_ctrl_inst_t
                                                          *const inst,
                                                      BT_Event_Cause_t
                                                          *ptBT_Event_Cause)
{
    // Send request to get bt event cause
    uint8_t buf[PD_BUFFER_SIZE] = {
        REQUEST_KEY, DUMMY_SEQ_NUM, GLOBAL_KEY, PREBT_EVENT_KEY, DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE,      DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE,      DUMMY_BYTE};
    ...
    meba_poe_status_t                *status)
{
    ...
    current_status->i2c_tx_error_counter =
        private_data->status.global.i2c_tx_error_counter;

    current_status->prod_number_detected =
        private_data->status.global.prod_number_detected;

    current_status->sw_version_major_detected =
        private_data->status.global.sw_version_major_detected;
    current_status->sw_version_minor_detected =
        private_data->status.global.sw_version_minor_detected;

    current_status->param_number_detected =
        private_data->status.global.param_number_detected;
    current_status->prod_number_from_file =
        private_data->status.global.prod_number_from_file;
    current_status->sw_version_from_file =
        private_data->status.global.sw_version_from_file;
    current_status->param_number_from_file =
        private_data->status.global.param_number_from_file;

    current_status->build_number = private_data->status.global.build_number;
    current_status->internal_sw_number =
        private_data->status.global.internal_sw_number;
    current_status->asic_patch_number =
        private_data->status.global.asic_patch_number;

    current_status->ePoE_Controller_Type =
        private_data->status.global.ePoE_Controller_Type;
    ...
    if (private_data->status.global.eDetected_poe_firmware_type ==
        MEBA_POE_FIRMWARE_TYPE_BT) {
        DEBUG(inst, MEBA_TRACE_LVL_DEBUG, "%s ,get BT globals_status info",
              __FUNCTION__);

        MESA_RC(meba_poe_pd69200_bt_event_cause_get(inst,
                                                    &(poe_controller[inst->index]
                                                          .tBT_event_cause)));
        current_status->vmain_out_of_range =
            poe_controller[inst->index]
                .tBT_event_cause.tSystem_event.vmain_fault;

        if (read_all_ports_once_at_startup < 3) {
            read_all_ports_once_at_startup++;
            DEBUG(inst, MEBA_TRACE_LVL_DEBUG,
                  "%s called line %d, force reading from all ports cycle: %d",
                  __FUNCTION__, __LINE__, read_all_ports_once_at_startup);
        ...
        MESA_RC(meba_poe_pd69200_get_total_power(
            inst, true, &(current_status->power_consumption_w),
            &(current_status->calculated_power_w),
            &(current_status->available_power_w),
            &(current_status->power_limit_w), &(current_status->power_bank),
            &(current_status->vmain_voltage_dv)));

        if (run_all_individual_masks_once_on_startup) {
            run_all_individual_masks_once_on_startup = false;
            MESA_RC(print_indv_masks_bt(inst, &(current_status
                                                    ->tPoe_individual_mask_info
                                                    .im_BT)));
        }
    } else if (private_data->status.global.eDetected_poe_firmware_type ==
               MEBA_POE_FIRMWARE_TYPE_PREBT) { // PREBT
        DEBUG(inst, MEBA_TRACE_LVL_DEBUG, "%s ,get PREBT globals_status info",
              __FUNCTION__);

        // lets check in which port
        MESA_RC(meba_poe_pd69200_prebt_event_cause_get(
            inst, &(poe_controller[inst->index].tBT_event_cause)));
        current_status->vmain_out_of_range =
            poe_controller[inst->index]
                .tBT_event_cause.tSystem_event
                .vmain_fault; // this bit is set to '1' when Vmain
                              // is out of range
        ...
        //----------------- get_total_power ---------------------------//
        MESA_RC(meba_poe_pd69200_get_total_power(
            inst, false, &(current_status->power_consumption_w),
            &(current_status->calculated_power_w),
            &(current_status->available_power_w),
            &(current_status->power_limit_w), &(current_status->power_bank),
            &(current_status->vmain_voltage_dv)));
        ...
        MESA_RC(meba_poe_pd69200_prebt_get_power_supply_measurements(
            inst, &(current_status->vmain_voltage_dv),
        ...
        DEBUG(inst, MEBA_TRACE_LVL_WARNING,
              "%s ,Unknown poe firmware type (PREBT/BT)", __FUNCTION__);
    }
...
static mesa_rc pd69200_active_matrix_verify(const meba_poe_ctrl_inst_t
                                                *const inst,
                                            uint8_t    prod_max_poe_ports)
{
    ...
    BOOL continue_to_update =
        TRUE; // detect the first unsigned port and stop the loop
    ...
    for (uint8_t i = 0; (i < POE_MAX_PORTS) && continue_to_update; i++) {
            ...
            if (inst->port_map[i].capabilities & MEBA_POE_PORT_CAP_POE) {
                ...
                if ((port_a != inst->port_map[i].phys_port_a) ||
                    ...
                    DEBUG(inst, MEBA_TRACE_LVL_INFO,
                          ...
                          __FUNCTION__, i, port_a,
                          inst->port_map[i].phys_port_a, port_b,
        ...
        if (bMatrixUpdateNeeded) {
            ...
            // ,port_a, port_b);
            MESA_RC(meba_poe_pd69200_set_temporary_matrix(inst, i, port_a,
                                                          port_b));
        }
    }

    if (bNeedToSaveMatrix) {
        DEBUG(inst, MEBA_TRACE_LVL_INFO, "%s ,matrix needs to be updated",
              __FUNCTION__);
        return MESA_RC_ERROR;
    } else {
...
mesa_rc meba_poe_ctrl_pd69200_do_detection(const meba_poe_ctrl_inst_t
                                               *const inst)
{
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG, "%s(%s)", __FUNCTION__,
          inst->adapter_name);
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "POE_BOOL_Is_system_status=%d ,Telemetry_at_Boot_Up_Error=%d ,rc=%d",
          ePOE_BOOL_Is_system_status, eTelemetry_at_boot_up_error, rc);

    if (ePOE_BOOL_Is_system_status == true) {
        DEBUG(inst, MEBA_TRACE_LVL_INFO,
              "System status on startup from app detected");
    ...
    if (rc == MESA_RC_ERROR) {
        // This is where we end if there are no PoE board detected.
        DEBUG(inst, MEBA_TRACE_LVL_INFO,
              "PD69200 No PoE chipset detected. Reason: rc: %d", rc);
        return rc;
    }
    ...
    if (rc == MESA_RC_ERROR) {
        // This is where we end if there are no PoE board detected.
        DEBUG(inst, MEBA_TRACE_LVL_INFO, "I2C driver error. Reason: rc: %d",
              rc);
        return rc;
    }
    ...
    if (rc == MESA_RC_OK) {
        ...
        case ePD69200M_PREBT: {
            private_data->status.global.eDetected_poe_firmware_type =
                MEBA_POE_FIRMWARE_TYPE_PREBT;
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "poe mcu type detected: PREBT firmware=%d",
                  tSoftware_version.poe_mcu_type);

            uint16_t pInterrupt_register;
            MESA_RC(meba_poe_pd69200_prebt_get_system_status(
                inst, &pInterrupt_register));
        ...
        case ePD69200M_BT: {
            private_data->status.global.eDetected_poe_firmware_type =
                MEBA_POE_FIRMWARE_TYPE_BT;
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "poe mcu type detected: BT firmware=%d",
                  tSoftware_version.poe_mcu_type);

            bt_system_status_t tBT_System_Status;
            MESA_RC(meba_poe_pd69200_bt_get_bt_system_status(inst,
                                                             &tBT_System_Status));
        ...
        default: {
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "Unknown detected poe mcu type=%d .\n",
                  tSoftware_version.poe_mcu_type);
            return MESA_RC_ERROR;
        }
        }
    ...
    if (rc == MESA_RC_ERROR) {
        ...
        DEBUG(inst, MEBA_TRACE_LVL_INFO, "%s(%s): STATUS OK", __FUNCTION__,
              inst->adapter_name);
        meba_poe_status_t *current_status =
            &(((poe_driver_private_t *)(inst->private_data))->status.global);
        MESA_RC(meba_poe_ctrl_pd69200_version_get(inst,
                                                  MEBA_POE_VERSION_STRING_SIZE,
                                                  current_status->version));
        MESA_RC(pd69200bt_get_serial_number(inst, &current_status->tSN));
    }

    DEBUG(inst, MEBA_TRACE_LVL_INFO, "%s(%s): exit", __FUNCTION__,
          inst->adapter_name);
    return MESA_RC_OK;
}
    ...
    int16_t                           restart_cause)
{
    ...
    //--- individual_masks ---//
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "%s(%s): syncing PREBT individual masks parameters", __FUNCTION__,
          inst->adapter_name);

    // Disable i2c ready interrupt
    uint8_t message_ready_notify;
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_PREBT_MESSAGE_READY_NOTIFY_0x1E, &message_ready_notify,
        "PREBT- message_ready_notify"));
    if (message_ready_notify !=
        tPoE_parameters.indv_mask_prebt_message_ready_notify_default) {
        MESA_RC(meba_poe_pd69200_set_individual_mask(
            inst, INDV_MASK_PREBT_MESSAGE_READY_NOTIFY_0x1E,
            tPoE_parameters.indv_mask_prebt_message_ready_notify_default));
        bChangedFlag = true;
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_PREBT_IGNORE_HIGHER_PRIORITY_0x00,
        &ignore_higher_priority, "PREBT- ignore_higher_priority"));
    if (ignore_higher_priority !=
        tPoE_parameters.indv_mask_prebt_ignore_higher_priority_default) {
        MESA_RC(meba_poe_pd69200_set_individual_mask(
            inst, INDV_MASK_PREBT_IGNORE_HIGHER_PRIORITY_0x00,
            tPoE_parameters.indv_mask_prebt_ignore_higher_priority_default));
        bChangedFlag = true;
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_PREBT_SUPPORTS_LEGACY_DETECTION_0x01,
        &supports_legacy_detection, "PREBT- supports_legacy_detection"));
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_PREBT_SUPPORTS_BACKOFF_0x11, &supports_backoff,
        "PREBT- supports_backoff"));
    if (supports_backoff !=
        tPoE_parameters.indv_mask_prebt_supports_backoff_default) {
        MESA_RC(meba_poe_pd69200_set_individual_mask(
            inst, INDV_MASK_PREBT_SUPPORTS_BACKOFF_0x11,
            tPoE_parameters.indv_mask_prebt_supports_backoff_default));
        bChangedFlag = true;
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_PREBT_LAYER2_LLDP_ENABLE_0x2E, &enable_LLDP_mode,
        "PREBT- enable_LLDP_mode"));
    if (enable_LLDP_mode !=
        tPoE_parameters.indv_mask_prebt_layer2_lldp_enable_default) {
        MESA_RC(meba_poe_pd69200_set_individual_mask(
            inst, INDV_MASK_PREBT_LAYER2_LLDP_ENABLE_0x2E,
            tPoE_parameters
                .indv_mask_prebt_layer2_lldp_enable_default)); // Enable LLDP
                                                               // mode.
        bChangedFlag = true;
    }
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_PREBT_LAYER2_PRIORITY_BY_PD_0x2F,
        &layer2_priority_by_PD,
        "PREBT- layer2_priority_by_PD")); // get Allow the priority to be
                                          // fetched from lldp
    if (layer2_priority_by_PD !=
        tPoE_parameters.indv_mask_prebt_layer2_priority_by_PD_default) {
        MESA_RC(meba_poe_pd69200_set_individual_mask(
            inst, INDV_MASK_PREBT_LAYER2_PRIORITY_BY_PD_0x2F,
            tPoE_parameters
                .indv_mask_prebt_layer2_priority_by_PD_default)); // Allow the
                                                                  // priority to
                                                                  // be fetched
                                                                  // from lldp
        bChangedFlag = true;
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_PREBT_MATRIX_SUPPORT_4P_0x34, &matrix_support_4p,
        "PREBT- matrix_support_4p")); // get matrix support 4p
    if (matrix_support_4p !=
        tPoE_parameters.indv_mask_prebt_matrix_support_4P_default) {
        MESA_RC(meba_poe_pd69200_set_individual_mask(
            inst, INDV_MASK_PREBT_MATRIX_SUPPORT_4P_0x34,
            tPoE_parameters
                .indv_mask_prebt_matrix_support_4P_default)); // Allow matrix to
                                                              // support 4p
        bChangedFlag = true;
    }
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_PREBT_LED_STREAM_TYPE_0x16, &led_stream_type,
        "PREBT- led_stream_type"));
    if (led_stream_type !=
        tPoE_parameters.indv_mask_prebt_led_stream_type_default) {
        MESA_RC(meba_poe_pd69200_set_individual_mask(
            inst, INDV_MASK_PREBT_LED_STREAM_TYPE_0x16,
            tPoE_parameters.indv_mask_prebt_led_stream_type_default));
        bChangedFlag = true;
    ...
    if (class4at_AF_reported_as_class4 !=
        ...
        MESA_RC(meba_poe_pd69200_set_individual_mask(
            inst, INDV_MASK_PREBT_CLASS4AT_AF_REPORTED_AS_CLASS4_0x12,
            prebt_individual_mask__class4at_AF_reported_as_class4_default)); // When the port is set to AF mode, class4 will be reported as class4, with class power limit of class0
        bChangedFlag = true;
    }

    uint8_t Class_0_equal_AF;
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_PREBT_CLASS_0_EQUAL_AF_0x38, &Class_0_equal_AF,
        "PREBT- Class_0_equal_AF"));
    if (Class_0_equal_AF != prebt_individual_mask__Class_0_equal_AF_default) {
        MESA_RC(meba_poe_pd69200_set_individual_mask(
            inst, INDV_MASK_PREBT_CLASS_0_EQUAL_AF_0x38,
            prebt_individual_mask__Class_0_equal_AF_default)); // Port that was
                                                               // detected as
                                                               // class 0 will
                                                               // behave based
                                                               // on port type
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_PREBT_SINGLE_DETECTION_FAIL_EVENT_0x46,
        &single_detection_fail_event, "PREBT- single_detection_fail_event"));
    if (single_detection_fail_event !=
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_PREBT_INVALID_SIG_COUNT_ONCE_0x55,
        &invalid_sig_count_once, "PREBT- invalid_sig_count_once"));
    if (invalid_sig_count_once !=
    ...
    if ((pm1 != tPoE_parameters.prebt_pm1_default) ||
        (pm2 != tPoE_parameters.prebt_pm2_default) ||
        (pm3 != tPoE_parameters.prebt_pm3_default)) {
        MESA_RC(meba_poe_pd69200_prebt_set_pm_method(
            inst, tPoE_parameters.prebt_pm1_default,
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "%s: power_supply_max_power_w=%d , power_limit_w=%d", __FUNCTION__,
    ...
    if (pd69200_active_matrix_verify(inst, inst->port_poe_length) !=
        MESA_RC_OK) {
        ...
        DEBUG(inst, MEBA_TRACE_LVL_INFO, "%s(%s): PROGRAM MATRIX DONE",
              __FUNCTION__, inst->adapter_name);
    }

    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "%s restart_cause= %d ,ChangedFlag= %d ,bMatrixUpdatedFlag: %d ",
          __FUNCTION__, restart_cause, bChangedFlag, bMatrixUpdatedFlag);
    ...
    if (bMatrixUpdatedFlag ||
        ((restart_cause == 0) &&
         (bChangedFlag == true))) // cold start and somthing was modified
    {
...
char *get_prebt_title_by_ports_status(uint8_t bt_port_status)
{
    ...
    case PREBT_0x41_OFF_CHIP_OVER_POWER: return "0x91: CHIP OVER POWER";
...
int findRangeIndex(double power)
{
    ...
    return class_numRanges -
           1; // If power exceeds the maximum range, return the last index
}
...
{
    if (port_status == NULL) {
        DEBUG(inst, MEBA_TRACE_LVL_ERROR, "%s port_status is NULL pointer",
              __FUNCTION__);
        return MESA_RC_ERROR;
    }
    ...
    current_port_status->port_status.chip_state =
        meba_poe_pd69200_get_chipset(inst);
    port_status->chip_state = current_port_status->port_status.chip_state;

    if (current_port_status->port_status.chip_state ==
        ...
        current_port_status->port_status.assigned_pd_class_a =
            POE_UNDETERMINED_CLASS;
        current_port_status->port_status.assigned_pd_class_b =
            POE_UNDETERMINED_CLASS;
        current_port_status->port_status.pd_structure =
    ...
    if (current_port_status->port_status.chip_state != MEBA_POE_CHIPSET_FOUND) {
        DEBUG(inst, MEBA_TRACE_LVL_DEBUG,
              "%s Device not ready for reading, chip_state=%d", __FUNCTION__,
              current_port_status->port_status.chip_state);
        return MESA_RC_ERROR;
    }
    ...
    if (handle >= inst->port_poe_length) {
        DEBUG(inst, MEBA_TRACE_LVL_DEBUG,
              "%s handle >= inst->port_poe_length \n", __FUNCTION__);
        return MESA_RC_OK;
    }

    // not a poe port
    if (!(inst->port_map[handle].capabilities & MEBA_POE_PORT_CAP_POE)) {
        DEBUG(inst, MEBA_TRACE_LVL_DEBUG, "%s Not PoE port=%d", __FUNCTION__,
              handle);
        return MESA_RC_ERR_PARM;
    }
    ...
    if (poe_controller[inst->index]
            .tBT_event_cause.all_ports_event_cause[handle]) {
        has_event = TRUE;
        DEBUG(inst, MEBA_TRACE_LVL_INFO, "%s port= %d, has port event cause",
              __FUNCTION__, handle);
    }
    ...
    if (poe_controller[inst->index].lldp_ports_event[handle]) {
        ...
        has_event = TRUE;
        DEBUG(inst, MEBA_TRACE_LVL_INFO, "%s port= %d, has lldp port event",
              __FUNCTION__, handle);
    }

    // in case that port was configured - lets read all status info
    if (poe_controller[inst->index].conf_all_ports_event_cause[handle]) {
        poe_controller[inst->index].conf_all_ports_event_cause[handle] =
            false; // reset flag
        has_event = TRUE;
        DEBUG(inst, MEBA_TRACE_LVL_INFO, "%s port= %d, has config event",
              __FUNCTION__, handle);
    }
    ...
    if (current_port_status->port_status.poe_internal_port_status ==
        ...
        has_event = TRUE;
        DEBUG(inst, MEBA_TRACE_LVL_INFO,
              "%s port= %d, has internal temporary disabled status=%s",
              __FUNCTION__, handle,
              get_prebt_title_by_ports_status(current_port_status->port_status
                                                  .poe_internal_port_status));
    }
    ...
    if (has_event) {
        DEBUG(inst, MEBA_TRACE_LVL_INFO, "%s port= %d, has_event", __FUNCTION__,
              handle);
    }
    ...
    if (bRead_port_status_and_port_class) {
        ...
        MESA_RC(meba_poe_pd69200_prebt_get_extended_port_status(
            inst, handle, &tPrebt_extended_port_status));
        ...
        current_port_status->port_status.bt_port_counters
        ...
        {
            current_port_status->port_status.power_requested_mw =
                POH_class_values[tPrebt_extended_port_status.class_num] *
                1000.0;
        } else {
            current_port_status->port_status.power_requested_mw = 0;
            DEBUG(inst, MEBA_TRACE_LVL_DEBUG,
                  "%s port= %d, tPrebt_single_port_status.af_at_poh value error=%d",
                  __FUNCTION__, handle, tPrebt_single_port_status.af_at_poh);
        }

        strncpy(current_port_status->port_status.poe_port_status_description,
                get_prebt_title_by_ports_status(tPrebt_extended_port_status
                                                    .port_status),
                MAX_STR_SIZE - 1);
        ...
        // will get power
        case PREBT_0x11_OFF_PWR_UP_IN_PROCESS: // should not happend since HC08
                                               // starts faster
        case PREBT_0x1B_OFF_DET_IN_PROCESS:    // Port Status #27 ------------
                                               // NORMAL SITUATION --------
        ...
        // Port was FORCED to ON, and due to some error  it enter to TEST-ERROR
        // state, or usual errors
        case PREBT_0x1F_OFF_OVERLOAD_OR_CAP_TO_BIG:
        case PREBT_0x25_OFF_IMPROPER_CAP_DET_MODE:
        case PREBT_0x31_OFF_FORCE_802_3AF_OVERLOAD:
        ...
        case PREBT_0x39_OFF_FORCE_PWR_ERR_CH_OVER_TEMP:
        case PREBT_0x3A_OFF_FORCE_PWR_ERR_CHIP_OVER_TEMP:
        case PREBT_0x3C_OFF_PWR_MANAGEMENT_STATIC:     // Port Status #60 - LED
                                                       // blinks at 2Hz rate
        case PREBT_0x3D_OFF_PWR_MANAGEMENT_STATIC_OV:  // Port Status #61 - LED
                                                       // blinks at 2Hz rate
        case PREBT_0x3E_OFF_FORCE_PWR_ERR_MNGMNT_STAT: // Port Status #62 - LED
                                                       // blinks at 2Hz rate
        case PREBT_0x3F_OFF_FORCE_PWR_ERR_MNGMNT_STAT_OV: // Port Status #63 -
                                                          // LED blinks at 2Hz
                                                          // rate
        ...
        default: {
            ...
            DEBUG(inst, MEBA_TRACE_LVL_ERROR,
                  "Port %d: Unknown port state= 0x%X",
        ...
        case PREBT_0x4A_OFF_RECOVERY_OVL:           {
            current_port_status->port_status.meba_poe_port_state =
                MEBA_POE_PD_OVERLOAD;
            break;
        }

        case PREBT_0x20_OFF_PWR_BUDGET_EXCEDDED: {
            current_port_status->port_status.meba_poe_port_state =
                MEBA_POE_POWER_BUDGET_EXCEEDED;
            break;
        }
        ...
        if ((current_port_status->port_status.poe_internal_port_status ==
            ...
            current_port_status->port_status.is_fault_link_without_power =
                false;
        }
    ...
    if (bRead_port_measurements) {
        if (current_port_status->port_status.meba_poe_ieee_port_state ==
            MEBA_POE_IEEE_PORT_STATE_DELIVERING_POWER) {
            meba_poe_pd69200_prebt_get_port_measurements(inst, handle,
                                                         &main_voltage,
                                                         &calculated_current,
    ...
    if (bRead_lldp_pse_data) {
        MESA_RC(meba_poe_pd69200_prebt_get_port_layer2_lldp_pse_data(
            inst, handle, &tPrebt_port_lldp_pse_data));
        ...
        current_port_status->port_status.pse_data.pse_power_type =
            tPrebt_port_lldp_pse_data
                .pse_power_type; // : Layer 2 PSE type as specified in the
                                 // IEEE802.3at standard
        current_port_status->port_status.pse_data.power_class =
            tPrebt_port_lldp_pse_data
                .power_class; // class0 -> 1 , class1 -> 2 ...
        current_port_status->port_status.pse_data.pse_power_pair =
            tPrebt_port_lldp_pse_data
                .pse_power_pair; // Data Pair -> 1. Spare Pair -> 2.
        current_port_status->port_status.pse_data.cable_len =
            tPrebt_port_lldp_pse_data.cable_len;
        ...
        if (tPrebt_port_lldp_pse_data.layer2_request_pending == 0) {
            // Layer2 LLDP/CDP request executed - no need to read lldp pse data
            // again
            poe_controller[inst->index].lldp_ports_event[handle] = FALSE;
        }
    }

    DEBUG(inst, MEBA_TRACE_LVL_NOISE, "CH=%d ,UNG status=%d ,int status=0x%X",
          handle, current_port_status->port_status.meba_poe_port_state,
          current_port_status->port_status.poe_internal_port_status);
                                         ...
                                         char    *msg)
{
    ...
    if (!pd69200_check_sum_ok(rx_buf)) {
        ...
        DEBUG(inst, MEBA_TRACE_LVL_INFO, "%s Failed, Invalid checksum: %s\n",
              __FUNCTION__,
              print_as_hex_string(rx_buf, PD_BUFFER_SIZE, dbg_txt,
                                  sizeof(dbg_txt)));
                                     ...
                                     char                             *msg)
{
    ...
    if (((tx_buf[0] == COMMAND_KEY || tx_buf[0] == PROGRAM_KEY)) &&
        (rx_buf[0] != REPORT_KEY)) {
        DEBUG(inst, MEBA_TRACE_LVL_WARNING,
              "%s COMMAND_KEY or PROGRAM_KEY should be answered with REPORT_KEY \n",
              __FUNCTION__);
    } else if ((tx_buf[0] == REQUEST_KEY) &&
               ((rx_buf[0] != TELEMETRY_KEY) && (rx_buf[0] != REPORT_KEY))) {
        DEBUG(inst, MEBA_TRACE_LVL_WARNING,
              "%s REQUEST_KEY should be answered with TELEMETRY_KEY or REPORT_KEY \n",
              __FUNCTION__);
    }
...
//
mesa_rc meba_poe_ctrl_pd69200_debug(const meba_poe_ctrl_inst_t *const inst,
                                    char                             *var,
                                    uint32_t                          str_len,
                                    char                             *title,
                                    char                             *tx_str,
                                    ...
                                    int max_msg_buf_size)
{
        ...
        && (args_count == 1))                // 13 or 15 bytes
    {
        ...
        if (!is_valid_hex_string(pStrTmp)) {
            strncpy(msg,
                    "  Arguments string is not valid Hex or Dec number \n\r",
                    max_msg_buf_size);
            return MESA_RC_ERROR;
        }
        ...
        for (int i = 0; i < (args_str_len / 2); i++) {
            ...
            DEBUG(inst, MEBA_TRACE_LVL_NOISE, "args_buf %d: %02X", i,
                  args_buf[i]);
        }
    ...
    // [<Num>] [<Num>] [<Num>] [<Num>] [<Num>]
        ...
        for (char *p_arg = strtok(s, " "); p_arg != NULL;
             p_arg = strtok(NULL, " ")) {
    ...
    sprintf(title,
            "              -ky-ec-s1-s2-s3-d0-d1-d2-d3-d4-d5-d6-d7-c1-c2-");

    int iBufIndex = 15;
    sprintf(tx_str, "        TX   : ");

    for (int i = 0;
         ((i < PD_BUFFER_SIZE) && ((iBufIndex + 3) < max_msg_buf_size)); i++) {
    ...
    if (pd69200_rd(inst, rx_buf, PD_BUFFER_SIZE) == MESA_RC_OK) {
        ...
        sprintf(rx_str, "        RX   : ");
        for (int i = 0;
             ((i < PD_BUFFER_SIZE) && ((iBufIndex + 3) < max_msg_buf_size));
             i++) {
        ...
        sprintf(msg, "%s Could not get 15bytes data from PoE MCU.",
                POEBT_TITLE);
        // T_WG_PORT(VTSS_TRACE_POEBT_GRP_CUSTOM, port_index, "%s", "Could not
        // read response.");
    ...
    meba_poe_port_cap_t              *capabilities)
{
    ...
    if (chip_state != MEBA_POE_CHIPSET_FOUND) {
        DEBUG(inst, MEBA_TRACE_LVL_WARNING,
              "%s(%s): Device not ready for reading, chip_state=%d",
...
mesa_rc meba_poe_ctrl_pd69200_firmware_upgrade(const meba_poe_ctrl_inst_t
                                                   *const  inst,
                                               mesa_bool_t reset,
                                               uint32_t    size,
                                               const char *data)
...
mesa_rc meba_poe_ctrl_pd69200_prepare_firmware_upgrade(const meba_poe_ctrl_inst_t
                                                           *const inst,
                                                       mesa_bool_t version_check,
                                                       uint32_t    size,
                                                       const char *data)
{
    return meba_poe_pd69200_prepare_firmware_upgrade(inst, version_check, size,
                                                     data);
}
    ...
    meba_poe_pd_data_t               *pd_data)
{
    ...
    default:
        DEBUG(inst, MEBA_TRACE_LVL_ERROR, "%s Invalid type parameter %d",
              __FUNCTION__, pd_data->type);
        return MESA_RC_ERR_PARM;
    }
    ...
    default:
        DEBUG(inst, MEBA_TRACE_LVL_ERROR, "%s Invalid source parameter %d",
              __FUNCTION__, pd_data->source);
        return MESA_RC_ERR_PARM;
    }
    ...
    default:
        DEBUG(inst, MEBA_TRACE_LVL_ERROR, "%s Invalid prio parameter %d",
              __FUNCTION__, pd_data->prio);
        return MESA_RC_ERR_PARM;
    }

    MESA_RC(meba_poe_pd69200_prebt_set_port_layer2_lldp_pd_data(
        inst, handle, type << 6 | source << 4 | prio,
        pd_data->pd_requested_power_mw /
            100, // convert from milliwatt to deciwatt
        pd_data->pse_allocated_power_mw /
            100, // convert from milliwatt to deciwatt
    ...
    meba_poe_port_handle_t            handle)
{
    ...
    return meba_poe_pd69200_prebt_set_port_layer2_lldp_pd_data(inst, handle,
                                                               pd69200_priority,
                                                               0, 0, 0, 3);
}
                              ...
                              meba_poe_parameters_t tMeba_poe_parameters)
{
    ...
    static meba_poe_ctrl_api_t meba_pd69200_api = {
        ...
        .meba_poe_ctrl_port_status_get =
            meba_poe_ctrl_pd69200_prebt_port_status_get,
        ...
        .meba_poe_ctrl_firmware_upgrade =
            meba_poe_ctrl_pd69200_firmware_upgrade,
        .meba_poe_ctrl_prepare_firmware_upgrade =
            meba_poe_ctrl_pd69200_prepare_firmware_upgrade,
        .meba_poe_ctrl_port_capabilities_get =
            meba_poe_ctrl_pd69200_port_capabilities_get,
        .meba_poe_ctrl_port_pd_bt_data_set =
            meba_poe_ctrl_pd69200_prebt_port_pd_bt_data_set,
        .meba_poe_ctrl_port_pd_data_set =
            meba_poe_ctrl_pd69200_prebt_port_pd_data_set,
        .meba_poe_ctrl_port_pd_data_clear =
            meba_poe_ctrl_pd69200_prebt_port_pd_data_clear,
    };
    ...
    private_data->cfg.ports =
        malloc(sizeof(meba_poe_port_cfg_t) * port_map_length);
    private_data->cfg_POEMCU.ports =
        malloc(sizeof(meba_poe_port_cfg_t) * port_map_length);
    private_data->status.ports =
        malloc(sizeof(meba_poe_port_private_status_t) * port_map_length);

    memset(private_data->cfg.ports, 0,
           sizeof(*(private_data->cfg.ports)) * port_map_length);
    memset(private_data->cfg_POEMCU.ports, 0,
           sizeof(*(private_data->cfg_POEMCU.ports)) * port_map_length);
    memset(private_data->status.ports, 0,
           sizeof(*(private_data->status.ports)) * port_map_length);
    ...
    private_data->cfg.global.power_supply_poe_limit_w =
        tMeba_poe_parameters.poe_init_params
            .power_supply_max_power_w; // Make sure initial value is different
                                       // from configured value as configuration
                                       // only is applied when values are
                                       // different.
...
static mesa_rc meba_poe_pd69200_bt_set_BT_port_parameters(
    const meba_poe_ctrl_inst_t *const inst,
    uint8_t                           channel,
    uint8_t                           cfg1,
    uint8_t                           cfg2,
    ...
    uint8_t                           priority)
{
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "[%s] CH=%d ,CFG1=%d ,CFG2=%d ,operation mode=0x%X ,add_power=%d ,priority=%d",
          fname, channel, cfg1, cfg2, operation_mode, add_power_for_port_mode,
          priority);

    return MESA_RC_OK;
}

static mesa_rc meba_poe_pd69200_bt_get_BT_port_parameters(
    const meba_poe_ctrl_inst_t *const inst,
    uint8_t                           handle,
    meba_poe_port_cfg_t              *cfg_POEMCU)
{
    // Transmit the command
    uint8_t buf[PD_BUFFER_SIZE] = {
        REQUEST_KEY, DUMMY_SEQ_NUM, CHANNEL_KEY, BT_PORT_CONFIG1_KEY,
        handle,      DUMMY_BYTE,    DUMMY_BYTE,  DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE,  DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE};
    ...
    cfg_POEMCU->enable =
        ((buf[3] & 0xF) > 0) ? 1 : 0; // bits[3..0] ? values 1,2
    cfg_POEMCU->ignored_inrush_check =
        ((buf[3] & 0xF) == 2) ? 1 : 0; // bits[3..0] ? value 2
    ...
    cfg_POEMCU->class_error_selection =
        (buf[4] >> 4) & 0xF; // bits[7..4] ? Class Error Operation Select

    cfg_POEMCU->bt_port_operation_mode = buf[5];
    cfg_POEMCU->bPoe_plus_mode =
        (buf[5] >= 0x10) ? true : false; // val < 0x10 = Compliant Modes, val >=
                                         // 0x10 = Non-Compliant Modes

    cfg_POEMCU->add_power_for_port_mode_dW = buf[6];
    cfg_POEMCU->priority = buf[7];

    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "[%s] CH=%d ,EnDis=%d ,IgInr=%d ,IgAuClass=%d ,PM_mode=0x%X ,CLS_err=0x%X ,Oper Mode=0x%X ,bPoe_plus_mode=%d ,Add pwr=%d ,Priority=0x%X",
          ...
          cfg_POEMCU->class_error_selection, cfg_POEMCU->bt_port_operation_mode,
          cfg_POEMCU->bPoe_plus_mode, cfg_POEMCU->add_power_for_port_mode_dW,
          cfg_POEMCU->priority);

    return MESA_RC_OK;
}

uint8_t get_port_type_operation_mode(uint8_t bt_pse_port_type,
                                     uint8_t bt_en,
                                     uint8_t legacy_pd_class_mode)
{
    ...
    return prod.port_type_operation_mode[bt_pse_port_type]
                                        [col_ind]; // pse_type , bt/legacy
}
 ...
 *classification (which result in port maximum power). BT_PORT_MAX_POWER_DEFAULT
 *is the enum for PoE port max power 15/30/60/90
 *
 * Return: void
 *********************************************************************/
void Set_BT_ParamsByOperationMode(meba_poe_ctrl_inst_t *inst)
{
    ...
    // 15W
    prod.port_type_operation_mode[0][0] =
        tPoE_parameters
            .bt_operation_mode_compliant_15w_default; // BT compliant 15w
    prod.port_type_operation_mode[0][1] =
        tPoE_parameters
            .bt_operation_mode_none_compliant_15w_default; // BT non compliant
                                                           // 15w legacy
    prod.port_type_operation_mode[0][2] =
        BT_Non_Compliant_4P_15w_2P_15w_Legacy; // poh - BT 15W legacy
    prod.port_type_operation_mode[0][3] =
        BT_Non_Compliant_4P_15w_2P_15w_Legacy; // ignore-pd-class - BT 15W legacy

    // 30W
    prod.port_type_operation_mode[1][0] =
        tPoE_parameters
            .bt_operation_mode_compliant_30w_default; // BT compliant 30w
    prod.port_type_operation_mode[1][1] =
        tPoE_parameters
            .bt_operation_mode_none_compliant_30w_default; // BT non compliant
                                                           // 30w legacy
    prod.port_type_operation_mode[1][2] =
        BT_Non_Compliant_4P_30w_2P_30w_Legacy; // poh - BT 30W legacy
    prod.port_type_operation_mode[1][3] =
        BT_Non_Compliant_4P_30w_2P_30w_Legacy; // ignore-pd-class - BT 30W legacy

    // 60W
    prod.port_type_operation_mode[2][0] =
        tPoE_parameters
            .bt_operation_mode_compliant_60w_default; // BT compliant 60w
    prod.port_type_operation_mode[2][1] =
        tPoE_parameters
            .bt_operation_mode_none_compliant_60w_default; // BT non compliant
                                                           // 60w legacy
    prod.port_type_operation_mode[2][2] =
        BT_Non_Compliant_4P_60w_2P_30w_Legacy; // poh - BT 60W legacy
    prod.port_type_operation_mode[2][3] =
        tPoE_parameters
            .bt_operation_mode_legacy_60W_ignore_pd_class_default; // ignore-pd-class

    // 90W
    prod.port_type_operation_mode[3][0] =
        tPoE_parameters
            .bt_operation_mode_compliant_90w_default; // BT compliant 90w
    prod.port_type_operation_mode[3][1] =
        tPoE_parameters
            .bt_operation_mode_none_compliant_90w_default; // BT non compliant
                                                           // 90w legacy
    prod.port_type_operation_mode[3][2] =
        tPoE_parameters.bt_operation_mode_legacy_90W_poh_default; // poh
    prod.port_type_operation_mode[3][3] =
        tPoE_parameters
            .bt_operation_mode_legacy_90W_ignore_pd_class_default; // ignore-pd-class
    ...
    prod.port_max_pwr_per_type_mW =
        poe_port_mode_max_power_w[prod.poe_port_mode_max_power_index] * 1000;
}

// 'Get BT Port Status' driver
static mesa_rc meba_poe_pd69200_bt_get_BT_port_status(
    const meba_poe_ctrl_inst_t *const inst,
    uint8_t                           channel,
    uint8_t                          *port_status,
    uint8_t                          *enable,
...
{
    uint8_t buf[PD_BUFFER_SIZE] = {
        REQUEST_KEY, DUMMY_SEQ_NUM, CHANNEL_KEY, BT_PORT_STATUS_KEY,
        channel,     DUMMY_BYTE,    DUMMY_BYTE,  DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE,  DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE};
    ...
    *port_event = buf[10];
    ...
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG,
          "[%s] CH=%d ,port status=0x%X ,enable=%d ,assigned class=%d ,measured port pwr=%lu ,last shutdown err status=0x%X ,port event=%d",
          fname, channel, *port_status, *enable, *assigned_class,
          *measured_port_power, *last_shutdown_error_status, *port_event);

    return MESA_RC_OK;
}

// 'Get BT Port Class' driver
static mesa_rc meba_poe_pd69200_bt_get_BT_port_class(
    const meba_poe_ctrl_inst_t *const inst,
    uint8_t                           channel,
    uint8_t                          *port_status,
...
{
    uint8_t buf[PD_BUFFER_SIZE] = {
        REQUEST_KEY, DUMMY_SEQ_NUM, CHANNEL_KEY, BT_PORT_CLASS_KEY, channel,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE,  DUMMY_BYTE,        DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE,  DUMMY_BYTE,        DUMMY_BYTE};
    ...
    *assigned_power_dW = (buf[9] << 8) + buf[10];
    uint16_t Auto_Class_Measurement_plus_Support_Result =
        (buf[11] << 8) + buf[12];

    *auto_class_measurement_dw = (Auto_Class_Measurement_plus_Support_Result &
                                  0xFFF); // AutoClass Measurement: Bits [11..0]
    *auto_class_support = ((Auto_Class_Measurement_plus_Support_Result >> 12) &
                           0xF); // AutoClass Support: Bits [15..12]

    DEBUG(inst, MEBA_TRACE_LVL_DEBUG,
          "[%s] CH=%d ,port status=0x%X ,port phy info=0x%X ,measured class=0x%X ,requested class=0x%X ,requested pwr_dW=%lu ,assigned_class=0x%X, assigned_power_dW=%lu ,AutoClass measure=%lu ,AutoClass Support=%d",
          fname, channel, *port_status, *port_phy_info, *measured_class,
          *requested_class, *requested_power_dW, *assigned_class,
          *assigned_power_dW, *auto_class_measurement_dw, *auto_class_support);
    ...
    uint16_t                         *port_voltage_dV)
{
    ...
    // Send request to get status
    unsigned char buf[PD_BUFFER_SIZE] = {
        REQUEST_KEY, DUMMY_SEQ_NUM, CHANNEL_KEY, BT_PORT_MEASE_KEY, channel,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE,  DUMMY_BYTE,        DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE,  DUMMY_BYTE,        DUMMY_BYTE};
    ...
    *port_voltage_dV = (buf[9] << 8) + buf[10];
    ...
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG,
          "[%s] CH=%d ,main voltage=%lu[dv] ,calculated current=%lu[mA] ,port pwr consumption=%lu[dW] ,port voltage=%lu[dV]",
          fname, channel, *main_voltage_dv, *calculated_current_mA,
...
mesa_rc meba_poe_ctrl_pd69200_bt_port_cfg_set(const meba_poe_ctrl_inst_t
                                                  *const             inst,
                                              meba_poe_port_handle_t handle,
                                              meba_poe_port_cfg_t *req_port_cfg)
{
    ...
    if (req_port_cfg->cable_length != port_cfg->cable_length) {
        if (port_cfg->cable_length != 255)
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "driver updating cable_length on CH=%d changed from %d to %d",
                  handle, port_cfg->cable_length, req_port_cfg->cable_length);
        else
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "driver cable_length on CH=%d is %d", handle,
    ...
    uint8_t req_bt_port_operation_mode =
    ...
    if (req_port_cfg->enable) {
        req_bt_port_operation_mode = get_port_type_operation_mode(
            req_port_cfg->bt_pse_port_type, !(req_port_cfg->bPoe_plus_mode),
            global_cfg->global_legacy_pd_class_mode); // pse_type , bt/legacy ,
                                                      // legacy_pd_class_mode

        // check if port_operation_mode was changed
        if (port_cfg_POEMCU->bt_port_operation_mode !=
    ...
    MESA_RC(meba_poe_pd69200_bt_get_BT_port_parameters(inst, handle,
                                                       port_cfg_POEMCU));
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "[ReqPortCfg_______] CH=%d ,EnDis=%d ,IgAutoClass=%d ,bt_pm_mode=%d ,bPoe_plus_mode=%d ,type=%d ,Priority=%d ,oper mode: 0x%X",
          handle, req_port_cfg->enable,
          req_port_cfg->ignore_pd_auto_class_request,
          req_port_cfg->bt_port_pm_mode, req_port_cfg->bPoe_plus_mode,
          req_port_cfg->bt_pse_port_type, req_port_cfg->priority,
          req_bt_port_operation_mode);

    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "[PortCfg__________] CH=%d ,EnDis=%d ,IgAutoClass=%d ,bt_pm_mode=%d ,bPoe_plus_mode=%d ,type=%d ,Priority=%d ,oper mode: 0x%X",
          handle, port_cfg->enable, port_cfg->ignore_pd_auto_class_request,
          port_cfg->bt_port_pm_mode, port_cfg->bPoe_plus_mode,
          port_cfg->bt_pse_port_type, port_cfg->priority,
          port_cfg_POEMCU
              ->bt_port_operation_mode); // current bt_port_operation_mode

    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "Update ports configuration for controller port %d", handle);

    mesa_bool_t bParamChanged =
        false; // Flag which indicate that we need to update the change into the
               // POEMCU. We need to send "Set BT Port Parameters"
    ...
    if (req_port_cfg->enable) {
        ...
        port_cfg->bt_port_operation_mode =
            req_bt_port_operation_mode; // composed from: pse_type , bt/legacy ,
                                        // legacy_pd_class_mode
        port_cfg->class_error_selection =
            prod.class_error_selection[port_cfg->bt_en];
    }
    ...
    port_cfg->ignore_pd_auto_class_request =
        req_port_cfg
            ->ignore_pd_auto_class_request; //- set by AutoClass global var
    Special_Port_Configuration_Bits_4_7 =
        req_port_cfg->ignore_pd_auto_class_request;
    ...
    if (port_cfg_POEMCU->bt_port_operation_mode !=
        port_cfg->bt_port_operation_mode) {
        DEBUG(inst, MEBA_TRACE_LVL_INFO,
              "* Change POEMCU CH-%02d Op mode from 0x%X to 0x%X", handle,
              port_cfg_POEMCU->bt_port_operation_mode,
              port_cfg->bt_port_operation_mode);
    ...
    if ((port_cfg_POEMCU->enable != port_cfg->enable) ||
        ((port_cfg_POEMCU->ignored_inrush_check !=
          port_cfg->ignored_inrush_check) &&
         port_cfg->enable)) {
        if (port_cfg_POEMCU->enable != port_cfg->enable) {
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "* Change POEMCU CH-%02d ON/OFF from %d to %d", handle,
                  port_cfg_POEMCU->enable, port_cfg->enable);
        }

        if (port_cfg_POEMCU->ignored_inrush_check !=
            port_cfg->ignored_inrush_check) {
            DEBUG(inst, MEBA_TRACE_LVL_INFO,
                  "* Change POEMCU CH-%02d Ignore inrush from %d to %d", handle,
                  port_cfg_POEMCU->ignored_inrush_check,
                  port_cfg->ignored_inrush_check);
        }
        ...
        if (port_cfg->enable &&
            port_cfg->ignored_inrush_check) { // in case Enabled and Legacy - 2
    ...
    if (port_cfg_POEMCU->ignore_pd_auto_class_request !=
        global_cfg->global_ignore_pd_auto_class_request) {
        DEBUG(inst, MEBA_TRACE_LVL_INFO,
              "* Change POEMCU CH-%02d Ignore Auto Class from %d to %d", handle,
              port_cfg_POEMCU->ignore_pd_auto_class_request,
              global_cfg->global_ignore_pd_auto_class_request);
        bParamChanged = true;
    ...
    if (port_cfg_POEMCU->bt_port_pm_mode != port_cfg->bt_port_pm_mode) {
        DEBUG(inst, MEBA_TRACE_LVL_INFO,
              "* Change POEMCU CH-%02d bt_port_pm_mode from %d to %d", handle,
              port_cfg_POEMCU->bt_port_pm_mode, port_cfg->bt_port_pm_mode);

        BT_port_PM_mode = port_cfg->bt_port_pm_mode;
        bParamChanged = true;
    }

    if (port_cfg->enable) // update only if port is active - otherwise - set "no
                          // change"
    {
        if ((port_cfg_POEMCU->class_error_selection !=
             prod.class_error_selection[port_cfg->bt_en]) ||
            (port_cfg->class_error_selection !=
             prod.class_error_selection[port_cfg->bt_en])) {
            if (port_cfg_POEMCU->class_error_selection !=
                prod.class_error_selection[port_cfg->bt_en]) {
                DEBUG(inst, MEBA_TRACE_LVL_INFO,
                      "* Change POEMCU CH-%02d class_error_selection from %d to %d",
                      handle, port_cfg_POEMCU->class_error_selection,
                      prod.class_error_selection[port_cfg->bt_en]);
            }

            if (port_cfg->class_error_selection !=
                prod.class_error_selection[port_cfg->bt_en]) {
                DEBUG(inst, MEBA_TRACE_LVL_INFO,
                      "* Set Config CH-%02d class_error_selection from %d to %d",
                      handle, port_cfg->class_error_selection,
                      prod.class_error_selection[port_cfg->bt_en]);
            }
    ...
    if (port_cfg_POEMCU->priority != port_cfg->priority) {
        DEBUG(inst, MEBA_TRACE_LVL_INFO,
              "* Change POEMCU CH-%02d Priority from %d to %d", handle,
    ...
    if (bParamChanged == true) {
        DEBUG(inst, MEBA_TRACE_LVL_INFO,
              "[CFG-SET BT PORT PARAMS] CH=%d ,En=%d ,IgInChk=%d ,IgAuClass=%d ,PM_mode=%d ,Class err=%d ,Op mode=0x%X ,bPoe_plus_mode=%d ,type=%d ,Add pwr=%d ,Priority=%d",
              ...
              port_cfg->bPoe_plus_mode, port_cfg->bt_pse_port_type,
        ...
        MESA_RC(meba_poe_pd69200_bt_set_BT_port_parameters(
            inst, handle, CFG1, CFG2, BT_port_Operation_Mode,
...
static mesa_rc meba_poe_pd69200_bt_set_port_layer2_lldp_pd_request(
    const meba_poe_ctrl_inst_t *const inst,
    meba_poe_port_handle_t            channel,
    ...
    uint8_t                           autoclass)
{
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "[%s] CH=%d ,req pwr single dw=%lu ,req pwr dual_a dw=%lu ,req pwr dual_b dw=%lu ,cable len=%d ,autoclass=%d",
...
static mesa_rc meba_poe_pd69200_bt_get_port_layer2_lldp_pse_data(
    const meba_poe_ctrl_inst_t *const inst,
    meba_poe_port_handle_t            channel,
    bt_lldp_pse_data_t               *ptBt_lldp_pse_data)
{
    uint8_t buf[PD_BUFFER_SIZE] = {
        REQUEST_KEY, DUMMY_SEQ_NUM, CHANNEL_KEY, BT_LAYER2_LLDP_PSE_KEY,
        channel,     DUMMY_BYTE,    DUMMY_BYTE,  DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE,  DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE};
...
mesa_rc meba_poe_ctrl_pd69200_bt_globals_cfg_set(const meba_poe_ctrl_inst_t
                                                     *const inst,
                                                 meba_poe_global_cfg_t
                                                     *cfg_global)
{
    ...
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG,
          "meba_poe_pd69200_bt_set_globals_cfg, current_global_cfg->power_supply_poe_limit_w= %d , cfg_global->power_supply_poe_limit_w=%d",
          current_global_cfg->power_supply_poe_limit_w,
          cfg_global->power_supply_poe_limit_w);

    if ((current_global_cfg->power_supply_poe_limit_w ==
         cfg_global->power_supply_poe_limit_w)) {
        return MESA_RC_OK;
    }
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "Update globals configuration for controller");
    ...
    uint16_t
        power_limit_w; // The disconnection power level of a specific power bank

    MESA_RC(pd69200_get_power_supply_parameters(inst, &power_consumption_w,
                                                &max_shutdown_voltage_dv,
                                                &min_shutdown_voltage_dv,
                                                &guard_band, &power_bank,
                                                &power_limit_w));

    if (power_limit_w != cfg_global->power_supply_poe_limit_w) {
        power_limit_w = cfg_global->power_supply_poe_limit_w;

        MESA_RC(meba_poe_pd69200_set_power_banks(inst, power_bank,
                                                 power_limit_w,
                                                 max_shutdown_voltage_dv,
                                                 min_shutdown_voltage_dv, 0));
    }
...
static mesa_rc meba_poe_pd69200_bt_get_BT_events_interrupt_mask(
    const meba_poe_ctrl_inst_t *const inst,
    bt_events_interrupt_mask_t       *cfg_events)
{
    // Transmit the command
    uint8_t buf[PD_BUFFER_SIZE] = {
        REQUEST_KEY, DUMMY_SEQ_NUM, GLOBAL_KEY, BT_IRQ_MASK_KEY, DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE,      DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE, DUMMY_BYTE,      DUMMY_BYTE};
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "[%s] system events=0x%X ,device events=0x%X ,port events=0x%X",
          fname, cfg_events->System_Events_Mask_Register,
...
static mesa_rc meba_poe_pd69200_bt_set_BT_events_interrupt_mask(
    const meba_poe_ctrl_inst_t *const inst,
    bt_events_interrupt_mask_t        cfg_events)
{
    uint8_t buf[PD_BUFFER_SIZE] = {COMMAND_KEY,
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "[%s] setting: system events=0x%X ,device events=0x%X ,port events=0x%X",
          fname, cfg_events.System_Events_Mask_Register,
          cfg_events.Device_Events_Mask_Register,
          cfg_events.Port_Events_Mask_Register);
...
static mesa_rc meba_poe_pd69200_bt_get_bt_port_counters(
    const meba_poe_ctrl_inst_t *const inst,
    meba_poe_port_handle_t            channel,
    bt_port_counters_t               *ptBT_port_counters)
{
    // Transmit the command
    uint8_t buf[PD_BUFFER_SIZE] = {
        REQUEST_KEY, DUMMY_SEQ_NUM, CHANNEL_KEY, BT_PORT_COUNTERS_KEY,
        channel,     DUMMY_BYTE,    DUMMY_BYTE,  DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE,  DUMMY_BYTE,
        DUMMY_BYTE,  DUMMY_BYTE,    DUMMY_BYTE};
    ...
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG,
          ...
          fname, channel, ptBT_port_counters->udl_count,
          ptBT_port_counters->ovl_count, ptBT_port_counters->sc_count,
...
mesa_rc meba_poe_ctrl_pd69200_bt_port_status_get(const meba_poe_ctrl_inst_t
                                                     *const             inst,
                                                 meba_poe_port_handle_t handle,
                                                     ...
                                                     *const port_status)
{
    ...
    if (current_port_status->port_status.chip_state ==
        MEBA_POE_FIRMWARE_UPGRADE) {
        DEBUG(inst, MEBA_TRACE_LVL_DEBUG, "%s Firmware upgrading",
              __FUNCTION__);
        current_port_status->port_status.assigned_pd_class_a =
            POE_UNDETERMINED_CLASS;
        current_port_status->port_status.assigned_pd_class_b =
    ...
    if (current_port_status->port_status.chip_state != MEBA_POE_CHIPSET_FOUND) {
        DEBUG(inst, MEBA_TRACE_LVL_DEBUG, "%s Device not ready for reading",
              __FUNCTION__);
        return MESA_RC_ERROR;
    }
    ...
    if ((current_port_status->port_status.meba_poe_ieee_port_state !=
        ...
        DEBUG(
            ...
            get_poe_ieee_port_state_description(current_port_status->port_status
                                                    .meba_poe_ieee_port_state),
            get_bt_port_internal_status_description(
                current_port_status->port_status.poe_internal_port_status));
    }
    ...
    if (bRead_port_status_and_port_class) {
        ...
        MESA_RC(meba_poe_pd69200_bt_get_BT_port_status(
            inst, handle, &port_state, &enable, &assigned_class,
            &measured_port_power, &last_shutdown_error_status, &port_event));

        bCounters_related_event =
            ((port_event & 4) !=
             0); // Counters Related Event bit 2 active - lets read counters

        current_port_status->port_status.assigned_pd_class_a =
            (assigned_class & 0xF0) >> 4;
        current_port_status->port_status.assigned_pd_class_b =
            assigned_class & 0xF;
        ...
        if (current_port_status->port_status.poe_internal_port_status !=
            port_state) {
            strncpy(current_port_status->port_status.poe_port_status_description,
                    get_bt_port_internal_status_description(port_state),
                    MAX_STR_SIZE - 1);
        }
        ...
        case BT_0x89_ON_4P_Port_delivering_4P_IEEE_DSPD:           {
            current_port_status->port_status.meba_poe_ieee_port_state =
                MEBA_POE_IEEE_PORT_STATE_DELIVERING_POWER; // POEBT
        ...
        case BT_0x84_ON_4P_Port_delivering_4P_non_IEEE:            {
            current_port_status->port_status.meba_poe_ieee_port_state =
                MEBA_POE_IEEE_PORT_STATE_DELIVERING_POWER; // LEGACY
        ...
        case BT_0x11_OFF_Port_is_yet_undefined:
        case BT_0x12_OFF_Internal_hardware_fault:
        case BT_0x1C_OFF_Non_802_3AF_AT_powered_device: // Non-standart PD
                                                        // connected
        ...
        case BT_0xA7_OFF_Connection_Check_error: // normal status when Ethernet
                                                 // link as laptop is conencted
                                                 // to this port
        ...
        if (current_port_status->port_status.meba_poe_port_state ==
            MEBA_POE_PD_ON) {
            if ((port_state ==
                 BT_0x87_ON_4P_Port_delivering_4P_IEEE_DSPD_1st_phase) ||
        ...
        // support auto classanalyze

        // adding
        current_port_status->port_status.bt_port_operation_mode =
            port_cfg_POEMCU->bt_port_operation_mode;
        current_port_status->port_status.bt_port_pm_mode =
            port_cfg_POEMCU->bt_port_pm_mode;
        current_port_status->port_status.bt_pse_port_type =
            port_cfg->bt_pse_port_type;

        DEBUG(inst, MEBA_TRACE_LVL_NOISE, "CH=%d ,bt_pse_port_type: 0x%X \n\r",
              handle, current_port_status->port_status.bt_pse_port_type);
        ...
        MESA_RC(meba_poe_pd69200_bt_get_BT_port_class(
            inst, handle, &port_state, &port_phy_info, &measured_class,
            &requested_class, &requested_power_dW, &assigned_class,
            &assigned_power_dW, &auto_class_support,
            &auto_class_measurement_dw));

        current_port_status->port_status.power_requested_mw =
            requested_power_dW * 100; // Convert from deciwatt to milliWatts
        current_port_status->port_status.pd_structure = port_phy_info & 0x0F;
        // current_port_status->port_status.poe_internal_port_status = port_state;

        current_port_status->port_status.power_assigned_mw =
            assigned_power_dW * 100; // Convert from deciwatt to milliWatts

        current_port_status->port_status.measured_pd_class_a =
            (measured_class & 0xF0) >> 4;

        current_port_status->port_status.measured_pd_class_b =
            measured_class & 0xF;

        current_port_status->port_status.requested_pd_class_a =
            (requested_class & 0xF0) >> 4;

        current_port_status->port_status.requested_pd_class_b =
            requested_class & 0xF;
        ...
        // current_port_status->port_status.autoclass_support =
        // auto_class_support;

        if (auto_class_support == 0x2) {
            current_port_status->port_status.measured_autoclass_power_mw =
                auto_class_measurement_dw *
                100; // Convert from deciwatt to milliWatts
        } else {
    ...
    if (bRead_port_measurements) {
        ...
        if (current_port_status->port_status.meba_poe_ieee_port_state ==
            ...
            meba_poe_pd69200_bt_get_BT_port_measurements(
                inst, handle, &main_voltage_dv, &calculated_current_ma,
                &port_power_consumption_dw, &port_voltage_dv);
        }

        current_port_status->port_status.power_mw =
            port_power_consumption_dw * 100; // dw -> mw
        current_port_status->port_status.current_ma = calculated_current_ma;
        current_port_status->port_status.voltage_mv =
            port_voltage_dv * 100; // dv -> mv
    }

    if (bRead_lldp_pse_data) {
        bt_lldp_pse_data_t tBt_lldp_pse_data;
        MESA_RC(meba_poe_pd69200_bt_get_port_layer2_lldp_pse_data(
            inst, handle, &tBt_lldp_pse_data));

        current_port_status->port_status.pse_data.power_pairs_control_ability =
            false;
        ...
        if (current_port_status->port_status.assigned_pd_class_a >= 4) {
            current_port_status->port_status.pse_data.power_class =
                5; // Note: value 1:class=0. value 2:class=1, etc. 5 = Class 4
                   // and above
        ...
        current_port_status->port_status.pse_data.cable_len =
            tBt_lldp_pse_data.cable_length_in_use;
        ...
        if (tBt_lldp_pse_data.layer2_usage_status >= 3) {
            if (current_port_status->port_status.pd_type_sspd_dspd ==
                2) { // DSPD
                current_port_status->port_status.power_requested_mw =
                    current_port_status->port_status.pse_data
                        .requested_power_mode_a_mw +
                    current_port_status->port_status.pse_data
                        .requested_power_mode_b_mw;
                // printf("\n\r
                ...
                // current_port_status->port_status.pse_data.requested_power_single_mw);
            }
        }
        ...
        current_port_status->port_status.system_setup =
            2; // This value is specificed in Table 79-6f IEEE 803.2bt/D3.7 Type
               // 3 PSE = 0. Type 4 PSE = 2.

        current_port_status->port_status.pse_data.layer2_execution_status =
            tBt_lldp_pse_data.layer2_execution_status;

        // Layer2 LLDP/CDP request not pending  (0x1—Layer2 LLDP/CDP request
        // pending)
        ...
        DEBUG(inst, MEBA_TRACE_LVL_NOISE,
              "CH=%d ,UNG status=%d ,intStatus=0x%X", handle,
              current_port_status->port_status.meba_poe_port_state,
              current_port_status->port_status.poe_internal_port_status);
    }
    ...
    if (bCounters_related_event) {
        ...
        // 'Get BT Class Power' driver
        MESA_RC(meba_poe_pd69200_bt_get_bt_port_counters(inst, handle,
                                                         &tBT_port_counters));

        current_port_status->port_status.bt_port_counters.udl_count =
            tBT_port_counters.udl_count;
        current_port_status->port_status.bt_port_counters.ovl_count =
            tBT_port_counters.ovl_count;
        current_port_status->port_status.bt_port_counters.sc_count =
            tBT_port_counters.sc_count;
        current_port_status->port_status.bt_port_counters
            .invalid_signature_count =
...
mesa_rc meba_poe_ctrl_pd69200_bt_port_pd_data_clear(const meba_poe_ctrl_inst_t
                                                        *const inst,
                                                    meba_poe_port_handle_t
                                                        handle)
{
    ...
    MESA_RC(meba_poe_ctrl_pd69200_bt_port_cfg_set(inst, handle, &req_port_cfg));
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "port=%d clear requested_power_mode_a_mw = %d \n\r", handle,
          current_port_status->requested_power_single_mw);

    return MESA_RC_OK;
}

mesa_rc meba_poe_ctrl_pd69200_bt_chip_initialization(const meba_poe_ctrl_inst_t
                                                         *const inst,
                                                     mesa_bool_t
                                                         interruptible_power,
                                                     int16_t restart_cause)
{
    ...
    if (inst->port_poe_length > inst->port_map_length) {
        DEBUG(inst, MEBA_TRACE_LVL_WARNING,
              "%s(%s): prod_max_poe_ports=%d is bigger then port_map_length=%d",
              __FUNCTION__, inst->adapter_name, inst->port_poe_length,
              inst->port_map_length);
        return MESA_RC_ERROR;
    }
    ...
    DEBUG(inst, MEBA_TRACE_LVL_DEBUG,
          ...
          __FUNCTION__, inst->adapter_name,
          tPoE_parameters.poe_init_params.power_supply_max_power_w,
          power_limit_w, current_global_cfg->power_supply_poe_limit_w);
    ...
    MESA_RC(meba_poe_pd69200_bt_get_BT_events_interrupt_mask(inst,
                                                             &cfg_events));

    //--- individual_masks ---//
    DEBUG(inst, MEBA_TRACE_LVL_INFO,
          "%s(%s): syncing BT individual masks parameters", __FUNCTION__,
          inst->adapter_name);

    // Turn off lowest priority port, when a higher priority has a PD connected.
    uint8_t ignore_high_priority;
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_BT_IGNORE_HIGHER_PRIORITY_0x00, &ignore_high_priority,
        "BT- ignore_high_priority"));
    if (ignore_high_priority !=
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_BT_SUPPORT_HIGH_RES_DETECTION_0x10,
        &support_high_res_detection, "BT- support_high_res_detection"));
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_BT_I2C_RESTART_ENABLE_0x1B, &i2c_restart_enable,
        "BT- i2c_restart_enable"));
    if (i2c_restart_enable !=
        tPoE_parameters.indv_mask_BT_i2c_restart_enable_default) {
        MESA_RC(meba_poe_pd69200_set_individual_mask(
            inst, INDV_MASK_BT_I2C_RESTART_ENABLE_0x1B,
            tPoE_parameters.indv_mask_BT_i2c_restart_enable_default));
        bChangedFlag = true;
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_BT_PSE_POWERING_PSE_CHECKING_0x1F,
        &pse_powering_pse_checking, "BT- pse_powering_pse_checking"));
    ...
    uint8_t led_stream_type;
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_BT_LED_STREAM_TYPE_0x20, &led_stream_type,
        "BT- led_stream_type"));
    if (led_stream_type !=
        tPoE_parameters.indv_mask_BT_led_stream_type_default) {
        MESA_RC(meba_poe_pd69200_set_individual_mask(
            inst, INDV_MASK_BT_LED_STREAM_TYPE_0x20,
            tPoE_parameters.indv_mask_BT_led_stream_type_default));
        bChangedFlag = true;
    }
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_BT_LAYER2_POWER_ALLOCATION_LIMIT_0x2C,
        &layer2_power_allocation_limit, "BT- layer2_power_allocation_limit"));
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_BT_SUPPORT_ADDING_LLDP_HALF_PRIORITY_0x4F,
        &support_lldp_half_priority, "BT- support_lldp_half_priority"));
    ...
    MESA_RC(meba_poe_pd69200_get_individual_mask(
        inst, INDV_MASK_BT_SINGLE_DETECTION_FAIL_EVENT_0x46,
        &bt_single_detection_fail_event, "BT- bt_single_detection_fail_event"));
    ...
    DEBUG(inst, MEBA_TRACE_LVL_INFO, "%s(%s): syncing poe lldp parameters",
          __FUNCTION__, inst->adapter_name);
    ...
    // cold start and somthing was modified
    if (bMatrixUpdatedFlag ||
        (/*(restart_cause == 0) &&*/ (bChangedFlag ==
                                      true))) // cold start and somthing was
                                              // modified
    {
        MESA_RC(meba_poe_ctrl_pd69200_save_command(inst));
        DEBUG(inst, MEBA_TRACE_LVL_INFO, "Save PoE settings");
                                ...
                                meba_poe_parameters_t tMeba_poe_parameters)
{
    ...
    static meba_poe_ctrl_api_t meba_pd69200bt_api = {
        .meba_poe_ctrl_do_detection = meba_poe_ctrl_pd69200_do_detection,
        .meba_poe_ctrl_chip_initialization =
            meba_poe_ctrl_pd69200_bt_chip_initialization,
        .meba_poe_ctrl_sync = meba_poe_ctrl_pd69200_sync,
        .meba_poe_ctrl_reset_command = meba_poe_ctrl_pd69200_do_reset,
        .meba_poe_ctrl_save_command = meba_poe_ctrl_pd69200_save_command,
        .meba_poe_ctrl_version_get = meba_poe_ctrl_pd69200_version_get,
        ...
        .meba_poe_ctrl_port_status_get =
            meba_poe_ctrl_pd69200_bt_port_status_get,
        ...
        .meba_poe_ctrl_port_capabilities_get =
            meba_poe_ctrl_pd69200_port_capabilities_get,
        .meba_poe_ctrl_port_pd_data_set =
            meba_poe_ctrl_pd69200_bt_port_prebt_pd_data_set,
        .meba_poe_ctrl_port_pd_bt_data_set =
            meba_poe_ctrl_pd69200_bt_port_bt_pd_data_set,
        .meba_poe_ctrl_port_pd_data_clear =
            meba_poe_ctrl_pd69200_bt_port_pd_data_clear,
    };
