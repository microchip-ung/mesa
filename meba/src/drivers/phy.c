...
mepa_rc meba_phy_fefi_set(meba_inst_t             inst,
...
mepa_rc meba_phy_fefi_get(meba_inst_t             inst,
                          mepa_port_no_t          port_no,
...
mepa_rc meba_phy_eee_mode_conf_set(meba_inst_t               inst,
                                   mepa_port_no_t            port_no,
                                   const mepa_phy_eee_conf_t conf)
{
    if ((port_no < 0) || (port_no >= inst->phy_device_cnt)) {
        return MESA_RC_ERR_INV_PORT_BOARD;
    }
...
mepa_rc meba_phy_eee_mode_conf_get(meba_inst_t                inst,
                                   mepa_port_no_t             port_no,
                                   mepa_phy_eee_conf_t *const conf)
{
    if ((port_no < 0) || (port_no >= inst->phy_device_cnt)) {
        return MESA_RC_ERR_INV_PORT_BOARD;
    }
...
// Read the current EEE mode Configuration.
mepa_rc meba_phy_eee_status_get(meba_inst_t        inst,
                                mepa_port_no_t     port_no,
                                uint8_t *const     advertisement,
                                mepa_bool_t *const rx_in_power_save_state,
                                mepa_bool_t *const tx_in_power_save_state)
{
    if ((port_no < 0) || (port_no >= inst->phy_device_cnt)) {
        return MESA_RC_ERR_INV_PORT_BOARD;
    }
...
mepa_rc meba_phy_if_get(meba_inst_t            inst,
                        mepa_port_no_t         port_no,
...
/* I2C Clock select */
mepa_rc meba_phy_i2c_clock_select(meba_inst_t                  inst,
                                  mepa_port_no_t               port_no,
...
mepa_rc meba_phy_media_set(meba_inst_t            inst,
                           mepa_port_no_t         port_no,
...
mepa_rc meba_phy_aneg_status_get(meba_inst_t         inst,
                                 mepa_port_no_t      port_no,
...
mepa_rc meba_phy_clause22_read(meba_inst_t     inst,
                               mepa_port_no_t  port_no,
                               uint32_t        address,
...
mepa_rc meba_phy_clause22_write(meba_inst_t    inst,
                                mepa_port_no_t port_no,
                                uint32_t       address,
                                uint16_t       value)
{
...
mepa_rc meba_phy_clause45_read(meba_inst_t     inst,
                               mepa_port_no_t  port_no,
                               uint32_t        address,
                               uint16_t *const value)
{
...
mepa_rc meba_phy_clause45_write(meba_inst_t    inst,
                                mepa_port_no_t port_no,
                                uint32_t       address,
                                uint16_t       value)
{
...
mepa_rc meba_phy_event_enable_set(meba_inst_t    inst,
                                  mepa_port_no_t port_no,
                                  mepa_event_t   event,
                                  mesa_bool_t    enable)
{
...
mepa_rc meba_phy_event_enable_get(meba_inst_t         inst,
                                  mepa_port_no_t      port_no,
...
mepa_rc meba_phy_loopback_get(meba_inst_t            inst,
                              mepa_port_no_t         port_no,
...
mepa_rc meba_phy_gpio_mode_set(meba_inst_t             inst,
                               mepa_port_no_t          port_no,
...
mepa_rc meba_phy_gpio_out_set(meba_inst_t    inst,
                              mepa_port_no_t port_no,
                              uint8_t        gpio_no,
                              mepa_bool_t    enable)
{
...
mepa_rc meba_phy_gpio_in_get(meba_inst_t        inst,
                             mepa_port_no_t     port_no,
                             uint8_t            gpio_no,
...
mepa_rc meba_phy_synce_clock_conf_set(meba_inst_t                    inst,
                                      mepa_port_no_t                 port_no,
                                 ...
                                 mesa_sd10g_media_type_t *media)
{
    ...
    if (inst->api.meba_port_entry_get(inst, port_no, &meba) == MESA_RC_OK) {
        if (meba.cap & MEBA_PORT_CAP_CU_BP) {
            *media = MESA_SD10G_MEDIA_BP;
            return MESA_RC_OK;
        }
    }
    switch (if_type) {
    ...
    case MESA_PORT_INTERFACE_DXGMII_10G: *media = MESA_SD10G_MEDIA_SR; break;
    default:                             *media = MESA_SD10G_MEDIA_PR_NONE; break;
    }
...
mepa_rc meba_port_test_conf_set(meba_inst_t                        inst,
                                mepa_port_no_t                     port_no,
                                const mesa_port_test_conf_t *const conf)
{
    ...
    case MESA_PORT_LB_NEAR_END:  lb.near_end_ena = 1; break;
    case MESA_PORT_LB_FAR_END:   lb.far_end_ena = 1; break;
    case MESA_PORT_LB_FACILITY:  lb.mac_serdes_facility_ena = 1; break;
    case MESA_PORT_LB_EQUIPMENT: lb.mac_serdes_equip_ena = 1; break;
    default:                     break;
    }
...
// Start PHY Self-Test
mepa_rc meba_selftest_start(meba_inst_t                 inst,
                            mepa_port_no_t              port_no,
...
mepa_rc meba_selftest_read(meba_inst_t                 inst,
                           mepa_port_no_t              port_no,
...
// To Set PRBS
mepa_rc meba_prbs_set(meba_inst_t                           inst,
                      mepa_port_no_t                        port_no,
...
// To Get PRBS
mepa_rc meba_prbs_get(meba_inst_t                           inst,
                      mepa_port_no_t                        port_no,
                      mepa_phy_prbs_type_t                  type,
                      mepa_phy_prbs_direction_t             direction,
                      mepa_phy_prbs_generator_conf_t *const prbs_conf)
...
mepa_rc meba_prbs_monitor_set(meba_inst_t                         inst,
                              mepa_port_no_t                      port_no,
...
mepa_rc meba_prbs_monitor_get(meba_inst_t                         inst,
                              mepa_port_no_t                      port_no,
                              mepa_phy_prbs_monitor_conf_t *const value)
{
    if ((port_no < 0) || (port_no >= inst->phy_device_cnt)) {
...
mepa_rc meba_warmrestart_conf_get(meba_inst_t           inst,
                                  mepa_port_no_t        port_no,
                                  mepa_restart_t *const restart)
{
    if ((port_no < 0) || (port_no >= inst->phy_device_cnt)) {
        return MESA_RC_ERR_INV_PORT_BOARD;
    }
...
mepa_rc meba_warmrestart_conf_set(meba_inst_t          inst,
                                  mepa_port_no_t       port_no,
                                  const mepa_restart_t restart)
{
    if ((port_no < 0) || (port_no >= inst->phy_device_cnt)) {
        return MESA_RC_ERR_INV_PORT_BOARD;
    }
