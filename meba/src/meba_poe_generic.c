                                     ...
                                     meba_poe_chip_state_t *chip_state)
{
    ...
    *chip_state = MEBA_POE_NO_CHIPSET_FOUND;
    if (inst->api_poe->meba_poe_get_controller_handle(inst, iport, &controller, &handle) ==
        MESA_RC_OK) {
        if (MESA_RC_OK ==
            controller->api->meba_poe_ctrl_chipset_get(controller, handle, chip_state)) {
            return MESA_RC_OK;
        }
    }
    return MESA_RC_ERROR;
...
mesa_rc meba_poe_generic_version_get(const meba_inst_t inst, uint32_t max_size, char *value)
{
    meba_poe_system_t *system;
    int                i;
    ...
    for (i = 0; i < system->controller_count; ++i) {
        if (MESA_RC_OK ==
            system->controllers[i].api->meba_poe_ctrl_version_get(&system->controllers[i], max_size,
                                                                  value)) {
            return MESA_RC_OK;
        }
    }
    ...
    if (inst && inst->api_poe && inst->api_poe->meba_poe_system_get) {
        if (inst->api_poe->meba_poe_system_get(inst, &system) == MESA_RC_OK) {
            ...
            for (i = 0; i < system->controller_count; ++i) {
                mesa_rc rc;
                rc =
                    system->controllers[i].api->meba_poe_ctrl_reset_command(&system->controllers[i]);
                if (rc != MESA_RC_OK) {
                    return rc;
                }
            }
    ...
    if (inst && inst->api_poe && inst->api_poe->meba_poe_system_get) {
        if (inst->api_poe->meba_poe_system_get(inst, &system) == MESA_RC_OK) {
            ...
            for (i = 0; i < system->controller_count; ++i) {
                mesa_rc rc;
                rc =
                    system->controllers[i].api->meba_poe_ctrl_save_command(&system->controllers[i]);
                if (rc != MESA_RC_OK) {
                    return rc;
                }
            }
                                      ...
                                      const char       *data)
{
    ...
    for (i = 0; i < system->controller_count; ++i) {
        if (MESA_RC_OK ==
            system->controllers[i].api->meba_poe_ctrl_firmware_upgrade(&system->controllers[i],
                                                                       reset, size, data)) {
            count++;
        }
    }
...
// return MESA_RC_ERR_POE_FIRMWARE_IS_UP_TO_DATE if firmware update is needed
// for any of the controllers. otherwise returns MESA_RC_OK
                                                  ...
                                                  const char       *data)
{
    ...
    for (i = 0; i < system->controller_count; ++i) {
        mesa_rc rc_;
        rc_ = system->controllers[i]
                  .api->meba_poe_ctrl_prepare_firmware_upgrade(&system->controllers[i],
                                                               version_check, size, data);
...
mesa_rc meba_poe_generic_cfg_set(const meba_inst_t inst, meba_poe_global_cfg_t *cfg)
{
    ...
    if (inst && inst->api_poe && inst->api_poe->meba_poe_system_get) {
        if (inst->api_poe->meba_poe_system_get(inst, &system) == MESA_RC_OK) {
            int i;
            for (i = 0; i < system->controller_count; ++i) {
                (void)system->controllers[i].api->meba_poe_ctrl_cfg_set(&system->controllers[i],
                                                                        cfg);
            }
        }
    }
...
mesa_rc meba_poe_generic_status_get(const meba_inst_t inst, meba_poe_status_t *status)
{
    ...
    if (inst && inst->api_poe && inst->api_poe->meba_poe_system_get) {
        if (inst->api_poe->meba_poe_system_get(inst, &system) == MESA_RC_OK) {
            ...
            for (i = 0; i < system->controller_count; ++i) {
                if (MESA_RC_OK ==
                    system->controllers[i].api->meba_poe_ctrl_status_get(&system->controllers[i],
                                                                         &local_status[i])) {
                    controller_count++;
                    valid_controller = i;
                }
...
mesa_rc meba_poe_generic_capabilities_get(const meba_inst_t inst, meba_poe_ctrl_cap_t *capabilities)
{
    meba_poe_system_t *system;
    if (inst && inst->api_poe && inst->api_poe->meba_poe_system_get) {
        if (inst->api_poe->meba_poe_system_get(inst, &system) == MESA_RC_OK) {
                                      ...
                                      meba_poe_port_cfg_t *cfg)
{
    ...
    meba_poe_port_handle_t handle;
    if (inst && inst->api_poe && inst->api_poe->meba_poe_get_controller_handle) {
        if (inst->api_poe->meba_poe_get_controller_handle(inst, port_no, &controller, &handle) ==
            MESA_RC_OK) {
            if (controller->api->meba_poe_ctrl_port_cfg_set(controller, handle, cfg) ==
                MESA_RC_OK) {
                return MESA_RC_OK;
            }
        }
                                           ...
                                           meba_poe_psu_input_prob_t *psu_prop)
{
    ...
    meba_poe_port_handle_t handle;
    if (inst && inst->api_poe && inst->api_poe->meba_poe_get_controller_handle) {
        if (inst->api_poe->meba_poe_get_controller_handle(inst, 0, &controller, &handle) ==
            MESA_RC_OK) {
            int i;
                                               ...
                                               meba_poe_port_cap_t *capabilities)
{
    ...
    if (inst && inst->api_poe && inst->api_poe->meba_poe_get_controller_handle) {
        if (inst->api_poe->meba_poe_get_controller_handle(inst, port_no, &controller, &handle) ==
            MESA_RC_OK) {
            if (controller->api->meba_poe_ctrl_port_capabilities_get(controller, handle,
                                                                     capabilities) == MESA_RC_OK) {
                return MESA_RC_OK;
            }
        }
    }
                               ...
                               int               max_msg_len)
{
    ...
    if (inst && inst->api_poe && inst->api_poe->meba_poe_get_controller_handle) {
        if (inst->api_poe->meba_poe_get_controller_handle(inst, port_no, &controller, &handle) ==
            MESA_RC_OK) {
            if (controller->api->meba_poe_ctrl_debug(controller, var, str_len, title, tx_str,
                                                     rx_str, msg, max_msg_len) == MESA_RC_OK) {
                return MESA_RC_OK;
            }
        } else
                                          ...
                                          meba_poe_pd_data_t *pd_data)
{
    ...
    if (inst && inst->api_poe && inst->api_poe->meba_poe_get_controller_handle) {
        if (inst->api_poe->meba_poe_get_controller_handle(inst, port_no, &controller, &handle) ==
            MESA_RC_OK) {
            if (controller->api->meba_poe_ctrl_port_pd_data_set(controller, handle, pd_data) ==
                MESA_RC_OK) {
                return MESA_RC_OK;
            }
        }
                                             ...
                                             meba_poe_pd_bt_data_t *pd_data)
{
    ...
    if (inst && inst->api_poe && inst->api_poe->meba_poe_get_controller_handle) {
        if (inst->api_poe->meba_poe_get_controller_handle(inst, port_no, &controller, &handle) ==
            MESA_RC_OK) {
            if (controller->api->meba_poe_ctrl_port_pd_bt_data_set(controller, handle, pd_data) ==
                MESA_RC_OK) {
                return MESA_RC_OK;
            }
        }
...
mesa_rc meba_poe_generic_port_pd_data_clear(const meba_inst_t inst, mesa_port_no_t port_no)
{
    ...
    if (inst && inst->api_poe && inst->api_poe->meba_poe_get_controller_handle) {
        if (inst->api_poe->meba_poe_get_controller_handle(inst, port_no, &controller, &handle) ==
            MESA_RC_OK) {
            if (controller->api->meba_poe_ctrl_port_pd_data_clear(controller, handle) ==
                MESA_RC_OK) {
                return MESA_RC_OK;
            }
        }
