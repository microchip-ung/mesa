...
void set_jr2_parameters(meba_poe_firmware_type_t poe_type) {
    ...
    //poe_default_parameters.ePoE_gen6_controller_detection_type_default = JR2_POE_GEN6_PD692x0_CONTROLLER_TYPE_DEFAULT;
    ...
    poe_default_parameters.bt_operation_mode_none_compliant_15w_default = JR2_BT_NON_COMPLIANT_15W_DEFAULT;
    poe_default_parameters.bt_operation_mode_none_compliant_30w_default = JR2_BT_NON_COMPLIANT_30W_DEFAULT;
    poe_default_parameters.bt_operation_mode_none_compliant_60w_default = JR2_BT_NON_COMPLIANT_60W_DEFAULT;
    poe_default_parameters.bt_operation_mode_none_compliant_90w_default = JR2_BT_NON_COMPLIANT_90W_DEFAULT;

    // special Port operation modes for legacy
    poe_default_parameters.bt_operation_mode_legacy_90W_poh_default = JR2_BT_OPERATION_MODE_LEGACY_90W_POH_DEFAULT;
    poe_default_parameters.bt_operation_mode_legacy_60W_ignore_pd_class_default = JR2_BT_OPERATION_MODE_LEGACY_60W_IGNORE_PD_CLASS_DEFAULT;
    poe_default_parameters.bt_operation_mode_legacy_90W_ignore_pd_class_default = JR2_BT_OPERATION_MODE_LEGACY_90W_IGNORE_PD_CLASS_DEFAULT;

    poe_default_parameters.reset_poe_gpio_number = JR2_RESET_POE_GPIO_NUMBER;

    // prebt led stream type
    poe_default_parameters.indv_mask_prebt_led_stream_type_default = JR2_INDV_MASK_PREBT_LED_STREAM_TYPE_DEFAULT;
    // power higher priority port
    poe_default_parameters.indv_mask_prebt_ignore_higher_priority_default = JR2_INDV_MASK_PREBT_IGNORE_HIGHER_PRIORITY_DEFAULT;
    // En/Dis support of legacy detection
    poe_default_parameters.indv_mask_prebt_supports_legact_detection_default = JR2_INDV_MASK_PREBT_SUPPORTS_LEGACY_DETECTION_DEFAULT;
    // En/Dis supports backoff
    poe_default_parameters.indv_mask_prebt_supports_backoff_default = JR2_INDV_MASK_PREBT_SUPPORTS_BACKOFF_DEFAULT;
    // en/Dis MESSAGE_READY pin notification
    poe_default_parameters.indv_mask_prebt_message_ready_notify_default = JR2_INDV_MASK_PREBT_MESSAGE_READY_NOTIFY_DEFAULT;
    // En/Dis Layer 2 PD commands
    poe_default_parameters.indv_mask_prebt_layer2_lldp_enable_default = JR2_INDV_MASK_PREBT_LAYER2_LLDP_ENABLE_DEFAULT;
    // accept/ignored Port Priority recived from the PD
    poe_default_parameters.indv_mask_prebt_layer2_priority_by_PD_default = JR2_INDV_MASK_PREBT_LAYER2_PRIORITY_BY_PD_DEFAULT;
    // use 4-pair matrix commands
    poe_default_parameters.indv_mask_prebt_matrix_support_4P_default = JR2_INDV_MASK_PREBT_MATRIX_SUPPORT_4P_DEFAULT;
    // power higher priority port
    poe_default_parameters.indv_mask_BT_ignore_higher_priority_default = JR2_INDV_MASK_BT_IGNORE_HIGHER_PRIORITY_DEFAULT;
    // expand Resistor detection range up to range to 55 K
    poe_default_parameters.indv_mask_BT_support_high_res_detection_default = JR2_INDV_MASK_BT_SUPPORT_HIGH_RES_DETECTION_DEFAULT;
    // Initialization of the I2C module system after 10 seconds of inactivity
    poe_default_parameters.indv_mask_BT_i2c_restart_enable_default = JR2_INDV_MASK_BT_I2C_RESTART_ENABLE_DEFAULT;
    // led stream type
    poe_default_parameters.indv_mask_BT_led_stream_type_default = JR2_INDV_MASK_BT_LED_STREAM_TYPE_DEFAULT;
    ...
    poe_default_parameters.indv_mask_BT_PSE_powering_PSE_checking_default = JR2_INDV_MASK_BT_PSE_POWERING_PSE_CHECKING_DEFAULT;
    // Layer2 Power Allocation Limit
    poe_default_parameters.indv_mask_BT_layer2_power_allocation_limit_default = JR2_INDV_MASK_BT_LAYER2_POWER_ALLOCATION_LIMIT_DEFAULT;
    // Port LED Blinks at invalid signature or connection-check error
    poe_default_parameters.indv_mask_BT_Port_LED_blinks_at_invalid_signature_or_connection_check_error_default = JR2_INDV_MASK_BT_PORT_LED_BLINKS_AT_INVALID_SIGNATURE_OR_CONNECTION_CHECK_ERROR_DEFAULT;
    // Support adding lldp half priority
    poe_default_parameters.indv_mask_BT_support_adding_lldp_half_priority_default = JR2_INDV_MASK_BT_SUPPORT_ADDING_LLDP_HALF_PRIORITY_DEFAULT;
...
mesa_rc meba_poe_jr2_system_initialize(meba_inst_t inst, meba_poe_init_params_t *tPoe_init_params)
{
    ...
    if ((tPoe_init_params->poe_controllers_count_from_tag == 1) || (tPoe_init_params->poe_controllers_count_from_tag == 2)) {
    ...
    if (poe_default_parameters.eMeba_poe_firmware_type == MEBA_POE_FIRMWARE_TYPE_GEN6_BT) {
        // overide tMeba_poe_init_params params if using H file parameters
        if (tPoe_init_params->use_poe_static_parameters) {
            poe_default_parameters.max_poe_ports = sizeof(jr2_pd69200_4pairs_port_map_1) / sizeof(meba_poe_port_properties_t);
      	}
        ...
        if(jr2_pd69200_system.controller_count == MEBA_POE_TWO_CONTROLLERS) {
            // overide tMeba_poe_init_params params if using H file parameters
            if (tPoe_init_params->use_poe_static_parameters) {
                poe_default_parameters.max_poe_ports = sizeof(jr2_pd69200_4pairs_port_map_2) / sizeof(meba_poe_port_properties_t);
           	}
        ...
        // overide tMeba_poe_init_params params if using H file parameters
        if (tPoe_init_params->use_poe_static_parameters) {
            poe_default_parameters.max_poe_ports = sizeof(jr2_pd69200_2pairs_port_map_1) / sizeof(meba_poe_port_properties_t);
        }
        ...
        if(jr2_pd69200_system.controller_count == MEBA_POE_TWO_CONTROLLERS) {
            // overide tMeba_poe_init_params params if using H file parameters
            if (tPoe_init_params->use_poe_static_parameters) {
                poe_default_parameters.max_poe_ports = sizeof(jr2_pd69200_2pairs_port_map_2) / sizeof(meba_poe_port_properties_t);
            }
